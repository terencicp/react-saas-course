...
- **From 10.4 (tenancy + RBAC):** `tenantDb(orgId)` in `src/lib/tenant-db.ts`, `authedAction(role, schema, fn)` in `src/lib/authed-action.ts`, `authedRoute(role, schema, fn)` in `src/lib/authed-route.ts`, the active-org slot in the session, `writeAuditLog(tx, event)`.
...
- **Server Action** (`src/lib/comments/actions.ts`):
  - `addCommentAction = authedAction({ role: 'member', schema: addCommentInput, fn: async ({ ctx, input }) => { const inserted = await tenantDb(ctx.orgId).transaction(async (tx) => { const [row] = await tx.insert(invoiceComments).values({ organizationId: ctx.orgId, invoiceId: input.invoiceId, authorId: ctx.userId, body: input.body }).returning(); await writeAuditLog(tx, { event: 'comment.added', subjectId: row.id }); return row; }); await revalidateTag(invoiceCommentsTag(input.invoiceId)); return { ok: true as const, data: { id: inserted.id, createdAt: inserted.createdAt.toISOString() } }; } })`.
...
- Fill `src/lib/comments/actions.ts`: `addCommentAction = authedAction({ role: 'member', schema: addCommentInput, fn })`. Inside `fn`, wrap the insert in a `tenantDb(ctx.orgId).transaction`: insert the row, call `writeAuditLog(tx, { event: 'comment.added', subjectId: row.id })`, return the row with `createdAt` ISO-stringified. After the transaction commits and **before** returning, call `await revalidateTag(invoiceCommentsTag(input.invoiceId))` so the Server Component invoice page's cached comment count refreshes on the next visit. Return `{ ok: true, data: { id, createdAt } }`. On Zod parse failure or DB error, the wrapper returns the canonical `{ ok: false, error }` shape — the rollback path the mutation will key on.
...
- Fill the `useMutation` inside `comment-thread.tsx`:
  - `mutationFn: async (body: string) => { const result = await addCommentAction({ invoiceId, body }); if (!result.ok) throw new Error(result.error.message); return result.data; }`.
...
- **Two-system invalidation reality:** check the action's source — `await revalidateTag(invoiceCommentsTag(invoiceId))` after commit. Check the mutation's source — `queryClient.invalidateQueries({ queryKey: commentKeys.list(invoiceId) })` in `onSettled`. Both fire on every successful add. Delete the `revalidateTag` call (deliberately) and observe: the comment thread updates (TanStack cache invalidated), but the parent Server-Component-rendered comment count widget (if the seed includes one) stays stale until the next visit. Add the call back. Then delete the `invalidateQueries` call: the optimistic row stays, the form clears, but the next poll is the only thing that brings in the canonical server row — for ~10 seconds the cache holds a row with the `optimistic-...` id. Add the call back.
