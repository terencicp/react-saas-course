...
- **`pnpm create next-app` — for recognition, not for use.** The course's starter (Chapter 1.4.1) is the source of truth; the wizard is named so the student recognizes its output (TypeScript, Tailwind v4, App Router, Turbopack, Biome, `@/*` alias, `AGENTS.md`) when seen elsewhere.
...
- **What lives outside `app/`.** The project-root surface for genuinely shared code: `lib/` (helpers used by 2+ features), `db/` (Drizzle, Unit 6), `components/` (app-wide UI, often shadcn), `actions/` (rare; shared server actions), `env.ts` (Chapter 1.4.5), config files. Threshold for top-level: "used by two or more features."
...
- **Top-level files in the `app/` directory.** Framework-read file conventions at `app/`'s root: `favicon.ico`, `globals.css`, plus the SEO conventions `robots.ts`, `sitemap.ts`, `opengraph-image.{ext}`, `twitter-image.{ext}`, `icon.{ext}`, `apple-icon.{ext}` (Chapter 5.6 owns the SEO set).
- **`route.ts` named once.** A `route.ts` in a route folder makes it a route handler (HTTP responses, not UI); Chapter 7.5 owns the full surface. Named here to prevent confusing it with `page.tsx`.
...
- Server Components vs. Client Components in depth — Chapter 5.2.
- `loading.tsx`, `error.tsx`, `not-found.tsx` — Chapter 5.3.
- The `metadata` export and `<head>` content — Chapter 4.1.2 introduced, Chapter 5.6 owns.
- `route.ts` and route handlers — Chapter 7.5.
- The `middleware.ts`-replaced-by-`proxy.ts` migration — Chapter 5.5.
...
- `loading.tsx`, `error.tsx`, `not-found.tsx` at the layout level — Chapter 5.3.
- Server Components vs. Client Components — Chapter 5.2.
- Multiple root layouts at depth (rare; named only).
- Routing in middleware / `proxy.ts` — Chapter 5.5.

Estimated student time: 50 to 65 minutes. Load-bearing for every multi-page surface in the course, the `<auth>` and `<app>` split in Unit 9, and the project in Chapter 5.7.
...
- **Why `params` is a Promise — the 2026 change.** Next.js 16 made `params`, `searchParams`, `cookies()`, `headers()`, and `draftMode()` Promises to enable Partial Prerendering. In Server Components, `await`; in Client Components, `React.use(params)`. Chapter 5.4 owns the rendering model.
...
- **`generateStaticParams` — pre-rendering dynamic routes at build time.** Named as a forward-reference; the route exports an array of params and the framework generates a static HTML per entry. Chapter 5.6.11 owns it in full.
- **Validating `params` with Zod — the senior shape.** The URL is untrusted input; the page often expects a number, UUID, or constrained value. Zod 4 (Chapter 7.1) at the entry; parse failure throws into the surrounding error boundary (Chapter 5.3).
- **`notFound()` when the resource doesn't exist.** Named in context; the framework catches the thrown signal and renders the closest `not-found.tsx`. Full treatment in 5.1.4.
...
- **Programmatic navigation — `useRouter` from `next/navigation`.** Used when navigation runs from a handler (form submit, conditional). Client Component only; `push`/`replace`/`back`/`refresh`. Full surface in Chapter 5.5.5.
...
- **`notFound()` — the "resource doesn't exist" signal.** Throws; the framework renders the closest `not-found.tsx`; HTTP 404. Every dynamic route is a candidate. Full `not-found.tsx` in Chapter 5.3.
...
- **Slots stream independently with Suspense.** Each slot with its own `loading.tsx` streams as an independent boundary; the layout shell appears immediately. Chapter 5.3 owns Suspense in full.
...
- The list-plus-detail project — Chapter 5.7.
...
- **The modal UI itself.** Wrapped in a `<Modal>`/`<Dialog>` (Client Component, Radix or shadcn — Chapter 4.11.1) for open/close, focus, and `Esc`-to-close. The data and content come from a shared `<PhotoDetail />` used in both renders.
...
- Parallel routes — installed in 5.1.5.
- Modal implementation details, focus management, `Esc`-to-close — Chapters 4.11.1 and 4.11.3.
