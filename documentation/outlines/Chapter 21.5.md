...
- **From 21.4.2:** the trigger map placing "splitting one column into two and dropping the original" squarely in the three-deploy list.
...
- **From 10.1 / 10.2 / 7.2:** `tenantDb(orgId)`, `authedAction(role, schema, fn)`, `logAudit(tx, event)`, the canonical Result shape.
...
src/
  ...
  db/
    schema.ts                     # provided: 11.3 schema with invoices.total numeric(12,2) NOT NULL only,
...
- Trigger map placement of "splitting one column into two and dropping the original" — 21.4.2.
- Preview-branch rehearsal checklist + split-coverage verification + data-integrity diff — 21.4.3.
...
- `authedAction` + `tenantDb` + `logAudit` — 10.2 + 10.1.
...
- Frame the build: take the 11.3 invoices project, ship it to a real production URL on Vercel against a Neon branch-per-PR workflow, then execute one cadence-class migration (split `invoices.total numeric NOT NULL` into separate `subtotal numeric NOT NULL` + `tax numeric NOT NULL` columns, dropping `total`) across three reviewed PRs without any moment of schema/app incompatibility in production. Close by rehearsing a production rollback against the contract PR.
...
- The starter ships the 11.3 surface end-to-end with a deliberately under-modeled shape: `total` is a single combined amount with no breakdown of subtotal and tax. The cadence fixes a real anti-pattern (a column that conflates two business concepts), not a contrived one.
...
## Lesson 21.5.3 — PR 1 (Expand): add the nullable subtotal and tax columns

Ships an additive-only migration that adds `subtotal numeric(12,2)` and `tax numeric(12,2)` as nullable columns, rehearses it on the Neon preview branch, merges, and verifies the running 11.3 app code stays healthy against the expanded schema.
...
Codebase state at exit: PR 1 merged; production runs the unchanged 11.3 app code against a schema that has `total` (still NOT NULL), plus `subtotal` and `tax` (both nullable, unwritten). Split-coverage 0%. Inspector confirms. Documented in the runbook. **Runnable — production is healthy on the expand schema.**
...
## Lesson 21.5.5 — PR 3 (Contract): drop the old column, promote the new pair
...
- This is the only PR in the cadence whose schema move is hard to undo. Rolling back the alias to PR 2's deployment restores the *code* that reads `total`, but `total` is gone — the rollback produces an immediate runtime error. The rehearsal in 21.5.6 makes this concrete; the runbook captures it explicitly.
