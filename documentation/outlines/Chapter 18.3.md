...
- **From 11.3:** `app/(app)/invoices/page.tsx` Server Component (URL-state filters, status filter, cursor pagination), `getInvoices` / `getInvoice`, soft-delete via `deletedAt`, `version`-precondition update action, the `invoices` / `invoice_lines` / `customers` schema with `currency` text column on `invoices` and `dueDate` on each row.
...
- **From 10.2 / 10.4 / 9.5 / 7.1:** `authedAction(role, schema, fn)`, `tenantDb(orgId)`, `writeAuditLog`, Better Auth session returning `{ user: { id, email, locale, timeZone }, orgId, role }`; Zod 4 `strictObject`; canonical Result.
