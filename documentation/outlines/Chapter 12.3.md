...
- **From 10.4 (tenancy + RBAC):** the `organizations` table with `id`, `name`, `ownerId`; the active-org slot in the session; `tenantDb(orgId)`; `authedAction(role, schema, fn)`; the `audit_logs` table and `writeAuditLog(tx, event)` helper. The starter adds an `ownerEmail` getter so `stripe.customers.create({ email })` has something to pass.
...
- **From 8.3 (Resend):** `sendEmail(to, template)` lives in `/lib/email.ts`; not exercised in this project but referenced when an entitlement change should email the org owner ("forward note — Unit 14 wires the notification").
...
- **The handlers** (`lib/webhooks/stripe.ts`):
  - `onCheckoutCompleted(tx, event)` — reads `session.customer` (the Stripe Customer ID), reads `session.subscription`, calls `stripe.subscriptions.retrieve(subscriptionId)` once (the one allowed `stripe.*` call inside a handler — names the carve-out), resolves `organizationId` via `session.subscription_data?.metadata?.organization_id` or by reading the `organizations` row with the matching `stripeCustomerId`, UPSERTs `plan_entitlements` from the projection with `last_event_at = event.created`, writes `audit_logs` row `billing.subscription.activated`.
  - `onSubscriptionUpdated(tx, event)` — projects the subscription, UPDATEs `plan_entitlements` with `WHERE organizationId = ? AND (last_event_at IS NULL OR last_event_at < ?)`, writes `audit_logs` `billing.subscription.updated` (only when the UPDATE returns a row).
  - `onSubscriptionDeleted(tx, event)` — UPDATEs `plan_entitlements` to `plan: 'free'`, `status: 'canceled'`, `subscriptionId: null`, same ordering predicate, writes `audit_logs` `billing.subscription.canceled`.
...
- Wire the inspector's Checkout and Portal buttons:
  - The "Upgrade to Pro" button is a `<form action={async () => { 'use server'; const result = await upgrade({ planSlug: 'pro' }); if (result.ok) redirect(result.data.url); }}>` Server-Action-inline form. (Or a Client Component calling the action and using `window.location.assign(result.data.url)`.)
  - The "Manage billing" button is the same shape with `openPortal({ returnPath: '/inspector' })`, opening the URL in a new tab via `target="_blank"`. Disabled (with tooltip) when `stripeCustomerId` is null.
