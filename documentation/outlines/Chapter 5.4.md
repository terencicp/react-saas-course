...
- **Enabling `cacheComponents` — the config line.** `next.config.ts` exports a config object with `cacheComponents: true`. The legacy `experimental.dynamicIO` and `experimental.useCache` flags are deprecated and removed; the top-level `cacheComponents` replaces both. The course pins this on from day one in the starter (5.1 reference). Named here so the student recognizes the line and knows what it does.
...
- The `use cache` directive in full (5.4.3).
- Partial Prerendering as a rendering shape (5.4.2).
- `cacheLife` and `cacheTag` (5.4.4).
- React `cache()` for per-request memoization (5.4.5).
- The post-mutation invalidation surface (5.4.6).
- Async request API specifics — `params`, `searchParams`, `cookies()`, `headers()`, `connection()` (5.4.8).
- The deprecated route segment config (5.4.7).
...
- **PPR and parallel routes.** Each `@slot` in a parallel route (5.1.5) is its own subtree; each can be independently cached or dynamic. The 5.7 project leans on this — a cached navigation slot alongside a dynamic detail slot, both under the same URL, each with its own loading and streaming behavior.
...
- The `use cache` directive's full syntax and semantics (5.4.3).
- `cacheLife` and `cacheTag` (5.4.4).
- React `cache()` for per-request work (5.4.5).
- Invalidation after a mutation (5.4.6).
- Async request APIs (5.4.8).
- Streaming and Suspense from scratch (5.3).
...
- **Serialization rules — the contract.** Arguments to a `use cache` function must be serializable (same shape as the server-client boundary in 5.2.4 — primitives, plain objects, arrays, `Date`, `Map`, `Set`, typed arrays). Return values must be serializable. Functions, class instances, Promises-of-Promises are rejected. The compiler enforces this at build; a violation is a build error with the offending parameter pointed at.
...
- **Function-level `use cache` — the cached fetcher.** An async function that fetches and returns data, marked `use cache`, becomes a deduplicated and persistent cache. Two callers across two requests with the same arguments get one fetch, one stored value. Distinct from React `cache()` (5.4.5) which is per-request, not cross-request.
...
- `cacheLife` for freshness windows and `cacheTag` for naming (5.4.4).
- React `cache()` and the request memo cache (5.4.5).
- Invalidating cached entries (5.4.6).
- Route segment config and its deprecations (5.4.7).
- Async request APIs (5.4.8).
...
- **`cacheTag` — the named handle.** `cacheTag('product-catalog')` inside a `use cache` function attaches the named tag to that cache entry. The tag is the address the invalidation API (5.4.6) uses to mark the entry stale or replaced. A function can attach multiple tags; an entry can be invalidated by any of them.
- **Tag naming — the senior shape.** Tags are strings. The senior pattern is `entity-type` for collections (`invoices`, `products`) and `entity-type:id` for individual records (`invoice:42`, `product:abc`). The reason: the mutation surface in 5.4.6 needs to express both "invalidate one record" and "invalidate the list that contains it," and the tag namespace is the way to do both.
...
- The `use cache` directive itself (5.4.3).
- `revalidateTag`, `updateTag`, `revalidatePath`, `router.refresh` — the invalidation surface (5.4.6).
- React `cache()` for per-request memoization (5.4.5).
- Cache backends and self-hosting (named in 5.4.3).
- Webhooks as invalidation triggers — Chapter 12.1.

Estimated student time: 45 to 55 minutes. Pairs with 5.4.3 as the two-part anatomy of a cached function.
...
- The `use cache` directive (5.4.3).
- `cacheLife` and `cacheTag` (5.4.4).
- The invalidation surface (5.4.6).
- Async request APIs that `cache()` is often paired with (5.4.8).
- Client-side state and `useMemo` (Chapter 4.8).
...
- **The post-mutation pattern — the canonical Server Action shape.** Pseudo-shape: validate input, write to the database, call `updateTag('invoices')` and `updateTag(\`invoice:${id}\`)`, redirect to the list. The user lands on the list, the cached list reads fresh, the user sees their change. Chapter 7.2 owns the full Server Action surface; this lesson names the invalidation slot.
...
- **What fires after a webhook — the asymmetric pattern.** A Stripe webhook updates a subscription. The user isn't in the loop — they were redirected to a Stripe-hosted page or notified by email. `revalidateTag('subscription:...')` is the right reach: the next time the user loads their billing page, they see the fresh state, and the stale read in the meantime is acceptable. Chapter 12.1 owns webhooks proper; this lesson sets the timing pattern.
- **What `router.refresh` does not do.** It does not invalidate any cached entries. It does not re-fetch on a different URL. It does not work from a Server Component (no router there). It's the client tool for "I changed local state, re-pull the server side." Common pairing: a "Mark all read" button in a Client Component triggers a Server Action that calls `revalidateTag('notifications')`, then calls `router.refresh()` so the route re-pulls.
...
- `cacheTag` and `cacheLife` (5.4.4).
- Server Actions in full — Chapter 7.2.
- Webhook ingestion patterns — Chapter 12.1.
- Optimistic UI with `useOptimistic` — Chapter 7.3.6.
- Path-based static generation — Chapter 5.6.11.
- Form-with-action wiring — Chapter 7.3.
...
- **`searchParams` — the URL query.** Same shape as `params` — `props.searchParams` is a Promise of `Record<string, string | string[] | undefined>`. Reading `searchParams` is the canonical URL-state pattern (5.5.4). The shape allows arrays (for repeated keys) and undefined (for missing keys). Validation with Zod is standard.
...
- **The Client-Component unwrap — `React.use(promise)`.** A Client Component that needs `params` or `searchParams` receives the Promise as a prop and unwraps with `React.use()`. Same shape as the streaming-Promise pattern in 5.2.4. The Client Component suspends until the Promise resolves; wrap the consumer in `<Suspense>` for the boundary.
...
- The `cookies()` and `headers()` server-side reads in full — Chapter 5.5.1.
- URL state patterns at depth — Chapter 5.5.4.
- Client-side navigation hooks (`useRouter`, `usePathname`, etc.) — Chapter 5.5.5.
- `proxy.ts` and request-level rewrites — Chapter 5.5.2.
- The Zod validation pattern in full — Chapter 7.1.
- `generateStaticParams` and SSG — Chapter 5.6.11.
