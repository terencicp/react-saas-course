...
Chapter 9.5 cashes in everything Unit 9 installed: the conceptual ground (9.1) — authn versus authz, server-stored opaque sessions in `__Host-`-prefixed cookies, the OAuth/PKCE shape named for downstream; the running Better Auth setup (9.2) — the `auth` instance, the catch-all handler, the Drizzle adapter and four canonical tables (`user`, `session`, `account`, `verification`), the session config with `freshAge` and the cookie cache, the `getCurrentUser()` / `requireUser()` helpers; the email+password sign-in surface (9.3.1–9.3.3) — `signUp.email`, `signIn.email`, the email-verification gate via `requireEmailVerification`; the request-time gate (9.4.1) — the two-layer pattern with `proxy.ts` doing cookie-presence redirects and the layout doing the validating read. The student ships the canonical first auth flow: sign up with email and password, receive a verification email through the Unit 8 Resend pipeline, click the link to verify, sign in, land on a protected `/dashboard`, sign out and watch the `session` row disappear. Nothing more — no OAuth, no passkeys, no 2FA, no password reset (those live in 9.3.4–9.3.8 chapter material; the project deliberately stays in the email+password lane).

Threads that run through every lesson: Better Auth's API is consumed directly per Principle #5 — the only thin modules are `lib/auth.ts` (server) and `lib/auth-client.ts` (browser); the catch-all route handler at `/api/auth/[...all]/route.ts` is the only auth route file the project writes — never per-action route files; the `nextCookies()` plugin is non-negotiable in Next.js 16 (without it, sign-up succeeds server-side but the cookie never reaches the browser — the canonical silent failure); the cookie defaults from 9.1.2 are honored at the call site (`__Host-`-prefixed, `HttpOnly`, `Secure`, `SameSite=Lax`); the verification email rides the existing Unit 8 send pipeline — the project does not re-implement Resend, only the `WelcomeVerification.tsx` React Email template and the `sendVerificationEmail` callback; `requireEmailVerification: true` is the senior reflex — unverified users cannot sign in, full stop, no half-state; the `proxy.ts` gate does cookie-presence only and the layout does the validating read (the 9.4.1 rule honored); sign-out is a Server Action that calls `auth.api.signOut`, and the session row's absence is the verification. Five build/walkthrough lessons plus a verify lesson, each ending on a runnable state.
...
- **From 9.4.1:** The two-layer gate — `proxy.ts` for cookie-presence redirects (`getSessionCookie` from `better-auth/cookies`), the layout for the validating read (`requireUser()`), the matcher pattern, the `?next=` round-trip with open-redirect closure, the inverse gate (signed-in users bounced from `/sign-in`).
...
- Authn vs. authz; sessions as opaque server-stored handles; `__Host-` cookie defaults — 9.1.1, 9.1.2.
- `betterAuth({ ... })` config, the catch-all handler, the `authClient`, `'server-only'` on `lib/auth.ts` — 9.2.1.
- Drizzle adapter, the four canonical tables (`user`, `session`, `account`, `verification`), Better Auth CLI `generate` — 9.2.2.
- Session config (`expiresIn`, `updateAge`, `freshAge`, `cookieCache`), cookie surface tuning — 9.2.3.
- `auth.api.getSession({ headers })`, `getCurrentUser`, `requireUser` — 9.2.4.
- `signUp.email` / `signIn.email`, `emailAndPassword` config, `requireEmailVerification`, `autoSignIn`, `minPasswordLength` — 9.3.1, 9.3.2.
- `emailVerification` config block, `sendVerificationEmail` callback, `autoSignInAfterVerification`, the `verification` table token row — 9.3.3.
- The two-layer gate — `proxy.ts` cookie-presence + layout validating read, `?next=` round-trip with open-redirect closure, inverse gate — 9.4.1.
- React Email template composition, transactional send through Resend with verified domain — 8.2, 8.3.
- Server Actions with `useActionState`, Zod at the action boundary, the `Result` discriminant — 7.2.
- Zod-validated env via `lib/env.ts` — 5.6.1.
- The `nextCookies()` plugin requirement in Next.js 16 — 9.2.1.
...
- Name the scope cut: no OAuth providers (9.3.8 chapter material), no passkeys (9.3.7), no 2FA (9.3.6), no magic links (9.3.5), no password reset (9.3.4), no account linking (9.3.9), no rate limiting (Unit 15.3 owns), no organization scoping (Unit 10 owns), no audit log (Unit 10.2). The project deliberately stays in the email+password lane.
...
- Link the starter via `degit react-saas-course-projects/9.5-email-password-auth/starter`.

Senior calls and watch-outs:

- The verified domain from 8.3 is non-negotiable. Resend's sandbox sender bounces verification emails into spam folders and the verification flow looks broken for reasons that have nothing to do with the code. The 8.3 prereq paragraph flagged this; reconfirm before starting.
...
- Walk the file tree, calling out provided vs. stubbed. Linger on five files:
  ...
  - `src/lib/email.ts` (carry-in from 8.3, provided; the `sendEmail({ to, subject, react })` signature the student calls from the verification callback).
...
- Read `src/lib/env.ts` — confirm `DATABASE_URL`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `RESEND_API_KEY`, `EMAIL_FROM`, `NEXT_PUBLIC_APP_URL` are all parsed with Zod. The senior reflex: any env entry the project depends on is in `lib/env.ts`, never `process.env.X` at the call site. From 5.6.1.
...
- Walk the Resend verification: send a test email via `sendEmail` to the student's own address using the existing `WelcomeEmail.tsx` from 8.3 (a small one-liner in a `scripts/smoke-email.ts` provided in the starter) — confirms DKIM/SPF pass and the verified domain still works. This is the 8.3 carry-in proven, not re-taught.

Senior calls and watch-outs:

- `BETTER_AUTH_SECRET` must be at least 32 bytes from a CSPRNG. Generated via `openssl rand -base64 32` and stored in `.env` (gitignored). Reusing across environments is the failure mode from 9.2.1.
...
- Fill `src/lib/auth.ts`:
  - `import 'server-only';` first line (the 9.2.1 reflex).
  ...
  - Session config: `session: { expiresIn: 60 * 60 * 24 * 30, updateAge: 60 * 60 * 24, freshAge: 60 * 60 * 24, cookieCache: { enabled: true, maxAge: 5 * 60 } }` — 30-day expiry, 1-day sliding renewal, 1-day fresh window, 5-minute cookie cache (the 9.2.3 defaults).
  - `advanced: { cookiePrefix: '__Host-', useSecureCookies: process.env.NODE_ENV === 'production' }` — `__Host-` prefix from 9.1.2; `Secure` flips off only in localhost dev (the browser refuses `Secure` cookies on `http://localhost` otherwise).
- Run `pnpm auth:generate` — the Better Auth CLI reads `lib/auth.ts`, knows Drizzle + Postgres, and writes `src/db/schema/auth.ts` with `user`, `session`, `account`, `verification`. Read the generated file column by column (revisit 9.2.2's walkthrough). Commit it.
...
- `nextCookies()` is the line that prevents the silent-failure pattern from 9.2.1. Forgetting it means the `signUp.email` server-side succeeds but the `Set-Cookie` header doesn't make it onto the Server Action response, and the user is "signed up but not signed in" with no indication of the bug.
- `requireEmailVerification: false` here is *only* for this lesson's smoke test. The next lesson flips it to `true` and a regression would mean anyone who typed a stranger's email controls that stranger's account. Flagged with a `// TODO: flip to true in 9.5.4` comment in the code itself.
- `minPasswordLength: 12` is the senior default in 2026, not 8. The library accepts 8 by default; 12 is the call.
- The catch-all `[...all]` segment is *not* optional. Writing `/api/auth/sign-up/route.ts` instead and trying to route manually breaks every endpoint the library expects to expose. The 9.2.1 rule honored.
...
- The verification email goes through *the same* `sendEmail` from 8.3. No re-implementation. The `react` field accepts the React Email template directly. The `email_suppressions` read inside `sendEmail` still applies — a suppressed verification address returns `{ ok: false }` and the sign-up still creates the user but the email never sent; the resend button on `/verify-email` is the user's escape hatch in that case.
...
- Returning the same opaque `'invalid-credentials'` for both "email not found" and "wrong password" closes the enumeration vector (9.3.2's rule). Returning `'email-not-verified'` as a distinct reason is fine — that information leaks only after the password matched, which is itself the proof of identity.
...
- The `?next=` sanitization must reject `//evil.com` (protocol-relative URLs that the browser resolves to `https://evil.com`) and `javascript:` and any absolute URL. The `sanitizeNext` helper is a small allowlist function — paths starting with `/` and not `//`, no colons. From the 9.4.1 watch-out list.
...
- Fill `src/app/(protected)/sign-out-action.ts`:
  ...
  - `redirect('/sign-in');`
  - The library clears the `__Host-better-auth.session_token` cookie and deletes the `session` row in one call (per 9.2.4 / 9.3 closeout). The student verifies the row's absence in Studio after sign-out.
- Smoke test:
...
- The proxy reads *only* the cookie. Any reach for `auth.api.getSession` inside `proxy.ts` collapses the two-layer model — the 9.4.1 rule honored at the call site. Inspect the imports in `proxy.ts` to confirm: `getSessionCookie` only, never `auth`.
...
- Adversarial probes:
  ...
  - Open two browsers signed in as the same user; sign out from browser A; refresh browser B; redirected to `/sign-in` because the `session` row is gone (cookie cache might briefly mask this within the 5-min window — name the trade from 9.4.3).
...
- Name the senior calls one more time:
  ...
  - Sign-out deletes the session row — the opaque-server-session model from 9.1.2 made concrete; the row's absence is the revocation.
- Forward references:
  - Chapter 9.3 (chapter material) layered passwords, magic links, TOTP, passkeys, OAuth providers on this same setup; none of them required re-shaping the schema.
  - Chapter 9.4.2 added credential changes and the elevation tier; the `freshAge` knob configured in 9.5.3 is what the change-password and change-email actions read.
  - Chapter 9.4.3 added the active-sessions list; the `ipAddress` / `userAgent` columns on `session` populated automatically.
  - Chapter 9.4.4 named the CSRF and XSS defaults that this setup already encoded — `SameSite=Lax` on the cookie + Server Actions origin check + React 19 auto-escape.
  ...
  - Unit 15.3 adds Upstash rate limiting to the sign-in, sign-up, and verify-resend endpoints — non-negotiable the moment this project ships to a public URL.
