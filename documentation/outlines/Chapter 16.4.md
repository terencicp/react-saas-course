...
- **From 11.3:** the toolbar / table / pagination pattern reused in the starter's `app/(app)/customers/page.tsx`, which is provided as a thin clone of 11.3's invoices surface.
- **From 10.4:** `tenantDb(orgId)`, `authedAction(role, schema, fn)`, the active-org session slot, `logAudit(tx, event)`.
- **From 6.6:** `customers` table (`id uuid pk`, `organizationId uuid fk`, `name`, `email`, `createdAt`). 16.4 ships a migration that adds `phone`, `line1`, `line2`, `city`, `region`, `postalCode`, `country`, `taxId`, `paymentTerms`, `defaultCurrency`, `language`, `notificationChannels jsonb` to match the four-slice payload.
...
src/
  db/
    schema.ts                   # provided: 6.6 customers table + 16.4 migration
...
- Read the `customers` table columns — 6.6's narrow row (`id`, `organizationId`, `name`, `email`, `createdAt`) extended by a 16.4 migration that adds `phone`, `line1`, `line2`, `city`, `region`, `postalCode`, `country`, `taxId`, `paymentTerms`, `defaultCurrency`, `language`, `notificationChannels jsonb` so the four-slice payload maps column-for-column. The action maps `{ contact: {firstName, lastName} }` into `name: \`${firstName} ${lastName}\`` plus email / phone / billing / preferences columns directly.
