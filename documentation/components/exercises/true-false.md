# `TrueFalse` + `Statement` + `TfWhy`

True/False drill played as a round. Each `<Statement>` carries an `answer` of `'true'` or `'false'` and an optional `<TfWhy>` explanation. Statements **shuffle** on each round; the student answers one at a time, watching pip-progress at the top. After the last card, a score panel and a collapsible card-by-card review appear, with each statement's `<TfWhy>` surfaced on the review row.
...
- **`TfWhy` default** — explanation rendered under the statement in the review. Markdown works.
...
Round of three with custom instructions and inline `<TfWhy>` on each card:
...
<TrueFalse instructions="Each claim is about how React's rendering model behaves under the hood.">
  <Statement answer="true">
    ...
    <TfWhy>React bails out of the update if the new state equals the old by `Object.is`. Useful for primitives — but for objects, "same reference" is what counts, so a fresh object with identical fields *will* re-render.</TfWhy>
  </Statement>

  <Statement answer="false">
    A component re-renders only when its own state or props change.
    <TfWhy>It also re-renders when its parent re-renders, regardless of whether props changed — unless wrapped in `React.memo`.</TfWhy>
  </Statement>

  <Statement answer="true">
    `useEffect` callbacks run *after* the browser has painted the new DOM.
    <TfWhy>Effects are deferred so they don't block paint. If you need to read layout before paint, use `useLayoutEffect`.</TfWhy>
  </Statement>
</TrueFalse>
````
...
<TrueFalse>
  <Statement answer="true">
    ...
    <TfWhy>Control-flow analysis narrows the union based on the guard. Outside the branch, `x` is back to `string | number`.</TfWhy>
  </Statement>

  <Statement answer="false">
    `Array.prototype.sort()` returns a new array and leaves the original untouched.
    <TfWhy>It sorts in place *and* returns the same array reference. Use `arr.toSorted()` (ES2023) when you want a non-mutating copy.</TfWhy>
  </Statement>
</TrueFalse>
````
