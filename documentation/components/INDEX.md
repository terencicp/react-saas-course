...
| `TSPlaygroundCallout` | Embedding a TypeScript Playground with either a pre-computed share URL or a raw `code` + `flags` pair that gets LZ-compressed at build time. | [ts-playground-callout.md](./embeds/ts-playground-callout.md) |
