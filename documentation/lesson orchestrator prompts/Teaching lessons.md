...
2 **lesson-writer**: Prompt this subagent only with the lesson number and title, the lesson outline path, and the path of the `src/content/docs/<X> <Unit name>/<X.Y> <Chapter name>` folder. After finishing, it will return the path of the new lesson file, and how many diagrams and exercises it contains and their unique ids.
...
7 **lesson-reviewer**: Prompt this subagent only with the lesson outline path and the MDX path. Surface any issues found in the chat.
