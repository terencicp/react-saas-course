...
- Form errors: `z.treeifyError(result.error)` returns a form-shaped tree. `z.flattenError(result.error)` returns a `{ formErrors, fieldErrors }` shape. Pick one per project; the course uses `treeifyError`.
...
- `drizzle-zod`'s `createSelectSchema`, `createInsertSchema`, and `createUpdateSchema` are the canonical way to derive validators from a Drizzle table. Per-column override map adds refinements (`email`, `url`, length caps) on top of the inferred shape. Hand-writing a parallel Zod schema for a table is a smell. Today this is the standalone `drizzle-zod` package; the v1 plan moves the same helpers into `drizzle-orm/zod` (one-line import change when the move lands).
...
- `casing: 'snake_case'` in **both** `drizzle.config.ts` (for the kit — introspect, migrations) and the runtime client (`drizzle(url, { casing: 'snake_case' })` — for query translation). TS code reads `createdAt`; SQL is `created_at`. Setting only one is a silent footgun.
...
- **Migration file naming.** Drizzle generates `NNNN_<random>.sql`. Rename to `NNNN_<verb>_<noun>.sql` (`0007_add_org_members_unique.sql`, `0014_backfill_invoice_customer_fk.sql`) before commit. The diff list is unreadable otherwise. Review every generated migration; never let an unread file ship.
...
- **Linter / formatter:** Biome 2.x as the single binary. The canonical `biome.json` lives in `configs/`. Biome covers the React Hooks rules (`useExhaustiveDependencies`, `useHookAtTopLevel`). `useExhaustiveDependencies` has minor behavioral gaps vs. `eslint-plugin-react-hooks` for some `useEffect` dependency edge cases — for most apps the gap is academic, but pin the canonical ESLint rule alongside Biome if you hit a specific case. ESLint is **not** installed unless a project genuinely needs a rule Biome hasn't ported — name the rule and the reason in `AGENTS.md` when you add it.
