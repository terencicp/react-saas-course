...
- **`lint/suspicious/noAssignInExpressions` rejects the `globalThis` lazy-init idiom** — the common `const store = (globalThis.__x ??= { … }).foo` (a module-singleton survival pattern for in-memory stores across the RSC read / Server Action write graphs) uses an assignment as an expression, which this default-on recommended rule errors on, failing `biome ci`. Fix: split it — `globalThis.__x ??= { … };` as its own statement, then read on the next line (`const store = globalThis.__x;`). Not auto-fixable.
...
## Zustand 5 (vanilla `createStore` + per-request provider)

- **A selector returning a fresh object/array hard-crashes under SSR, not just over-renders** — `useStore(api, selector)` runs the selector inside React's `useSyncExternalStore`; a selector that builds a new reference each call (e.g. a derived `selectErrors` doing `safeParse(...).flatten().fieldErrors`, or a bare `(s) => ({ a: s.a })` pick) fails the snapshot-stability check and throws "The result of getSnapshot should be cached to avoid an infinite loop" — the Next dev overlay shows *This page couldn't load* (a render crash, not a console warning). Fix: read a **primitive** per consumer (`useStore(api, (s) => selectErrors(s).field?.[0])` → `string | undefined`), or wrap the object selector in `useShallow` from `zustand/react/shallow`. Plans mandating "atomic selectors" must extend that rule to every derived object/array selector, not only whole-slice picks.
- **Composing slices: type the combiner as `StateCreator`, not `Parameters<typeof createStore<T>>`** — `Parameters<...>` over `createStore<T>` resolves to `undefined`/`never` (the factory arg is optional in the overload), so a `composeSlices(...a: Parameters<...>)` does not compile. Type the combiner itself `const composeSlices: StateCreator<T, [], [], T> = (...a) => ({ …createSliceA(...a), … })`; then `...a` is the real `(set, get, store)` tuple every slice receives and `createStore<T>()(composeSlices)` typechecks.
- **The provider ref/Context hold the store *handle*, not the state** — `createWizardStore()` returns `StoreApi<T>` (call it `WizardStoreApi = ReturnType<typeof createWizardStore>`), so the `useRef`/`createContext` must be typed `WizardStoreApi | null`, never `T | null`; mistyping them as the state object breaks `useStore`/`subscribe`. A `useWizardStore<U>(selector)` hook that needs to select store-level members (e.g. a store-wide `reset`) must type its `selector` on the **full store type** (the one carrying `reset`), not a narrower read-only `State` projection — a `(s: State) => U` selector cannot select `s.reset`. Read-only `State`-typed selectors still pass into a `Store`-typed parameter by contravariance.
