...
- **Server-side data fetching in the body.** Direct `await db.query(...)`, `await fetch(...)`, `await fs.readFile(...)`. The senior reflex: data lives next to where it renders, no loader functions, no `getServerSideProps`. The Next.js 16 fetch cache and React `cache()` are named in context; Chapter 032 owns caching.
...
- **Where the boundary lives.** The student's job is to keep the Server Component tree wide and the Client Component leaves narrow. A page is Server, its layout shell is Server, the interactive `<DatePicker />` leaf is Client. Full Principle in lesson 3 of chapter 030.
- **The RSC payload — named once.** What the server sends back is not HTML alone; it's HTML plus a serialized React tree the client uses to reconcile. Full surface in lesson 4 of chapter 030.
- **Watch-outs.** No hooks; `async` is fine in a Server Component but not in a Client Component (use `React.use()` instead); reading cookies/headers/searchParams is async in Next.js 16 (lesson 8 of chapter 032); importing a Node-only module accidentally into a Client Component crashes the build (lesson 3 of chapter 030 owns the structural fix); secrets in a Server Component are safe but secrets *passed as props* to a Client Component leak — the RSC wire is visible in network DevTools.

What this lesson does not cover:

- Client Components and the boundary contract (lesson 2 of chapter 030).
- The `"use client"` and `"use server"` directives in depth (lesson 3 of chapter 030).
- `server-only` / `client-only` packages (lesson 3 of chapter 030).
- What crosses the RSC wire and serialization rules (lesson 4 of chapter 030).
- Hydration (lesson 5 of chapter 030).
- Suspense and streaming — Chapter 031.
- Caching, `use cache`, `cacheLife`, `cacheTag` — Chapter 032.
- Async Request APIs (`cookies()`, `headers()`, `params`, `searchParams`) at depth — lesson 8 of chapter 032 and lesson 1 of chapter 033.
- Server Actions and `"use server"` for mutations — Chapter 043.

Estimated student time: 55 to 70 minutes. Load-bearing for every page in Chapter 031 onward, every data-fetching pattern in Unit 5, and every server-rendered surface in the course.
...
- The full `"use client"` and `"use server"` directive semantics, edge cases, and bundler behavior (lesson 3 of chapter 030).
- Architectural Principle #6 (lesson 3 of chapter 030).
- `server-only` / `client-only` packages and structural enforcement (lesson 3 of chapter 030).
- The exact list of types that survive RSC serialization (lesson 4 of chapter 030).
- Hydration mechanics and mismatch causes (lesson 5 of chapter 030).
- Suspense and `loading.tsx` for client transitions — Chapter 031.

Estimated student time: 55 to 70 minutes. Load-bearing for every interactive component in the course, the form patterns in Unit 6, and the auth flows in Unit 8.
...
- **Watch-outs.** Directive must be the first non-comment line; typos in directives fail silently; `"use server"` does not make a file a Server Component (Server Components are the default everywhere); `"use server"` exports are RPC endpoints — anyone who can reach them can call them, validate inputs (Chapter 043); `server-only` and `'use server'` solve different problems; Context providers are Client-bounded — they need `"use client"`; importing a `"use client"` file from a Server Component is fine, importing it from another `"use client"` file is fine (the directive is a one-way door); third-party packages without directives sometimes need a wrapper file; the build error from `server-only` is the early warning system — don't suppress it.

What this lesson does not cover:

- The full Server Actions surface (`'use server'` as RPC, validation, errors, redirects) — Chapter 043.
- The exact serialization rules at the boundary (lesson 4 of chapter 030).
- Hydration (lesson 5 of chapter 030).
- Async Request APIs at depth — lesson 8 of chapter 032.
- Route handlers as the alternative to Server Actions — Chapter 046.

Estimated student time: 50 to 65 minutes. Load-bearing for every Server Action in Unit 6, every server-only helper in Units 5 and 8, and the security baseline in Chapter 081.
...
- **Watch-outs.** "Cannot pass function" is the canonical first error; convert to a Server Action or move the handler inside the Client Component; class instances fail silently or with a confusing message — flatten to plain objects; Errors don't fully cross — use error boundaries (Chapter 031) and `error.tsx` for the user-visible surface; the RSC payload is visible in DevTools — treat it like any other network response; keep props narrow, both for security and for wire size; large server-rendered trees in props are still wire bytes; Promise props plus `React.use()` is the streaming pattern (Chapter 031); `Date` is fine, `dayjs.Dayjs` is a class instance and fails — pass the raw `Date` (Chapter 083).

What this lesson does not cover:

- Server Actions in full — Chapter 043.
- Suspense, `React.use()`, streaming — Chapter 031.
- The full caching model — Chapter 032.
- Error handling and error boundaries — Chapter 031 and chapter 080.
- The structured clone API itself in browser contexts (`postMessage`, `IndexedDB`) — Chapter 016 in passing.
- Hydration (lesson 5 of chapter 030).

Estimated student time: 50 to 65 minutes. Load-bearing for every Server-to-Client prop in the course, the Promise-streaming pattern in Chapter 031, and the data shape decisions in Unit 5.
...
- Suspense, streaming, and how server-rendered async content arrives — Chapter 031.
- `error.tsx` and `global-error.tsx` — Chapter 031.
- Static vs. dynamic rendering and Cache Components — Chapter 032.
- Time, timezone, and locale handling in depth — Chapters 087 and chapter 084.
- `useId` semantics beyond hydration (form/ARIA at depth) — Chapter 024.

Estimated student time: 50 to 60 minutes. Load-bearing for every Client Component in the course, the form patterns in Unit 6, and the time/locale patterns in Unit 17.
