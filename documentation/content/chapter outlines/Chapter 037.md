...
Chapter 037 turns the relational model from chapter 036 into a Drizzle schema file, the artifact the rest of the course derives from. By the end the student writes a `db/schema.ts` for a small SaaS domain — organizations, invoices, line items, tags — with the right columns, the right types, the right constraints, and the right relationship graph; runs no migrations yet (Chapter 040 owns Drizzle Kit); writes no queries yet (Chapter 038 owns the data-access surface); and can hand the file's `$inferSelect` types to a Server Component, a Server Action, or a Zod schema (Unit 6) without restating the shape. The chapter installs Architectural Principle #2 — the schema is the source of truth — at lesson one and cashes it in at the last teaching lesson when the inferred types replace every hand-written row interface in the codebase.

Threads that run through every lesson: Drizzle is the only data-access layer the course teaches; `db/schema.ts` is the single source of truth — column names, types, defaults, and constraints flow from this file into migrations (chapter 040), queries (chapter 038), Zod via drizzle-zod (lesson 8 of chapter 042), and RLS (chapter 056); TS camelCase maps to SQL snake_case via Drizzle's `casing: 'snake_case'` config; `timestamptz` with `defaultNow()` is the only timestamp type; UUIDv7 via Postgres 18's built-in `uuidv7()` is the default PK for user-facing entities, `bigint generatedAlwaysAsIdentity` is the reach for high-volume internal tables; FKs declared column-level via `references(() => other.id, { onDelete: 'cascade' })`, with the cascade rule a deliberate per-relationship decision; NOT NULL is the senior default, nullability is opt-in; the Drizzle Relations v2 API (`defineRelations`) sits next to the schema, separate from FK declaration, and feeds the relational query API in lesson 3 of chapter 038; `$inferSelect` / `$inferInsert` are the canonical row types — a hand-typed `Invoice` interface is the smell that says Principle #2 was skipped. The chapter ships ten teaching lessons plus a quiz, ordered by dependency: principle, schema file anatomy, types, column modifiers, primary keys, foreign keys, constraints, junction tables, the relations API, and inferred types.
...
- **Principle #2 stated.** The schema is the source of truth for every typed shape downstream. Row types from `$inferSelect`; insert types from `$inferInsert`; Zod validators from drizzle-zod (lesson 8 of chapter 042); form field sets from the same Zod; RLS policies (chapter 056) reference the same column names. One file changes, every downstream layer's type checker catches the drift.
...
- **Where the principle bites later.** Named ahead, not taught: chapter 038 queries return `$inferSelect` types; lesson 8 of chapter 042 drizzle-zod turns schema into Zod; chapter 043 Server Actions parse with that Zod; chapter 047 forms read the Zod; chapter 056 RLS reads column names; 16 API contracts derive from the schema.
...
- **The order of operations.** Change the column in `db/schema.ts` first; generate the migration (lesson 1 of chapter 040); the type checker walks the codebase and surfaces every consumer; ship.
...
- The `pgTable` API surface and the schema file's anatomy — lesson 2 of chapter 037.
- drizzle-zod and the schema-to-Zod pipeline — lesson 8 of chapter 042.
- `$inferSelect` / `$inferInsert` mechanics — lesson 10 of chapter 037.
- RLS policies reading the schema — Chapter 056.
...
- **The `db/` folder layout.** `db/schema.ts` for tables, `db/relations.ts` for the Relations v2 declarations (lesson 9 of chapter 037), `db/index.ts` for the `db` client (wired in chapter 040 setup). Lives at project root — schema is shared by every feature.
...
- **The `logger` option.** `drizzle({ casing, logger: true })` is the opt-in flag that logs every emitted SQL statement to stdout — invaluable for N+1 diagnosis (lesson 2 of chapter 039) and the EXPLAIN-copy reflex (lesson 3 of chapter 039); off by default, dev-only.
- **Naming convention for tables.** Plural snake_case (`organizations`, `invoice_line_items`); the exported `const` matches.
- **Where the file's exports flow.** `db/schema.ts` exports tables; `db/relations.ts` imports them; `db/index.ts` re-exports as a namespace for the Drizzle client; Drizzle Kit (lesson 1 of chapter 040) reads the file for migrations; drizzle-zod (lesson 8 of chapter 042) reads the same exports.
...
- **Watch-outs:** mixing the SQL-name argument with `casing` config silently drifts between tables — pick one; bare `text()` is nullable (lesson 4 of chapter 037 fixes the default); Drizzle Kit treats the schema file as input — unexported tables do not exist for migrations.

What this lesson does not cover:

- Specific Postgres data types and which to reach for — lesson 3 of chapter 037.
- Column modifiers (`.notNull()`, `.default()`, generated columns) — lesson 4 of chapter 037.
- Primary keys, foreign keys, unique/check constraints — Lessons lesson 5 of chapter 037–lesson 7 of chapter 037.
- Many-to-many junction tables — lesson 8 of chapter 037.
- The Relations v2 API — lesson 9 of chapter 037.
- The `db` client wiring, `drizzle({ casing })`, `db/index.ts` — Chapter 040 setup walkthrough.
- Drizzle Kit migrations from the schema — lesson 1 of chapter 040.
...
- **UUID.** `uuid` for surrogate keys; `.defaultRandom()` generates UUIDv4, Postgres 18's `uuidv7()` is the senior reach for time-sortable keys (lesson 5 of chapter 037 owns the decision). Drizzle maps `uuid` to a string.
...
- **Geographic, full-text, binary — named, deferred.** `point`/`polygon`/`geography` (PostGIS, out of scope); `tsvector` + generated columns (lesson 8 of chapter 038); `bytea` (the course stores binary in R2, keeps a URL).
...
- Column modifiers (`.notNull()`, `.default()`, generated columns) — lesson 4 of chapter 037.
- Primary keys and the UUIDv7 vs. bigserial decision at depth — lesson 5 of chapter 037.
- `jsonb` query syntax (`->`, `->>`, `@>`) — lesson 9 of chapter 038.
- Full-text search columns — lesson 8 of chapter 038.
- Zod validation of payload shapes at the boundary — Chapter 042.
- Object storage for binary — Chapter 14.

Estimated student time: 45 to 55 minutes. Load-bearing for every column the student writes in this chapter, in chapter 041's project, and in every later schema.
...
- **The Zod boundary parallel.** Same default at the validation layer (chapter 042) — `.optional()` is opt-in. drizzle-zod (lesson 8 of chapter 042) reads NOT NULL from the schema and emits the matching required/optional Zod field.
...
- **The `updatedAt` pattern.** `timestamp({ withTimezone: true }).defaultNow().notNull().$onUpdate(() => new Date())`. `$onUpdate` runs at the Drizzle layer — direct SQL bypasses it. Prod-grade reach: a Postgres trigger (named once; chapter 040 setup adds it as a one-time migration).
...
- **Generated columns — `.generatedAlwaysAs(…)`.** A column Postgres computes from other columns. `STORED` (persisted, indexable, costs disk) is the production default; `VIRTUAL` (Postgres 18, computed on read) is named but newer. Senior reaches: lowercased email for case-insensitive uniqueness; `tsvector` for full-text (lesson 8 of chapter 038); derived totals. Trade-off: read-only — inserts can't supply it; updating inputs recomputes.
- **Worked example — case-insensitive email.** `email text not null` plus `emailLowercased text generatedAlwaysAs(sql\`lower(email)\`)`. A unique index on `emailLowercased` (lesson 7 of chapter 037) enforces case-insensitive uniqueness without application code.
...
- Primary-key generation specifically — lesson 5 of chapter 037.
- Foreign keys and cascade — lesson 6 of chapter 037.
- UNIQUE indexes that pair with generated columns — lesson 7 of chapter 037.
- `$inferInsert` type behavior with defaults and generated columns — lesson 10 of chapter 037.
- Postgres triggers for `updatedAt` — Chapter 040 setup mentions, not in this chapter.
- Full-text `tsvector` generated columns — lesson 8 of chapter 038.
...
- **Composite primary keys.** Junction tables and only them: `primaryKey({ columns: [t.invoiceId, t.tagId] })` in `pgTable`'s third argument. Full pattern in lesson 8 of chapter 037; mechanic introduced here so the next two lessons can use it.
...
- Foreign keys, `references()`, and cascade — lesson 6 of chapter 037.
- UNIQUE constraints that aren't primary keys — lesson 7 of chapter 037.
- Junction tables at depth — lesson 8 of chapter 037.
- The `db.transaction` shape for monotonic ID assignment — lesson 4 of chapter 039.
- Snowflake / KSUID / ULID variants — named once for recognition, not taught.

Estimated student time: 35 to 45 minutes. Load-bearing for the project schema in chapter 041 and every later table the student creates.
...
- **What an FK buys.** Postgres rejects writes pointing at a non-existent parent; column types must match the PK type exactly; an index on the FK column is not automatic (lesson 1 of chapter 039 owns indexing).
...
- **Worked example — invoices' four relationships.** `invoices.organizationId` → `organizations.id` (`restrict`); `invoice_line_items.invoiceId` → `invoices.id` (`cascade`); `invoices.createdById` → `users.id` (`set null`); `invoice_tags.*` (cascade on both — lesson 8 of chapter 037 owns).
- **Multi-tenancy foreshadowed.** Every tenant-owned table carries an `organizationId` FK with `restrict`; tenancy enforcement at the query layer is Unit 9.
- **Watch-outs:** missing `onDelete` means `NO ACTION` — the app handles cleanup every time, in every code path; cascade can wipe more than expected when relationship graphs branch — model first; missing FK indexes make cascade deletes table scans (lesson 1 of chapter 039); circular FKs need deferrable constraints — the course breaks cycles by structure instead.

What this lesson does not cover:

- UNIQUE and CHECK constraints — lesson 7 of chapter 037.
- Junction tables (the canonical many-to-many shape) — lesson 8 of chapter 037.
- Indexing foreign-key columns — lesson 1 of chapter 039.
- The Drizzle Relations v2 API (separate from `references()`) — lesson 9 of chapter 037.
- Soft-delete patterns and `deletedAt` filtering at depth — Chapter 039.
- Multi-tenancy scoping with `organizationId` — Unit 9.

Estimated student time: 40 to 50 minutes. Load-bearing for the project schema in chapter 041 and every later table that has a parent.
...
- **Partial unique indexes — `.where(…)`.** `uniqueIndex(...).on(...).where(sql\`…\`)` enforces uniqueness only for rows matching a predicate. Canonical use: "one primary contact per org where `isPrimary = true`." This is technically a unique index (lesson 1 of chapter 039 owns indexes), not a `UNIQUE` constraint — same correctness.
- **Case-insensitive uniqueness — generated-column pattern.** `emailLowercased` as STORED generated column (lesson 4 of chapter 037), then `.unique()` on it. Cleaner than `LOWER(email)` expression indexes; emits a real column the app can read.
...
- **The Zod parallel.** Length caps, ranges, regex — Zod (chapter 042) catches them at the API boundary with friendly errors; database `CHECK` catches them if Zod is bypassed. Both correct: database is the safety net, Zod is the user experience. drizzle-zod does not generate Zod from `CHECK` — that's hand-wired.
- **What these are not for.** Cross-row invariants ("revenue per org under quota") — application logic plus a transaction (lesson 4 of chapter 039); not constraints. Multi-table predicates — same.
...
- Many-to-many junction tables and their composite primary keys (which double as uniques) — lesson 8 of chapter 037.
- The Drizzle Relations v2 API — lesson 9 of chapter 037.
- Index strategy beyond what unique constraints add for free — lesson 1 of chapter 039.
- Transactions for cross-row invariants — lesson 4 of chapter 039.
- Zod refinement at the API boundary — Chapter 042.
- Error catching at the Server Action layer when a constraint fires — Chapter 043.

Estimated student time: 35 to 45 minutes. Load-bearing for the project schema in chapter 041 and every later table with multi-column invariants.
...
- **Worked examples.** `invoice_tags` as pure junction (two FKs + composite PK, no metadata). `memberships` as entity (surrogate `id`, `userId` + `organizationId` FKs, `role` enum, `joinedAt`, `unique(...)` on the pair) — foreshadowed for Unit 9 tenancy.
- **Relations v2 cliff-hanger.** The junction is data; lesson 9 of chapter 037 wires `db.query.invoices.findFirst({ with: { tags: true } })` to traverse it automatically.
...
- **Watch-outs:** three-or-more FK junctions are almost always two relationships hiding as one — split them; FK indexes aren't automatic — the composite PK indexes `(invoiceId, tagId)` and serves `WHERE invoiceId = …` but not `WHERE tagId = …`, so add the second index (lesson 1 of chapter 039); cascade on both is right for ownership but switch to `restrict` if either endpoint should block deletion while the relationship exists; adding `createdAt` is the trigger that says "promote to entity."

What this lesson does not cover:

- The Drizzle Relations v2 API and `db.query.…({ with: … })` traversal — lesson 9 of chapter 037.
- Indexing the second column of a junction — lesson 1 of chapter 039.
- Querying through junctions with explicit joins — lesson 2 of chapter 038 / lesson 3 of chapter 038.
- Multi-tenancy via memberships at depth — Unit 9.
...
Declares the TS-side traversal graph with `defineRelations` in `db/relations.ts` — one/many/through shapes — that enables `db.query.…({ with: … })` nested reads in lesson 3 of chapter 038.

Topics to cover:

- **The senior question.** FKs connect rows; how does the application traverse them? `references()` (lesson 6 of chapter 037) declares the database constraint but does not enable Drizzle's relational query API. The v2 declarative relations layer (`defineRelations(...)` in `db/relations.ts`) does, feeding `db.query.invoices.findFirst({ with: { lineItems: true, tags: true } })` in lesson 3 of chapter 038.
- **Two layers.** `db/schema.ts` says what's in the database; `db/relations.ts` says how the relational query API walks it. Postgres only sees the FKs from lesson 6 of chapter 037; the relations file is a TS-side traversal graph.
...
- **v2 vs. v1, why v2 only.** v1 (`relations(table, …)`) compiles but is deprecated. v2 is more compact, infers better types, and is what `db.query` (lesson 3 of chapter 038) reads; v1's query helper moved to `db._query` as compatibility.
- **Where the file plugs in.** `db/index.ts` wires `drizzle(connectionString, { schema, relations })` so the client sees both columns and traversal graph (chapter 040 setup ties it).
- **What lesson 3 of chapter 038 buys.** `db.query.invoices.findMany({ where: …, with: { lineItems: true, tags: true, organization: { columns: { name: true } } } })` returns a typed nested object — the join graph as data. Without `defineRelations`, the same result needs hand-written joins (lesson 2 of chapter 038). Both work; relational API is the senior reach for nested reads.
- **When to skip the relations layer.** Aggregates, complex predicates across joined tables, non-tree shapes — drop to `db.select(...).leftJoin(...)` (lesson 2 of chapter 038).
- **Watch-outs:** `references()` alone does not declare a relation — `db.query.…` returns undefined without `defineRelations`; both directions of a one-to-many usually declared so the next reader doesn't have to guess; v1 and v2 in one project produce inconsistent call shapes — pick one; the relational API plans its own SQL — large joins can surprise on performance, `EXPLAIN ANALYZE` (lesson 3 of chapter 039) is the diagnostic.

What this lesson does not cover:

- Actually writing queries through the relational API (`db.query.…`) — lesson 3 of chapter 038.
- Hand-written joins via `db.select(...).leftJoin(...)` — lesson 2 of chapter 038.
- The N+1 problem at the relations layer — lesson 2 of chapter 039.
- `db/index.ts` wiring of `drizzle(...)` with `schema` and `relations` — Chapter 040 setup.
- The v1 relations API at depth — the course teaches v2 only.

Estimated student time: 40 to 50 minutes. Load-bearing for lesson 3 of chapter 038 (the relational query API), lesson 2 of chapter 039 (N+1 at this layer), and every nested read in Units 6 through 23.
...
- **Prop pattern.** Page reads `Invoice[]` from a query, hands it to a Client Component whose prop type is `Invoice[]`. One type, one source. Updates accept `Partial<NewInvoice>`; the matching Zod via drizzle-zod (lesson 8 of chapter 042) ships from the same table — every layer aligns.
- **drizzle-zod foreshadowed.** lesson 8 of chapter 042 owns `createInsertSchema(invoices)` / `createSelectSchema(invoices)` — same field names, same nullability, same enums. Chain: Drizzle schema → `$inferInsert` for the type → drizzle-zod for the validator → both consumed by the Server Action.
...
- **Watch-outs:** `$inferSelect` returns the full-row shape — partial selects produce narrower inferred shapes, don't restate them; `$inferInsert` makes defaulted columns optional, not nullable (passing `undefined` is fine, `null` only if the column is nullable); `Partial<NewInvoice>` for updates is fine, `Required<Pick<Invoice, 'id'>> & Partial<NewInvoice>` is more accurate; inferred types don't include relations — the relational query API has its own inferred shapes (lesson 3 of chapter 038).

What this lesson does not cover:

- drizzle-zod and the Zod-from-schema pipeline — lesson 8 of chapter 042.
- Inferring result types from custom `db.select(...)` shapes — lesson 1 of chapter 038.
- The relational query API's nested-result types — lesson 3 of chapter 038.
- Server Action argument typing and the Result return shape — Chapter 043.
- Branded ID types layered on `$inferSelect` — Chapter 005 (recap site if needed).
...
- The Drizzle Relations v2 API — `defineRelations` in a separate file; the difference between `references()` (database FK) and a relation (TS-side traversal hint); how the relations file enables `db.query.…({ with: … })` nested reads in lesson 3 of chapter 038.
