...
- **DevTools — reading the live tree.** The Elements panel *is* the DOM tree, not the HTML source. The "Inspect" reflex; `$0` in the Console as the last-inspected element; right-click → "Store as global variable" for repeated probing. Cross-reference to Chapter 1.3.3.
...
- The attribute-versus-property split (3.5.2).
- The event model and delegation (3.5.3).
- React refs and `useRef` — Chapter 4.6 owns the React side; this lesson frames the DOM nodes refs *point at*.
- React Portals — Chapter 4.6.5; named here as a `DocumentFragment` use case.
...
- **The `dataset` property and `data-*` attributes.** `<div data-user-id="42">` is read as `element.dataset.userId`. The rename: kebab-case → camelCase, `data-` prefix stripped. The 2026 reach: state hooks for Tailwind variants (`data-[state=open]`), event-delegation discriminators (in 3.5.3), test selectors. The student writes `data-*` in JSX and reads it as `dataset.*` in delegation handlers.
...
- Tree structure, node hierarchy, query methods (3.5.1).
- The event model (3.5.3).
...
- `style` and `CSSStyleDeclaration` at depth — Chapter 4.3 owns CSS and tokens.
- The `dataset` API beyond recognition — used in 3.5.3 for delegation.
- ARIA semantics — Chapter 4.1.6 owns the `aria-*` surface.
...
- **The `AbortController` cleanup pattern.** The senior 2026 form for any non-React listener:
  ...
  - This pattern composes directly with React 19 ref callback cleanup (cross-reference forward to Chapter 4.6) and with `useEffect` teardown. The mental model is "one shutdown switch per scope" instead of "match each `add` with an exact `remove`."
...
- React's `onClick` / `onChange` / `onSubmit` props at depth — Chapter 4.6 owns React events.
- React Synthetic Events versus native events at depth — recognition only.
- `useEffect` cleanup and ref callback cleanup — Chapter 4.6.
...
- The `KeyboardEvent` keycode/key surface at depth — Chapter 4.6 or 4.11 will handle keyboard shortcuts when it earns its weight.
