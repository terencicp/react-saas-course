...
Chapter 010 closed the module-graph story — the four import shapes, depth-first evaluation, live bindings, the `'server-only'` boundary, and the top-level-await-vs-lazy-init call. The student can read any file as a node in a typed graph. This chapter zooms into what happens *inside* a node when the work it does isn't synchronous: how the runtime decides what runs next (the event loop and the microtask queue), how Promises model the three-state work-not-done-yet contract, how `async`/`await` collapses Promise chains into linear-looking code without losing the parallel-by-default reflex, and how `AbortSignal` carries cancellation through every modern web and Node API the course writes against. Every later chapter — `fetch` (chapter 019), the four async files (chapter 035), Server Actions (chapter 047), webhook ingestion (chapter 067), background work (chapter 070), streaming AI responses (chapter 110) — assumes the student can predict the order of any small async program and reach for `AbortSignal.any([...])` without ceremony.

Threads that must run through every lesson:

- **The student leaves able to predict execution order.** The event-loop model from lesson 1 of chapter 011 is the substrate the rest of the chapter rides on. Every later snippet that interleaves `await`, `setTimeout`, and `queueMicrotask` should be readable as a tick-by-tick trace.
- **Parallel by default, sequential by dependency.** The senior reflex is to check whether each `await` depends on the previous one's result. If not, the work runs through `Promise.all`. This rule fires in lesson 3 of chapter 011 and recurs in lesson 2 of chapter 035 (streaming a page), lesson 2 of chapter 043 (N+1), and lesson 6 of chapter 098 (RSC waterfalls).
...
- **Errors at the async seam preview Chapter 012.** Async throws, `AbortError` discrimination, the `unknown`-in-catch narrow — named at the moment they apply in this chapter, taught at depth in chapter 012. The chapter does not re-teach `try`/`catch` mechanics; it shows where they land in async code.
...
The student finishes the chapter able to trace `await`/`setTimeout`/`queueMicrotask` interleavings tick-by-tick, pick the right combinator for a "what counts as done" contract, write `Promise.withResolvers()` instead of the deferred-pattern boilerplate, rewrite serial `await`s into `Promise.all` when no data flows between them, bound parallelism with `for await...of` or `pMap` instead of unbounded `.map(async ...)`, wire `AbortSignal` through every `fetch` and timer with `AbortSignal.timeout(ms)` and `AbortSignal.any([...])`, and discriminate `AbortError` at the catch from real failures. Chapter 012 lands on this floor with the error-channel discipline.
...
- Authoring Promises (lesson 2 of chapter 011 owns the constructor and combinators).
- The `async`/`await` syntax beyond the microtask framing (lesson 3 of chapter 011 owns the parallel-by-default and sequential-by-dependency rules).
- Cancellation (lesson 4 of chapter 011).
- `try`/`catch` discipline at depth (Chapter 012).
...
- React's scheduler or `useTransition`'s priority queue. Unit chapter 029 owns that.
...
- The `async`/`await` mechanics in depth (lesson 3 of chapter 011).
- Cancellation (lesson 4 of chapter 011).
- `try`/`catch` discipline and the `Result<T, E>` return shape (lesson 1 of chapter 012).
- Authoring custom `AggregateError` subclasses. The default `AggregateError` is enough; lesson 2 of chapter 012 covers the broader custom-error story.
- Async iterators (`for await...of`). lesson 3 of chapter 011 owns the iteration shape.
...
- The three correct fixes for `.map(async ...)`, named with the trigger.
  ...
  - **Database batch.** If the work is N database reads, the right answer is one query: `db.select().where(inArray(table.id, items.map((i) => i.id)))`. Forward link to lesson 2 of chapter 043 (N+1) for the deeper treatment. Named here because the reflex is general — when N awaits in a loop are all hitting the same backend, the right fix is to ask the backend for the batch.
...
- `return await` — the stack-trace discipline. A one-line primer on the minimal shape used below: `try { … } catch (err) { … }` runs the `try` block and, if any awaited Promise inside rejects (or any synchronous `throw` fires), jumps to `catch` with the rejection reason bound as the block-scoped `err`; full treatment in lesson 1 of chapter 012. Two snippets.
...
- Top-level await pattern, named in one line. lesson 3 of chapter 010 owns the decision (top-level await vs. lazy init). Here, the student should recognize that top-level `await` is legal in modules and reaches for it only on the senior-call cases from lesson 3 of chapter 010. No re-teaching.

What this lesson does not cover:

- Authoring the Promises themselves (lesson 2 of chapter 011 owns the constructor and combinators).
- Cancellation (lesson 4 of chapter 011).
- `try`/`catch` discipline and `Result<T, E>` (Chapter 012).
- The Drizzle batch-query API at depth (lesson 2 of chapter 043).
...
- React's `use(promise)` hook for unwrapping Promises in Server Components (lesson 7 of chapter 029 and chapter 035 own that).
...
- `AbortError` discrimination at the catch. A `fetch` that aborts rejects with a `DOMException` whose `name === 'AbortError'`. The senior reflex: at every catch around an abortable call, the first branch distinguishes the abort from a real failure.
  - **The shape.** The `try`/`catch` form introduced in lesson 3 of chapter 011 (full treatment in lesson 1 of chapter 012) wraps the abortable call: `catch (err) { if (err instanceof Error && err.name === 'AbortError') return; throw err }` — the abort is intentional; treat it as a no-op. Anything else is a real failure and should propagate.
  - The cross-realm `instanceof DOMException` gotcha lives in lesson 2 of chapter 012; here, name the `error.name` check as the portable form.
...
- Propagation discipline. Every `async` function in the chain that *does* I/O should accept `signal` and pass it through. A function that only awaits work it already kicked off doesn't need to thread the signal — but the moment it adds an I/O step, it does. The senior reflex: when authoring an async function, **the `signal` parameter is part of the signature, not an afterthought**. Forward link to chapter 047 (Server Actions) where every action accepts and threads a signal.
...
- The React-side wiring (effect cleanup, ref-held controllers, request deduplication). lesson 2 of chapter 029 owns it.
- The Drizzle and `pg` cancellation specifics (lesson 4 of chapter 043 names the connection-level effect briefly).
- Server-side request cancellation in Next.js (Server Actions, route handlers — Unit chapter 047 and chapter 050).
- The AI SDK's streaming cancellation contract (Unit chapter 110).
- Background-job cancellation (`ctx.run.abortSignal` from Trigger.dev). lesson 5 of chapter 070 owns it.
- Authoring custom abort reasons and reading `signal.reason`. Named in one line — `controller.abort(new Error('user-canceled'))` makes the reason readable in the catch; the deeper custom-error story is in lesson 2 of chapter 012.
...
Roughly 150 to 190 minutes across the four content lessons plus the quiz. The chapter splits naturally across three evenings — lesson 1 of chapter 011 (the event loop and microtask queue) as a tight first sitting that earns its own evening because the mental model is load-bearing for everything after, lesson 2 of chapter 011 + lesson 3 of chapter 011 (Promises and `async`/`await` patterns) as the second sitting since they share the parallel-by-default rule, and lesson 4 of chapter 011 (cancellation) plus the quiz as the third. At the end the student can predict the execution order of any small async program, pick the right combinator for any "what counts as done" contract, reach for `Promise.withResolvers()` instead of the deferred-pattern boilerplate, rewrite sequential `await`s into `Promise.all` when no data flows between them, bound parallelism with `pMap` or pivot to a batched query, iterate streams and paginated APIs with `for await...of`, thread `AbortSignal` through every I/O call with `AbortSignal.timeout(ms)` and `AbortSignal.any([...])` as the composition reach, and discriminate `AbortError` at the catch. Chapter 012 lands on this floor and installs the error-channel discipline (throw vs. return, the `unknown`-in-catch narrow, custom domain errors) on top of the async substrate.
