...
8. **N+1 in the invoice list (20.3.7).** `src/lib/invoices/queries.ts` runs `db.select().from(invoices)` then loops to fetch each customer with a separate query; the Drizzle relations API (`with: { customer: true }`) is the fix.
