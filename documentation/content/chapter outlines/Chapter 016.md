...
- Senior watch-outs.
  ...
  - The `URL` constructor lowercases the hostname and normalizes the default port off (`:80` for http, `:443` for https). This matters for origin comparisons in lesson 2 of chapter 016.

What this lesson does not cover:

- The same-origin policy and how the browser uses `origin` — lesson 2 of chapter 016 owns it.
- CORS headers and the cross-origin opt-in — lesson 3 of chapter 016 owns it.
- Cookie attributes and the cookie trust model — Chapter 017.
- The `fetch` body, methods, and `AbortSignal` shape — Chapter 019.
- Next.js route matching, `params`, and `searchParams` in Server Components — Unit 5.
- The `nuqs` URL-state library for typed search-params state in React — Unit chapter 028 names it.
...
- Cross-origin object access on the page. Two pages from different origins can hold references to each other (via `window.open` or `<iframe>`), but the same-origin policy restricts which properties they can touch. The cross-origin-accessible subset is short: `location.href` (write-only for navigation), `location.replace`, `postMessage`, `closed`, `close`. Everything else throws. `postMessage` is the load-bearing primitive for cross-origin window communication — named here in one line; Chapter 018 owns the event model that fires `message`.
...
- The `Origin` request header named here. When a browser makes a cross-origin request (any non-`GET`/`HEAD` request, or any CORS-eligible request), it attaches an `Origin: https://app.acme.com` header. The server reads it and decides what to return. This is the single line that connects lesson 2 of chapter 016 to lesson 3 of chapter 016: CORS is the protocol the server uses to *answer* `Origin`.

What this lesson does not cover:

- CORS headers themselves, preflight, and credentials — lesson 3 of chapter 016.
- Cookie `SameSite`, `Secure`, `HttpOnly`, `Partitioned` — Chapter 017.
- CSRF token patterns — chapter 085.
- `Cross-Origin-Resource-Policy`, `Cross-Origin-Opener-Policy`, `Cross-Origin-Embedder-Policy` headers in depth — Unit 17.
- `postMessage` event-handler shape and origin validation — Chapter 018 names it.
...
- The cookie attribute surface — Chapter 017. CORS-with-credentials cross-references `SameSite`; the cookie chapter owns the full attribute table.
- CSRF token patterns and double-submit cookies — chapter 085.
- Server Actions vs. Route Handlers as a design decision — Unit 5 and Unit 7.
- The `fetch` request body, methods, status, and `AbortSignal` — Chapter 019.
...
Roughly 115 to 150 minutes across the three teaching lessons plus the quiz. The chapter splits naturally across two evenings — lesson 1 of chapter 016 + lesson 2 of chapter 016 (the parser and the policy) as one sitting, lesson 3 of chapter 016 + the quiz (the cross-origin opt-in) as the second since the preflight lesson carries the chapter's mechanics weight. The student finishes with a working `URLSearchParams` reflex for every URL they will build in Units 4 through 7, a precise origin-versus-site vocabulary that Chapter 017 will lean on for `SameSite`, and a Next.js CORS Route Handler shape they can paste into any production allow-list. Chapter 017 lands on this foundation and teaches the cookie attribute table without restating origin or site.
