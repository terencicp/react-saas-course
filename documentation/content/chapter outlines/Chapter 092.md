...
- **AsyncLocalStorage as the route-handler escape hatch.** Route handlers can't accept a `db` parameter — their signature is fixed by Next.js (`POST(request: Request)`). The contained alternative: a Node `AsyncLocalStorage<DbOrTx>` instance wired in `src/db/test-tx-context.ts` whose `.run(tx, fn)` enters the handler inside a test-owned async context, and a production `getDb()` helper that returns `als.getStore() ?? defaultDb`. Explicit handles remain the course default for Server Actions and `/lib`; ALS is the contained escape hatch used in lesson 2 of chapter 095's webhook-handler test.
- **What does NOT roll back.** Sequences advance and stay advanced. `pg_notify` fires regardless. External side effects (`fetch`, queue enqueues, email sends) are not transactional; they go through MSW (lesson 5 of chapter 092) or in-memory stubs. The test asserts on *intent* (the MSW handler was called with shape X), not the side effect.
- **Test data — seed once, override per test.** Worker setup migrates the schema and inserts a tiny baseline (a `seed_org`, a `seed_admin_user`). Per-test factories (lesson 2 of chapter 091, extended in lesson 3 of chapter 092) insert fresh rows inside the transaction. Don't reseed per test; don't seed heavy realistic data — that's `drizzle-seed` for dev (lesson 3 of chapter 044).
- **Test file convention — `.int.test.ts`.** The `integration` project glob from lesson 1 of chapter 090 picks up `src/**/*.int.test.ts`. The suffix is the discriminator: `invoice.test.ts` is unit, `invoice.int.test.ts` is integration. Prevents accidental promotion of a unit test into the slow lane.
...
- Test-DB lifecycle, migrations, per-worker isolation, Neon — lesson 2 of chapter 092.
- Auth fixtures — lesson 3 of chapter 092.
- Network-boundary mocking — Lessons lesson 4 of chapter 092 and lesson 5 of chapter 092.
- Webhook receiver testing — lesson 6 of chapter 092.
- Server Action end-to-end — lesson 7 of chapter 092.
- Flake taxonomy — lesson 8 of chapter 092.
...
- **Seed step — one baseline.** After migrations, insert a tiny baseline: one `seed_org`, one `seed_admin_user`. Heavy realistic data belongs in `drizzle-seed` (lesson 3 of chapter 044), not test setup. Per-test data comes from factories inside the transaction.
...
- **GitHub Actions wiring.** Job runs `pnpm install`, `docker-compose up -d postgres` (or Neon action), `pnpm drizzle-kit migrate`, `pnpm vitest run --project integration`. Reporter is JUnit (chapter 101). Cleanup on `if: always()`.
...
- **Runner commands.** Local: `vitest --project integration` (watch). CI: `vitest run --project integration --reporter=junit`. Pre-push (lefthook from chapter 100): `vitest run --project integration --changed` for affected tests only.
...
- The rollback pattern — lesson 1 of chapter 092.
- Auth fixtures — lesson 3 of chapter 092.
- Migration authoring — Chapter 043.
- Dev seeding — lesson 3 of chapter 044.
- CI depth (matrix, caching, reporters) — Chapter 101.
- RLS testing — Chapter 11.
...
- **Where the fixture lives.** `src/test/fixtures/auth.ts`. Imports factories from lesson 2 of chapter 091, the test `tx`, and the cookie/auth mocks. Exports `signedInAs` and `anonymous` — nothing else.
...
- **MSW as the boundary tool.** Mock Service Worker (`setupServer` for Node) intercepts at the request layer; the SDK runs unchanged. Handlers describe what the wire looks like. Practical setup in lesson 5 of chapter 092.
...
- MSW API surface — lesson 5 of chapter 092.
- Webhook receiver testing — lesson 6 of chapter 092.
- Contract testing — out of scope.
- E2E network mocking — Chapter 094.
- DI in unit tests — Chapter 091.
...
- **Failure injection.** `HttpResponse.error()` for network failure; `await new Promise((r) => setTimeout(r, 5000))` for slow upstream — pair with `vi.useFakeTimers()` (lesson 3 of chapter 091) and `advanceTimersByTimeAsync`. Status codes (`{ status: 503 }`) test the SDK's real retry.
...
- Boundary-mocking rationale — lesson 4 of chapter 092.
- Webhook receiver (incoming) testing — lesson 6 of chapter 092.
- jsdom/browser MSW — Chapter 093.
- Playwright network mocking — Chapter 094.
- Contract testing — out of scope.
...
- **The timestamp-tolerance path.** Stripe rejects signatures older than 5 minutes. Sign with timestamp 10 minutes in the past — assert 400. Sign within tolerance — assert 200. Clock seam from lesson 3 of chapter 091 controls the timestamp.
...
- Outbound MSW mocking — Lessons lesson 4 of chapter 092, lesson 5 of chapter 092.
- Server Action testing — lesson 7 of chapter 092.
- Stripe receiver implementation — Chapter 13.
- Resend receiver — Chapter 8.
- `processed_events` design — Chapter 13.
- Full Stripe project — Chapter 095.
...
- **The senior question.** A Server Action `createInvoice(input)` does six things: parse input through Zod, read the session via `await auth()`, check the plan, write to Postgres, revalidate a path, return a typed `Result`. A unit test for the inner function skips the wrapper, the session check, the revalidate. Reach: **test the exported action itself**, with auth fixtures (lesson 3 of chapter 092), real `tx` (lesson 1 of chapter 092), boundary network mocks (lesson 5 of chapter 092), and assertions on return, DB, and revalidate.
- **The action under test.** `export const createInvoice = authedAction(InvoiceSchema, async (input, { user, db }) => { ... })`. The `authedAction` wrapper from chapter 050 handles session, parsing, error mapping. The test imports the *exported* action and calls it as production calls it.
...
- **Typed return assertions.** Actions return `Result<TData, ErrorCode>` (chapter 047). Type tests (lesson 4 of chapter 091) cover the shape; integration asserts values via custom `toBeOkResult` / `toBeErrResult` (lesson 6 of chapter 091).
...
- `authedAction` implementation — Chapter 050.
- The `Result` shape — Chapter 047.
- MSW setup — lesson 5 of chapter 092.
- Auth fixtures — lesson 3 of chapter 092.
- Transaction wrapper — lesson 1 of chapter 092.
- Browser E2E — Chapter 094.
- Component form tests — Chapter 093.
...
- **Transaction-rollback isolation.** From lesson 1 of chapter 092: every test in `tx` and rolls back. Eliminates 80% of integration flake.
- **MSW handler reset.** From lesson 5 of chapter 092: `server.resetHandlers()` in `afterEach`. Override leaking into the next test is the bug class.
...
- **Clock and timer reset.** From lesson 3 of chapter 091: `vi.useFakeTimers()` without `vi.useRealTimers()` in `afterEach` leaks fake time forward. A later "after 1s" test hangs indefinitely.
...
- Rollback pattern — lesson 1 of chapter 092.
- MSW setup — lesson 5 of chapter 092.
- Clock/timer fakes — lesson 3 of chapter 091.
- `expect.assertions(n)` / forgotten-`await` — lesson 5 of chapter 091.
- CI infrastructure flake — Chapter 101.
- Playwright flake — Chapter 094.
...
- Integration project scope — `.int.test.ts` files under `src/**`; `vitest run --project integration` in CI; faster than E2E (chapter 094), slower than `/lib` unit tests (chapter 091), placed where bug density is highest.
