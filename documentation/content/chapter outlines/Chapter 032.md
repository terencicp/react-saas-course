...
- **Enabling `cacheComponents` — the config line.** `next.config.ts` exports a config object with `cacheComponents: true`. The legacy `experimental.dynamicIO` and `experimental.useCache` flags are deprecated and removed; the top-level `cacheComponents` replaces both. The course pins this on from day one in the starter (chapter 029 reference). Named here so the student recognizes the line and knows what it does.
...
- The `use cache` directive in full (lesson 3 of chapter 032).
- Partial Prerendering as a rendering shape (lesson 2 of chapter 032).
- `cacheLife` and `cacheTag` (lesson 4 of chapter 032).
- React `cache()` for per-request memoization (lesson 5 of chapter 032).
- The post-mutation invalidation surface (lesson 6 of chapter 032).
- Async request API specifics — `params`, `searchParams`, `cookies()`, `headers()`, `connection()` (lesson 8 of chapter 032).
- The deprecated route segment config (lesson 7 of chapter 032).
...
- **PPR and parallel routes.** Each `@slot` in a parallel route (lesson 5 of chapter 029) is its own subtree; each can be independently cached or dynamic. The chapter 035 project leans on this — a cached navigation slot alongside a dynamic detail slot, both under the same URL, each with its own loading and streaming behavior.
...
- The `use cache` directive's full syntax and semantics (lesson 3 of chapter 032).
- `cacheLife` and `cacheTag` (lesson 4 of chapter 032).
- React `cache()` for per-request work (lesson 5 of chapter 032).
- Invalidation after a mutation (lesson 6 of chapter 032).
- Async request APIs (lesson 8 of chapter 032).
- Streaming and Suspense from scratch (chapter 031).
...
- **Serialization rules — the contract.** Arguments to a `use cache` function must be serializable (same shape as the server-client boundary in lesson 4 of chapter 030 — primitives, plain objects, arrays, `Date`, `Map`, `Set`, typed arrays). Return values must be serializable. Functions, class instances, Promises-of-Promises are rejected. The compiler enforces this at build; a violation is a build error with the offending parameter pointed at.
...
- **Function-level `use cache` — the cached fetcher.** An async function that fetches and returns data, marked `use cache`, becomes a deduplicated and persistent cache. Two callers across two requests with the same arguments get one fetch, one stored value. Distinct from React `cache()` (lesson 5 of chapter 032) which is per-request, not cross-request.
...
- `cacheLife` for freshness windows and `cacheTag` for naming (lesson 4 of chapter 032).
- React `cache()` and the request memo cache (lesson 5 of chapter 032).
- Invalidating cached entries (lesson 6 of chapter 032).
- Route segment config and its deprecations (lesson 7 of chapter 032).
- Async request APIs (lesson 8 of chapter 032).
...
- **`cacheTag` — the named handle.** `cacheTag('product-catalog')` inside a `use cache` function attaches the named tag to that cache entry. The tag is the address the invalidation API (lesson 6 of chapter 032) uses to mark the entry stale or replaced. A function can attach multiple tags; an entry can be invalidated by any of them.
- **Tag naming — the senior shape.** Tags are strings. The senior pattern is `entity-type` for collections (`invoices`, `products`) and `entity-type:id` for individual records (`invoice:42`, `product:abc`). The reason: the mutation surface in lesson 6 of chapter 032 needs to express both "invalidate one record" and "invalidate the list that contains it," and the tag namespace is the way to do both.
...
- The `use cache` directive itself (lesson 3 of chapter 032).
- `revalidateTag`, `updateTag`, `revalidatePath`, `router.refresh` — the invalidation surface (lesson 6 of chapter 032).
- React `cache()` for per-request memoization (lesson 5 of chapter 032).
- Cache backends and self-hosting (named in lesson 3 of chapter 032).
- Webhooks as invalidation triggers — Chapter 063.

Estimated student time: 45 to 55 minutes. Pairs with lesson 3 of chapter 032 as the two-part anatomy of a cached function.
...
- The `use cache` directive (lesson 3 of chapter 032).
- `cacheLife` and `cacheTag` (lesson 4 of chapter 032).
- The invalidation surface (lesson 6 of chapter 032).
- Async request APIs that `cache()` is often paired with (lesson 8 of chapter 032).
- Client-side state and `useMemo` (Chapter 024).
- Database access patterns at scale — Unit 5.
...
- **The post-mutation pattern — the canonical Server Action shape.** Pseudo-shape: validate input, write to the database, call `updateTag('invoices')` and `updateTag(\`invoice:${id}\`)`, redirect to the list. The user lands on the list, the cached list reads fresh, the user sees their change. Chapter 043 owns the full Server Action surface; this lesson names the invalidation slot.
...
- `cacheTag` and `cacheLife` (lesson 4 of chapter 032).
- Server Actions in full — Chapter 043.
- Webhook ingestion patterns — Chapter 063.
- Optimistic UI with `useOptimistic` — lesson 6 of chapter 044.
- Path-based static generation — lesson 8 of chapter 034.
- Form-with-action wiring — Chapter 044.

Estimated student time: 50 to 65 minutes. Closes the loop on the cache lifecycle and prepares for Server Actions in Unit 6.
...
- The `cookies()` and `headers()` server-side reads in full — lesson 1 of chapter 033.
- URL state patterns at depth — lesson 4 of chapter 033.
- Client-side navigation hooks (`useRouter`, `usePathname`, etc.) — lesson 5 of chapter 033.
- `proxy.ts` and request-level rewrites — lesson 2 of chapter 033.
- The Zod validation pattern in full — Chapter 042.
- `generateStaticParams` and SSG — lesson 8 of chapter 034.
