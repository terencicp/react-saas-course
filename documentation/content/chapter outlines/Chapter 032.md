...
- **From chapter 021 (JSX/HTML semantics):** semantic landmarks (`<header>`, `<main>`, `<nav>`, `<footer>`), heading hierarchy (one `<h1>`, no level skips), `<button type>`, `<label htmlFor>`, `<img alt>`.
- **From chapter 022 (Tailwind):** utility-first reflex, `@theme` config, `cn()`, the `dark:` variant, the `next-themes` wiring shape (`<ThemeProvider attribute="class" defaultTheme="system" enableSystem disableTransitionOnChange>` plus `suppressHydrationWarning` on `<html>`).
- **From chapter 023 (cascade and tokens):** the semantic-token model (`--background`/`--foreground`/`--primary`/`--muted`/`--ring`/`--border`) and OKLCH color values.
- **From chapter 024 (layout):** flex axes, CSS Grid with `grid-template-columns`, container queries when applicable, the `min-h-dvh` reach.
- **From chapter 025 (typography/color/motion/responsive):** breakpoint utilities, `prefers-reduced-motion`, `tw-animate-css`, fluid type with `clamp()`.
- **From chapter 026 (components and composition):** the canonical Button shape (`cva` + `Slot` + `cn()` + `asChild`), `VariantProps<typeof variants>`, discriminated unions for mutually exclusive props.
- **From chapter 028 (state and refs):** `useState`, `useRef`, `useId`.
- **From chapter 029 (effects):** `useEffect` for body-scroll lock with cleanup; closing-on-escape via event listeners.
- **From chapter 030 (custom hooks):** `useLockBodyScroll()` shape — toggles `overflow: hidden` on `body` with cleanup.
- **From chapter 031 (shadcn + a11y):** the shadcn CLI install pattern, `asChild` composition, the four discipline-level commitments, focus-trap as the primitive's job, the icon-only button label pattern.
...
- Tailwind v4 with `@theme` — lesson 2 of chapter 022.
- `cn()` helper composition — lesson 3 of chapter 022.
- CVA variant tables (`cva` + `VariantProps`) — lesson 3 of chapter 026.
- `Slot` + `asChild` polymorphism — lesson 3 of chapter 026.
- `dark:` variant and the `.dark` class flip — lesson 5 of chapter 022.
- `next-themes` wiring without FOUC — lesson 6 of chapter 022.
- Semantic-token model (`--background`, `--foreground`, `--primary`) — lesson 4 of chapter 023.
- Flex and CSS Grid layout — lesson 2 of chapter 024, lesson 3 of chapter 024.
- Responsive breakpoints (`sm:`, `md:`, `lg:`) — lesson 6 of chapter 025.
- `prefers-reduced-motion` and `motion-reduce:` — lesson 5 of chapter 025.
- shadcn primitives (`Button`, `Sheet`, `Card`, `Badge`, `Separator`) — lesson 1 of chapter 031.
- The four discipline-level a11y commitments — lesson 2 of chapter 031.
- ARIA: `aria-label` on icon-only buttons, `aria-hidden` on decorative icons — lesson 3 of chapter 031.
- Focus trap (Radix handles it inside `Sheet`) and return-focus contract — lesson 4 of chapter 031.
- `useState`, `useRef`, `useId` — Chapter 028.
- `useEffect` with cleanup — lesson 1 of chapter 029.
- `useLockBodyScroll` custom hook — lesson 1 of chapter 030.
...
- Open `next.config.ts` — `reactCompiler: true`; cross-ref lesson 2 of chapter 030.
...
- `(marketing)` is a route group — the parentheses do not appear in the URL. Cross-ref lesson 2 of chapter 033 (forward).
...
## Lesson 3 — Header, hero, and feature grid
...
- Fill `components/hero.tsx`: semantic `<section>` with an `<h1>`, supporting paragraph, two CTAs (`<Button asChild><Link href="/signup">Get started</Link></Button>` and `<Button variant="outline" asChild>...</Button>`), and a `<ThemeAwareImage>` to the right on `lg:`, stacking above on `md:` and below. Cross-ref the canonical Button shape from lesson 3 of chapter 026.
...
- `cva` is the senior shape for variant tables. Three booleans (`isBrand`, `isLoud`, `isMuted`) becomes one `tone` union and one `emphasis` union — the variant table makes invalid states unrepresentable. Cross-ref lesson 3 of chapter 026.
- Icon-only buttons (none yet in this lesson, but the theme toggle and mobile trigger appear in lesson 4 of chapter 032 and lesson 5 of chapter 032) require `aria-label`. Pattern named now.
...
- `<Button asChild>` wraps a Next.js `<Link>` — the polymorphism is the Slot pattern from lesson 3 of chapter 026. Naming this once because it appears in every CTA.
- Heading hierarchy: one `<h1>` in the hero, `<h2>` for each section's title, no skipping levels. Cross-ref lesson 3 of chapter 021.

Codebase state at entry: shell with TODO placeholders from lesson 2 of chapter 032.
Codebase state at exit: header (desktop nav only), hero, and feature grid render correctly on desktop and below `md`. Page is responsive at 1280 and 768; mobile nav slot still empty (drawer ships in lesson 5 of chapter 032). Theme toggles via the OS preference; no manual toggle yet.

Estimated student time: 50 to 65 minutes.

---

## Lesson 4 — Pricing, footer, and a flicker-free theme toggle
...
- Icon-only buttons (theme toggle and social icons in the footer) all need `aria-label`. Decorative icons inside labeled buttons get `aria-hidden`. Cross-ref lesson 3 of chapter 031.
...
Codebase state at entry: header, hero, feature grid render from lesson 3 of chapter 032.
...
## Lesson 5 — Mobile drawer with scroll lock
...
- The focus trap is Radix's job, not yours. Writing a focus trap by hand is the canonical lesson 4 of chapter 031 watch-out — don't.
...
- Returning focus to the hamburger trigger on close is automatic with Radix. The exception (lesson 4 of chapter 031) is unmounted triggers, which doesn't apply here.
...
- The mobile nav must not duplicate the desktop nav's links in the DOM. The `hidden md:flex` and `md:hidden` cut keeps each surface single-source. Cross-ref lesson 6 of chapter 025.
...
Codebase state at entry: full surface from lesson 4 of chapter 032 with empty mobile-nav slot.
Codebase state at exit: full responsive surface works. Mobile drawer opens, traps focus, locks scroll, closes on `Esc` and on link click. Theme toggle accessible from inside the drawer on mobile. All Done-when clauses are now achievable in lesson 6 of chapter 032.
...
- Lighthouse a11y: run in Chrome incognito; the score is 100; if not, audit findings against the four lesson 2 of chapter 031 commitments and the icon-only-button labeling from lesson 3 of chapter 031.
...
- Run axe DevTools (extension) as the secondary auditor — its rule coverage exceeds Lighthouse's and catches what Lighthouse misses (lesson 2 of chapter 031). Note any new findings.
...
- Forward references:
  ...
  - Chapter 085 will add the security headers (CSP, HSTS) this marketing page needs in production.
  - Chapter 098 will add the performance and Core Web Vitals pass; this project ships the a11y baseline, not the perf baseline.
