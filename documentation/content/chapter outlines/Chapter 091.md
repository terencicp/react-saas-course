...
- **Colocation and the `.test.ts` suffix.** `src/lib/error-mapping.ts` and `src/lib/error-mapping.test.ts` sit side by side. The `unit` project's include glob from lesson 1 of chapter 090 picks it up. Why colocation over a parallel `tests/` tree: short import paths, renames move both files together, deleted code can't leave orphaned tests, and the test reads as documentation in the same folder.
- **AAA in the `/lib` shape.** Arrange is one line (the input). Act is one line (the call). Assert is one or two. A `/lib` unit test fits on a screen with no scrolling. If arrange grows past three lines, the fixture moves to a factory (lesson 2 of chapter 091); if the act has multiple calls, the test is two behaviors and should split.
...
- **Matcher selection by shape.** **Primitive** — `toBe` (SameValue). **Object / array** — `toEqual` (deep). **Partial object** — `toMatchObject`. **Array contains** — `toContainEqual` (deep) or `toContain` (primitive). **Float tolerance** — `toBeCloseTo`. **Throws** — `toThrow(MessageOrClass)` (depth in lesson 6 of chapter 091). **Class** — `toBeInstanceOf`. Wrong matcher loses signal: `toBe` on an object compares references; `toEqual` on a `Temporal.Instant` doesn't compare instants — compare `.epochMilliseconds` or use a custom matcher.
...
- **`beforeEach` discipline in `/lib`.** Pure functions need no setup. A `beforeEach` here is a smell: either a shared fixture (move to a factory call inside `it`) or the unit isn't pure (move to chapter 092). Exception: `vi.useFakeTimers()` / `vi.useRealTimers()` for time-touching code (lesson 3 of chapter 091).
...
- **The `vitest --project unit` reflex.** All chapter tests live here. `vitest --project unit --watch` is the inner-loop reflex. Coverage thresholds from lesson 3 of chapter 090 ride this glob — `/lib/**` at 90% lines / 85% branches.
...
- Fixture factories and the Builder pattern — lesson 2 of chapter 091.
- Time, randomness, and ID determinism — lesson 3 of chapter 091.
- Type-level assertions — lesson 4 of chapter 091.
- Async-specific traps and `vi.useFakeTimers()` with promises — lesson 5 of chapter 091.
- Unhappy-path matchers (`toThrow`, `rejects.toThrow`, `Result.err` shape) — lesson 6 of chapter 091.
- Integration tests at the seams (Server Actions, route handlers, DB) — Chapter 092.
- Component tests — Chapter 093.
- Snapshot testing for email templates — Chapter 8 / chapter 093.
...
- **Where factories live.** `src/test/factories/` — one file per entity (`users.ts`, `orgs.ts`, `invoices.ts`, `webhookEvents.ts`). Imported by `/lib` tests and integration tests (chapter 092) alike; the factory is the test-data contract for the whole suite. Don't colocate with `/lib` tests — factories are cross-cutting.
...
- **Sequence helpers for uniqueness.** Integration (chapter 092) DBs reject duplicate emails. The reach: `const userSeq = sequence('user'); buildUser({ email: \`user-${userSeq.next()}@test.com\` })`. The sequence resets per test or per worker — never global. Deterministic IDs cross-reference lesson 3 of chapter 091.
...
- **Factories vs. fixtures vs. seeds.** Three nouns the team confuses. **Fixtures** are static test data (JSON files with Stripe webhook payloads). **Factories** are functions producing fresh instances. **Seeds** populate a dev or test DB with realistic volumes (lesson 3 of chapter 044). Conflating them is the bug class: a "fixture" that mutates is a factory in the wrong shape; a "seed" used in tests creates run-order coupling.
- **Static fixture files for external payloads.** A Stripe webhook test needs a realistic payload — captured once from Stripe's test dashboard, saved to `src/test/fixtures/stripe/checkout-completed.json`, imported by the test. Static JSON because exact shape matters (signature verification in lesson 6 of chapter 092). Update by re-capturing, not by hand-editing beyond the field under test. Same pattern for signed JWTs, Resend bounce webhooks, S3 events.
...
- **Faker — when it earns weight.** `@faker-js/faker` for dev seeds (lesson 3 of chapter 044), not unit tests. A unit test wants *deterministic* data — `email: 'test@example.com'` debugs better than `email: 'wilma.lakin@example.org'`. Faker without an explicit seed is the source of "flaky, can't reproduce" failures.
...
- **Watch-outs.** A factory with no `overrides` argument forces mutation — back to shared mutable state; a factory returning a cached object isn't a factory; defaults failing validation ship false negatives; `Math.random()` in a default kills reproducibility — use the seeded RNG or a sequence (lesson 3 of chapter 091); DB-touching factories belong in chapter 092's integration-fixture layer; factories imported from `app/**` reverse dependency direction — they live in `src/test/factories/`; builder-DSL chaining (`buildUser().withRole('admin').build()`) reads like Spring — the override-spread shape is enough.

What this lesson does not cover:

- Deterministic `now()` / `uuid()` / RNG — lesson 3 of chapter 091.
- Type-level assertions on factory return shapes — lesson 4 of chapter 091.
- Integration-test fixtures (signed-in user, org seed) — lesson 3 of chapter 092.
- `drizzle-seed` for dev databases — lesson 3 of chapter 044 owns it.
- `MSW` handler factories — lesson 5 of chapter 092.
...
- **The forgotten-`await` trap with fake timers.** Code does `await sleep(5000); doWork()`. The test `vi.advanceTimersByTime(5000)` fires the timer but `sleep`'s promise resolves on the *next microtask*; `doWork` hasn't run yet. Fix: `await vi.advanceTimersByTimeAsync(5000)` — the async variants (`runAllTimersAsync`, `runOnlyPendingTimersAsync`) flush microtasks between ticks. lesson 5 of chapter 091 owns the depth.
...
- The factory pattern for entity defaults — lesson 2 of chapter 091.
- Type-level testing — lesson 4 of chapter 091.
- Async traps and promise-aware timer advancement at depth — lesson 5 of chapter 091.
- Unhappy-path assertions — lesson 6 of chapter 091.
- Time semantics, IANA timezones, Temporal codecs — Chapter 087 owns the production surface.
- MSW handler determinism (request-level randomness) — lesson 5 of chapter 092.
...
- **Testing Zod and Drizzle inferred types.** `expectTypeOf<z.infer<typeof InvoiceSchema>>().toEqualTypeOf<Invoice>()` — the schema-as-source pattern (chapter 046) means the type test catches drift between schema and domain type. Same for `InferSelectModel<typeof users>` against the application's `User` type.
...
- Runtime assertions on values (matchers, `Result.ok` checks) — Lessons lesson 1 of chapter 091, lesson 6 of chapter 091.
- Principle #7 from Unit 2 — owned by Chapter 009.
- Drizzle schema inference — Unit 6.
- Zod schema definition — Chapter 046.
...
- **Async error assertions — `rejects.toThrow`.** `await expect(fetchInvoice('bad-id')).rejects.toThrow(NotFoundError)`. Assert on class or structured `code`, never message. Depth in lesson 6 of chapter 091.
...
- Fake timer mechanics on the time / clock axis — lesson 3 of chapter 091.
- Unhappy-path error-shape assertions — lesson 6 of chapter 091.
- Integration-level async (MSW handlers, Drizzle transactions) — Chapter 092.
- Server Action async testing — lesson 7 of chapter 092.
- Playwright's async waiting — Chapter 094.
...
- **Asserting on `Result.err` shape.** The course's `Result` from lesson 3 of chapter 047 is `{ ok: false, error: { code, userMessage, fieldErrors? } }`. Use `toMatchObject({ ok: false, error: { code: 'NOT_FOUND' } })` — partial match ignores fields not under test. Don't `toEqual` the full shape; every test breaks when one field gets added.
...
- **`Error.cause` chain assertions.** When the unit re-wraps with `new MyError('...', { cause: originalErr })`: `expect(err).toBeInstanceOf(MyError); expect(err.cause).toBeInstanceOf(NetworkError)`. The wrap-and-throw discipline from lesson 2 of chapter 012 carries production debug info; tests verify the chain isn't broken.
- **`not.toThrow`.** `expect(() => fn(good)).not.toThrow()` — for "doesn't throw, returns void" paths. Pairs with positive assertions on side effects (the row written, the audit log entry — in chapter 092).
...
- **Unhappy-path coverage audit.** From lesson 3 of chapter 090: a file with 100% line coverage and no throw-branch test is a smell. Branch coverage first surfaces it. Audit reflex: for every exported function, "is there a test for the throw / error branch?"
- **Watch-outs.** `expect(fn(bad)).toThrow()` without the arrow — the call runs and throws before `expect` sees anything; `toThrow('Invalid email')` is a *substring* match, not full-string; matching message strings breaks on i18n updates; narrowing `result` for `error.code` access — assert via `toMatchObject({ ok: false, error: { code } })` to skip the narrowing dance; testing success and failure in one `it` block conflates behaviors — split; `it.fails` committed without a tracking issue lingers; `expect(promise).rejects.toThrow()` without `await` is the lesson 5 of chapter 091 trap; mocking a collaborator's throw and asserting the unit re-threw it is a tautology unless the wrap behavior (cause, code mapping) is what's being tested; asserting on stack traces is brittle — class and code only.

What this lesson does not cover:

- The full `Result` shape and contract — Chapter 047 owns it.
- The error mapper's seam — lesson 3 of chapter 047 and Chapter 17 own it.
- Integration-level error testing (DB serialization failures, MSW failure responses) — Chapter 092.
- Sentry capture in tests — Chapter 096.
- Error rendering in the UI (toast, form errors) — Chapter 7, Chapter 17.
- Snapshot testing for error responses (RFC 9457 bodies) — Chapter 092 integration tests.

Estimated student time: 35 to 45 minutes. Pattern lesson closing the chapter; pairs with lesson 5 of chapter 091 (async error patterns) as the unhappy-path duo.
