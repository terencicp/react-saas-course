...
Chapter 081 cashes in the threshold and wiring discipline of chapter 080 — the four-trigger funnel (lesson 1 of chapter 080), the read/write split between `useInfiniteQuery` and the Server Action (lesson 4 of chapter 080), the per-request `getQueryClient()` rule on the server (lesson 3 of chapter 080), the `<HydrationBoundary>` SSR-hydrated shape (lesson 3 of chapter 080), the cache-update optimistic pattern with `cancelQueries` + snapshot + restore (lesson 2 of chapter 080 / lesson 4 of chapter 080), the `commentKeys` helper (lesson 2 of chapter 080), and the two-system invalidation reality (lesson 3 of chapter 080 / lesson 4 of chapter 080) — as one runnable per-invoice comment thread on top of the Unit chapter 066 invoices surface. The student wires `<QueryClientProvider>` and the senior defaults, prefetches the thread's first page on the Server Component invoice page, hydrates the client cache, runs `useInfiniteQuery` with cursor paging and a 10-second `refetchInterval`, posts new comments through a Server Action, then fires the cache-update optimistic shape with rollback on `onError` and `invalidateQueries` from the client after the action returns. Each build closes on a runnable state: lesson 3 of chapter 081 ends with the provider wired and the seeded first page rendering with no client loading state on first paint; lesson 4 of chapter 081 ends with infinite scroll and polling live; lesson 5 of chapter 081 ends with optimistic add and rollback wired; lesson 6 of chapter 081 walks the "Done when" clause-by-clause.

Threads through every lesson: TanStack Query is **scoped to the comment thread leaf only** — the rest of the invoice detail page stays Server Components, no `useQuery` creeps outside the `<CommentThread />` subtree; the `QueryClient` is per-request on the server via React's `cache()` and a singleton on the client, never module-scoped on the server; the `queryFn` calls a route handler (`GET /api/invoices/[id]/comments`) so the read seam stays a public HTTP contract that a future mobile or third-party client can hit, and the same handler the Server Component's prefetch can read directly via the in-process branch; the write seam is a Server Action (`addCommentAction`) — Server Actions own form semantics, audit-log writes, and the canonical Result — and the client invalidates the TanStack cache after the action resolves; `commentKeys` is the only place query-key arrays exist, same structural enforcement as `tags.ts` from lesson 1 of chapter 076; the optimistic update is the **cache-update** shape (snapshot via `getQueryData`, write via `setQueryData`, restore in `onError`, invalidate in `onSettled`) because the new row has to land inside `data.pages[0].comments` of the infinite-query cache; `cancelQueries` runs before every optimistic `setQueryData` to prevent an in-flight refetch from overwriting the optimism; polling uses `refetchInterval: 10_000` plus `refetchIntervalInBackground: false` so polling pauses when the tab is hidden; the two-system invalidation (Server Action's `revalidateTag` + client's `invalidateQueries`) is the architectural price tag of bringing TanStack Query in, surfaced deliberately at the seam.

### Dependency carry-in

- **From chapter 066 (the starter base):** `app/(app)/invoices/page.tsx` Server Component reading `invoiceListSearchParamsCache`, `app/(app)/invoices/[id]/page.tsx` Server Component reading `getInvoiceDetail({ orgId, id })`, the toolbar / table / pagination shells, the four lifecycle Server Actions, the version-precondition update path.
- **From chapter 063 (tenancy + RBAC):** `tenantDb(orgId)` in `src/lib/tenant-db.ts`, `authedAction(role, schema, fn)` in `src/lib/authed-action.ts`, `authedRoute(role, schema, fn)` in `src/lib/authed-route.ts`, the active-org slot in the session, `logAudit(tx, event)`.
- **From chapter 045 (schema):** `invoices`, `invoice_lines`, `customers`, `organizations`, `org_members`, `audit_logs`, the cursor helpers in `src/db/cursor.ts`. New for this project: `invoice_comments` table — `id uuid pk`, `organizationId uuid fk`, `invoiceId uuid fk`, `authorId uuid fk references users(id)`, `body text not null`, `createdAt timestamptz default now()`. Composite index `(invoiceId, createdAt desc, id desc)` for the cursor.
- **From chapter 050 (route handlers):** `authedRoute(role, schema, fn)` wrapper, the canonical JSON response shape `{ data } | { error }`, the Zod parse at the boundary, the shared response schema imported by both the handler and the client `queryFn`.
- **From chapter 047 + chapter 051:** canonical Result shape, Zod 4 `strictObject` at the action boundary, `'use server'`, `useActionState`.
- **From lesson 6 of chapter 036 / lesson 2 of chapter 076:** `revalidateTag` for the Server Component cache invalidation from the action.
- **From lesson 1 of chapter 076 (`tags.ts`):** the existing `invoiceTag(id)` helper and the new `invoiceCommentsTag(invoiceId)` helper, both invalidated by the action via `revalidateTag` so the Server Component invoice page's cached count refreshes alongside the TanStack cache.
- **From lesson 2 of chapter 080:** `useQuery`, `useMutation`, `useInfiniteQuery` primitives; `commentKeys.list(invoiceId)`; `cancelQueries` + `getQueryData` + `setQueryData` + restore pattern; `maxPages` cap; `refetchInterval` + `refetchIntervalInBackground: false`.
- **From lesson 3 of chapter 080:** `getQueryClient()` with the `typeof window` branch and React `cache()` on the server; `<QueryClientProvider>` with senior defaults (`staleTime: 60_000`, `gcTime: 5 * 60_000`); `<HydrationBoundary>`; `<ReactQueryDevtools />` gated on `NODE_ENV`.
- **From lesson 4 of chapter 080:** the read/write split (infinite query reads, Server Action writes), the cache-update optimistic shape (not via-variables), the two-system invalidation, the 10-second polling cadence with background pause.
...
scripts/
  seed.ts                       # provided: two orgs, 60 invoices per org from chapter 066, plus 80+ seeded
                                #           comments on one focal invoice per org (so the infinite-query
                                #           has real pages to fetch and the polling demo has a coworker
                                #           insert path)
src/
  db/
    schema.ts                   # provided: full chapter 066 schema + new invoice_comments table
    client.ts                   # provided
  ...
  lib/
    ...
    authed-route.ts             # provided (chapter 050)
...
- **Env entries:** unchanged from chapter 066. No new third-party services for this project.
...
- The four-trigger funnel and the comment-thread case clearing three of them — lesson 1 of chapter 080 + lesson 4 of chapter 080.
- `useInfiniteQuery` with `getNextPageParam`, `initialPageParam`, `maxPages` — lesson 2 of chapter 080.
- `useMutation` lifecycle and the cache-update optimistic shape — lesson 2 of chapter 080 + lesson 4 of chapter 080.
- `cancelQueries` + `getQueryData` + `setQueryData` + restore — lesson 2 of chapter 080.
- Polling with `refetchInterval` + `refetchIntervalInBackground: false` — lesson 2 of chapter 080 + lesson 4 of chapter 080.
- `queryClient.invalidateQueries` after the Server Action returns — lesson 3 of chapter 080.
- `commentKeys` as the structural enforcement — lesson 2 of chapter 080 (mirrors lesson 1 of chapter 076's `tags.ts`).
- Per-request `QueryClient` on the server via React `cache()` — lesson 3 of chapter 080.
- `<QueryClientProvider>` + senior defaults (`staleTime`, `gcTime`, `refetchOnWindowFocus: false`) — lesson 3 of chapter 080.
- `<HydrationBoundary>` and `dehydrate` for SSR-hydrated initial data — lesson 3 of chapter 080.
- The fetcher branch on `typeof window` for in-process server reads — lesson 3 of chapter 080.
- `<ReactQueryDevtools />` gated on `NODE_ENV` — lesson 3 of chapter 080.
- `'use client'` boundary placement at the leaf, not the page — lesson 3 of chapter 080 (lesson 2 of chapter 034 frame).
- Route handlers as the read seam — chapter 050 (lesson 4 of chapter 080 frame).
- Server Actions as the write seam, canonical Result, audit-log write — chapter 047 / chapter 063.
- The two-system invalidation reality (Server Component cache vs. TanStack cache) — lesson 3 of chapter 080 + lesson 4 of chapter 080.
- Zod parse at every boundary (route handler, fetcher, action) — chapter 046.

---

## Lesson 1 — Project brief

Frames the build: bolt a polling, infinite-scrolling, optimistically-added comment thread onto the chapter 066 invoice detail page with TanStack Query scoped to the leaf, names the "Done when" clauses, and links the starter.

Goals:

- Frame the build: take the chapter 066 invoice detail page and add a real-time-ish comment thread — `<QueryClientProvider>` wired in the root layout, the invoice page's Server Component prefetches the first page via `prefetchInfiniteQuery` and hands a dehydrated cache to `<HydrationBoundary>`, the client `<CommentThread />` runs `useInfiniteQuery` with 10-second polling and infinite-scroll-up paging, posting a comment fires a Server Action with cache-update optimistic add and rollback on failure. Show one screenshot of the finished invoice page: header, customer card, line items, and the comment thread at the bottom with a posted optimistic row visible.
...
- The starter ships chapter 066 working end-to-end plus the new `invoice_comments` table with 80+ seeded rows per focal invoice. The chapter 066 surface stays untouched — every change in this project lives under `src/lib/query/`, `src/lib/comments/`, the new route handler, the invoice detail page's prefetch wiring, and the leaf `<CommentThread />` component.
- The TOC suggests "QueryClientProvider + HydrationBoundary" and "useInfiniteQuery with polling" as two lessons. The outline splits them deliberately: lesson 3 of chapter 081 lands the provider, the per-request factory, the prefetch, and the hydration boundary so the seeded first page renders SSR-hydrated; lesson 4 of chapter 081 layers infinite scroll and polling on top. Each is independently verifiable.
- The cache-update optimistic shape (snapshot + setQueryData + restore + invalidate) is the heavier path — the via-variables shape from lesson 2 of chapter 080 doesn't fit because the optimistic row has to land inside the infinite-query cache's `data.pages[0].comments`, not be rendered inline by reading `mutation.variables`. The lesson surfaces this decision once and commits.

Codebase state at entry: empty repo (student runs `degit`).
Codebase state at exit: starter cloned, Postgres up, schema migrated (including the new `invoice_comments` table), seed loaded; `pnpm dev` shows the chapter 066 invoice detail page working with no comment thread yet (the leaf component is unimported); `/inspector` loads but the cache-state panel is empty (no provider).

Estimated student time: 10 to 15 minutes.

---

## Lesson 2 — Tour the starter and the inspector
...
- Walk the file tree, calling out provided vs. stubbed. Linger on six files: `src/lib/query/client.ts` (empty — lesson 3 of chapter 081), `src/lib/query/keys.ts` (empty — lesson 3 of chapter 081), `src/lib/query/provider.tsx` (empty — lesson 3 of chapter 081), `src/lib/comments/fetcher.ts` (empty — lesson 4 of chapter 081), `app/api/invoices/[id]/comments/route.ts` (empty — lesson 4 of chapter 081), `src/lib/comments/actions.ts` (empty — lesson 5 of chapter 081), plus the two consumer files `app/(app)/invoices/[id]/page.tsx` (provided Server Component shell — student adds prefetch in lesson 3 of chapter 081) and `app/(app)/invoices/[id]/comment-thread.tsx` (empty — lesson 4 of chapter 081 + lesson 5 of chapter 081).
...
- Read the route-handler scaffolding: `authedRoute` is the chapter 050 wrapper; the empty `route.ts` exports a placeholder that returns 501. The Zod schemas in `src/lib/comments/schema.ts` are shipped, shared by the handler and the client fetcher.
- Read `src/lib/comments/queries.ts` — the `listCommentsPage` direct DB read is provided in full because the SQL is not the lesson (cursor pagination was the chapter 045 lesson, scoped queries chapter 066). The student calls it from two places: the route handler (server side, public) and the in-process branch of `fetcher.ts` (server side, internal). The same function, two callers.
- Read `next.config.ts`: `cacheComponents: true` is on from chapter 066; the invoice detail page's existing Server-Component reads stay cached. Adding the comment thread does not touch that — the cached subtree is the invoice header, the comment thread is a fresh dynamic-with-client-cache leaf.
- Read the inspector end-to-end — every panel, button, debug toggle. The "Force 500 on next POST", "Insert coworker comment", "Clear client cache", and "Toggle background polling" controls are the verification surface for lesson 6 of chapter 081.
...
- Run the app: `/invoices/[id]` renders the chapter 066 invoice detail page; the comment-thread section is empty (the leaf component is unimported). `/inspector` loads but the query-state panel is blank.
...
- `commentKeys` lives in `src/lib/query/keys.ts` because key arrays are query-system identifiers — the parallel to `tags.ts` from lesson 1 of chapter 076. Future projects will extend `keys.ts` with `notificationKeys`, `jobKeys`, etc. Same discipline.
...
## Lesson 3 — Provider, per-request factory, and the SSR-hydrated first page
...
- Fill `src/lib/query/client.ts`: define `makeQueryClient()` with the senior defaults (`staleTime: 60_000`, `gcTime: 5 * 60_000`, `refetchOnWindowFocus: false`). Export `getQueryClient` that branches on `typeof window === 'undefined'`: on the server, return `cache(makeQueryClient)()` (React's per-request memoization); on the client, return a module-scoped singleton. Comment the branch with the cross-request-leak failure mode named in lesson 3 of chapter 080 — name the bug at the call site so future readers know why the branch is there.
- Fill `src/lib/query/keys.ts`: `commentKeys = { all: ['comments'] as const, list: (invoiceId: string) => ['comments', invoiceId, 'list'] as const }`. Lock the hierarchy now so lesson 4 of chapter 081 and lesson 5 of chapter 081 share the same identity. `as const` keeps the tuples narrow.
...
- `<HydrationBoundary>` only needs to wrap the subtree that will run `useQuery` / `useInfiniteQuery` on those keys. Wrapping the entire page is fine but unnecessary — the comment-thread leaf is what reads from the hydrated cache. The lesson 2 of chapter 034 boundary discipline: push `'use client'` as deep as it goes.
- `staleTime: 60_000` at the provider level is the SaaS default — without it, every mount and tab focus would refetch, which is the 2022-era React Query behavior that produces refetch storms. The polling cadence in lesson 4 of chapter 081 overrides this per-query via `refetchInterval` (which works alongside `staleTime`, not against it).
...
## Lesson 4 — Infinite scroll, polling, and the route handler
...
- Fill the client branch of `src/lib/comments/fetcher.ts`: build the URL with `cursor` as an optional search param, `fetch` it with `credentials: 'same-origin'`, parse the response envelope (`{ data }` shape from chapter 050), validate with `commentsPageSchema.parse`, throw on `!res.ok` so `useInfiniteQuery` surfaces the error. The fetcher now works on both sides — server-side prefetch unchanged, client-side polls and scroll-fetches via this branch.
...
- Rewrite `app/(app)/invoices/[id]/comment-thread.tsx` to the real shape:
  ...
  - Render an `isFetching` indicator (a subtle dot or "Updating..." chip) at the top of the thread so the user can tell when a poll is in flight without confusing it with the per-page spinner — lesson 2 of chapter 080's `isPending` vs. `isFetching` distinction applied.
...
- `maxPages: 10` is the senior memory cap from lesson 2 of chapter 080. For a 1000-comment thread, scrolling deep keeps the 10 most-recent loaded pages in memory; older pages drop and refetch if the user scrolls back. Without the cap, an unbounded `useInfiniteQuery` accumulates pages until the tab is closed. The senior call: chat-style threads cap at 10; feed-style read-once surfaces are unbounded.
- `refetchInterval: 10_000` with `refetchIntervalInBackground: false` is the threshold-met polling cadence from lesson 4 of chapter 080. Faster (1s, 2s) floods the connection pool and burns mobile battery; slower (30s, 60s) feels stale. The framework's `document.hidden` check is what pauses polling — flipping `refetchIntervalInBackground: true` (the inspector toggle) keeps polling when hidden, useful for status-panel surfaces but wrong for a thread.
...
- The Zod parse on the client side (`commentsPageSchema.parse(json.data)`) is the contract enforcement from chapter 046 / chapter 050. If the route handler drifts (a new field added without updating the schema), the client fails loudly with a parse error rather than silently rendering broken UI. The schema lives in `src/lib/comments/schema.ts` — both the handler and the fetcher import it.
...
## Lesson 6 — Verify against "Done when"
...
- **Org-switch cache-clear discipline (forward pointer):** the chapter does **not** ship a `queryClient.clear()` on org switch — the inspector's "Clear client cache" button demonstrates the manual call, and lesson 3 of chapter 080's framing names this as the discipline at the tenancy boundary. The forward direction: production should wire `queryClient.clear()` (or `removeQueries` per org-scoped subtree) into the active-org-switch action. Name once; do not implement here.
...
- Forward references:
  - Chapter 082 — Zustand for genuinely shared client state; the chapter project layers a wizard's per-step shared state over four routes; the threshold question is run again against a different surface.
  - Unit chapter 093 — component tests for the comment thread; the optimistic-add-and-rollback assertions are mechanical against a mocked `addCommentAction`.
  - Unit chapter 085 — security baseline; the route handler's tenancy check is one of the audited findings (`authedRoute` on every read seam, not just write seams).
  - Unit chapter 074 — notifications dispatcher; a future "@-mention" feature on the thread would route through the dispatcher rather than the polling loop.
  - Unit chapter 096 — structured logs; the `comment.added` audit-log entry is the operator-truth side, the in-app notification (Unit 14) is the user-facing side — same notifiable-vs-logged distinction from chapter 074.
...
- The org-switch cache-clear is a forward pointer, not an implementation step here. The chapter explicitly does not ship it because the active-org-switch action lives in chapter 060 and the cache-clear hook into that action is a single line — name where it goes, don't reach in.
