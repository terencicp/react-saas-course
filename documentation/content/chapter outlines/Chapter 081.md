...
- **The threshold — three triggers, any one.** (a) Endpoint costs money per call (LLM, transactional email, SMS). (b) Can be used to attack a third party (victim's inbox, outbound fetch). (c) Touches state addressable without auth (sign-up, invite-accept by token, password reset). Endpoints failing all three get the wrapper's default per-user/per-org defensive limit, not a hand-rolled one. Limiters are not for "every endpoint" — only for endpoints matching one of the three.
  - **SSRF carve-out on trigger (b).** The outbound-fetch half of (b) is the vuln class SSRF: attacker supplies the URL the server fetches and aims it at an internal service, a private-IP database, or the cloud metadata endpoint `169.254.169.254`. A rate limiter does nothing here; mitigation is a destination-host allowlist that blocks private/loopback/link-local ranges plus the metadata IP by name. Carve-out only (~3 sentences, single `<Term>` gloss), not a section. Revisit trigger is generic — any future server-side fetch of a user-controlled URL (image proxy, link unfurler, webhook tester) — explicitly NOT RAG.
