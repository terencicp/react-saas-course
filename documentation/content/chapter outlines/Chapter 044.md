...
- **The shadcn `Form` primitive — what it adds, what it doesn't.** Chapter 027 introduced the shadcn UI primitives; for forms, shadcn ships `<Form>` (a thin wrapper over React Hook Form) plus `<FormField>`, `<FormItem>`, `<FormLabel>`, `<FormControl>`, `<FormDescription>`, `<FormMessage>` — primitives for the form's *layout* (label, control, error message in a vertical stack with consistent spacing) regardless of the form-handling library. The senior call for this chapter, corrected against the shipped shadcn API: only `<FormItem>` (a `grid gap-2` div) is genuinely RHF-free — `<FormLabel>`, `<FormControl>`, `<FormDescription>`, and `<FormMessage>` all call `useFormField()`, which reads RHF context and throws outside a `<FormField>`. So the chapter's default form stays the hand-rolled cluster (`<label htmlFor>` + shadcn `<Input>` + the course's own `<FieldError>` reading the flat `Result.error.fieldErrors`); `<FormItem>` is the one borrowable layout primitive. The project chapter (chapter 047) builds its Server-Action forms this way and must not render `<FormLabel>`/`<FormControl>`/`<FormMessage>` without RHF; the full shadcn form stack earns its weight only with RHF in Chapter 045.
...
- **The `permalink` argument of `useActionState`.** The third argument to `useActionState` is a URL string the browser *navigates to* when a Server-Function action is submitted before the bundle hydrates (it does not POST to it); the destination must render the same form component. The 2026 reach: name the route's own URL as the permalink so the no-JS submit lands back on the same page. Named once; usually the framework's default handling makes this unnecessary.
