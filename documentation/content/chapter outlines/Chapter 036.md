...
Chapter 036 establishes the data backplane the rest of Unit 5 (schema in chapter 037, queries in chapter 038, performance in chapter 039, migrations in chapter 040, project in chapter 041) and every later unit reads and writes through. It does two things and only two: it installs the mental model — rows, tables, foreign keys, normalization to 3NF as the default — that Drizzle schemas in chapter 037 will encode, and it stands up the running Postgres the student's `DATABASE_URL` will point at. The chapter does not write a `pgTable`, run a query, or touch Drizzle Kit; those are owned by chapter 037, chapter 038, and chapter 040 respectively. Postgres is the only database the course teaches; alternative engines (MySQL, SQLite, document stores) get one line for recognition.

Threads that run through every lesson: Postgres is the assumed engine; `DATABASE_URL` is the single connection-string contract every lesson consumes (introduced in lesson 2 of chapter 037); normalize to 3NF first and only denormalize against a measured read pattern; the local-dev database is a senior call between Docker Postgres (offline, drifts from prod) and Neon dev branch (matches prod exactly, needs connectivity), with Neon Local as the 2026 hybrid; production runs on Neon with a branch per preview deployment, scale-to-zero pricing, and the `@neondatabase/serverless` driver; the HTTP driver is the default for one-shot Server Component reads and the WebSocket/Pool driver is the conditional reach for transactions and Server Actions; the pooled connection string (PgBouncer transaction mode) is mandatory for any TCP/WebSocket use from serverless runtimes. The chapter ships four teaching lessons plus a quiz.
...
- **The senior question.** Given a SaaS feature spec (invoices belonging to organizations, each with line items, each tagged), what shape does the data take on disk before any code is written? The lesson establishes that the schema is decided up front, in the relational shape Postgres expects, and that this shape constrains every component, query, and API surface downstream. Connects to Architectural Principle #2 (lesson 1 of chapter 037) — the schema is the source of truth.
...
- **Primary keys named, deferred.** Every row needs a unique identifier; the surrogate (UUID, bigserial) vs. natural (email, slug) decision is lesson 5 of chapter 037. Named here so the student knows what "primary key" means when normalization needs it.
- **Foreign keys named, deferred.** A column in table A whose values must exist as a primary key in table B; the cascade options (`ON DELETE`, `ON UPDATE`) are lesson 6 of chapter 037. Named here so "this row belongs to that one" has a vocabulary.
...
- **When to denormalize — the senior call.** Denormalization is a measured response to a read pattern, never a starting point. Three legitimate triggers: a hot read path where the join cost is measured and material (a feed showing usernames per comment), reporting tables built by jobs (not by hand), and `jsonb` for genuinely shapeless data (audit-log payloads, third-party webhook bodies — lesson 9 of chapter 038 owns this). The wrong trigger: "joins feel slow" without `EXPLAIN ANALYZE` (lesson 3 of chapter 039).
...
- **One-to-one, one-to-many, many-to-many — the three cardinalities.** Each gets a one-line shape: 1:1 collapses into one table or two with a shared key; 1:N is one foreign-key column on the many side; N:M is a junction table with two foreign keys (lesson 8 of chapter 037 owns the junction shape).
...
- **Watch-outs:** comma-separated columns are a normalization failure that compounds; the `jsonb`-everywhere temptation moves type safety out of the database; denormalization without a measurement is premature optimization; "we'll just join more tables" is rarely the actual bottleneck — indexes (lesson 1 of chapter 039) are; relational shape is decided before code, not refactored after.

What this lesson does not cover:

- Drizzle's `pgTable`, columns, or relations — Chapter 037.
- Postgres data types in depth — lesson 3 of chapter 037.
- Primary keys (surrogate vs. natural) — lesson 5 of chapter 037.
- Foreign keys and cascade behavior — lesson 6 of chapter 037.
- Junction-table mechanics — lesson 8 of chapter 037.
- `jsonb` querying — lesson 9 of chapter 038.
- Indexes and `EXPLAIN ANALYZE` — Chapter 039.
- Multi-tenant scoping patterns — Unit 9.

Estimated student time: 45 to 55 minutes. Load-bearing for every schema, query, and migration in Units 5 through 23.
...
- **`DATABASE_URL` as the single contract.** Whatever runs the database, the app reads a Postgres connection string from `env.ts` (lesson 2 of chapter 037). Switching providers is changing one environment variable. This is the abstraction the lesson protects.
...
- **The course's default.** The course assumes Neon dev branch in subsequent chapters; the Docker Compose alternative is shown once here and the lesson notes that swapping providers is one env variable. Project starters in Unit chapter 041 and later assume `DATABASE_URL` is set, agnostic to source.
- **Database GUIs — named, dropped.** TablePlus, pgAdmin, DataGrip, Neon's web console, Drizzle Studio (lesson 1 of chapter 040). The course teaches Drizzle Studio as the in-stack default; alternatives named once for the student to pick later.
- **Migrations and seeds named, deferred.** Whichever provider runs, the same Drizzle Kit migrations apply (lesson 1 of chapter 040) and the same seed script populates (lesson 2 of chapter 040). The lesson does not run them.
- **Watch-outs:** `DATABASE_URL` in `.env.local` only — never committed (lesson 2 of chapter 037); the Docker volume persists data across `docker compose down`, but `docker compose down -v` nukes it; Neon dev branches scale to zero and the first query after idle takes a second or two — this is the feature, not a bug; running both Docker Postgres and Neon Local at once collides on port 5432; Postgres major-version drift between local and prod is the single most common "works on my machine" cause.

What this lesson does not cover:

- Neon production branching per preview deploy and scale-to-zero economics — lesson 3 of chapter 036.
- The `@neondatabase/serverless` driver, HTTP vs. WebSocket, pooling — lesson 4 of chapter 036.
- Drizzle's connection setup in code — Chapter 037.
- Drizzle Kit migrations, the studio — lesson 1 of chapter 040.
- Seeding for dev and test — lesson 2 of chapter 040.
- Self-hosted Postgres on RDS/Supabase/Fly — named once, never taught; the course commits to Neon.

Estimated student time: 35 to 45 minutes. Load-bearing for every later lesson in Unit 5, which assumes a running Postgres at `DATABASE_URL`.
...
- **Branching for data migrations and "what-if" debugging.** A senior reach: branch production, run the destructive migration or the exploratory query on the branch, throw the branch away. Named here so the student knows it's available; the migration workflow is lesson 1 of chapter 040.
...
- The `@neondatabase/serverless` driver, HTTP vs. WebSocket, connection pooling — lesson 4 of chapter 036.
- Drizzle's connection code — Chapter 037.
- Migration workflow against branches — lesson 1 of chapter 040.
- Vercel deployment, environment variables, preview deployments at depth — Unit 20.
...
Estimated student time: 40 to 50 minutes. Load-bearing for lesson 4 of chapter 036 (the driver choice depends on understanding that production runs on Neon), Unit 20 (deployment wires the Vercel integration), and the project in Chapter 041.
...
- Drizzle's `db` wiring, `drizzle-orm/neon-http`, `drizzle-orm/neon-serverless` setup code — Chapter 037.
- Server Actions and where transactions earn their weight — Chapter 043.
- Migrations under the unpooled URL — lesson 1 of chapter 040.
...
- Background workers and long-lived Postgres clients — Unit 12.

Estimated student time: 45 to 55 minutes. Load-bearing for Chapter 037 (Drizzle's `db` setup mirrors the two-client pattern), Chapter 038 (every query in the course runs through one of the two clients), Chapter 040 (migrations use the unpooled URL), and Chapter 043 (Server Actions reach for the transactional client).
