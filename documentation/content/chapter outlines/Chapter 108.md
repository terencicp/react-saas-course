...
src/
  ...
  lib/
    ...
    llm/
      models.ts                   # provided (lesson 3 of chapter 105): chatModel = openai('gpt-chapter 017-mini')
...
- Scope cuts: no `generateObject` / `streamObject` here (taught in lesson 2 of chapter 106); no `ai/rsc` / `streamUI` (named once as the experimental alternative in lesson 2 of chapter 107); no embeddings or RAG (out of scope — lesson 3 of chapter 107's lesson stays the reference); no destructive tools (propose-then-confirm pattern named in lesson 2 of chapter 107 stays a reference); no multi-modal inputs; no persisted conversation history (`useChat` keeps it in component state — refresh loses; persistence pattern named at the end as a forward pointer); no provider-swap demo (the model is fixed via `lib/llm/models.ts`); no human-in-the-loop confirmation; no E2E test against a real OpenAI call (deferred to the team's CI strategy).
...
- Starter ships chapter 062 end-to-end plus the AI SDK packages, `OPENAI_API_KEY` env entry, the two new tables (`usage_quota_daily`, `llm_audit_events`), the provided model registry, the inspector, and every UI shell. Every change lives under `src/lib/llm/`, `src/app/api/chat/`, `src/app/api/usage/`, and three client components in `src/app/(app)/invoices/`.
- The TOC's three-build split is preserved because each closes on a runnable state: lesson 3 of chapter 108 ends with the route streaming text-only answers under auth; lesson 4 of chapter 108 ends with the tool callable and quotas live; lesson 5 of chapter 108 ends with the client rendering parts and the usage panel.

Codebase state at entry: empty repo (student runs `degit`).
Codebase state at exit: starter cloned, Postgres up, schema migrated, seed loaded; `/invoices` renders the chapter 062 list with an empty right-rail chat shell (input disabled); `/inspector` loads with usage at 0, audit-log empty, no events.
...
- Read `src/lib/llm/models.ts` (provided): one export, `chatModel = openai('gpt-chapter 017-mini')`. The lesson notes that swapping providers is a one-line change here — provider-abstraction discipline from lesson 3 of chapter 105.
...
- Read `app/(app)/invoices/page.tsx`: the chapter 062 list view, plus a new right-rail slot rendering `<InvoiceChat />`. The slot is a shell pointing at an unwritten component.
- Read `package.json`: `ai@^5`, `@ai-sdk/openai`, `@ai-sdk/react`. Note the v5 import paths.
- Read `next.config.ts`: `cacheComponents: true` from chapter 062. The chat surface is fully client-side and won't interact with Cache Components; the streaming route handler is dynamic by definition.
...
- Seven student files plus three client components is the entire build surface. The chapter 062 list, the schema, and the auth wrappers don't change.
...
- Note that the route file is `app/api/chat/route.ts`, not a Server Action. Streaming responses are route handlers; the `authedRoute` wrapper from lesson 3 of chapter 057 is the variant of `authedAction` adapted to `Request`/`Response` shapes.
...
- Fill `src/lib/llm/prompts.ts`: a single export `invoiceQAPrompt({ orgName })` returning the system prompt string. Three load-bearing lines: enforce tool-grounding ("Always call getInvoiceStats before stating numeric facts"), refuse cross-org questions, define error behavior ("If a tool returns { error }, explain and ask the user to rephrase"). The prompt is the controller (lesson 1 of chapter 106's posture); user messages are untrusted input.
- Fill `src/lib/llm/audit.ts`: two direct inserts into `llm_audit_events` that mirror the audit-log discipline from lesson 5 of chapter 057's `logAudit` (one row per event, no off-transaction writes); the table differs from `auditLogs` so these aren't `logAudit` wrappers: `writeLlmStepEvent({ userId, orgId, toolCalls, finishReason, usage })` and `writeLlmFinishEvent({ userId, orgId, finishReason, usage })`. Both insert into `llm_audit_events`. Each wrapper opens a one-statement `tenantDb(ctx.orgId).transaction(async (tx) => logAudit(tx, { ... }))` — `logAudit`'s signature from lesson 5 of chapter 057 requires a `tx` so off-transaction calls don't typecheck. The route handler itself isn't transactional; the wrapper supplies the bounded one-row transaction the signature demands.
...
- `onError` returns a sanitized log; the client's `useChat` `onError` will toast a friendly message — do not leak raw errors. The chapter 106 pattern repeated.
- The audit write at `onFinish` is the aggregate event. Per-step audit lands in lesson 4 of chapter 108 once tools are wired.
...
- Fill `src/lib/llm/tools.ts`. The factory shape:
  ...
  - The closure over `ctx.orgId` is the load-bearing pattern: `orgId` is **never** in the tool's `inputSchema`. The model cannot pass it; the model cannot fake it; the model cannot ask for another org's data. The senior rule from lesson 1 of chapter 107 made structural.
  - `outputSchema` projects an aggregate, not raw rows. A `count`, a `totalAmount`, a `byStatus` map, and a single date. This is the "return minimal" discipline from lesson 1 of chapter 107 — feeding raw rows back to the model would compound input tokens across loop steps.
...
- Wire the tool and quota into the route. Update `app/api/chat/route.ts`:
  ...
  - `onFinish` unchanged from lesson 3 of chapter 108.
...
- "Return don't throw" inside `execute` is the lesson 1 of chapter 107 rule. The `try/catch` wraps the Drizzle call; programmer errors (bad Drizzle types, undefined references) still bubble — those are bugs, not user-facing failures.
...
- Replace the smoke-test `invoice-chat.tsx` with the full typed component. `'use client';`
  ...
  - Form submit: `const onSubmit = (e) => { e.preventDefault(); if (status === 'streaming' || status === 'submitted' || !input.trim()) return; const text = input; setInput(''); sendMessage({ text }) };` — the `status` guard prevents double-submits, matching chapter 079's `useTransition` pattern.
...
- Fill `invoice-stats-card.tsx`. `'use client';` accepts `{ state, input, output }` typed via `InferUITools<InvoiceTools>['getInvoiceStats']`. Switch on state:
  - `input-streaming` → return null (the brief argument-arrival window — skip rendering, the parts protocol from lesson 2 of chapter 107).
...
- `useChat<InvoiceUIMessage>` is the typed shape. Without the generic, `part.output` is `unknown` and every switch branch needs `as` casts — the type contract is the whole point of the `InferUITools` pattern from lesson 2 of chapter 107. Name at the call site.
...
- The `status` guard prevents double-submits the way `useTransition` does in chapter 079. The patterns rhyme: an in-flight request's button is disabled.
- The per-tool skeleton over a generic spinner is the lesson 2 of chapter 107 rule. A 5-tool chat would have 5 different skeletons; this chat has one. The shape matters because the skeleton conveys what's coming.
...
- The `default` case in the parts switch returns `null` — graceful fallback for unknown part types (future tools added, transient parts, etc.). lesson 2 of chapter 107's "always have a default case" rule.
...
- Forward references:
  - Chapter 080 — error discipline; the LLM surface's user/operator message split is one audit row, the sanitized toast vs. the audit-events payload.
  - Chapter 082 — security baseline audit; the route's `authedRoute` wrap and the closure-`orgId` rule are two seeded audit findings.
  - Unit 18 — testing; the integration tests for this route mock the model via the AI SDK's `MockLanguageModelV2` (named once); the tool's `execute` is unit-testable as a plain function.
  - Chapter 092 — structured logs; the `llm_audit_events` table is the operator-truth side, the chat UI is the user-facing side.
  ...
  - lesson 3 of chapter 107 — RAG; the next senior reach when invoice-stats questions outgrow what aggregate tools can answer ("explain why we have so many overdue this month" needs grounding in invoice descriptions, which exceeds aggregates).
