...
| Preview branch verification ran on every PR | Each PR's `vercel-build` check in CI shows `pnpm db:migrate && next build` succeeding; the PR's preview URL exercised the new code against a Neon branch; the rehearsal checklist (lesson 3 of chapter 099) was applied. |
...
- Frame the build: take the chapter 062 invoices project, ship it to a real production URL on Vercel against a Neon branch-per-PR workflow, then execute one cadence-class migration (split `invoices.total numeric NOT NULL` into separate `subtotal numeric NOT NULL` + `tax numeric NOT NULL` columns, dropping `total`) across three reviewed PRs without any moment of schema/app incompatibility in production. Close by rehearsing a production rollback against the contract PR.
...
- Scope cuts: no custom domain provisioned (use the `*.vercel.app` URL — the gesture for domain swap was lesson 4 of chapter 098); no Cloudflare-in-front; no OIDC for cloud credentials (R2 credentials carry from Unit 12 as long-lived env vars; OIDC upgrade is named not done); no rolling release (full cutover with fast rollback is the default); no production migration via a separate gated CI job (the project uses the build-command migration path that lesson 5 of chapter 098 sets up, which is correct for the cadence the project exercises and is named as the simplification in chapter 099); no pgroll, no schema-management tooling beyond Drizzle Kit; not solving cross-tenant data corrections or business-logic flag-rollouts bundled with the migration — the cadence covers schema choreography only; not modeling per-line-item tax rates or jurisdiction lookups (the backfill assumes `tax = 0` against the seed; real-world tax computation is its own problem).
...
- The starter ships the chapter 062 surface end-to-end with a deliberately under-modeled shape: `total` is a single combined amount with no breakdown of subtotal and tax. The cadence fixes a real anti-pattern (a column that conflates two business concepts), not a contrived one.
...
- One full chapter (~3h) is the realistic clock; the chapter can be sliced across two sittings — the first ending after lesson 3 of chapter 100 (expand merged, prod healthy) or lesson 4 of chapter 100 (migrate merged, backfill green), the second finishing with contract and rollback. The PR-level grain makes the natural pause points obvious.
...
Codebase state at exit: starter cloned locally; `pnpm install` succeeds; `pnpm dev` against local docker postgres renders chapter 062's surface; the student has read this brief and knows the three PRs ahead. No code written; no deploy yet.
...
- Run the lesson 2 of chapter 098 + lesson 3 of chapter 098 + lesson 5 of chapter 098 + lesson 6 of chapter 098 + lesson 8 of chapter 098 wirings concretely on this repo. The chapter material taught the moves; this lesson does them once against the project, end to end, producing the production URL the rest of the chapter targets.
...
- Push the starter to a fresh private GitHub repo. Confirm branch protection on `main` (lesson 4 of chapter 096): no direct pushes, require PR + green CI before merge, require at least one review (in a solo course this is a soft self-attestation, but the rule is set).
...
- Watch the second build succeed: `pnpm install` → `pnpm db:migrate` (against the Neon `main` branch; applies every migration up to the chapter 062 baseline) → `next build`. The route summary shows static vs. dynamic, function bundle sizes.
- Set the Function Region to match the Neon region (Project Settings → Functions). lesson 3 of chapter 098's load-bearing diff. Default `iad1` matches Neon's `aws-us-east-1`; the student verifies their region before clicking.
...
- Sentry's `SENTRY_DSN` is required. Even if Sentry was wired in Unit 19, the project assumes the student has an account and a project there; if not, the launch checklist's row 2 cannot be completed and the project is not done. The course material made this clear in Unit 1's account-setup brief.
- `pnpm db:migrate` in the build command is the project's chosen pattern (matches lesson 5 of chapter 098's wiring and the cadence the chapter exercises). The trade-off — production migrations run on every deploy, which is safe under the cadence's discipline but would be reckless without it — is named once and not re-litigated; lesson 1 of chapter 099's discussion is the canonical reference.
...
Codebase state at exit: production URL live serving the chapter 062 surface against the Neon `main` branch; preview-per-PR workflow verified end-to-end; launch checklist green (recorded); throwaway PR closed; Neon branch auto-deleted. **Runnable — production is live on the baseline chapter 062 schema; ready to begin the cadence.**
...
Ships an additive-only migration that adds `subtotal numeric(12,2)` and `tax numeric(12,2)` as nullable columns, rehearses it on the Neon preview branch, merges, and verifies the running chapter 062 app code stays healthy against the expanded schema.
...
Codebase state at entry: chapter 062 baseline live in production; no migration in flight.
Codebase state at exit: PR 1 merged; production runs the unchanged chapter 062 app code against a schema that has `total` (still NOT NULL), plus `subtotal` and `tax` (both nullable, unwritten). Split-coverage 0%. Inspector confirms. Documented in the runbook. **Runnable — production is healthy on the expand schema.**
...
- Document the gesture in `docs/runbooks/rollback.md`:
  ...
  - Step 5: open a `git revert` PR for the bad commit (lesson 2 of chapter 096 gesture); merge after CI; the next prod deploy ships the reverted code.
...
- Sentry's reaction during the rehearsal is the verification — if Sentry doesn't notice, observability is broken. The chapter's launch checklist (lesson 8 of chapter 098) row 2 includes Sentry receiving errors; the rehearsal exercises it on a real (transient) error.
...
- The cadence + the rollback rehearsal close out lesson 7 of chapter 098's instant-rollback mechanic and lesson 1 of chapter 099's forward-only premise as one integrated discipline. The student should be able to articulate: "alias rollback is fast and free for code; schema rollback is slow and expensive forward-fix. The cadence is what keeps the latter rare."
