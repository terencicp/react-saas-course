...
- The *deep* Sources / server-side debugger workflow — the course's day-to-day on this stack is heavy on Server Components and Server Actions, where the debugger lives in the Node process, not the browser. Deferred to the error-monitoring chapter. One browser-side beat — line breakpoint, `debugger;`, step over/into/resume, the Scope pane — is now taught in lesson 3.
