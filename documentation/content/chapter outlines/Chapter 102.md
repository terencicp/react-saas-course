...
- **What does *not* happen automatically.** No automatic database migration (Drizzle migrations are a separate CI step in chapter 103). No automatic external-CDN purge (Cloudflare-in-front needs its own). No automatic secret rotation. The deploy ships code; everything stateful is the engineer's responsibility.
...
- **The trust model.** Vercel reads from GitHub via OAuth, never has push access. The audit trail lives in GitHub commit history plus Vercel's deployment log. Senior reflex: never run unreviewed scripts at build time — a malicious dep in `pnpm install` runs on Vercel's builder with full env-var access; SHA-pinned actions (chapter 101) and `minimumReleaseAge` in `.npmrc` are the prevention.
...
- First-deploy mechanics — lesson 2 of chapter 102; runtime/region — lesson 3 of chapter 102; domains — lesson 4 of chapter 102; preview+Neon — lesson 5 of chapter 102; env scoping — lesson 6 of chapter 102; rollback — lesson 7 of chapter 102; launch checklist — lesson 8 of chapter 102; schema migrations — Chapter 103.
...
- **Prerequisites.** GitHub repo with the Next.js 16 starter and a green `main` (CI gate from chapter 101 is live). Vercel account. `package.json` declares `"packageManager": "pnpm@9.x"` so Vercel auto-detects pnpm.
...
- **The Configure Project screen.** Project name (becomes the `*.vercel.app` subdomain; rename later costs the old URL). Framework Preset (auto-detected, leave). Root Directory (`./` for single-app; subdirectory for monorepo). **Skip env vars on first deploy** — added in lesson 6 of chapter 102. Click Deploy.
...
- **The first production URL.** `<project-name>.vercel.app` becomes the production deployment URL. Live on the public internet. Custom domain (lesson 4 of chapter 102) replaces this as primary.
...
- **The first preview URL.** Open any PR. Bot comments: "Visit Preview" → `your-app-git-<branch>-<org>.vercel.app`. Fully functional, billed against the project. **At this point** (before lesson 5 of chapter 102) the preview shares the production database — don't run destructive operations against it.
...
- **`vercel.json` — when to add it.** Unnecessary for the default Next.js project on Node.js runtime. Reach for it for: `headers` security headers (lesson 8 of chapter 102), per-route runtime override, `crons` (use Trigger.dev instead — Unit 13). Most SaaS projects ship with no `vercel.json` or a 10-line one.
...
- **The deployment notification surface.** GitHub commit status checks (build pass/fail dot), GitHub PR comments with preview URLs, optional Slack via Settings → Integrations. The CI gate's checks (chapter 101) and Vercel's build check live side by side on each PR. Both must be green to merge.
- **Watch-outs:** broad GitHub App permissions at install — scope to one repo; project name typos — subdomain renames hurt; first preview hitting prod DB — destructive ops dangerous until lesson 5 of chapter 102; sharing `*.vercel.app` URLs as production — get a custom domain first; missing `packageManager` field — falls back to npm.

What this lesson does not cover:

- Deployment model — lesson 1 of chapter 102; runtime/region — lesson 3 of chapter 102; domains — lesson 4 of chapter 102; preview+Neon — lesson 5 of chapter 102; env scoping — lesson 6 of chapter 102; rollback — lesson 7 of chapter 102.
...
- **The Node.js runtime as the default.** Vercel Functions ship on Node.js in 2026. Full Node.js API surface, native modules, all npm packages, streaming. 15s timeout on Hobby, up to 15 min on Pro. chapter 025 MB payload. **The default the course commits to** for every server action, route handler, and RSC fetch.
...
- **Function memory, CPU, payload, timeout.** 1024 MB default, configurable to 3009 MB. Memory and CPU scale together. chapter 025 MB request body limit; 25 MB chunked streaming response. Uploads larger than chapter 025 MB route through R2 presigned URLs (Chapter 073) — file never traverses Vercel. Hobby 10s timeout, Pro 60s default up to 15 min. Anything plausibly slow goes async (Trigger.dev, Unit 13); the timeout is the canary, not the budget.
...
- Deployment model — lesson 1 of chapter 102; first deploy — lesson 2 of chapter 102; domains — lesson 4 of chapter 102; preview+Neon — lesson 5 of chapter 102; background jobs — Unit 13; Sentry — Unit 20.
...
- **HTTPS-only enforcement.** Vercel auto-redirects HTTP → HTTPS on custom domains. The HSTS header (`Strict-Transport-Security`) needs to be set explicitly in the app's response headers — Vercel doesn't add it. The launch checklist (lesson 8 of chapter 102) names this.
...
- Deployment model — lesson 1 of chapter 102; runtime/region — lesson 3 of chapter 102; preview deployments — lesson 5 of chapter 102; security headers — lesson 8 of chapter 102 / Chapter 085; tenant-subdomain routing — Unit 10.
...
- **Schema migration on preview branches.** When a PR's `drizzle/migrations/` changes, the preview's DB starts as a copy of production *before* the migration. The preview's build needs to run the migration against the preview branch before the app boots. Standard wiring: `pnpm db:migrate` runs as part of `next build` via the Build Command override (`pnpm db:migrate && next build`) or a `prebuild` script. Reads the deployment's `DATABASE_URL` (the preview branch), applies migrations. Chapter 103 owns the safety story; this lesson treats migrations as a black box.
- **Production caveat on the build-command migration.** With migrations in the build command, every prod deploy runs migrations against `main`'s database. Safe with the expand-migrate-contract cadence (Chapter 103), risky with a naive destructive migration. Senior call: migrations in build command for previews; production migrations gated through a CI "migrate" job with explicit approval. Chapter 103 owns the full story.
...
- Deployment model — lesson 1 of chapter 102; env variable scoping — lesson 6 of chapter 102; rollback — lesson 7 of chapter 102; expand-migrate-contract — Chapter 103; Drizzle framework — Unit 6; tenant-data isolation — Unit 10.
...
- **Managed variables.** Some integrations inject vars you can't edit by hand. The Neon integration (lesson 5 of chapter 102) manages `DATABASE_URL` in Preview. Vercel marks these; editing requires uninstalling. Managed vars are correct by construction — don't fight them.
...
- Deployment model — lesson 1 of chapter 102; preview+Neon — lesson 5 of chapter 102; rollback — lesson 7 of chapter 102; the validator schema in detail — Unit 5; Stripe webhook secrets — Unit 12; full rotation playbook — Chapter 085.
...
- **The code-side `git revert`.** From lesson 2 of chapter 100: `git revert <bad-sha>` creates a new commit applying the inverse. PR as normal (or direct to `main` with team consent). The CI gate (chapter 101) gates the revert PR — yes, it slows emergency response, but bypassing it on revert is a habit-corruption risk; keep the gate, trust the four-minute build.
...
- Deployment model — lesson 1 of chapter 102; env scoping — lesson 6 of chapter 102; migration cadence — Chapter 103; `git revert` — lesson 2 of chapter 100; postmortems — Unit 22; feature flags — chapter 097.
...
- **The mindset — launch is not done at the URL.** The first production URL was live by lesson 4 of chapter 102. Launch means the URL is *defensible* — the app degrades gracefully on errors, doesn't leak data, doesn't fall over under load, has someone watching when it does.
- **Row 1 — Env validation green in production.** Verify: production build logs show env-validator passed; `SKIP_ENV_VALIDATION` is not set in production. A missing required env var means the first runtime call fails with an opaque error; the build-time validator catches it before the deploy. Cross-ref: lesson 6 of chapter 102.
- **Row 2 — Error monitoring wired and receiving.** Verify: Sentry initialized in `instrumentation.ts`; a deliberate test error in production shows in the Sentry dashboard within seconds; source maps uploaded so stack traces are readable. Errors invisible to the team compound. Cross-ref: chapter 096.
- **Row 3 — Rate limits live on the abuse surface.** Verify: auth endpoints (sign-in, sign-up, password reset, magic link) are rate-limited via Upstash from Unit chapter 079; a script hitting `/api/auth/sign-in` 50 times in 10 seconds gets 429s after threshold. Reach for `hey` as the simple HTTP load-testing CLI for this check (`hey -n 50 -c 5 <URL>`); equivalents include `oha` or a curl loop. Unrated auth endpoints are credential-stuffing targets on day one. Cross-ref: chapter 079.
- **Row 4 — Audit logs writing.** Verify: every privileged action (org-membership changes, role changes, billing, data exports) writes a row to the audit table from Unit chapter 061; `select * from audit_log order by created_at desc limit 10` shows recent activity. Missing rows mean compliance and incident-response fly blind. Cross-ref: chapter 061.
...
- **Row 6 — Database pooling and matching region.** Verify: Drizzle uses Neon's pooled connection string (`-pooler` in the hostname); production function region matches the DB region (lesson 3 of chapter 102). Unpooled connections exhaust the database under load; cross-region functions pay 80ms+ per query. Cross-ref: lesson 3 of chapter 102, Unit 6.
...
- Rollback mechanic — lesson 7 of chapter 102; Sentry — Unit chapter 096; rate limiting — Unit chapter 079; audit log schema — Unit chapter 061; full CSP — Chapter 085; runbook templates — Unit 22.
