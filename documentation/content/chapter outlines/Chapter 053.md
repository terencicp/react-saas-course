...
- **Argon2id by default.** Memory cost 64 MiB, time 3, parallelism 4, per-password salt. Hash lands in `account.password` (the column on `account`, not `user`, from lesson 2 of chapter 052). Override only for measured constraints.
...
- **The Server Action wrapping.** Zod on input (lesson 4 of chapter 043), `auth.api.signUpEmail({ body, headers })` server-side, return the `Result` shape with `'ok'` / `'invalid-input'` / `'email-taken'` discriminants.
...
- **The `additionalFields` extension hook.** Mandatory: email, password. Optional: `name`. Custom fields (`role`, `companyName`, `timezone`) land via `user.additionalFields` from lesson 2 of chapter 052 — typed on `signUpEmail`'s body and on `getSession`'s user. Senior reach: keep sign-up minimal, defer everything else to onboarding.
...
- **Database state post-sign-up.** One `user` (`emailVerified: false`), one `account` (`providerId: 'credential'`, `password: <hash>`), one `verification` row keyed by email. lesson 3 of chapter 053 consumes the verification row; lesson 2 of chapter 053 reads `emailVerified`.
- **Rate limits — named, deferred.** Built-in plugin (in-memory dev, `secondaryStorage`-backed prod) sits on every public auth endpoint. Default per-IP on sign-up is conservative; tighten for production. Full wiring in Chapter 074.
- **Watch-outs.** `autoSignIn: true` with `requireEmailVerification: true` — broken state; surfacing `'email-taken'` distinctly — enumeration leak undone; trusting client-side strength server-side; missing email trim/lowercase — `Ada@acme.com` and `ada@acme.com` slip past the unique index; sending verification email through anything other than the Unit 7 pipeline — suppression list / DKIM / template anatomy live there; password floor below 8.

What this lesson does not cover:

- Verification token shape, click-handler, post-verify redirect (lesson 3 of chapter 053).
- Sign-in itself, failed-attempt counter (lesson 2 of chapter 053).
- Password reset (lesson 4 of chapter 053), 2FA enrollment (lesson 6 of chapter 053), OAuth sign-up (lesson 8 of chapter 053).
- Full rate-limit wiring (Chapter 074), email template anatomy (Chapter 049).
...
- **The library flow and the discriminant surface.** Library resolves the email, finds the `'credential'` account, verifies the hash in constant time, checks `emailVerified`, checks the failed-attempt counter, returns "requires second factor" if 2FA is enabled. Result discriminants:
  ...
  - `'email-not-verified'` — known account, correct password, `emailVerified: false`. Re-sends verification when `sendOnSignIn: true` (lesson 3 of chapter 053); UI renders "check your inbox."
  - `'too-many-attempts'` — failed-attempt counter tripped; UI shows "try again in N minutes."
  - `'requires-second-factor'` — credential validated, 2FA enabled; carries a `factor-token` consumed by the 2FA prompt (lesson 6 of chapter 053).
...
- **The Server Action wrapping.** Same shape as sign-up — Zod (`email`, `password`, `rememberMe`), the call, the `Result` mapping. Form uses `useActionState` (lesson 2 of chapter 045) and surfaces per-discriminant copy.
...
- **Session rotation on sign-in.** Better Auth issues a fresh `session.token` on every successful sign-in — never reusing any pre-auth token. The fixation defense from lesson 2 of chapter 051 at the call site; no config required.
...
- Full rate-limit wiring (Chapter 074).
- Verification flow (lesson 3 of chapter 053), password reset (lesson 4 of chapter 053).
- TOTP enrollment and challenge UI (lesson 6 of chapter 053).
- OAuth sign-in (lesson 8 of chapter 053), active-sessions list (lesson 3 of chapter 054).
- 401-versus-403 wire shapes (lesson 3 of chapter 046, lesson 4 of chapter 054).
...
Teaches how to wire `sendVerificationEmail` through the Unit 7 Resend pipeline, the hashed-token row in the `verification` table, the click-through endpoint that flips `emailVerified`, and the enumeration discipline that holds across every entry point.

Topics to cover:

- **The senior question.** Sign-up queued an email; sign-in refuses unverified accounts. What's the token shape, how does the click-through endpoint verify it, what's the rotation/expiry policy, and how does enumeration discipline survive the verification surface? Configure `emailVerification`, wire the React Email template through Unit 7, land the verify endpoint that flips `emailVerified`.
...
- **The link shape.** `https://app.example.com/api/auth/verify-email?token=<token>&callbackURL=<dest>`. The catch-all from lesson 1 of chapter 052 routes it; library hashes the token, looks up the row, checks expiry, flips `emailVerified`, deletes the verification row, and (with `autoSignInAfterVerification`) issues a session and redirects to `callbackURL`.
- **The React Email template.** Lives in `emails/verify-email.tsx`. One CTA button (`<Button href={url}>Verify email</Button>`), a fallback plain-text URL, an expiry note, the from-address discipline from Chapter 048. No marketing, no cross-sells — one job.
...
- **The downstream gate.** Once flipped, sign-in lets the user through, the proxy.ts gate from lesson 4 of chapter 052 lets them past `/dashboard`, and lesson 1 of chapter 054's pattern reads the flag for billing-tier gating. `emailVerified` is the *floor* for capability, not the ceiling — every per-action authz still applies (Unit 9).
...
- **Email-change verification — same flow, different namespace.** When the user changes email from account settings (lesson 2 of chapter 054), the same table holds the new-email token with `identifier` namespaced to the user ID. Reuses the same primitives.
- **Watch-outs.** Sending through anything other than the Unit 7 pipeline; storing the raw token; distinct copy for "invalid" vs "expired" — enumeration; letting expired rows accumulate without cleanup; reflecting `callbackURL` without allowlisting; surfacing the token in analytics / Sentry breadcrumbs.

What this lesson does not cover:

- Password reset (lesson 4 of chapter 053), magic-link (lesson 5 of chapter 053), email-change at settings (lesson 2 of chapter 054).
- React Email template anatomy (chapter 049), send pipeline (chapter 050).
- Rate-limit windows (Chapter 074).
...
- **Token-on-URL discipline.** Visible in history, referer, logs. Mitigations: short expiry, one-time use (row deletes on success), `Referrer-Policy: same-origin` (lesson 4 of chapter 054 default), don't log full URLs. Some products move the token to the URL fragment for extra defense; default is fine.
- **The "session invalidation on reset" property — the load-bearing trade.** Reset's purpose is "the old credential might be in attacker hands." If sessions stayed alive, the attacker keeps acting after the legitimate user resets. Non-negotiable. Same discipline applies to password *change* from settings (lesson 2 of chapter 054 — every session except the current one is revoked).
...
- **Rate limits.** Per-IP stops harvesting reset emails for many users; per-email stops weaponized inbox flooding. Library defaults sensible; tighten for prod. Full wiring in chapter 074.
- **2FA composition.** If 2FA is enabled, reset alone doesn't bypass it — sign-in after reset still requires the second factor. The reason 2FA matters: a leaked password (plus a network attacker who controls the email forwarder) still doesn't yield access. If the second factor is also lost, recovery codes from lesson 6 of chapter 053 are the path; absent that, support-driven identity verification (out of scope for the library).
...
- Verify-email flow (lesson 3 of chapter 053), change-password from settings (lesson 2 of chapter 054).
- Recovery codes (lesson 6 of chapter 053), send pipeline (chapter 050).
- Rate-limit windows (chapter 074).
...
- `verification` table mechanics (lesson 3 of chapter 053), send pipeline (chapter 050).
...
- 2FA composition surface (lesson 6 of chapter 053).
- Rate-limit windows (chapter 074).
...
- **Six-step enrollment.**
  ...
  2. Action calls `authClient.twoFactor.enable({ password })` — the elevation check (current password or fresh session via `freshAge` from lesson 3 of chapter 052).
...
- **Step-up for sensitive actions.** Some actions require a fresh 2FA challenge regardless of session age — exporting data, billing, ownership transfer. Library surfaces `auth.api.requireTwoFactor(...)`; full coverage in Unit 9.
...
- Passkeys (lesson 7 of chapter 053), SMS (not covered — TOTP/passkeys are the senior baseline).
- Full elevation-tier UI and credential changes (lesson 2 of chapter 054).
- Step-up middleware pattern (chapter 057), rate-limit windows (chapter 074).
...
- **Recovery path.** A user with one synced passkey and a lost iCloud account has lost the credential. Recovery codes from lesson 6 of chapter 053 serve as the universal fallback. Every passkey enrollment also surfaces recovery codes; no-recovery is an explicit opt-in.
...
- TOTP (lesson 6 of chapter 053), step-up surface (chapter 057).
...
- **The senior question.** lesson 3 of chapter 051 installed the OAuth 2.1 + PKCE model. Practical: a "Sign in with Google" button. What's the config, what's the redirect URI to register, where does the callback live (catch-all from lesson 1 of chapter 052), what's in the `account` table post-callback, what's the difference between sign-in-with-existing and first-time-OAuth-sign-up, and what are the per-provider quirks? Configure Google as the canonical, name the others, surface the post-callback hooks.
...
- **First-time-OAuth vs existing — lookup logic.** Library callback first looks up by `(providerId, accountId)` — does an account row already link this Google identity to a user? If yes, sign in. If no, look up by email — does a user with this email already exist? If yes and `accountLinking.enabled` is true (lesson 9 of chapter 053), link the new provider. If no, create new user + account. The lookup order matters; the linking config decides whether two paths to the same email become one account or two.
...
- **The security trade.** Linking is a trust transfer — "whoever controls this Google identity is the same person who knows this password." If `email_verified` is wrong (provider bug, Workspace edge), an attacker who controls a Google account with `ada@acme.com` (via domain takeover) can sign in as Ada without her password. Mitigations:
  ...
  - Require fresh authentication before link-from-settings (`freshAge` from lesson 3 of chapter 052).
  - Send a "we linked a new provider" notification email.
  - Audit-log the linking event (Chapter 057).
...
- OAuth flow mechanics (lesson 3 of chapter 051), per-provider config and quirks (lesson 8 of chapter 053).
- Change-email at account settings (lesson 2 of chapter 054), active-sessions / revocation (lesson 3 of chapter 054).
- Audit-log table (chapter 057).
...
- Configuring email+password — `requireEmailVerification` vs `autoSignIn`, `minPasswordLength` floor, why `nextCookies` from chapter 052 is non-negotiable, where the password hash lives (`account.password`, not `user`).
