...
- **From prior units (the principles and patterns each seeded finding maps to):**
  ...
  - `logAudit(tx, event)` and the audit-log canonical event set — 10.2.5 / 17.2.3.
...
- `logAudit(tx, event)` and the canonical audit-log event set — 10.2.5, 17.2.3.
...
- The diff ships with all the canonical helpers available (`authedAction`, `tenantDb`, `logAudit`, the Temporal helpers). Bypasses live in the call sites that *don't reach for* the helpers, not in the helpers themselves. Reading the helpers first calibrates the eye for the bypass — same reflex as 17.3.2.
...
- **Finding 5 — missing audit-log write on plan label change.** Re-open `app/(app)/plan/actions.ts`, point at the `db.transaction(async (tx) => { await tx.update(organizations).set({ planLabel }).where(...) })` block. Quote the failure mode: the compliance trail is silent on a security-relevant mutation (`org.plan.label_updated` is in the canonical event set from 17.2.3); the operator side of the audit history loses the event; a future GDPR or SOC2 audit can't reconstruct who changed the label and when. Pattern violated: the canonical audit-log event catalog (10.2.5, 17.2.3). Severity: `blocking:`. Action: add `await logAudit(tx, { action: 'organization.plan-label-changed', subjectType: 'organization', subjectId: ctx.orgId, actorUserId: ctx.user.id, orgId: ctx.orgId, payload: { previousLabel, nextLabel } })` inside the transaction. Write the comment.
