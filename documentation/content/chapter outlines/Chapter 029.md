...
- **`pnpm create next-app` — for recognition, not for use.** The course's starter (lesson 2 of chapter 031) is the source of truth; the wizard is named so the student recognizes its output (TypeScript, Tailwind v4, App Router, Turbopack, Biome, `@/*` alias, `AGENTS.md`) when seen elsewhere.
...
- **What lives outside `app/`.** The project-root surface for genuinely shared code: `lib/` (helpers used by 2+ features), `db/` (Drizzle, Unit 5), `components/` (app-wide UI, often shadcn), `actions/` (rare; shared server actions), `env.ts` (lesson 2 of chapter 037), config files. Threshold for top-level: "used by two or more features."
...
- **Top-level files in the `app/` directory.** Framework-read file conventions at `app/`'s root: `favicon.ico`, `globals.css`, plus the SEO conventions `robots.ts`, `sitemap.ts`, `opengraph-image.{ext}`, `twitter-image.{ext}`, `icon.{ext}`, `apple-icon.{ext}` (Chapter 034 owns the SEO set).
- **`route.ts` named once.** A `route.ts` in a route folder makes it a route handler (HTTP responses, not UI); Chapter 046 owns the full surface. Named here to prevent confusing it with `page.tsx`.
...
- Layouts, nested layouts, and the layout/page render boundary (lesson 2 of chapter 029).
- Route groups and how they organize layouts (lesson 2 of chapter 029).
- Dynamic segments and catch-all routes (lesson 3 of chapter 029).
- Navigation between routes (lesson 4 of chapter 029).
- Parallel routes and slots (lesson 5 of chapter 029).
- Intercepting routes (lesson 6 of chapter 029).
- Server Components vs. Client Components in depth — Chapter 030.
- `loading.tsx`, `error.tsx`, `not-found.tsx` — Chapter 031.
- The `metadata` export and `<head>` content — lesson 2 of chapter 017 introduced, Chapter 034 owns.
- `route.ts` and route handlers — Chapter 046.
- The `middleware.ts`-replaced-by-`proxy.ts` migration — Chapter 033.
- The Pages Router — named once for recognition, never taught.

Estimated student time: 50 to 65 minutes. Load-bearing for every later lesson in the chapter, every route in Chapters 034 through 039, and the entire project structure in Units 5 through 23.
...
- The root layout's `<html>` and `<body>` and the metadata API (lesson 2 of chapter 017).
- Dynamic segments and catch-all (lesson 3 of chapter 029).
- Parallel routes and slots — a different model from nested layouts (lesson 5 of chapter 029).
- `loading.tsx`, `error.tsx`, `not-found.tsx` at the layout level — Chapter 031.
- Server Components vs. Client Components — Chapter 030.
- Multiple root layouts at depth (rare; named only).
- Routing in middleware / `proxy.ts` — Chapter 033.

Estimated student time: 50 to 65 minutes. Load-bearing for every multi-page surface in the course, the `<auth>` and `<app>` split in Unit 8, and the project in Chapter 035.
...
- **Why `params` is a Promise — the 2026 change.** Next.js 16 made `params`, `searchParams`, `cookies()`, `headers()`, and `draftMode()` Promises to enable Partial Prerendering. In Server Components, `await`; in Client Components, `React.use(params)`. Chapter 032 owns the rendering model.
...
- **`generateStaticParams` — pre-rendering dynamic routes at build time.** Named as a forward-reference; the route exports an array of params and the framework generates a static HTML per entry. lesson 8 of chapter 034 owns it in full.
- **Validating `params` with Zod — the senior shape.** The URL is untrusted input; the page often expects a number, UUID, or constrained value. Zod 4 (Chapter 042) at the entry; parse failure throws into the surrounding error boundary (Chapter 031).
- **`notFound()` when the resource doesn't exist.** Named in context; the framework catches the thrown signal and renders the closest `not-found.tsx`. Full treatment in lesson 4 of chapter 029.
...
- Navigation to dynamic routes via `<Link>` (lesson 4 of chapter 029).
- `notFound`, `redirect`, `permanentRedirect` — named in context, owned by lesson 4 of chapter 029.
- `not-found.tsx` and the error boundary — Chapter 031.
- `searchParams` and URL state — Chapter 033.
- `generateStaticParams` at depth and the rendering model — Chapters 036 and chapter 034.
- The full async-request-API surface (`cookies()`, `headers()`, `draftMode()`) — lesson 8 of chapter 032.
- Zod 4 at depth — Chapter 042.

Estimated student time: 45 to 55 minutes. Load-bearing for every dynamic-route page in the course, the `searchParams` model in Chapter 033, and the project in Chapter 035.
...
- **Programmatic navigation — `useRouter` from `next/navigation`.** Used when navigation runs from a handler (form submit, conditional). Client Component only; `push`/`replace`/`back`/`refresh`. Full surface in lesson 5 of chapter 033.
...
- **`notFound()` — the "resource doesn't exist" signal.** Throws; the framework renders the closest `not-found.tsx`; HTTP 404. Every dynamic route is a candidate. Full `not-found.tsx` in Chapter 031.
...
- **Watch-outs:** `<Link>` for in-app, `<a>` for external; the throwing trio (`redirect`/`notFound`/`permanentRedirect`) doesn't execute code after them — don't `try/catch`; `redirect()` in Server Actions is 303; `useRouter` is `next/navigation`, not `next/router`; don't `prefetch={true}` blindly; `router.refresh()` is the post-mutation reach (lesson 6 of chapter 032); `<Link>` to external loses prefetching; `<Link replace>` skips history; `<Link scroll={false}>` preserves scroll.

What this lesson does not cover:

- The full `useRouter` / `usePathname` / `useSearchParams` / `useParams` surface — lesson 5 of chapter 033.
- Server Actions and the post-action `redirect()` pattern — Chapter 043.
- `not-found.tsx`, `error.tsx`, `global-error.tsx` — Chapter 031.
- Middleware-level redirects in `proxy.ts` — lesson 3 of chapter 033.
- The full mutation-revalidation tree (`revalidatePath`, `revalidateTag`, `router.refresh`) — lesson 6 of chapter 032.
- `next.config.ts` redirects for project-wide URL maps — lesson 3 of chapter 034.

Estimated student time: 50 to 60 minutes. Load-bearing for every navigation in the course, the auth guard pattern in Unit 8, and the project in Chapter 035.
...
- **Slots stream independently with Suspense.** Each slot with its own `loading.tsx` streams as an independent boundary; the layout shell appears immediately. Chapter 031 owns Suspense in full.
...
- Intercepting routes — the modal pattern (lesson 6 of chapter 029).
- Suspense, streaming, `loading.tsx`, `error.tsx` at depth — Chapter 031.
- `searchParams` at depth and the URL-state SaaS pattern — lesson 4 of chapter 033.
- The list-plus-detail project — Chapter 035.
- Routing in `proxy.ts` for tenant-scoped slot rendering — Chapter 033.

Estimated student time: 50 to 65 minutes. Load-bearing for lesson 6 of chapter 029 (intercepting routes build on the slot model), Chapter 031 (Suspense at the slot boundary), and Chapter 035 (the project wires the full list-plus-detail surface).
...
- **The modal UI itself.** Wrapped in a `<Modal>`/`<Dialog>` (Client Component, Radix or shadcn — lesson 1 of chapter 027) for open/close, focus, and `Esc`-to-close. The data and content come from a shared `<PhotoDetail />` used in both renders.
...
- Parallel routes — installed in lesson 5 of chapter 029.
- Modal implementation details, focus management, `Esc`-to-close — Chapters lesson 1 of chapter 027 and lesson 3 of chapter 027.
- `loading.tsx`, `error.tsx`, `not-found.tsx` at the slot level — Chapter 031.
- The list-plus-detail project — Chapter 035.
- Animations and transitions for the modal opening — lesson 5 of chapter 021.

Estimated student time: 50 to 65 minutes. Load-bearing for Chapter 035 (the project's intercepting modal), Chapter 060 (URL-state list views), and the senior reflex of reaching for URL-backed UI in every later SaaS pattern.
