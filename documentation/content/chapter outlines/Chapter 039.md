...
Chapter 039 cashes in the App Router primitives the unit installed: parallel routes (lesson 5 of chapter 033), intercepting routes (lesson 6 of chapter 033), server-side `searchParams` reads (lesson 4 of chapter 037), Suspense and `loading.tsx` at the segment boundary (lesson 1 of chapter 035, lesson 3 of chapter 035), `default.tsx` for unmatched slots, and the server/client boundary (chapter 034). The student ships the canonical SaaS list-plus-detail surface — invoices on the left, the selected invoice's detail on the right — with the "new invoice" form opening as a soft-navigation modal that has a real URL, refreshes correctly, survives `Cmd+click`, and falls back to a full page on direct visit. The list reads a `?status=` filter from `searchParams` server-side; both slots stream independently under their own Suspense boundaries; the surface degrades to a working no-JavaScript flow.
...
- **From chapter 033 (routing):** `app/` shape, `layout.tsx` / `page.tsx` semantics, parallel-route `@slot` naming, `default.tsx` per slot, intercepting-route prefixes (`(.)`, `(..)`).
- **From chapter 034 (server/client boundary):** Server Components as the default, `"use client"` only where state or events appear.
- **From chapter 035 (async UI):** `<Suspense>` as the streaming seam, `loading.tsx` at the segment, sibling-independent streaming.
- **From chapter 036 (cache):** Under Cache Components every route is dynamic by default; `searchParams` is an explicit dynamic signal.
- **From chapter 037 (server-side reads):** `await searchParams` in Server Components, Zod validation at the boundary.
- **From chapter 031 (shadcn):** `<Dialog>` for the modal shell (assumed installed in starter); `<Skeleton>` for skeleton primitives.
- **From chapter 008 / chapter 009 (TypeScript):** Zod schemas, `z.infer`, narrowed enum types.
...
- Parallel routes with `default.tsx` fallbacks — lesson 5 of chapter 033.
- Intercepting routes paired with non-intercepting `page.tsx` — lesson 6 of chapter 033.
- Layouts vs. pages — lesson 2 of chapter 033.
- Server Components with async data — lesson 1 of chapter 034.
- Server-side `searchParams` reads with Zod validation — lesson 4 of chapter 037.
- Suspense at the segment + `loading.tsx` — lesson 1 of chapter 035, lesson 3 of chapter 035.
- RSC prop-serialization boundary — lesson 4 of chapter 034.
- Architectural Principle #6 (explicit over magic) at `"use client"` — lesson 3 of chapter 034.
- Dynamic-by-default rendering under Cache Components — lesson 1 of chapter 036.
...
- The `searchParamsSchema` is in `/lib` so the same Zod schema validates the URL and would validate a form payload later — this is Architectural Principle #3 (pure `/lib`) (introduced formally in lesson 4 of chapter 047).
...
## Lesson 3 — Wiring the @list and @detail slots
...
Codebase state at entry: empty stubs from lesson 2 of chapter 039.
...
## Lesson 4 — Modal with a real URL
...
- The `(.)` prefix matches "same-level segment" — `(..)` and `(...)` exist for cross-level interception (named once, referenced to lesson 6 of chapter 033).
- Closing the modal is a navigation, not a state toggle. `router.back()` on the close action keeps history clean.
- The shadcn `<Dialog>` portal-renders to `<body>` — confirming the modal escapes any ancestor stacking context. Cross-reference lesson 9 of chapter 024 if the student saw the trap.
...
## Lesson 5 — Independent streaming per slot
...
- Optional reach: an explicit `<Suspense>` inside `@detail/[id]/page.tsx` around a slow sub-section (a related-invoices panel that takes longer than the detail header) — name the pattern, point to lesson 2 of chapter 035.
...
Codebase state at exit: both slots stream independently under their own skeletons. Done-when clauses are now achievable in lesson 6 of chapter 039.
