...
Chapter 1.2 pinned the runtime, the package manager, and the way TypeScript runs. The student can already execute `.ts` files. This chapter pins the next layer — the editor the code is written in, the tool that keeps it consistent, and the panel that proves it works in the browser — so the feedback loop closes inside the IDE and the browser before the first framework lesson lands in Chapter 1.4.
...
- **Minimum stack thinking applies to the editor too.** Same filter as Chapter 1.1.2: the smallest set of extensions, settings, and panels that earns its weight. Marketplace tours are out. Every extension, setting, or DevTools panel named answers a concrete senior question.
- **Editor configuration is part of the repo, not the machine.** `.editorconfig`, `.vscode/settings.json`, `.vscode/extensions.json`, and `biome.json` live in version control next to the lockfile from Chapter 1.2.3. The student leaves the chapter understanding the editor as a teammate-shared surface, not a personal preference.
...
- **No application code yet.** The chapter ships editor configuration, a Biome config, and a tour of DevTools panels against any existing tab the student has open. The Next.js scaffold lands in Chapter 1.4.

The student finishes the chapter with VS Code installed, a small workspace-checked-in settings file and extensions recommendations file, an `.editorconfig` at the repo root, a `biome.json` with `useEditorconfig: true` and the recommended rule set, format-on-save wired through the Biome extension, and the muscle memory to open the four DevTools panels and read what each is for. Chapter 1.4 lands on that floor and scaffolds Next.js into it.
...
- The minimum-viable extension set, with one senior reason each.
  - Biome — the lint-and-format LSP that 1.3.2 will configure. Installed here, configured there.
  - Tailwind CSS IntelliSense — autocomplete, hover-previewed generated CSS, class-name linting. Earns its weight from Chapter 4.3 onward; installed here so it never has to be revisited.
...
- The Biome configuration itself — that's 1.3.2.
- The full `tsconfig.json` — that's Chapter 1.4.3 and 1.4.4.
...
- The minimum-viable `biome.json` for the course. Walk through the fields the student actually touches:
  ...
  - `"formatter": { "enabled": true, "indentStyle": "tab" | "space", "indentWidth": 2, "lineWidth": 100 }` — name the senior debate (tab vs. space) and pick one (the course uses spaces, two wide, matching the `.editorconfig` from 1.3.1) so the file stays consistent with itself.
  ...
  - `"useEditorconfig": true` — Biome reads the `.editorconfig` from 1.3.1 for indentation, line endings, and final-newline so the two files agree by construction instead of by hand.
...
- The editor integration. Format-on-save and `source.organizeImports.biome` were wired in 1.3.1; this lesson is the moment that wiring actually fires because the config now exists. Demonstrate one round: open a file, save it ugly, watch Biome format on save and surface a lint warning inline (the Error Lens extension from 1.3.1 puts it on the same line).
...
- Two senior watch-outs.
  ...
  - The CI rule. Biome runs in CI on every PR. The `--frozen-lockfile`/`only-allow pnpm` discipline from Chapter 1.2.3 has a Biome cousin — the `biome ci` script is what closes the loop. Foreshadowed only; the actual CI job lives in Unit 21.
...
- **Elements panel.** What it is — the live DOM, not the source HTML, with the styles cascade visible per-element. The senior workflow:
  ...
  - Read the rules in the right pane in cascade order, with overridden rules struck through. This is *how* the cascade visualizes — the topic itself is taught in Unit 4.4, but the panel is the place a senior reads the cascade in practice.
...
- The Device Mode toolbar — viewport simulation for responsive testing. Named once because the Tailwind chapter (4.6) will lean on it; the workflow itself is taught there.
...
- React DevTools mechanics in depth — installed here, taught at the call site in Unit 4.7.
...
Roughly 95 to 110 minutes across the three lessons. The chapter fits a single evening for a focused reader or splits naturally across two — 1.3.1 + 1.3.2 as the editor evening, 1.3.3 as the DevTools evening. At the end the student has VS Code installed with the minimum-viable extension set, a `.vscode/extensions.json`, a `.vscode/settings.json`, an `.editorconfig`, a `biome.json`, four Biome scripts in `package.json`, React DevTools installed in their browser, and the muscle memory to open the four DevTools panels and read what each is for. Chapter 1.4 scaffolds Next.js into that floor without re-explaining any of it.
