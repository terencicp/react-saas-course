...
The TOC packs all of that into one bullet (lesson 1 of chapter 001). One bullet is too much for one lesson at the grain rule (§5: one TOC bullet, less than 1h student time — and the introduction is a place students bounce off if it's a wall of text). Splitting it gives each idea room to land and lets the student stop after any lesson and still know where they are.

Threads that must run through every lesson in this chapter:

- **Adult, terse, opinionated voice.** This is the first contact with the course's voice. If lesson lesson 1 of chapter 001 reads like a bootcamp prologue, the cut is broken before it starts. No celebratory tone. No "we'll go on a journey." Direct.
...
- Time and shape: 6 to 10 hours per week across roughly 4 to 6 months, sized for evenings rather than a bootcamp sprint. Each lesson is under an hour. Some chapters end in a 1.5 to 3 hour project that ships a real surface (the projects are introduced in lesson 5 of chapter 001).
...
- The stack itself (handled in lesson 3 of chapter 001).
- The full prerequisites list (handled in lesson 4 of chapter 001).
- How a lesson is structured (handled in lesson 5 of chapter 001).
...
## Lesson 3 — The stack at a glance
...
- One forward link: Chapter 002 starts pinning these tools on the student's machine.

What this lesson does not cover:

- Installation or setup of any tool (Chapter 002 onwards).
- The Architectural Principles or SaaS Patterns (introduced inline across the course).
- Hosted-service account creation (Projects.md flags this; Chapter 004 surfaces it for the first scaffold).
...
- Tool-specific prerequisites (Node, pnpm versions) — those are in Chapter 002.
...
## Lesson 5 — How to read this course
...
- One forward link: Chapter 002 pins the runtime.
