...
- **What does *not* happen automatically.** No automatic database migration (Drizzle migrations are a separate CI step in chapter 099). No automatic external-CDN purge (Cloudflare-in-front needs its own). No automatic secret rotation. The deploy ships code; everything stateful is the engineer's responsibility.
...
- **The trust model.** Vercel reads from GitHub via OAuth, never has push access. The audit trail lives in GitHub commit history plus Vercel's deployment log. Senior reflex: never run unreviewed scripts at build time — a malicious dep in `pnpm install` runs on Vercel's builder with full env-var access; SHA-pinned actions (chapter 097) and `minimumReleaseAge` in `.npmrc` are the prevention.
...
- First-deploy mechanics — lesson 2 of chapter 098; runtime/region — lesson 3 of chapter 098; domains — lesson 4 of chapter 098; preview+Neon — lesson 5 of chapter 098; env scoping — lesson 6 of chapter 098; rollback — lesson 7 of chapter 098; launch checklist — lesson 8 of chapter 098; schema migrations — Chapter 099.
...
- **Prerequisites.** GitHub repo with the Next.js 16 starter and a green `main` (CI gate from chapter 097 is live). Vercel account. `package.json` declares `"packageManager": "pnpm@9.x"` so Vercel auto-detects pnpm.
...
- **The Configure Project screen.** Project name (becomes the `*.vercel.app` subdomain; rename later costs the old URL). Framework Preset (auto-detected, leave). Root Directory (`./` for single-app; subdirectory for monorepo). **Skip env vars on first deploy** — added in lesson 6 of chapter 098. Click Deploy.
...
- **The first production URL.** `<project-name>.vercel.app` becomes the production deployment URL. Live on the public internet. Custom domain (lesson 4 of chapter 098) replaces this as primary.
...
- **The first preview URL.** Open any PR. Bot comments: "Visit Preview" → `your-app-git-<branch>-<org>.vercel.app`. Fully functional, billed against the project. **At this point** (before lesson 5 of chapter 098) the preview shares the production database — don't run destructive operations against it.
...
- **`vercel.json` — when to add it.** Unnecessary for the default Next.js project on Node.js runtime. Reach for it for: `headers` security headers (lesson 8 of chapter 098), per-route runtime override, `crons` (use Trigger.dev instead — Unit 12). Most SaaS projects ship with no `vercel.json` or a 10-line one.
...
- **The deployment notification surface.** GitHub commit status checks (build pass/fail dot), GitHub PR comments with preview URLs, optional Slack via Settings → Integrations. The CI gate's checks (chapter 097) and Vercel's build check live side by side on each PR. Both must be green to merge.
- **Watch-outs:** broad GitHub App permissions at install — scope to one repo; project name typos — subdomain renames hurt; first preview hitting prod DB — destructive ops dangerous until lesson 5 of chapter 098; sharing `*.vercel.app` URLs as production — get a custom domain first; missing `packageManager` field — falls back to npm.

What this lesson does not cover:

- Deployment model — lesson 1 of chapter 098; runtime/region — lesson 3 of chapter 098; domains — lesson 4 of chapter 098; preview+Neon — lesson 5 of chapter 098; env scoping — lesson 6 of chapter 098; rollback — lesson 7 of chapter 098.
...
- **The single-region rule for SaaS.** Vercel Functions execute in one region by default — `iad1` (Washington, D.C.). For SaaS with one database, **the function region must match the database region**; otherwise every server action pays a cross-region round trip. The Neon DB from Unit 5 was deployed to a specific region; match it. Project Settings → Functions → Region. Multi-region functions are a Pro+ feature for global SaaS with regional DBs — out of scope.
- **The Node.js runtime as the default.** Vercel Functions ship on Node.js in 2026. Full Node.js API surface, native modules, all npm packages, streaming. 15s timeout on Hobby, up to 15 min on Pro. chapter 021 MB payload. **The default the course commits to** for every server action, route handler, and RSC fetch.
...
- **Function memory, CPU, payload, timeout.** 1024 MB default, configurable to 3009 MB. Memory and CPU scale together. chapter 021 MB request body limit; 25 MB chunked streaming response. Uploads larger than chapter 021 MB route through R2 presigned URLs (Chapter 069) — file never traverses Vercel. Hobby 10s timeout, Pro 60s default up to 15 min. Anything plausibly slow goes async (Trigger.dev, Unit 12); the timeout is the canary, not the budget.
...
- **The `instrumentation.ts` hook.** Next.js 16's entry point for server startup work — Sentry init, structured-logger init, OpenTelemetry tracers. Runs once per cold start. Wired in Unit 19; named here.
...
- Deployment model — lesson 1 of chapter 098; first deploy — lesson 2 of chapter 098; domains — lesson 4 of chapter 098; preview+Neon — lesson 5 of chapter 098; background jobs — Unit 12; Sentry — Unit 19.
...
- **Automatic SSL via Let's Encrypt.** Vercel provisions a Let's Encrypt cert automatically once DNS resolves. No upload, no manual renewal — auto-renews before expiry. Modern TLS (chapter 002 and chapter 003) by default. Custom cert upload is Enterprise-only.
- **HTTPS-only enforcement.** Vercel auto-redirects HTTP → HTTPS on custom domains. The HSTS header (`Strict-Transport-Security`) needs to be set explicitly in the app's response headers — Vercel doesn't add it. The launch checklist (lesson 8 of chapter 098) names this.
...
- **Subdomain patterns for SaaS.** `app.example.com` for the application, `docs.example.com` for docs, `<tenant>.example.com` for tenant-isolated subdomains (Unit 9 — named once). Wildcard `*.example.com` CNAME issues a wildcard cert; reach for dynamic tenant subdomain provisioning, out of scope here.
...
- Deployment model — lesson 1 of chapter 098; runtime/region — lesson 3 of chapter 098; preview deployments — lesson 5 of chapter 098; security headers — lesson 8 of chapter 098 / Chapter 081; tenant-subdomain routing — Unit 9.
...
- **`neonctl` for manual branch operations.** The Native Integration handles preview branches automatically; `neonctl` is the CLI for one-off branch work outside that flow (debugging a closed preview's branch before deletion, reset-on-demand, branching for an ad-hoc experiment). Install via `pnpm add -g neonctl` (or `npm i -g neonctl`), then `neonctl auth` to log in, `neonctl branches create --parent main` to fork manually, `neonctl branches reset` to roll a branch back to its parent.
...
- **Vercel-Managed vs. Neon-Managed Integration.** Functionally equivalent for the preview-branching feature. Vercel-Managed — Neon provisioned and billed through Vercel; suited for greenfield. Neon-Managed — Neon is the system of record; Vercel pulls connection strings; billing stays on Neon. Course default: Neon-Managed (matches Unit 5's setup).
- **Schema migration on preview branches.** When a PR's `drizzle/migrations/` changes, the preview's DB starts as a copy of production *before* the migration. The preview's build needs to run the migration against the preview branch before the app boots. Standard wiring: `pnpm db:migrate` runs as part of `next build` via the Build Command override (`pnpm db:migrate && next build`) or a `prebuild` script. Reads the deployment's `DATABASE_URL` (the preview branch), applies migrations. Chapter 099 owns the safety story; this lesson treats migrations as a black box.
- **Production caveat on the build-command migration.** With migrations in the build command, every prod deploy runs migrations against `main`'s database. Safe with the expand-migrate-contract cadence (Chapter 099), risky with a naive destructive migration. Senior call: migrations in build command for previews; production migrations gated through a CI "migrate" job with explicit approval. Chapter 099 owns the full story.
...
- Deployment model — lesson 1 of chapter 098; env variable scoping — lesson 6 of chapter 098; rollback — lesson 7 of chapter 098; expand-migrate-contract — Chapter 099; Drizzle framework — Unit 5; tenant-data isolation — Unit 9.
...
- **The senior question.** Three environments each have their own set of env vars; some are shared, some scoped to one. Where does each variable belong, who can read it, and how does the env validator from Unit 4 enforce that the build fails when something's missing?
...
- **Managed variables.** Some integrations inject vars you can't edit by hand. The Neon integration (lesson 5 of chapter 098) manages `DATABASE_URL` in Preview. Vercel marks these; editing requires uninstalling. Managed vars are correct by construction — don't fight them.
...
- Deployment model — lesson 1 of chapter 098; preview+Neon — lesson 5 of chapter 098; rollback — lesson 7 of chapter 098; the validator schema in detail — Unit 4; Stripe webhook secrets — Unit 11; full rotation playbook — Chapter 081.
...
- **The code-side `git revert`.** From lesson 2 of chapter 096: `git revert <bad-sha>` creates a new commit applying the inverse. PR as normal (or direct to `main` with team consent). The CI gate (chapter 097) gates the revert PR — yes, it slows emergency response, but bypassing it on revert is a habit-corruption risk; keep the gate, trust the four-minute build.
- **The full incident-response runbook (one shape).** (1) Detect — alert fires. (2) Triage — confirm broken, identify bad deployment. (3) Rollback the alias — verify traffic flips. (4) Communicate — status page / Slack / customer comms. (5) Revert the code — PR, review, merge. (6) Re-enable auto-assignment — only after the next prod deploy from reverted `main` is verified. (7) Postmortem — what got past CI, what's the structural fix. Runbook lives in `docs/runbooks/`; named here, owned by Unit 21.
- **Database state on rollback.** If the bad deploy ran a destructive migration, alias rollback doesn't undo it. Drizzle migrations are forward-only. The expand-migrate-contract cadence (Chapter 099) is specifically designed to prevent this — every migration step is reversible by the next deploy *without* a code rollback. If a non-cadence migration shipped and broke things: write a forward-fix migration; don't rewind.
- **The feature-flag escape hatch.** If the bad behavior is gated behind a feature flag (PostHog from lesson 4 of chapter 093), flipping the flag off is faster than rollback — no deployment changes, no DNS, no CDN cache. Ship risky features behind a flag; the rollback story for flagged code is "flip the flag." The strongest preventive layer.
...
- Deployment model — lesson 1 of chapter 098; env scoping — lesson 6 of chapter 098; migration cadence — Chapter 099; `git revert` — lesson 2 of chapter 096; postmortems — Unit 21; feature flags — chapter 093.
...
- **The mindset — launch is not done at the URL.** The first production URL was live by lesson 4 of chapter 098. Launch means the URL is *defensible* — the app degrades gracefully on errors, doesn't leak data, doesn't fall over under load, has someone watching when it does.
- **Row 1 — Env validation green in production.** Verify: production build logs show env-validator passed; `SKIP_ENV_VALIDATION` is not set in production. A missing required env var means the first runtime call fails with an opaque error; the build-time validator catches it before the deploy. Cross-ref: lesson 6 of chapter 098.
- **Row 2 — Error monitoring wired and receiving.** Verify: Sentry initialized in `instrumentation.ts`; a deliberate test error in production shows in the Sentry dashboard within seconds; source maps uploaded so stack traces are readable. Errors invisible to the team compound. Cross-ref: chapter 092.
- **Row 3 — Rate limits live on the abuse surface.** Verify: auth endpoints (sign-in, sign-up, password reset, magic link) are rate-limited via Upstash from Unit chapter 075; a script hitting `/api/auth/sign-in` 50 times in 10 seconds gets 429s after threshold. Reach for `hey` as the simple HTTP load-testing CLI for this check (`hey -n 50 -c 5 <URL>`); equivalents include `oha` or a curl loop. Unrated auth endpoints are credential-stuffing targets on day one. Cross-ref: chapter 075.
- **Row 4 — Audit logs writing.** Verify: every privileged action (org-membership changes, role changes, billing, data exports) writes a row to the audit table from Unit chapter 057; `select * from audit_log order by created_at desc limit 10` shows recent activity. Missing rows mean compliance and incident-response fly blind. Cross-ref: chapter 057.
...
- **Row 6 — Database pooling and matching region.** Verify: Drizzle uses Neon's pooled connection string (`-pooler` in the hostname); production function region matches the DB region (lesson 3 of chapter 098). Unpooled connections exhaust the database under load; cross-region functions pay 80ms+ per query. Cross-ref: lesson 3 of chapter 098, Unit 5.
...
- **Row 9 (soft) — Runbooks for the top three incidents.** Verify: `docs/runbooks/` contains markdown for (1) production rollback, (2) database restore, (3) credential rotation. Under one page each. Incidents happen at 2 AM; the engineer paged needs a checklist, not a memory test. Cross-ref: Unit 21.
...
- Rollback mechanic — lesson 7 of chapter 098; Sentry — Unit chapter 092; rate limiting — Unit chapter 075; audit log schema — Unit chapter 057; full CSP — Chapter 081; runbook templates — Unit 21.
