...
Chapter 098 closes Unit 20 by turning the observability work of chapter 096 (Sentry + structured logs) and chapter 097 (Speed Insights + PostHog) into a *vigilance discipline*: a small set of recurring checks and irreversible defaults that keep the app fast as it grows. The student arrives with Speed Insights streaming real-user Vitals from lesson 1 of chapter 097, `next/image` introduced as the only image primitive in lesson 2 of chapter 038, `Promise.all` as the sequential-vs-parallel decision from lesson 3 of chapter 011, N+1 cataloged at the Drizzle layer in lesson 2 of chapter 043, and `EXPLAIN ANALYZE` from lesson 3 of chapter 043. None of those get re-taught; this chapter binds them into the performance-vigilance pattern (SaaS pattern #15) and adds Core Web Vitals as the canonical metric set, two audit tools (`@next/bundle-analyzer` and Lighthouse), and the RSC waterfall trap that the Server Component thread finally cashes in on.
...
- **The one structural reach for LCP.** `next/image` with `priority` on the LCP element (lesson 2 of chapter 098), `next/font` for the primary typeface (lesson 4 of chapter 038), and data-fetch off the critical path (lesson 6 of chapter 098).
...
- **Supporting cast — TTFB and FCP.** Not Vitals but Speed Insights reports them; the upstream causes when LCP is bad. TTFB high → server slow (Lessons lesson 6 of chapter 098 or lesson 7 of chapter 098). FCP high but LCP close → LCP element is the bottleneck (lesson 2 of chapter 098).
- **The 28-day window cadence.** Field data is rolling 28 days — a regression won't move the score for a week or two. The implication: regression detection happens before deploy (Lighthouse in CI, lesson 5 of chapter 098) or by alerting on the perf events. The window is a stability feature; not for catching same-day regressions.
- **The chapter map.** lesson 2 of chapter 098 → LCP. lesson 3 of chapter 098 and lesson 4 of chapter 098 → INP via bundle weight. lesson 5 of chapter 098 → all three via pre-launch gate. lesson 6 of chapter 098 → TTFB therefore LCP. lesson 7 of chapter 098 → TTFB upstream.
...
- Image priority — lesson 2 of chapter 098.
- Bundle analysis — Lessons lesson 3 of chapter 098 and lesson 4 of chapter 098.
- Lighthouse in CI — lesson 5 of chapter 098.
- RSC waterfalls — lesson 6 of chapter 098.
- DB perf — lesson 7 of chapter 098.
- Speed Insights install — lesson 1 of chapter 097.
...
- **The senior question.** LCP on the marketing page is chapter 015s — over the chapter 009s threshold. The hero image is the LCP element. The team needs the hint that tells the browser to fetch it immediately. The lesson revisits `next/image` from lesson 2 of chapter 038 through the LCP-vigilance lens: the `priority` prop, the `fetchPriority` hint it generates, and the structural ban on `<img>`.
...
- **Required props (revisited from lesson 2 of chapter 038).** `src`, `alt`, `width`+`height` *or* `fill` with sized container. New-in-16 `qualities` config requirement. The `sizes` prop on responsive images — without it the browser fetches the largest variant, doubling LCP bytes.
...
- `next/image` first install and `remotePatterns` — lesson 2 of chapter 038.
- `next/font` — lesson 4 of chapter 038.
- R2 hosting — Chapter 072.
- LCP measurement methodology — lesson 1 of chapter 098.
...
- **The lucide-react case — 1500 icons.** Without optimization, `import { Pencil } from 'lucide-react'` pulls roughly 1500 icon modules. The analyzer (lesson 4 of chapter 098) catches it; the student sees a 600KB `lucide-react` chunk where actual icons used would total under 30KB.
...
- **Verifying — bridge to lesson 4 of chapter 098.** After applying `optimizePackageImports`, run `@next/bundle-analyzer`; the lucide chunk drops from ~600KB to ~30KB. Without the analyzer, the fix is unverified.
...
- The bundle analyzer — lesson 4 of chapter 098.
- Image bundle weight — lesson 2 of chapter 098.
...
- **Next.js chapter 080 built-in.** Next.js chapter 080 ships an experimental built-in bundle analyzer that works with Turbopack (flag name pending, check release notes). Interactive UI. Course default in 2026: prefer the built-in once stable, fall back to `@next/bundle-analyzer` for the established workflow.
...
- **Triage decision tree.** Surprise dep → remove or replace. Heavy per-route chunk → `dynamic(() => import('...'), { ssr: false })` for client-interactive below-fold, or move work server-side. Duplicate dep → `pnpm dedupe` plus peer-dep audit. Barrel-bloat → `optimizePackageImports` (lesson 3 of chapter 098).
...
- **CI artifact pattern.** Produce the analyzer HTML in CI on dep- or route-touching PRs, upload as workflow artifact, reviewer eyeballs the diff against `main` (CI workflow primitives: lesson 1 of chapter 101). No bot needed — habit plus PR-template checklist line covers the regression class. `hashicorp/nextjs-bundle-analysis` exists for the formal version; named once.
...
- Lighthouse — lesson 5 of chapter 098.
- Icon bundle tactics — lesson 3 of chapter 098.
- Image optimization — lesson 2 of chapter 098.
...
- **The senior question.** The product ships in two weeks. The team needs a structural gate that catches regressions before production, plus a one-off pre-launch deep audit of high-traffic pages. The reach is Lighthouse — run against the marketing page and one critical authenticated screen, scoring against the Vitals from lesson 1 of chapter 098. The lesson installs the pre-launch audit, the CI regression gate (`@lhci/cli`), and the lab-vs-field discipline.
- **What Lighthouse does.** Full page load in a controlled environment — simulated Slow 4G, mid-tier mobile CPU, no extensions, no caching — reports synthetic LCP, INP (estimated, approximate), CLS, plus accessibility, SEO, best-practices audits. Does *not* measure real users; that's Speed Insights from lesson 1 of chapter 097. Lighthouse is the regression catcher and pre-launch deep dive; Speed Insights is the production verdict.
...
- **A11y and SEO bonus.** Lighthouse catches structural a11y gaps (missing alt, labels, contrast, landmarks), meta-tag SEO basics, HTTPS/mixed-content best-practices. Not taught at depth here (Unit 4 owns a11y, lesson 7 of chapter 038 owns SEO); the audit surfaces gaps cheaply.
- **One-off vs. recurring.** **Pre-launch one-off**: PageSpeed Insights against marketing, dashboard, 2–3 other critical screens; triage findings (lesson 4 of chapter 098 bundle, lesson 2 of chapter 098 images, lesson 6 of chapter 098 waterfalls, lesson 7 of chapter 098 DB); ship fixes; re-audit. **Recurring CI gate**: `@lhci/cli` on PRs against two routes with budgets. Gate is the floor; one-off is the deep dive.
...
- Speed Insights setup — lesson 1 of chapter 097.
- Vitals definitions — lesson 1 of chapter 098.
- Sentry performance traces — Chapter 096.
- A11y at depth — Unit 4.
- SEO at depth — lesson 7 of chapter 038.
...
- **The dependency-check reflex.** lesson 3 of chapter 011 introduced sequential-vs-parallel as a JS pattern. This lesson cashes in: before adding a second `await` in an RSC, ask "does this depend on what I just awaited?" If yes, sequential. If no, `Promise.all`.
...
- **Server-side caching as the upstream fix.** When the same fetch happens across many pages (user, org), React `cache()` and Next.js's `unstable_cache` (Chapter 036 / chapter 076) dedup at request scope. Parallel awaits remove serialization; caching removes duplication.
- **The N+1 cousin.** lesson 2 of chapter 043 owns N+1 at the Drizzle layer (the list loop). This lesson's RSC waterfall is the same shape at the component layer: list renders, each child component awaits its own fetch, DB sees N queries serial. Fix shape is identical: hoist, batch with join or `whereIn`, pass down. lesson 7 of chapter 098 owns the SQL side; this owns the component side.
...
- React `cache()` mechanics — Chapters 036 and chapter 076.
- N+1 at Drizzle — Lessons lesson 2 of chapter 043 and lesson 7 of chapter 098.
- Suspense depth — Chapter 4.
- Trace install — Chapter 096.
...
- **Pagination and the limit-offset trap.** Limit-offset scans (offset + limit) rows on every page — page 100 scans 1000+ rows for a 10-row result. Cursor pagination (lesson 4 of chapter 037) avoids it: `where created_at < $cursor order by ... limit 10`. With the right index, O(limit) regardless of depth.
- **Connection-pool saturation — the shifted bottleneck.** A `Promise.all` fan-out (lesson 6 of chapter 098's fix) plus a small pool can saturate Postgres. Neon's serverless driver uses HTTP, sidestepping the persistent-connection cost; classic `pg` plus PgBouncer is the alternative. Surface so the student doesn't accept the new failure mode silently.
...
- Index types and `EXPLAIN ANALYZE` depth — Chapter 043.
- Drizzle relations API depth — Chapter 042.
- N+1 introduction — lesson 2 of chapter 043.
- Cursor pagination shape — lesson 4 of chapter 037 and Unit 11.
- Neon driver — Chapter 040.
