...
- The seed populates two orgs with overlapping members (at least one user in both) and 100+ invoices total, surfaced in the inspector banner as `organizations: 2`, `org_members: 5`, `invoices: ≥ 100`.
- The seed is idempotent: running it twice produces identical row counts and reproduces the same business data on a sampled table — a sampled invoice's `number` is unchanged. (Primary keys come from the `uuidv7()` default and are freshly generated each run, so they are not byte-identical across runs; determinism is asserted against the reproduced `number`, not the `id`.)
...
- The list query's plan uses `idx_invoices_org_created_at_id` (no status) or `idx_invoices_org_status_created_at_id` (with status), never a `Seq Scan`.
...
- **From lesson 2 of chapter 036 / lesson 4 of chapter 036:** Docker Postgres compose file for local; the unpooled `DATABASE_URL_UNPOOLED` wired for Drizzle Kit, the pooled `DATABASE_URL` wired for the app's `db` client. The pooled/unpooled split is a no-op locally (`dbUnpooled` aliases `db`), staged for Unit 20's Neon swap.
- **From lesson 2 of chapter 037–lesson 10 of chapter 037:** `pgTable`, `casing: 'snake_case'`, the Postgres types (`uuid`, `numeric({ precision: 12, scale: 2 })`, `timestamp({ withTimezone: true })`, `text`, enum via `pgEnum`), `notNull()` / `default(...)`, UUIDv7 primary keys via `default(sql\`uuidv7()\`)`, FK + `ON DELETE`, `unique()` + `check()`, junction-table shape for `org_members`, `relations()` per table in `db/relations.ts`, `$inferSelect` / `$inferInsert`.
...
- **From lesson 1 of chapter 040 / lesson 3 of chapter 040:** `drizzle-kit generate`, the meta snapshot, `db:migrate` / `db:seed` scripts, `reset(db, schema)` then deterministic direct inserts, the fixed-seed idempotency move.
...
```
docker-compose.yml             # provided: postgres:18 service on :5432, pgdata volume
drizzle.config.ts              # provided: dialect, schema path, out, dbCredentials (unpooled URL), casing
.env.example                   # provided: DATABASE_URL, DATABASE_URL_UNPOOLED, SEED
package.json                   # provided: db:generate, db:migrate, db:seed, db:studio, test:lesson scripts
src/
  env.ts                       # provided: @t3-oss/env-nextjs boundary (3 server vars)
  db/
    index.ts                   # provided: postgres-js `db` + `dbUnpooled` alias, schema+relations wired
    schema.ts                  # TODO(L3) student: enums, FKs, constraints, indexes on the six tables
    relations.ts               # TODO(L3) student: relations() per table
    columns.ts                 # provided: reusable `timestamps` column group (createdAt)
    cursor.ts                  # provided: base64url encode/decode + Zod cursor schema
  lib/
    invoices/
      queries.ts               # TODO(L5/L6) student: listInvoices, getInvoiceDetail
      schema.ts                # provided: statusSchema, listInvoicesInputSchema
      counts.ts                # provided: getRowCounts, listOrgs
      explain.ts               # provided: getDetailPlan, getListPlan EXPLAIN probes
    utils.ts                   # provided: cn()
  app/
    page.tsx                   # provided: redirect('/inspector')
    layout.tsx                 # provided: root layout + Providers
    inspector/
      page.tsx                 # provided: RSC, reads ?orgId & ?status & ?cursor & ?invoiceId, renders four panels
      actions.ts               # provided: 'use server' reseed action wrapping runSeed
      loading.tsx              # provided: Suspense shell
      _components/             # provided: counts-banner, inspector-header, list/detail/plan panels
scripts/
  seed.ts                      # TODO(L4) student: reset + deterministic PRNG inserts, two orgs, 100+ invoices
tests/lessons/                 # provided: Lesson 2–6 placeholder specs (describe.todo)
drizzle/                       # generated by db:generate (absent in start), committed
```

### Reference solution signatures lessons display

All PKs are `id uuid pk default uuidv7()`, and every table carries the shared `timestamps` group (`createdAt timestamptz not null default now()`, precision 3) from `db/columns.ts`.

- `organizations`: `name text not null`, `slug text not null unique('organizations_slug_unique')`.
- `users` (stub for Unit 8): `email text not null unique('users_email_unique')`, `name text not null`.
- `orgMembers` (junction): composite PK `(organizationId, userId)`, `role` via the `memberRole` pgEnum `('owner', 'admin', 'member')` not null, both FKs `on delete cascade`. No surrogate `id`.
- `customers`: `organizationId uuid not null references organizations(id) on delete cascade`, `name text not null`, `email text not null`, `unique('customers_org_email_unique') (organizationId, email)`.
- `invoices`: `organizationId uuid not null references organizations(id) on delete cascade`, `customerId uuid not null references customers(id) on delete restrict`, `createdBy uuid not null references users(id) on delete restrict`, `number text not null`, `status` via the `invoiceStatus` pgEnum `('draft', 'sent', 'paid', 'overdue')` not null default `'draft'`, `total numeric(12, 2) not null`, `currency text not null default 'USD'`, `issuedAt timestamptz not null`, `dueAt timestamptz not null`, `unique('invoices_org_number_unique') (organizationId, number)`, `check('invoices_total_nonneg', total >= 0)`.
- `invoiceLines`: `invoiceId uuid not null references invoices(id) on delete cascade`, `description text not null`, `quantity numeric(12, 2) not null`, `unitPrice numeric(12, 2) not null`, `position integer not null`, `unique('invoice_lines_invoice_position_unique') (invoiceId, position)`.
- Enums (named, declared at the top of `schema.ts`): `memberRole = pgEnum('member_role', [...])`, `invoiceStatus = pgEnum('invoice_status', [...])`.
- Indexes (named): `idx_invoices_org_status_created_at_id` on `(organizationId, status, createdAt desc, id desc)`; `idx_invoices_org_created_at_id` on `(organizationId, createdAt desc, id desc)`; `idx_invoices_customer_id` on `(customerId)`.
- Relations (Drizzle Relations v1 — `relations()` per table, spread into the schema object in `db/index.ts`): `organizations -> many(members, customers, invoices)`; `users -> many(members, invoices)` (the invoices side tagged `relationName: 'createdByUser'`); `orgMembers -> one(organization), one(user)`; `customers -> one(organization), many(invoices)`; `invoices -> one(organization), one(customer), one(createdByUser via relationName), many(lines)`; `invoiceLines -> one(invoice)`.
- Cursor type: `{ createdAt: string; id: string }`, base64url-encoded JSON, validated with Zod (`createdAt: z.string()`, `id: z.uuid()`); `decodeCursor` returns `null` on a bad token.
- `listInvoices(input: ListInvoicesInput): Promise<{ rows: InvoiceListRow[]; nextCursor: string | null }>` — uses `db.query.invoices.findMany` with the callback `where` form `(t, { and, eq, lt, or }) => and(...)`: the tenant guard `eq(t.organizationId, ...)`, an optional `eq(t.status, status)`, and the cursor compound predicate `or(lt(t.createdAt, c), and(eq(t.createdAt, c), lt(t.id, id)))` (omitted on the first page), with `orderBy: (t, { desc }) => [desc(t.createdAt), desc(t.id)]`, `limit: pageSize + 1`, and `with: { customer: true }` for the list cell. `InvoiceListRow` is inferred from the query, not from `Invoice`.
- `getInvoiceDetail({ organizationId: string, invoiceId: string }): Promise<InvoiceDetail | null>` — uses `db.query.invoices.findFirst` with a callback `where` that AND-includes `eq(t.id, ...)` and `eq(t.organizationId, ...)` (tenant guard), and `with: { customer: true, lines: { orderBy: (t, { asc }) => [asc(t.position)] } }`; returns `result ?? null`.
- Env entries (in `.env.example`): `DATABASE_URL=postgres://postgres:postgres@localhost:5432/app`, `DATABASE_URL_UNPOOLED=postgres://postgres:postgres@localhost:5432/app` (same URL locally; the variable split exists so the deploy story in Unit 20 plugs Neon in without renaming), `SEED=1`. All three are validated in the `server` block of `env.ts`.
...
- **Plan panel (bottom):** A `<details>` that renders the plan text in a `<pre>`, sourced from the provided `getListPlan` / `getDetailPlan` probes in `lib/invoices/explain.ts` (each runs `EXPLAIN (ANALYZE, BUFFERS)` over the list or detail query). Pre-built so the student doesn't write it; verifies that the index gets used.
...
- `pgTable`, casing — lesson 2 of chapter 037. Postgres types via Drizzle (`uuid`, `numeric`, `timestamptz`, `pgEnum`; `jsonb` taught there but unused here) — lesson 3 of chapter 037.
- Column modifiers (NOT NULL, DEFAULT) — lesson 4 of chapter 037. UUIDv7 PKs via `default(sql\`uuidv7()\`)` — lesson 5 of chapter 037.
...
- Drizzle Relations v1 (`relations()` per table) — lesson 9 of chapter 037. `$inferSelect` / `$inferInsert` — lesson 10 of chapter 037.
...
- `drizzle-kit generate` / `migrate` / studio — lesson 1 of chapter 040. The `reset(db, schema)`-then-deterministic-insert idempotent seed pattern (fixed-seed PRNG, weighted status mix) — lesson 3 of chapter 040.
...
1. Open the `start` project directory (name the repo path).
2. `pnpm install` — completes with no errors.
3. Copy `.env.example` to `.env` (the `db:*` scripts load `.env` via `dotenv-cli`) and fill the values. Env vars: `DATABASE_URL` and `DATABASE_URL_UNPOOLED` (the Postgres connection string; locally the same value, obtained from the Docker Compose service), `SEED` (the fixed seed number, default `1`).
...
The starter ships `env.ts` with a `server` block holding all three server-only variables (`DATABASE_URL: z.url()`, `DATABASE_URL_UNPOOLED: z.url()`, `SEED: z.coerce.number().default(1)`), an empty `client` block (populated when PostHog and others land in later units), and the `runtimeEnv` map that threads `process.env.X` to each schema so the validator knows which variables Next.js inlines and which stay dynamic; the student's job is to wire the project to it and prove the boundary fires, not to re-derive the file.
...
- `.env` holds the real secret and is git-ignored; `.env.example` is committed and names every variable the app expects.
...
- The `env.ts` shape shown as it sits in the repo (three server vars, empty client, the `runtimeEnv` map), with one or two sentences on why the `runtimeEnv` map exists and why imports go through `env` rather than `process.env`.
- The `.env.example` → `.env` split: what each file is for, that `.gitignore` excludes `.env`, and the one-line Vercel connection (production vars set in the dashboard still validate at build, so a missing one fails the deploy before traffic shifts — Unit 20 owns the deploy chapter).
...
- By hand: `pnpm build` succeeds. Remove `DATABASE_URL` from `.env`, run `pnpm build` again, read the error naming the missing variable, then restore it and confirm the build passes. Tick each as you go.
...
The starter wires the tooling you build on: `drizzle.config.ts` drives `generate`/`migrate`/`studio` and points Drizzle Kit at the unpooled URL, the schema path, and snake-case casing; `src/db/index.ts` exports the `db` client (the `postgres-js` driver over `DATABASE_URL`, with `{ ...tables, ...relations }` and `casing: 'snake_case'`) plus the `dbUnpooled` alias; the `db:migrate`/`db:studio`/`db:seed` scripts wrap their tools through `dotenv-cli` because Drizzle Kit and `tsx` do not read `.env` by themselves.
The decisions that shape the schema, each carried in from Unit 5: UUIDv7 primary keys via `default(sql\`uuidv7()\`)` (Postgres 18 generates the ID) for sortable, non-guessable IDs; `timestamp({ withTimezone: true })` for every timestamp and `numeric({ precision: 12, scale: 2 })` for money; the named `memberRole` and `invoiceStatus` `pgEnum`s so the database enforces the domain; FK `ON DELETE` decided per edge rather than copied — cascade for owned children that lose meaning without their parent, restrict for referenced entities the schema can't make sense of without (a customer must not vanish under an invoice); every uniqueness scope and every index made tenant-aware, with composite index column order matching the list query's `where` plus `orderBy` direction.
...
- The emitted migration SQL, read before applying, shows every `CREATE TABLE`, each FK with its intended `ON DELETE`, each tenant-scoped unique, the `total >= 0` check, and the three named indexes (`idx_invoices_org_status_created_at_id`, `idx_invoices_org_created_at_id`, `idx_invoices_customer_id`).
- A second `pnpm db:generate` immediately after reports no changes — the snapshot is in sync.
- The relations resolve the two `invoices → users` edges distinctly: both `members` and the invoice author point at `users`, so the author edge is tagged `relationName: 'createdByUser'` on both sides to keep them apart, and the exported `$inferSelect` row types are what `queries.ts` and the inspector consume.

### Coding time

One line directing the student to author `db/schema.ts` and `db/relations.ts` against the reference signatures and the tests, run `pnpm db:generate --name init_schema`, read the emitted `0000_init_schema.sql`, then migrate.

Hidden `<details>` solution walkthrough:

- `src/db/schema.ts` in repo order — the two `pgEnum`s, then `organizations`, `users` (stub), `orgMembers` (composite PK), `customers`, `invoices`, `invoiceLines` — built in the order FKs demand, with one or two sentences each on the UUIDv7/`timestamptz`/`numeric`/`pgEnum` choices, the per-edge `ON DELETE` rationale (named once at `customers → invoices`), the tenant-aware uniques, and the three indexes declared at the bottom with column order matching the list `orderBy`. The shared `timestamps` group from `db/columns.ts` is spread into every table.
- `src/db/relations.ts` via Drizzle Relations v1 (`relations()` per table), calling out the `relationName: 'createdByUser'` tag that disambiguates the two `invoices → users` edges. Mention that these consts are spread into the schema object in `db/index.ts` (the v1 shape), not passed as a separate option.
- The `$inferSelect` / `$inferInsert` exports as the canonical row types.
- A callout on the provided `drizzle.config.ts`: the unpooled URL drives Drizzle Kit while the pooled URL drives the app's runtime queries (locally the same URL, `dbUnpooled` aliases `db`), and running long DDL against a pooled URL is the thing to avoid once Unit 20 makes the split real with Neon — the watch-out is reaching for the pooled URL in any new migrate/seed script written later.
...
Finished result: `pnpm db:seed` makes the inspector banner read `organizations: 2`, `org_members: 5` (Ada in both orgs), and `invoices: ≥ 100`, and running it a second time leaves row counts and sampled primary keys unchanged.
...
You will write `runSeed` in `scripts/seed.ts` as `reset(dbUnpooled, schema)` followed by a sequence of typed direct inserts (`organizations`, `users`, `orgMembers`, `customers`, `invoices`, `invoiceLines`), all randomness drawn from a small fixed-seed PRNG so every run is byte-identical; the `db:seed` script is already wired (`dotenv -e .env -- tsx scripts/seed.ts`).
The shape carried in from chapter 040 is the idempotency move — `reset(db, schema)` issues `TRUNCATE ... CASCADE` to clear before re-inserting — but here the data itself comes from hand-written inserts rather than `drizzle-seed`'s generators, because the invoicing model has too many cross-row invariants (overlapping membership, per-invoice `number` sequences, `position` renumbered per invoice, `dueAt` derived from `issuedAt`, FK targets captured from `.returning()`) for the bulk seeder to express cleanly. The senior takeaway: `drizzle-seed` is the right reach for shapeless fixtures; once the data has structure, a deterministic PRNG plus direct inserts is clearer and fully under your control.
The PRNG is a tiny linear-congruential generator seeded by `env.SEED`; it exposes `int`, `money`, `pick`, and a `weightedStatus` helper driven by `STATUS_BANDS` (paid 50, sent 25, draft 15, overdue 10) for a realistic status mix.
The defining constraint is determinism: the fixed seed number is the contract, so the same number must always produce the same data — bumping `SEED` deliberately shifts the shape, while changing the insert logic without bumping it silently breaks the contract.
The data shape: 2 orgs (Acme, Globex), 4 users, 5 `orgMembers` with Ada Lovelace in BOTH orgs (the overlapping-membership invariant), 40 customers alternating across the orgs, 12–18 invoices per customer (well past 100 total), and 2–4 line items per invoice numbered by `position`.
Two traps to avoid: `reset` issues `TRUNCATE ... CASCADE`, which is fine against local Docker but would hold locks on a shared branch, so this stays local-only and reaches `dbUnpooled` for the long transaction, the same rule as migrations; and PKs come from the schema's `uuidv7()` default plus `.returning()` to capture parent ids for the child inserts — don't hand-generate ids.

- Running `pnpm db:seed` makes the inspector banner show two orgs, 5 memberships with at least one user (Ada) belonging to both orgs, 40 customers spread across the orgs, and 100+ invoices.
- The seeded invoices show a realistic status distribution and totals, with each invoice's line items numbered by `position` and `dueAt` falling 30 days after `issuedAt`.
- Running `pnpm db:seed` a second time leaves the row counts unchanged and reproduces a sampled invoice's `number` from the first run (the `uuidv7()` primary key is regenerated each run, so determinism is checked against `number`, not `id`).

### Coding time

One line directing the student to write `runSeed` in `scripts/seed.ts` against the reference shape and the tests, then run `pnpm db:seed` twice and compare.

Hidden `<details>` solution walkthrough:

- `scripts/seed.ts` in repo order: the LCG PRNG factory (`createPrng`, seeded by `env.SEED`), the `reset(dbUnpooled, schema)` call, then the insert sequence — orgs and users via `.returning({ id })`, the five `orgMembers` rows (Ada in both orgs), 40 customers alternating orgs, the invoice loop (12–18 per customer, `INV-#####` numbering, `weightedStatus`, `money` totals, dates off `SEED_EPOCH`, `dueAt = issuedAt + 30d`), and the line-item `flatMap` (2–4 per invoice, `position` 1..n) — with a sentence on why the structure forces direct inserts over a generator.
- The CLI entry guard using `pathToFileURL` (so the guard fires even when the project path contains a space), and that the script exits 0 on success.
- The arithmetic that lands invoices past 100 (40 customers × 12–18 each).
- A callout that the fixed seed number is the determinism contract and what bumping `SEED` versus editing the insert logic does, linking the chapter 040 seed lesson rather than re-explaining it.
- A note that `reset`'s `TRUNCATE ... CASCADE` and the long transaction are why the seed is local-only and uses `dbUnpooled`.

### Moment of truth

- Run the lesson's test suite (name the command); expected pass output.
- By hand: run `pnpm db:seed`, read the inspector banner for two orgs, 5 memberships (Ada in both orgs), and 100+ invoices; open Studio to eyeball the status spread and line-item counts. Run `pnpm db:seed` again and confirm identical row counts and a reproduced sampled invoice `number` across both runs (the `id` is regenerated by `uuidv7()` each run, so compare `number`). Tick each as you go.
...
You will write `listInvoices` in `src/lib/invoices/queries.ts` against the provided `listInvoicesInputSchema` (orgId required, status optional, cursor optional as a decoded `Cursor` object — the inspector page already decodes the URL token via `decodeCursor` from `db/cursor.ts` before calling — pageSize default 20 capped at 100) — the same input shape Unit 6 will reuse against a Server Action's `formData`, which is why the schema lives in `/lib` for callers to compose.
The query uses the relational API's `findMany` with the callback `where` form `(t, { and, eq, lt, or }) => and(...)` that AND-combines the tenant guard `eq(t.organizationId, ...)`, the optional `eq(t.status, status)`, and the cursor predicate; `orderBy: (t, { desc }) => [desc(t.createdAt), desc(t.id)]`; and `with: { customer: true }` so the list cell's customer name comes back in the same round trip.
Two constraints define correctness here: cursor pagination needs the composite tiebreaker — ordering by `createdAt` alone skips or duplicates rows whenever two invoices share a timestamp, so the compound predicate is written with the relational API's `where`-callback helpers as `or(lt(t.createdAt, c.createdAt), and(eq(t.createdAt, c.createdAt), lt(t.id, c.id)))` (`createdAt` is pinned to millisecond precision so the cursor's ISO string round-trips exactly), conditionally included only when a cursor is present; and the page boundary uses the `limit: pageSize + 1` trick — fetch one extra row to learn whether a next page exists, set `nextCursor` from the last kept row via `encodeCursor`, and avoid a separate `count()` round trip.
...
- The list query's plan uses `idx_invoices_org_created_at_id` without a status filter and `idx_invoices_org_status_created_at_id` with one, never a `Seq Scan`.
...
- Run the lesson's test suite (name the command); expected pass output.
- By hand: click "Next page" three times against one org and confirm no row repeats across the three pages (open Studio if needed); switch orgs and confirm the rows differ. Click `paid`, confirm the URL shows `?status=paid`, only paid rows render, and a hard reload preserves the view. Switch the plan panel to the list query and confirm it uses `idx_invoices_org_created_at_id` (no status) and `idx_invoices_org_status_created_at_id` (with status), never a `Seq Scan` — if it falls back, the index order from lesson 3 of chapter 041 is wrong. Tick each as you go.
...
You will write `getInvoiceDetail({ organizationId, invoiceId })` in `src/lib/invoices/queries.ts` using `findFirst` with the callback `where` and `with: { customer: true, lines: { orderBy: (t, { asc }) => [asc(t.position)] } }`, returning `result ?? null` when nothing matches.
The defining constraint is the tenant guard: the callback `where` must AND-include both `eq(t.id, invoiceId)` and `eq(t.organizationId, organizationId)`, so the security lives in the query rather than in a check after the load — the same structural rule the list read follows and the discipline Unit 9's `tenantDb` will enforce.
...
- `getInvoiceDetail` as it sits in `queries.ts`: the `findFirst` with the callback `where` AND-combining `eq(t.id, ...)` and `eq(t.organizationId, ...)`, the `with` block ordering lines by `position` and pulling the customer, and the `result ?? null` return. Note `InvoiceDetail` is `NonNullable<Awaited<ReturnType<typeof findFirst>>>`, inferred from the query rather than hand-typed.
