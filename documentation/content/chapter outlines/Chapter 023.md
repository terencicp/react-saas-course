...
- **The source-order trap and `cn()` / `tailwind-merge`.** Cross-reference to lesson 3 of chapter 022. Class-string order doesn't decide a utility conflict; `cn()` resolves the conflict before it reaches the cascade.
...
- CSS inheritance and the `inherit`/`initial`/`unset`/`revert` keywords (lesson 2 of chapter 023).
- Preflight's specific reset rules and what gets stripped (lesson 3 of chapter 023).
- CSS custom properties at depth and the design-token model (lesson 4 of chapter 023).
- The `cn()` and `tailwind-merge` composition primitive — installed in lesson 3 of chapter 022, only cross-referenced here.
- The `@custom-variant` and `@utility` directives — installed in lesson 2 of chapter 022.
- The browser layout engine and how the cascade interacts with the box model (Chapter 024).
...
- Opens with the form-element surprise: `<body className="text-foreground">` styles most descendants but `<button>` ignores it. Names the user-agent override and motivates Preflight (lesson 3 of chapter 023).
...
- **CSS custom properties inherit.** Cross-reference to lesson 4 of chapter 023. The reason a `.dark { --color-foreground: ... }` swap on `<html>` reaches every descendant.
...
- The cascade resolution algorithm and layers (lesson 1 of chapter 023).
- Preflight's specific reset rules (lesson 3 of chapter 023).
- CSS custom properties at depth and the design-token model (lesson 4 of chapter 023).
- Typography utilities (`text-*`, `font-*`, `leading-*`) at depth — lesson 1 of chapter 025 owns typography.
- Color spaces, OKLCH, and color theory — lesson 2 of chapter 025 owns color.
- SVG icon component patterns — lesson 1 of chapter 031 cashes in shadcn's icon usage with `currentColor`.
- The CSS box model — lesson 1 of chapter 024 owns it.
...
- **Customizing prose tokens.** Themed via `@theme` (`--prose-body`, `--prose-headings`, `--prose-links`, etc.). lesson 5 of chapter 031 cashes in the prose case for empty-state UI; this lesson names it only.
- **Preflight and form elements.** Cross-reference to lesson 5 of chapter 021 (form element surface) and lesson 1 of chapter 031 (shadcn `<Button>` as the canonical wrapper). The student doesn't fight Preflight on form elements; they style them.
...
- The cascade resolution algorithm and layers in general (lesson 1 of chapter 023) — Preflight's layer is named here, but the model lives in lesson 1 of chapter 023.
- CSS inheritance — Preflight's inheritance choices are cited (`font: inherit` on form elements) but the model lives in lesson 2 of chapter 023.
- The full `@tailwindcss/typography` API — cashed in lightly when needed.
- The box model and `box-sizing` — lesson 1 of chapter 024 owns it (Preflight sets the default, that's all).
- Form element styling at depth — Chapter 031 owns shadcn `<Button>`, `<Input>`, `<Form>`; Chapter 7 owns the form pattern.
- Typography sizing and line-height systems — lesson 1 of chapter 025.
...
- **The Tailwind v4 `@theme`-to-utility flow.** Cross-reference to lesson 2 of chapter 022. Token definitions become `:root` rules in `@layer theme`; utilities compile to `var(--token)` reads; the namespace prefix determines the utility family (`--color-primary` produces `bg-primary`, `text-primary`, `border-primary`).
- **Inheritance plus the cascade — subtree overrides.** The senior pattern: defaults on `:root`, dark theme on `.dark`, scoped overrides on `.marketing-hero` or `[data-org="acme"]`. Use cases named: theme switching (cross-ref lesson 5 of chapter 022), multi-tenant branding, section-specific theming, component-scoped properties.
- **Reading and writing from JavaScript.** The DOM API named (`getPropertyValue`, `setProperty`, `removeProperty`). Use cases: a theme color picker; multi-tenant branding written from server data (inline `<script>` in `<head>` to avoid FOUC, parallel to lesson 6 of chapter 022); slider-driven CSS animations. The recognition: visual updates happen without React re-renders.
...
- The cascade and specificity (lesson 1 of chapter 023).
- CSS inheritance at depth (lesson 2 of chapter 023).
- Preflight (lesson 3 of chapter 023).
- The `@theme`, `@utility`, `@container` directives (lesson 2 of chapter 022).
- The semantic-token shape for dark mode (lesson 5 of chapter 022) — cross-referenced.
- `next-themes` and the React-side theme wiring (lesson 6 of chapter 022).
- Color spaces (OKLCH, sRGB, P3) — lesson 2 of chapter 025.
- CSS animations and transitions — lesson 5 of chapter 025.
- shadcn's exact token list — lesson 1 of chapter 031.
