  ...
  - source: 66.1
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      The analytics event is the textbook `after()` case — lose-it-once-in-a-thousand is acceptable. The invitation email is the opposite: the user was told it sent, so it must be confirmed *before* the success message. `after()` gives it no retry and no failure path, so a throw vanishes and the toast becomes a lie. Defer what the user needn't see; never defer what they do.
  ...
  - source: 66.3
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      Fan-out with concurrency control is condition 4 — a real, testable property the cheap tiers physically can't provide. Mere slowness under the time wall isn't a trigger: if the user needs the result you're stuck with the latency, and a job platform only adds a "where did my result go" problem. "Cleaner separation" is an aesthetic a plain helper satisfies — escalate on a condition, never on a vibe.
  ...
  - source: 66.5
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      A run-level retry replays every side-effecting line that ran before the throw — the source of nearly every duplicate-email incident. A per-member key with `scope: 'run'` ties each key to the parent run, so a retry regenerates identical keys and the runtime returns the cached result for members already done. A hand-rolled retry loop stacks a redundant layer on the runtime's; killing retries throws away durability and still loses the failing member's work.
  ...
  - source: 66.5
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      `wait.for` is a checkpoint: the worker is released (zero compute billed) and the pause lives as a durable snapshot, so a redeploy or OOM mid-wait resumes cleanly. `setTimeout` keeps the worker alive and idle for the full wait, and because the timer is in process memory, a crash loses it and the run never resumes. Durability lives in the seams.
  ...
  - source: 66.6
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      `token.url` is the server-to-server completion webhook (no CORS), so a backend partner completes it directly and the run resumes — the runtime owns the URL, the auth, the dedup, and the resume. `token.id` is an internal identifier, not a URL. `token.publicAccessToken` is the browser handle. And the timeout is mandatory: the default is only 10 minutes, which would kill a multi-hour render, so size it explicitly and handle `!result.ok`.
  ...
  - source: 66.6
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      `batchTriggerAndWait` is the idiomatic fan-in for your own children: the runtime creates and owns a waitpoint per child, parks the parent on all of them, and returns a typed result array once they settle (so you still check each `ok`). There is no `wait.forWaitpoint` API in v4 — a stale completion will suggest it. Raw tokens are for *external* completers; aiming 200 hand-managed tokens at your own tasks is the right tool for the wrong problem.
  ...
  - source: 66.7
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      The app is the caller and the task is the callee. Ship the app first and there's a window where live code triggers a task version the workers lack, failing on a real user's request. So deploy the callee before the caller — the principle Trigger.dev v4's atomic deploys and Vercel integration encode for you, and the failure you'll recognize the moment that automation is off.
