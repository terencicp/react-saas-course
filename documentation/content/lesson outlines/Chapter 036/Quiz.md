  ...
  - source: 36.1
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      "X can have many Ys *and* a Y can belong to many Xs" is many-to-many, and neither side can hold a single foreign key — so you introduce a junction table with one foreign key to each side, keyed on the composite pair. A lone `project_id` column expresses one-to-many (each user in exactly one project), which the spec contradicts. The comma-separated columns are a 1NF violation and can't be foreign-key-enforced or indexed.
