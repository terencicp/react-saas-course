  ...
  - source: 51.1
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      This collapses authorization into a render-time check. Two problems: layouts can be bypassed under partial pre-rendering, so the gate isn't guaranteed to run; and even when it does, a component that renders nothing is UX politeness, not a security boundary — a direct request to the underlying action still goes through. Authentication lives at the session-read boundary, but authorization belongs at the action boundary, on the server, evaluated per-action and per-resource every time.
  ...
  - source: 51.1
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      Elevation is the cleanest demonstration that authn and authz are related without being the same. A policy — pure authorization — declares that a high-stakes capability requires *recent* proof. What that policy triggers is a fresh password or passkey check — pure authentication. The session hasn't expired; the user is authenticated. The authz rule is demanding a fresh authn on top of the baseline.
  ...
  - source: 51.2
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      Two real bugs. A `===` comparison short-circuits on the first mismatched byte, so response timing reveals how much of the token was correct — a constant-time compare is the fix. And trusting a role baked into the cookie lets a stale cookie keep capabilities after a server-side change, which is why the action boundary always re-reads authz from the source of truth. The distractors invert facts: `crypto.randomUUID()` (122 bits) clears the 128-bit-ballpark bar fine, and each device should get its *own* row — that's what makes "sign out everywhere" a `DELETE WHERE userId = ?` and powers the active-sessions list.
  ...
  - source: 51.3
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      OAuth only ever proves authorization — it gives the app a token meaning "may act on the user's behalf," not who the user is. OIDC is the authentication layer bolted on top: request the `openid` scope and the provider returns an `id_token` and standardizes `userinfo`, so the app can read identity. The senior shorthand is "OIDC over OAuth" — authentication standing on the shoulders of an authorization protocol.
  ...
  - source: 51.3
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      The "secret is enough" reasoning is the pre-2.1 framing where PKCE was sold as a fix only for clients that can't keep a secret. OAuth 2.1 makes PKCE required for all clients because a static `client_secret` can't bind *this code* to *this flow* — and secrets get shared across deployments, codes get replayed against staging, and code-injection variants slip in before the secret matters. PKCE closes all of that; per-environment secrets are good hygiene but don't replace it.
  ...
  - source: 51.3
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      The front channel — redirects and URLs — is visible in history, logs, and to extensions, so only values that are useless on their own go there: the `code_challenge` (a one-way hash that leaks nothing about the verifier) and the short-lived `code`. The secrets — the `code_verifier` and the `client_secret` — travel only on the direct server-to-server back channel during the token exchange. That split is the whole reason a leaked `code` is survivable: without the verifier, it can't be redeemed.
  ...
  - source: 51.3
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      The OAuth tokens are upstream of the session, never a replacement for it. Once the verified identity is read, the app provisions or links a local user and issues its own `__Host-` session cookie — the same hardened cookie from the sessions lesson. For a pure login the access and refresh tokens can be thrown away; you only keep a refresh token (server-side, never in the browser) if the app needs to call the provider's APIs later without the user present.
