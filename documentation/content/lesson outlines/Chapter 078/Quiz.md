  ...
  - source: 78.1
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      Run the gate that sits *before* the funnel: do we even own this state on the client? A polled thread is the server's data — TanStack Query owns its cache, refetch, and staleness. A store would just be a poor man's database that never refetches. Per-feature scoping and `persist` are real rules, but neither fixes the fundamental error of caching server state in a store.
  ...
  - source: 78.1
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      The trigger's qualifier is "more than two unrelated subtrees" — three scattered callers meet it, and a store beats threading a callback across half the tree. A modal absolutely can justify a store (it's the canonical imperative-action case), and you can't split the action surface in two and leave callers inconsistent.
  ...
  - source: 78.2
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      Server module scope is shared across all concurrent requests, so a module-scoped `create` store is a cross-request data-isolation leak — the same shape as the module-scoped `QueryClient`. The fix is a fresh store per request, pinned with `useRef` in a provider. Both `create` and `createStore` support generics, and the leak is about shared scope, not re-running the creator.
  ...
  - source: 78.3
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      Surviving a refresh means the state must live somewhere durable — that's server persistence (a drafts table with GC and tenancy) or `sessionStorage` if product accepts tab durability. Either is a deliberate product decision with real cost; the wizard declines it because a lingering half-finished draft on a shared machine is the worse outcome. `persist` is compatible with the provider, and `sessionStorage` is per-tab, not cross-tab.
