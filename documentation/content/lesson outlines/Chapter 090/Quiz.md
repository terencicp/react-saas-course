  ...
  - source: 90.1
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      The webhook-flips-Postgres test and the renders-the-badge test each see one isolated layer. Neither sees the *composition*: the redirect round-trip to checkout.stripe.com and the success page waiting on an out-of-band webhook before showing "Pro." That's a money path only the browser can catch — and Stripe test mode plus the 4242 card is a stable, drivable contract, not the undrivable case.
  ...
  - source: 90.1
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      Year-one zero is honest, not a shortcut. The integration suite owns the seam where bugs cluster, observability owns the unknown-unknowns, and a human clicking the money paths before release covers the rest. The trigger language tells the team *when to start*, not that they should already have started.
  ...
  - source: 90.2
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      A single retry differentiates a flake from a true failure. Bumping the count toward `retries: 3` is the anti-pattern from lesson 1 — it mutes the alarm on a real race instead of fixing it. The runtime and trace-capture framings are plausible but not the reason; the reason is keeping the flake signal honest.
  ...
  - source: 90.2
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      Per-test rollback works for integration tests because the test code and the DB connection share one process — open transaction, write, roll back, all in one runtime. The E2E test only talks to its server over HTTP and never holds that transaction, so the process boundary that makes E2E realistic also makes per-test rollback impossible. The fix is a deterministic reset between full runs.
  ...
  - source: 90.2
    ...
    choices:
      ...
      - text: |
        ...
        correct: false
    why: |
      A web-first matcher only runs when you `await` it. Drop the `await` and it returns a promise nobody waits on — the assertion never executes and the test passes while checking nothing, worse than a crash. The fix is one keyword. `toBeVisible` already auto-waits, so no `waitForSelector` is needed.
