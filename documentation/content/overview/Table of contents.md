# Web Dev Course (2026) — Proposed Curriculum Structure (v4)

---

## Unit 1 — JavaScript and TypeScript
...
- 1 The bar and the brief (Frame the static marketing surface as a SaaS pattern, state the seven "Done when" verifications, show the final UX at three widths, set the scope cut, and commit to the from-scratch build (no starter clone).)
- 2 pnpm and the lockfile contract (Install pnpm through mise, write the minimum-viable `package.json` with `packageManager` and `.npmrc`, commit `pnpm-lock.yaml` as the deterministic resolution record with `--frozen-lockfile` in CI, and guard against mixed package managers with `only-allow pnpm`.)
- 3 AGENTS.md as the next contributor's briefing (Author `AGENTS.md` for what earns a place (thesis, pinned stack, layout, commands, conventions pointers) and what doesn't (aspirational prose, duplicated rules, hand-maintained file lists), with the deeper documentation doctrine deferred to the documentation unit.)
- 4 Configuring tsconfig (Author the project-owned strictness floor (`strict`, `noUncheckedIndexedAccess`, `noFallthroughCasesInSwitch`, `noImplicitOverride`, `forceConsistentCasingInFileNames`, the `@/*` path aliases) alongside the framework-owned compatibility flags (`target`/`lib`, `module`/`moduleResolution: "bundler"`, the transpiler-alignment trio, `jsx: "preserve"`, `noEmit`, the Next.js plugin, `next-env.d.ts`).)
- 5 Biome, the single-binary linter and formatter (Adopt Biome as the 2026 default over ESLint+Prettier (single Rust binary, dependency-aware domains), the `next lint` removal in Next.js 16, the minimum-viable `biome.json` wired to `.editorconfig`, the four daily scripts, and the safe-versus-unsafe fix distinction.)
- 6 Header, hero, and feature grid (Build the semantic header with desktop nav, the hero with `<Button asChild>` CTAs and a CSS-only `ThemeAwareImage`, and a `cva`-driven `FeatureCard` mapped into a responsive three-column grid.)
- 7 Pricing, footer, and a flicker-free theme toggle (Compose the pricing cards with a data-driven featured tier, build the three-column footer, and wire a `next-themes` toggle that respects `prefers-reduced-motion` and survives hard reload without hydration warnings or FOUC.)
- 8 Mobile drawer with scroll lock (Write the `useLockBodyScroll` hook that restores prior overflow on cleanup, then drop it inside a shadcn `Sheet`-based `MobileNav` that traps focus, closes on `Esc` and link click, and keeps the desktop and mobile navs single-source via `hidden md:flex` / `md:hidden`.)
- 9 Verify clause by clause (Walk every "Done when" check — no-FOUC reload, Lighthouse 100, keyboard-only traversal, reflow at 360/768/1280, drawer focus trap, scroll lock, and `Esc` close — plus an axe DevTools audit, and name the senior calls one more time.)

---

## Unit 4 — Next.js and the App Router
...
- 2 Cloning the starter and the dev/build cycle (Fetch the course's pinned Next.js 16 scaffold with `degit`, read the file tree end to end, and run `pnpm dev` and `pnpm build` to commit the project's toolchain decisions into a real first commit — the first project in the course that starts from a starter rather than from scratch.)
- 3 Starter walkthrough (Tours the provided file tree, the queries and Zod schema in `/lib`, the pure render components, and the two-slot layout the student will fill in.)
- 4 Wiring the @list and @detail slots (Fills the slot `page.tsx` and `default.tsx` files, reads `searchParams` server-side with Zod validation, and uses `notFound()` for missing records.)
- 5 Modal with a real URL (Builds the `(.)new` intercepting modal and its non-intercepting twin so soft nav opens a dialog while refresh and `Cmd+click` open the full page.)
- 6 Independent streaming per slot (Adds `loading.tsx` and skeleton components to each slot so the list and detail stream independently under throttled network.)
- 7 Verify (Walks every "Done when" clause as a verification step, names the senior calls, and points at the forward references in Units 5, 6, and 10.)
...
- 9 Drizzle Relations v2 (Declares the TS-side traversal graph with `defineRelations` in `db/relations.ts` — one/many/through shapes — that enables `db.query.…({ with: … })` nested reads in 038.3.)
...
- 3 Tour of the starter and the inspector contract (Walks the provided file tree (`drizzle.config.ts`, the pooled `db` client, the cursor helpers, the inspector page, the `db:*` scripts), brings up Docker Postgres, and pins the contracts the student's queries must satisfy.)
- 4 Authoring the schema and shipping the init migration (Fills `db/schema.ts` and `db/relations.ts` with the six tables (`organizations`, `users` stub, `org_members`, `customers`, `invoices`, `invoice_lines`) including UUIDv7 PKs, FK `ON DELETE` decisions, tenant-scoped uniques, the three composite indexes, and the `$inferSelect` row types, then generates and runs the initial migration.)
- 5 A deterministic, idempotent seed for two orgs (Writes `scripts/seed.ts` using `reset` plus `seed().refine(...)` with `weightedRandom`, `valuesFromArray`, and `with` to produce two orgs with overlapping members and 100+ invoices, dropping to direct `db.insert` where the seeder's shape doesn't fit.)
- 6 Writing the two tenant-scoped reads (Implements `listInvoices` (cursor pagination with the composite tiebreaker predicate and the `limit(pageSize + 1)` trick) and `getInvoiceDetail` (relational `findFirst` with `lines` and `customer`), with the `organizationId` tenant guard baked into every `where`.)
- 7 Verifying the seven "Done when" clauses (Runs each Done-when check end-to-end (clean migrate, idempotent seed, cursor pagination, server-side status filter, cross-org tenant guard, single-round-trip detail, `EXPLAIN ANALYZE` showing the right indexes) and forward-references Units 6, 8, 9, and 10.)
...
- 7 Verify and forward-reference (Walking each "Done when" clause as a test (JS-disabled submit, field errors, conflict path, optimistic rollback, transactional delete, revalidation) and naming the Units 7–14 layers that will extend each discipline.)

---

## Unit 7 — Transactional Email
...
- 2 Tour the starter (Walk the provided file tree, confirm the Zod-validated env entries, bring up Postgres, and prove the Unit 7 Resend pipeline still works before any code is written.)
...
- 4 Lock the verification gate (Build the React Email verification template, flip `requireEmailVerification` on, wire `sendVerificationEmail` through the Unit 7 pipeline, and add the sign-in action with opaque errors and `?next=` sanitization.)
...
## Unit 10 — Lists, URL State, and Soft Delete
...
- 1 The list view every SaaS ships (Frames the project goal — turning the Unit 6 invoice CRUD into a URL-state list with soft delete, archive, restore, and optimistic concurrency — and names the "Done when" verifications, scope cuts, and senior payoff.)
...
## Unit 12 — Background Work and Object Storage
...
- 6 Real downloadUrl for the export (Retrofits the 67 export task to do a server-side `PutObjectCommand` under the `org/<id>/exports/` prefix, hand a fresh `getSignedGetForKey` URL to the email, and rely on the lifecycle rule for cleanup with no `file_metadata` row written.)
...
- 2 Starter tour and inspector surface (Walk the 62-based starter, the new `org_invoice_summaries` table, the empty stubs (`tags.ts`, queries directives, action invalidations, task body), and every inspector panel that will verify later lessons.)
...
- 1 Project brief (Frames the build: wrap the 55 sign-in, sign-up, and reset actions with three Upstash limiters, swap out Better Auth's built-in limiter, and verify the 11th-request 429 against the inspector and the Upstash dashboard.)
- 2 Tour the 55 auth starter and the inspector (Reads the provided file tree, the still-on Better Auth built-in limiter, the empty `lib/` stubs, the seeded `alice` / `bob` / `eve` accounts, the mocked email counter, and every panel and toggle on `/inspector`.)
...
- 1 Project brief (Frames the build: bolt a polling, infinite-scrolling, optimistically-added comment thread onto the 62 invoice detail page with TanStack Query scoped to the leaf, names the "Done when" clauses, and links the starter.)
...
- 1 Project brief (Frames the four-step routed customer wizard you will build on top of the 62 customers surface, states the "Done when" clauses, and calls out the two structural decisions (shared-layout provider, vanilla `createStore`) that prevent the canonical bugs.)
...
- 5 Commit, then self-grade (Teaches the irreversible-commit-then-peek discipline, the clause-by-clause comparison against the `v1.0-answer-key` tag, the partial-credit scoring rule (rule and location are the floor, fix detail is the senior reach), and the forward references into Units 18 through 22.)

---

## Unit 17 — Time and Internationalization
...
- 1 Brief and Done-when (Frames the build, scope cuts, and the "Done when" clauses for lifting the 62 invoices list into a tri-locale, tz-aware surface with `hreflang` and per-locale sitemap entries.)
