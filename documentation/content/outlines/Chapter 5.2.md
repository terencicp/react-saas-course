...
- **Server-side data fetching in the body.** Direct `await db.query(...)`, `await fetch(...)`, `await fs.readFile(...)`. The senior reflex: data lives next to where it renders, no loader functions, no `getServerSideProps`. The Next.js 16 fetch cache and React `cache()` are named in context; Chapter 5.4 owns caching.
...
- **Where the boundary lives.** The student's job is to keep the Server Component tree wide and the Client Component leaves narrow. A page is Server, its layout shell is Server, the interactive `<DatePicker />` leaf is Client. Full Principle in 5.2.3.
- **The RSC payload — named once.** What the server sends back is not HTML alone; it's HTML plus a serialized React tree the client uses to reconcile. Full surface in 5.2.4.
- **Watch-outs.** No hooks; `async` is fine in a Server Component but not in a Client Component (use `React.use()` instead); reading cookies/headers/searchParams is async in Next.js 16 (5.4.8); importing a Node-only module accidentally into a Client Component crashes the build (5.2.3 owns the structural fix); secrets in a Server Component are safe but secrets *passed as props* to a Client Component leak — the RSC wire is visible in network DevTools.

What this lesson does not cover:

- Client Components and the boundary contract (5.2.2).
- The `"use client"` and `"use server"` directives in depth (5.2.3).
- `server-only` / `client-only` packages (5.2.3).
- What crosses the RSC wire and serialization rules (5.2.4).
- Hydration (5.2.5).
- Suspense and streaming — Chapter 5.3.
- Caching, `use cache`, `cacheLife`, `cacheTag` — Chapter 5.4.
- Async Request APIs (`cookies()`, `headers()`, `params`, `searchParams`) at depth — Chapter 5.4.8 and 5.5.1.
- Server Actions and `"use server"` for mutations — Chapter 7.2.

Estimated student time: 55 to 70 minutes. Load-bearing for every page in Chapter 5.3 onward, every data-fetching pattern in Unit 6, and every server-rendered surface in the course.
...
- **The two-render model.** Client Components render on the server to produce initial HTML (so the user sees content before JS loads), then hydrate in the browser to attach event listeners and become interactive. Both renders must produce the same output — divergence is a hydration mismatch (5.2.5).
- **The boundary contract — what crosses, in shape.** A Server Component renders a Client Component by importing it and passing props. Props must be serializable through the RSC wire — strings, numbers, plain objects, arrays, Promises, JSX (as `children`), Maps, Sets, Dates. No functions, no class instances. Full rules in 5.2.4.
...
- The full `"use client"` and `"use server"` directive semantics, edge cases, and bundler behavior (5.2.3).
- Architectural Principle #6 (5.2.3).
- `server-only` / `client-only` packages and structural enforcement (5.2.3).
- The exact list of types that survive RSC serialization (5.2.4).
- Hydration mechanics and mismatch causes (5.2.5).
- Suspense and `loading.tsx` for client transitions — Chapter 5.3.
...
- **Watch-outs.** Directive must be the first non-comment line; typos in directives fail silently; `"use server"` does not make a file a Server Component (Server Components are the default everywhere); `"use server"` exports are RPC endpoints — anyone who can reach them can call them, validate inputs (Chapter 7.2); `server-only` and `'use server'` solve different problems; Context providers are Client-bounded — they need `"use client"`; importing a `"use client"` file from a Server Component is fine, importing it from another `"use client"` file is fine (the directive is a one-way door); third-party packages without directives sometimes need a wrapper file; the build error from `server-only` is the early warning system — don't suppress it.

What this lesson does not cover:

- The full Server Actions surface (`'use server'` as RPC, validation, errors, redirects) — Chapter 7.2.
- The exact serialization rules at the boundary (5.2.4).
- Hydration (5.2.5).
- Async Request APIs at depth — Chapter 5.4.8.
- Route handlers as the alternative to Server Actions — Chapter 7.5.

Estimated student time: 50 to 65 minutes. Load-bearing for every Server Action in Unit 7, every server-only helper in Units 6 and 9, and the security baseline in Chapter 17.2.
...
- **Watch-outs.** "Cannot pass function" is the canonical first error; convert to a Server Action or move the handler inside the Client Component; class instances fail silently or with a confusing message — flatten to plain objects; Errors don't fully cross — use error boundaries (Chapter 5.3) and `error.tsx` for the user-visible surface; the RSC payload is visible in DevTools — treat it like any other network response; keep props narrow, both for security and for wire size; large server-rendered trees in props are still wire bytes; Promise props plus `React.use()` is the streaming pattern (Chapter 5.3); `Date` is fine, `dayjs.Dayjs` is a class instance and fails — pass the raw `Date` (Chapter 18.1).

What this lesson does not cover:

- Server Actions in full — Chapter 7.2.
- Suspense, `React.use()`, streaming — Chapter 5.3.
- The full caching model — Chapter 5.4.
- Error handling and error boundaries — Chapter 5.3 and 17.1.
- The structured clone API itself in browser contexts (`postMessage`, `IndexedDB`) — Chapter 3.7 in passing.
- Hydration (5.2.5).

Estimated student time: 50 to 65 minutes. Load-bearing for every Server-to-Client prop in the course, the Promise-streaming pattern in Chapter 5.3, and the data shape decisions in Unit 6.
...
- Suspense, streaming, and how server-rendered async content arrives — Chapter 5.3.
- `error.tsx` and `global-error.tsx` — Chapter 5.3.
- Static vs. dynamic rendering and Cache Components — Chapter 5.4.
- Time, timezone, and locale handling in depth — Chapters 18.1 and 18.2.
- `useId` semantics beyond hydration (form/ARIA at depth) — Chapter 4.8.
