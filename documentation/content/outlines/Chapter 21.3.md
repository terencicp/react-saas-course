...
- **Row 3 — Rate limits live on the abuse surface.** Verify: auth endpoints (sign-in, sign-up, password reset, magic link) are rate-limited via Upstash from Unit 15.4; a script hitting `/api/auth/sign-in` 50 times in 10 seconds gets 429s after threshold. Reach for `hey` as the simple HTTP load-testing CLI for this check (`hey -n 50 -c 5 <URL>`); equivalents include `oha` or a curl loop. Unrated auth endpoints are credential-stuffing targets on day one. Cross-ref: Unit 15.4.
