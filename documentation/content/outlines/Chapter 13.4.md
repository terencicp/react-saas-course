...
Chapter 13.4 cashes in the 13.3 primitives — scoped credentials and CORS (13.3.2), presigned PUT/GET with the two-step write (13.3.3), the `file_metadata` row shape with tenancy at the read boundary (13.3.4), the workload split between user uploads and export outputs (13.3.5) — and lands them as one runnable upload surface. The student writes the `presignedPut` Server Action that signs against R2 without piping bytes through the function, the browser-side direct-to-R2 PUT with XHR progress, the `finalizeUpload` action that HEADs the object and inserts the row, the `Files` list rendering rows with fresh per-render presigned GETs, and the retrofit on the 13.2 export job so the `console.log`-shaped `downloadUrl` becomes a real R2 object and the email's link is a 10-minute presigned GET. Each build lesson closes on a runnable state: 13.4.3 ends with a fireable action verified by `curl`-PUT; 13.4.4 ends with the browser upload landing the bytes and the row; 13.4.5 ends with the `Files` page rendering downloads; 13.4.6 ends with the export emailing a working R2 link. Verify walks the "Done when" clause-by-clause, including the long-tail proof — a refresh 11 minutes later still works because GETs are re-issued per render.
...
trigger.config.ts              # provided (from 13.2)
.env.example                   # provided: 13.2 vars + R2_ACCOUNT_ID/ACCESS_KEY_ID/SECRET_ACCESS_KEY/BUCKET_NAME
package.json                   # provided: 13.2 scripts + r2:cors, r2:lifecycle
scripts/
  seed.ts                      # provided: orgs + invoices from 13.2; no file_metadata rows
  r2-cors.ts                   # provided: PutBucketCors with the CORS JSON
  r2-lifecycle.ts              # provided: 7-day rule on org/*/exports/*
src/
  db/schema.ts                 # provided: 13.2 schema; file_metadata TODO student
  lib/
    ...
    invoices/                  # provided (13.2)
    exports/                   # provided (13.2) — student edits in 13.4.6
  trigger/
    export-invoices.ts         # provided (13.2) — student edits in 13.4.6
    paginate-page.ts           # provided (13.2)
    send-export-email.ts       # provided (13.2) — payload already accepts downloadUrl
  emails/ExportReadyEmail.tsx  # provided (13.2)
  app/
    files/
    ...
    inspector/page.tsx         # provided: 13.2 surface + downloadUrl renders as clickable link
```
...
**`/inspector` page** (provided, from 13.2 + one addition): the run-status panel renders `metadata.downloadUrl` as a clickable link once the export completes.
...
- Threshold for object storage, R2 vs. S3 — 13.3.1.
- Bucket + scoped credentials + CORS, S3-compatible endpoint, `region: 'auto'` — 13.3.2.
- Presigned PUT/GET mechanics, `ContentType`/`ContentLength` pinning, expiry trade-offs — 13.3.3.
- Two-step write, layered size defense, R2-specific `ContentLength` non-enforcement — 13.3.3.
- `file_metadata` shape, `id` as key segment, no-`url`-column rule — 13.3.4.
- Tenancy at read via `tenantDb(orgId)`, audit on upload/download/delete — 13.3.4 / 10.1 / 10.2.
- Soft delete with cooled-off cleanup (named) — 13.3.4 / 11.2.
- One bucket two prefixes, browser-vs-worker byte-pipe rule — 13.3.5.
- `authedAction('member', ...)` — 10.2. Result shape — 7.2. Zod validation — 7.1/7.2.
- `XMLHttpRequest` for upload progress — closes 3.7.
- Principle #3 (named seams in `lib/files/`) — 5.7/7.2. Principle #5 (R2 SDK at the call site) — 13.3.2.
...
- Frame the build: direct-browser-to-R2 upload with `Files` list, plus the 13.2 export retrofit so the email carries a real R2 link. One screenshot: `/files` with form, progress mid-flight, list of completed rows.
...
- Senior payoff: the canonical R2 shape every later upload feature copies — `lib/r2.ts` once, presigned PUT, two-step write with HEAD, fresh-per-render GETs, `file_metadata` as canonical identity. The retrofit on 13.2 proves the same primitives work from a worker with server-side PUT.
...
- Starter inherits 13.2's Trigger.dev project. 13.4.6 needs both `pnpm trigger:dev` and `pnpm dev` in two terminals.
...
Codebase state at exit: starter cloned, deps installed, Postgres up, 13.2 schema migrated and seeded, R2 bucket/token/env created, `pnpm r2:cors` run, `/files` renders empty list with broken form, `/inspector` carries the 13.2 surface.
...
- Walk the file tree, calling out provided vs. stubbed. Linger on the five TODO files in `lib/files/` and the two in `app/files/`. The 13.2 task files are provided in full 13.2 form; student edits them in 13.4.6.
- Read the provided pieces: `lib/r2.ts` (singleton `S3Client` + constants), `lib/files/keys.ts` (pure `buildObjectKey` — extension from the validated content type, never from the user filename), `lib/files/errors.ts` (the four error codes), `lib/files/soft-delete.ts` (named, not exercised), `scripts/r2-cors.ts` (CORS JSON per 13.3.2). Confirm `pnpm r2:cors` ran successfully.
- Read `app/inspector/page.tsx` — the 13.2 surface intact, the new "downloadUrl as clickable link" rendering provided.
- Bring up the dev rhythm: `pnpm trigger:dev` in one terminal, `pnpm dev` in another. Visit `/files`: form renders, picking a file does nothing (action empty). Visit `/inspector`: 13.2 surface works (export still produces `console.log`-shaped placeholder, not R2 yet).
...
- `ContentLength` is signed but R2 does not enforce it server-side (the 13.3.3 quirk). Signing is still correct — it documents intent and matches the AWS SDK shape — but the *enforcement* is the post-upload HEAD in `finalizeUpload`.
...
- Migration adds the table; re-running `db:generate` emits "no changes" (determinism signal from 6.5).
...
- Edit `trigger/export-invoices.ts`. After the page loop accumulates `csvAccumulator`: `const buffer = Buffer.from(csvAccumulator, 'utf8'); const objectKey = \`org/${organizationId}/exports/${ctx.run.id}.csv\`; await r2.send(new PutObjectCommand({ Bucket, Key: objectKey, Body: buffer, ContentType: 'text/csv', ContentDisposition: \`attachment; filename="export-${dayBucket()}.csv"\` }))`. **Server-side PUT** — bytes pass through this Trigger.dev worker, which is correct per 13.3.5.
- `const { url: downloadUrl } = await getSignedGetForKey({ objectKey, expiresIn: 600 })`. `metadata.set('downloadUrl', downloadUrl)`. Pass `downloadUrl` to `sendExportEmail.triggerAndWait({ ..., downloadUrl })` (payload schema already accepts it from 13.2).
...
Codebase state at entry: 13.2 export works against placeholder URL.
