...
- **A minimal `useState` primer (for the next two lessons).** Before the initializer trap and 4.7.4's snapshot examples, the basic shape: `const [value, setValue] = useState(initial)` declares a piece of local state. Reading `value` returns the current render's snapshot; calling `setValue(next)` schedules a re-render in which `value` reflects `next`. The full API surface — lazy initialization, functional updates, the `useState` vs. `useReducer` cut — lands in 4.8.1; this primer exists so 4.7.3 and 4.7.4 are readable inline.
...
- **The updater function form.** `setCount(c => c + 1)` passes a function that receives the *queued* value and returns the next (full treatment in 4.8.1). React calls each updater in order. Three calls now produce 0→1→2→3. The senior rule: when the next state depends on the previous, use the updater form. When it doesn't (`setCount(0)`, `setUser(newUser)`), the direct form is fine and arguably clearer.
