...
- **CI artifact pattern.** Produce the analyzer HTML in CI on dep- or route-touching PRs, upload as workflow artifact, reviewer eyeballs the diff against `main` (CI workflow primitives: 21.2.1). No bot needed — habit plus PR-template checklist line covers the regression class. `hashicorp/nextjs-bundle-analysis` exists for the formal version; named once.
