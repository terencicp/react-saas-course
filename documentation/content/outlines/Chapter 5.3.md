...
- **The interaction with `useTransition` — named once.** A state update wrapped in `startTransition` does not re-trigger the Suspense fallback; instead the previous tree stays visible and `isPending` reads true. The pattern earns its weight when the fallback would be jarring (search-as-you-type, tab switches). Chapter 4.9.5 owns the hook; named here so the student knows the option exists.
- **What Suspense does not do.** It does not catch errors (`error.tsx` and Error Boundaries own that, 5.3.3). It does not retry. It does not deduplicate fetches (React `cache()` and the request memo cache own that, 5.4.5). It does not auto-skeleton the children — the student writes the fallback.
...
- **Watch-outs.** Suspense is not for errors — wrap with `error.tsx` or a separate Error Boundary; fallbacks that themselves suspend make the boundary useless; `key`-less Suspense around param-driven children shows stale content during transitions; nesting too deeply produces a flash-cascade of skeletons; promise creation in Client Components for `use()` is not stable across renders — create on the server, pass down, or wrap in a memo with `cache()` (5.4.5); a Suspense boundary inside an `async` Server Component still has to wait for the parent's await before rendering its child.

What this lesson does not cover:

- Streaming and how the route shell sends fallbacks before resolved content (5.3.2).
- `loading.tsx` as Suspense at the route boundary (5.3.3).
- `error.tsx`, `not-found.tsx`, and Error Boundary semantics (5.3.3).
- `global-error.tsx` (5.3.4).
- `useTransition`, `useDeferredValue` in full — Chapter 4.9.5.
- `useOptimistic` and Server Action UX — Chapter 7.3.6.
- The full `React.use()` API and Promise creation rules — Chapter 4.9 in context, 5.2.4 introduced.
- Caching and `cache()` for request memoization — Chapter 5.4.

Estimated student time: 50 to 65 minutes. Load-bearing for every async UI in the course, the project in 5.7, and every list-plus-detail surface in Units 11 and 12.
...
- **The relationship to Partial Prerendering — named, deferred.** PPR (5.4.2) extends streaming with a build-time static shell: the static parts ship from the CDN, the dynamic parts stream in. Same Suspense boundaries, two transports. Chapter 5.4.2 owns the rendering decision; this lesson focuses on the runtime mechanics of streaming a single request.
...
- **Streaming and parallel routes — 5.1.5 revisited.** Each `@slot` in a parallel route is its own independent subtree and gets its own Suspense boundary. List-plus-detail surfaces stream the list and the detail in parallel; either can show its skeleton while the other resolves. The 5.7 project leans on this.
...
- **Cancellation and abandonment.** When the user navigates away mid-stream, Next.js aborts the request. Server work in flight (DB queries, fetch calls) does not auto-cancel — pass `signal` from `connection()` or accept the cost. Named here; full cancellation surface in 5.4.8.
...
- The file conventions — `loading.tsx`, `error.tsx`, `not-found.tsx` (5.3.3).
- `global-error.tsx` (5.3.4).
- Partial Prerendering, `cacheLife`, `cacheTag` — Chapter 5.4.
- `connection()` and dynamic-rendering opt-in — Chapter 5.4.8.
- Real-time push (SSE, WebSockets) at depth.
- Parallel routes in full — Chapter 5.1.5.
- React `cache()` for deduplicating parallel queries — Chapter 5.4.5.

Estimated student time: 45 to 60 minutes. Load-bearing for every page with more than one slow read, the project in 5.7, and the PPR rendering model in 5.4.2.
...
- **`error.tsx` does not catch errors in its own layout.** A throw in the layout that wraps `error.tsx` escapes — the boundary is *inside* the layout's children. The fix is to put the `error.tsx` one segment up, or to add `global-error.tsx` at the root (5.3.4) for the layout-throws case.
- **`not-found.tsx` — the platform's 404 surface.** A file named `not-found.tsx` is rendered when the framework triggers a not-found for the segment. The trigger is the `notFound()` function from `next/navigation` (named in 5.1.4): called inside a Server Component, it short-circuits rendering and renders the nearest `not-found.tsx` up the tree.
...
- **Status codes — what each file emits.** `loading.tsx` is part of a 200 streaming response. `error.tsx` emits 500 by default for an uncaught throw; for known errors, throw inside a Server Action and handle in the action return shape (Chapter 7.2). `not-found.tsx` emits 404 — correct for SEO and for monitoring.
...
- **Watch-outs.** `error.tsx` must have `"use client"` or the build fails; `error.tsx` does not catch errors in the layout that wraps it (5.3.4 owns the fix); `loading.tsx` is a Server Component, animations inside it should be in a Client child; `not-found.tsx` is triggered by `notFound()`, not by a 404 status from `fetch`; throwing inside a Server Action goes through different channels (Chapter 7.2); `reset` re-renders the segment, it doesn't refetch — if data is the cause, the new render will reproduce the error (combine with `router.refresh()` to retry data); the same `error.tsx` catches errors at the layout level for nested routes — write it as if it's the only safety net for everything below.

What this lesson does not cover:

- `global-error.tsx` for the root-layout case (5.3.4).
- `notFound()` and other `next/navigation` exports in full — Chapter 5.1.4.
- Server Action error returns — Chapter 7.2.
- Custom error classes and the error-handling discipline — Chapter 17.1.
- Sentry and error monitoring — Chapter 20.1.
- Page-level metadata for error pages — Chapter 5.6.9.

Estimated student time: 55 to 70 minutes. Load-bearing for every route in the course and the project in 5.7.
...
- **Internationalization caveat — named.** Locale-aware messages require the i18n setup to have loaded *before* the crash. For top-of-layout failures it usually hasn't — keep `global-error.tsx` in the default locale or with stripped strings ("Something went wrong" plus a digest). Chapter 18.2 owns i18n proper.
- **Monitoring integration — the senior reach.** `global-error.tsx` is the canonical place to call `captureException(error)` from the monitoring SDK (Sentry, Datadog) so the catastrophe is logged. Chapter 20.1 owns the full integration; named here as the location.
...
- Segment-level `error.tsx` (5.3.3).
- `not-found.tsx` and the not-found surface (5.3.3).
- Error monitoring and Sentry — Chapter 20.1.
- Custom error classes and discipline — Chapter 17.1.
- Internationalization at depth — Chapter 18.2.
- Server Action error handling — Chapter 7.2.

Estimated student time: 30 to 40 minutes. Load-bearing for production resilience in every project from 5.7 onward and the observability work in Chapter 20.1.
