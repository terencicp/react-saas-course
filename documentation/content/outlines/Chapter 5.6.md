...
- **`cacheComponents: true`.** The 5.4 rendering model is opted in here; flag stays on for every new project.
...
- **Security headers — forward reference.** `headers()` in config is the canonical place for CSP, HSTS, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`. Full baseline in 17.2.
...
- **Next.js 16.1 transitive externalization.** Turbopack now correctly externalizes transitive dependencies of packages in `serverExternalPackages` — no more declaring an SDK's internal deps. Named so older configs are recognized.
...
- Redirects and rewrites in detail (5.6.3).
- Security headers (Chapter 17.2).
- The `cacheComponents` model itself (5.4.1).
...
- **Self-hosted deploys.** Optimizer requires a sharp-capable function; outside Vercel, `loader`/`loaderFile` route to an alternative. Full pattern in 21.3.
...
- Security headers for image sources (17.2).
- Object storage and presigned uploads (13.3).
- Self-hosted deployment specifics (21.3).
- `next/font` (5.6.4).
...
- **Decision tree.** Static and request-independent (legacy URL migrations, marketing renames): `next.config.ts`. Request-conditional (auth, A/B, geo): `proxy.ts` (5.5.2–5.5.3). After a Server Action: `redirect()` from `next/navigation` (5.1.4).
...
- **Headers in `next.config.ts`.** Same shape (`source`/`headers`), applied at the edge. Canonical use is the security baseline (17.2).
...
- Conditional redirects in `proxy.ts` (5.5.3).
- Security headers in depth (17.2).
- Route-level redirects via `redirect()` and `notFound()` (5.1.4).
- I18n routing (18.2).
...
- **What `next/font` does not do.** No runtime CDN fetches; no font without a subset declared; no icon-font optimization (use SVG sprites or Lucide — 4.11).
...
- Tailwind font configuration in depth (4.5).
- Icon libraries (4.11).
- Design tokens (4.3).
...
- Analytics setup (20.2).
- Error monitoring SDKs (20.1).
- Stripe integration (12.2).
...
- **Promise shape.** `params` and `searchParams` are Promises in 16; `await` inside. Fetches here dedupe with the page's fetch via React's `cache()` (5.4.5).
...
- **`opengraph-image.tsx` and `twitter-image.tsx`.** Recapped briefly; full coverage in 5.6.6. A root-level `opengraph-image.tsx` provides the brand fallback for every route that doesn't override.
...
- Dynamic OG generation in depth (5.6.6).
- Full PWA implementation.
- Schema.org in depth.
- I18n sitemaps with `hreflang` (18.2).
...
- **Pairing with `use cache`.** A `[slug]` page with `generateStaticParams` plus a `use cache` body and `cacheTag(\`post:${slug}\`)` ships static for the build-time set; post-edit invalidation (5.4.6) busts the cache for the changed slug. Production shape for content surfaces.
...
- **What it does not do.** No request-time run; no per-request data (`cookies()`, `headers()`); not a replacement for `use cache` on arbitrary functions (5.4.3); doesn't apply without a dynamic segment.
...
- **Watch-outs.** Empty array makes every URL fall to runtime; function failing at build breaks the deploy — guard with try/catch and a fallback if non-critical; `use cache` body reading `cookies()` is a build error — must be deterministic; tens of thousands of slugs slow the build noticeably; `dynamicParams: false` with an incomplete list returns 404s; `searchParams` doesn't participate in static generation — a route with `searchParams` is dynamic regardless (5.5.4).

What this lesson does not cover:

- `use cache` and `cacheTag` in depth (5.4.3, 5.4.4).
- Post-mutation invalidation (5.4.6).
- Cursor pagination (6.3).
