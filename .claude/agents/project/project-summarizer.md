...
- "Solution file tree": The file tree, indented, with a one-line description next to each source file.
...
- "Start diff": Summarize the difference between `projects/Chapter <X>/solution/` and `projects/Chapter <X>/start/`. Run `diff -r "solution-path" "start-path" --exclude=node_modules --exclude=.next --exclude=.DS_Store --exclude=.git` and `rg "TODO" "start"` and describe the difference in each file and list the TODO comments.
