...
List the start dir's files with `find . \( -name node_modules -o -name .next -o -name dist -o -name build -o -name .git \) -prune -o -print` to confirm the derivation landed: every file the `Start derivation` named is present, nothing the plan said to delete is still there. Then read back every file you overwrote or stubbed and check the body matches the plan — no solution code leaked into stubs, no half-applied edits, no missing imports. Fix any gaps.
