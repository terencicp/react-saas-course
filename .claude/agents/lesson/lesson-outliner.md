...
If diagrams are necessary to convey the lessons concepts, describe each in detail in the relevant section and explain what the pedagogical goal of the diagram is. Remember a diagram is not just a complex system graph, any simple visual aid that enriches the lesson is a positive addition. Use the TabbedContent component to group multiple diagrams or the DiagramSequence to show step-by-step animations if necessary.

If exercises are helpful to allow the student to check its understanding or practice the concepts it just learned add them to the relevant section. Exercises can be live coding pre-built components, sandboxes embedded using the SandboxCallout component, or other pre-built exercise components. If no pre-built exercise component is the best way to test understanding or practice a specific topic, propose building a custom exercise, describe its UI, mechanics, goals and grading criteria so the corresponding agent can build it.
...
Finally, return the path of the newly created lesson outline file. If you had any issues or have any ideas to improve the work of agents carrying out these tasks in the future, describe them briefly and concisely as feedback.
