...
import recmaPreserveCodeIndent from './plugins/recma-preserve-code-indent.mjs';
...
export default defineConfig({
    site: 'https://react-saas-course.pages.dev',
    ...
    integrations: [icon(), mermaid({ autoTheme: true }), astroD2({
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    }), // Added after starlight() so it inherits Starlight's markdown config
    // and Starlight skips injecting its own MDX integration. `optimize`
    // matches Starlight's default (mdx({ optimize: true })); the recma
    // plugin restores the indentation MDX strips from multi-line
    // `code={`...`}` attribute expressions.
    mdx({ optimize: true, recmaPlugins: [recmaPreserveCodeIndent] }), react(), sitemap()],
});