...
So where does the "time zone" in the name come from? From the conversion Postgres does on the way in and the way out. That conversion is driven by a per-connection setting called the <Term definition="A per-connection Postgres setting that controls how timestamptz input strings are interpreted and how output is rendered as text. It does not change the stored bytes, only the translation on input and output.">session `TimeZone`</Term>. On `INSERT`, Postgres reads that setting, interprets your input string as if it were written in that zone, converts the result to UTC, and stores the UTC bytes. On `SELECT`, the same setting drives how the stored bytes get rendered back into text.
...
<VideoCallout videoId="oWF5jTFBSHw" videoTitle="Tips for handling timezones in Postgres">
  Aaron Francis (Mastering Postgres) demonstrates the session `TimeZone` mechanic live in 14 minutes. He covers the same "keep it UTC, convert at the edge" rule, plus the difference between named zones and offsets.
</VideoCallout>
...
<CodeVariants>
  ...
  <CodeVariant label="mode: 'string' — wrong shape">
    ...
    **Hands back Postgres's raw text**, `'2026-03-09 07:47:00.038+00'`. It has a space where the canonical form uses `T`, and, the part that causes trouble, an hours-only `+00` offset instead of the `±HH:mm` shape (`+00:00`) a clean ISO 8601 carrier round-trips. The type is right (a string), but the shape is non-canonical, so you don't hand it to Temporal raw; you normalize it first.
  </CodeVariant>
</CodeVariants>
...
<CodeVariants>
  ...
  <CodeVariant label="Server Action">
    ...
    ```
    **The write direction takes a `Temporal.Instant` too.** The action re-derives the instant on the server rather than accepting one from the client, and the next section is entirely about why.
  </CodeVariant>
  ...
  <CodeVariant label="Route handler">
    ...
    ```
    **The wire direction is the one manual encode.** A `Temporal.Instant` can't cross a JSON response or the React Server Component wire as-is, because it's a class instance the boundary can't serialize. Call `.toString()` (or let `JSON.stringify` invoke `toJSON()`) to emit the ISO string.
  </CodeVariant>
</CodeVariants>
