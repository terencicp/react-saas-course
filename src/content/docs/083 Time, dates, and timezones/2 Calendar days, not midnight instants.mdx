...
<VideoCallout videoId="P7TC3-Lvpf8" videoTitle="Temporal API Explained">
  Tom Dohnal's 11-minute walkthrough, with the chapter at 5:00 on `Temporal.PlainDate` showing exactly why a calendar day stored as an instant lands on a different date in each timezone, and why a `Date` can't hold one safely.
</VideoCallout>
