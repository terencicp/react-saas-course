...
Resend doesn't roll its own signing scheme. It signs webhooks with <Term definition={"A hosted webhooks-as-a-service layer used by many SaaS providers. Standardizes the signature header format and delivery (retries, etc.) so every Svix-backed webhook verifies the same way."}>Svix</Term>, a webhooks-as-a-service layer that a lot of SaaS products use so their customers don't each have to invent a new verification scheme. Svix sends three headers with every delivery:
...
`}>
  ...
  <AnnotatedStep meta="{23}" color="green">
    The dedup key is the `svix-id` header, the Svix message id, not anything inside the body. This matters because a bounce and a *later* complaint for the same email arrive as two deliveries with two different `svix-id`s, so both must process. Dedup on the body's email id and you'd wrongly collapse them into one.
  </AnnotatedStep>
...
:::note
The sandbox spells out SQL column names like `text('provider_event_id')` and swaps the production `reason` pgEnum for plain `text`, because it has no client-level `casing` config and PGlite can't take the full schema. In the real project you set `casing: 'snake_case'` once and write plain camelCase keys against the ch048 `emailSuppressions` schema. It's the same SQL; the sandbox just writes it out longhand.
:::
...
<Figure caption="The top three rows change with every provider; the bottom three never do. That fused green block is the spine you reuse, and a new provider is just the amber edges.">
  <PortabilityGrid />
</Figure>
