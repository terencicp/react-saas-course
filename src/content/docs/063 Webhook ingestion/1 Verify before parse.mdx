...
It helps to make the threat concrete, because the stakes are the whole reason the mechanism exists. Without a threat behind it, the cryptography is just ceremony.
...
Now we open the box. "Verify the signature" has three moving parts, and they're easiest to hold one at a time. We'll build them up in order: the header Stripe sends, the exact string it signs, and the check you run.
...
The timestamp is glued in front so the signature covers it too, which is what makes the freshness check later trustworthy. And `rawBody` means the exact bytes Stripe sent, character for character, not a parsed-and-reprinted version. Hold onto that; the next section is entirely about it.

**Three: the verification.** You compute an <Term definition="Hash-based Message Authentication Code. A keyed hash: feed it the same bytes and the same secret key and it always returns the same digest; without the key you can't produce a matching digest. It proves both that the message wasn't altered and that the sender held the key.">HMAC</Term>-SHA-256 of that signed-payload string, keyed by your webhook secret. That gives you a <Term definition="The fixed-length output of a hash function — here, the 32 bytes (64 hex characters) HMAC-SHA-256 produces.">digest</Term>. Then you compare your digest against the `v1` value from the header. A match means the request is authentic. No match means it's forged or your config is wrong, and you treat both the same way.
...
`}>
  ...
  <AnnotatedStep meta="{17}" color="orange">
    **Build the signed payload.** Timestamp, a literal dot, then the exact raw body. This is the single line beginners get wrong: it is the raw bytes off the wire, never re-stringified JSON. The whole verification hinges on it.
  </AnnotatedStep>
  ...
  <AnnotatedStep meta="{26-29}" color="blue">
    **Sign, then decode `v1` to bytes.** `crypto.subtle.sign` returns an `ArrayBuffer`, so you wrap it in a `Uint8Array`, the same view-over-bytes move from the Web Crypto lesson. The header's `v1` is hex, so `hexToBytes` decodes it to the matching `Uint8Array`, putting both sides in the same shape to compare.
  </AnnotatedStep>

  <AnnotatedStep meta="{31}" color="orange">
    **Constant-time compare the two byte buffers.** `constantTimeEqual` is the length-checked XOR loop you wrote in the Web Crypto lesson: it takes two `Uint8Array`s and always runs the full length. Never `===` here, because a byte-by-byte early exit leaks timing.
  </AnnotatedStep>

  <AnnotatedStep meta="{33-34}" color="green">
    **Check the timestamp tolerance.** A digest can match and the event can still be a stale replay. Rejecting anything older than 300 seconds closes that hole, and the next section explains why 300.
  </AnnotatedStep>
</AnnotatedCode>
...
:::note
Web Crypto has no built-in constant-time comparison the way Node's `node:crypto` ships `timingSafeEqual`. That's why the hand-rolled path leans on the XOR-loop helper from the Web Crypto lesson. When we switch to the SDK, this nuance resolves itself, because the SDK picks the right primitive for the runtime.
:::

## The raw body is sacred

Of every rule in this lesson, this is the one that most often fails in production, so it gets its own section. The trap is comfortable and reasonable-looking, which is exactly why it catches people.
...
Step six of the hand-rolled verifier checked that `|now − t|` was under five minutes, and we left the "why" for later. Here it is.
...
:::caution
The tolerance check is not optional, and it's easy to leave out of a hand-rolled verifier because the signature already passes without it. A verifier that skips the timestamp has a permanent replay hole even though every test you write will go green. Both Stripe's SDK and the hand-rolled helper above bake the check in, which is why it's step six rather than an afterthought.
:::

## `constructEvent`: the box, closed

You've now seen every gear: parse the header, rebuild `` `${t}.${rawBody}` ``, HMAC it, constant-time compare, check the tolerance. Now we close the box and never open it again, because Stripe's SDK does all five of those steps in a single call.
...
Then create one shared client and export it. Every call site in the app imports this same instance, so you never write `new Stripe(...)` per request.
...
`}>
  <AnnotatedStep meta="{7}" color="blue">
    **Declare the Node runtime.** Next.js 16 already defaults route handlers to Node, so this line doesn't change anything; it states intent. The full rationale is in the prose below.
  </AnnotatedStep>

  <AnnotatedStep meta="{10}" color="green">
    **Read the raw body once.** `request.text()` gives you the exact bytes off the wire, read a single time. This is the source of truth for the HMAC, exactly as the "raw body is sacred" rule demands.
  </AnnotatedStep>

  <AnnotatedStep meta="{11}" color="blue">
    **Read the signature header, and treat null as failure.** A missing header gets no benefit of the doubt: `null` means not from Stripe, or misconfigured. It flows into the same 400 via `signature ?? ''`, which `constructEvent` rejects.
  </AnnotatedStep>
  ...
  <AnnotatedStep meta="{32-33}" color="green">
    **On success, hand off.** This is a deliberate stub: claiming the event and writing to the database is the next lesson. This lesson ends the moment the signature passes.
  </AnnotatedStep>
</AnnotatedCode>
...
When it starts, `stripe listen` prints something you should not skip past: a webhook signing secret, a string beginning with `whsec_`. That is the secret for this local session, and it's the value you paste into `STRIPE_WEBHOOK_SECRET` in your local `.env` so `constructEvent` can verify the forwarded events.
...
This makes Stripe generate a realistic `checkout.session.completed` event and send it down the tunnel to your handler: no real checkout, no real card, just a properly-signed event you can develop against. Set a breakpoint, watch it verify, and iterate.
...
:::caution
The `whsec_` secret that `stripe listen` prints is **not** the same secret as the one configured for your endpoint in the Stripe Dashboard. Each environment has its own signing secret. Paste the local CLI secret into production (or your production secret into local dev) and every single event fails verification with a 400, because the HMAC computes a different digest when the key is different, and the symptom looks exactly like a broken verifier. The fix is the env discipline you already use: one `STRIPE_WEBHOOK_SECRET` value per environment, never crossed.
:::
...
One last concern follows straight from "the secret is the trust root": eventually you'll need to rotate that secret, either on a schedule or because you suspect it leaked. The naive picture is alarming. If `constructEvent` verifies against exactly one secret, swapping it seems to require a moment where in-flight events signed with the old secret get rejected, which would mean an outage just to rotate a key.
...
Two quick drills lock in the decisions before the next lesson builds on them. The first cements the order: verify before parse before process. The sequence is the whole point.
