...
`auto-install-peers=true` tells pnpm to install <Term definition="A package a library declares it expects the host project to provide, rather than bundling itself — React is the classic example for a React component library.">peer dependencies</Term> automatically. This is already the modern default; the starter sets it explicitly so contributors on an older pnpm version get the same behavior.
