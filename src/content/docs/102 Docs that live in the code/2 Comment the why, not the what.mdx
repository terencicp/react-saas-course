...
<CodeVariants syncKey="why-test-pair">
  ...
  <CodeVariant label="Earns it">
    ...
    **Identical code, opposite verdict.** The *why* lives in Stripe's retry behavior, outside this file and invisible to anyone reading the line. Without the comment, `retries += 1` looks arbitrary, and a future reader "simplifies" the cap away.
  </CodeVariant>
</CodeVariants>
...
:::note
Single-line `//` for one-line whys; for a multi-line why, stack more `//` lines (block `/* ... */` is reserved for TSDoc). Sentence fragments need no terminal punctuation; full sentences get it. A lowercase first letter is fine on a fragment. Above all, match the file's existing style, since consistency reads faster than any one rule.
:::
...
- **An ordering constraint becomes a transactional function.** `// audit must commit before the email enqueue` becomes a single function that does both inside a transaction, in the guaranteed order. The ordering is now structural, since there's no longer a way to write the awaits in the wrong sequence, and the comment dissolves. This is the [thin-action, pure-`lib`](/043-server-actions/4-thin-actions-pure--lib/) shape you already use.
- **A "remember to validate" becomes a Zod parse.** `// validate the input first` becomes a `safeParse` at the boundary. The validation is now *enforced* rather than *remembered*, exactly the [parse-on-entry](/043-server-actions/2-parse-on-entry-every-time/) discipline, seen from a new angle. The comment was a reminder; the parse is a guarantee.
...
<CodeVariants syncKey="comment-to-enforcement">
  ...
  <CodeVariant label="Enforced by the type system">
    ...
    **The parse runs on entry, every time, and there's no path into the insert that skips it.** `valid` is now a typed, validated value. The constraint moved from prose to the boundary, and the comment is gone because it's no longer needed. The strongest form of documenting a why is making the wrong thing impossible to write.
  </CodeVariant>
</CodeVariants>
...
CodeAesthetic's six-minute essay makes the same argument from the other end: most comments are a missed chance to refactor the code or lift the constraint into a type, and the few that survive are the *why*s no type can express.
