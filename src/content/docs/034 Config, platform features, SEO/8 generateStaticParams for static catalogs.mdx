...
But under `cacheComponents: true`, the flag you turned on in [next.config.ts](/034-config-platform-features-seo/1-the-typed-nextconfigts/), each `[slug]` segment is **dynamic by default**: its render function runs on every request.
