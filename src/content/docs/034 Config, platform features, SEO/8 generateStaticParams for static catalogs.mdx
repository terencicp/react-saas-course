...
But under `cacheComponents: true`, the flag you turned on in [The typed next.config.ts](/034-project-config-platform-primitives-seo/1-the-typed-nextconfigts/), each `[slug]` segment is **dynamic by default**: its render function runs on every request.
