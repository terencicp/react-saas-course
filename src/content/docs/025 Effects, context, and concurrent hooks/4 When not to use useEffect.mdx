...
tagline: The React judgment of when useEffect is the wrong tool, and which closer-fitting tool to reach for instead.
chapter-id: 25
course-progress: 0.2261
rewrites: 1
sidebar:
  order: 4
  label: When not to use useEffect
---
...
1. **Is this value <Term definition="A value computed from existing props or state, recomputed on every render rather than stored. Lives in the render body, not in useState.">derived</Term> from props or state you already have?** Compute it in render. As you saw in ["Derive in render, do not mirror into state"](/024-hooks-for-holding-state/2-derive-in-render-do-not-mirror-into-state/), a value you can calculate from what you hold doesn't get its own `useState`.
...
3. **Is it the page's initial data?** It's a <Term definition="A Server Component or framework data function that fetches the page's data before render, on the server. The 2026 default for initial page data — no effect, present at first paint.">route loader</Term> or a Server Component that fetches before the page renders. You'll meet this path with the App Router.
4. **Is it cached server state, something you refetch, poll, or update optimistically?** It's <Term definition="A client-side server-state cache library: it handles fetching, caching across views, refetching, polling, invalidation, and optimistic updates so you don't hand-roll them in an effect.">TanStack Query</Term>, or <Term definition="A React hook that reads the value of a promise (or context) during render, suspending until it resolves. Used to consume a promise passed down from a Server Component.">use()</Term> for a simple read. Caching, invalidation, and polling are a solved problem, and the solution isn't an effect.
...
<StateMachineWalker kind="decision" title="Before you write the effect…">
  ...
  <Leaf id="leaf-nothing" verdict="You don't need to do anything">
    If it isn't derived, isn't an event, isn't data to load, isn't cached state, and there's no external system, the value already exists and already flows.
...
<CodeVariants>
  ...
  <CodeVariant label="Derived (the fix)">
    ...
    **`total` isn't state, it's an expression.** Compute it in the render body and it's always correct, in a single render, with nothing to sync. The `useState` and the `useEffect` both disappear, and so does the class of bug they introduced.
  </CodeVariant>
</CodeVariants>
...
<VideoCallout videoId="E1cklb4aeXA" videoTitle="Most Beginner React Developers Do This Wrong">
  Web Dev Simplified builds a search filter two naive ways, including storing the filtered list in state, then derives it in render and adds `useMemo` only where it's earned. 14 minutes.
</VideoCallout>
...
<CodeVariants>
  ...
  <CodeVariant label="key reset (the fix)">
    ...
    **A changed `key` makes React throw the old instance away and mount a fresh one**, with state initialized straight from the new `record`. No effect, no stale flash. You don't *do* the reset on a prop change; it falls out of the component being a different instance.
  </CodeVariant>
</CodeVariants>

The lever here is <Term definition="React identifies a component instance by its position in the tree plus its key. Change the key and React treats it as a different instance — it unmounts the old one and mounts a fresh one, so all its state resets.">component identity</Term>. Give a different `key` and React discards the old instance, state and all, and mounts a new one. You don't synchronize state back to the prop; you replace the component whose state it was. This is the same mechanism from ["Remounting with key"](/023-the-render-model/5-remounting-with-key/), reused for reset-on-prop-change.
...
:::caution
Reach for `key` first. The `setState`-during-render reset is only for when you can't remount the whole subtree, because you need to keep most of the state and reset one slice of it. React supports it deliberately, but don't make it a habit: if you're resetting *everything*, you wanted `key`.
:::
...
<CodeVariants>
  ...
  <CodeVariant label="Handler (the fix)">
    ...
    **The toast lives where the save happens.** It fires exactly once, on exactly the path the user took, the moment the save resolves. No `isSaved` state, no effect watching it, no phantom toast on remount.
  </CodeVariant>
</CodeVariants>
...
<CodeVariants>
  ...
  <CodeVariant label="Notify in handler (the fix)">
    ...
    **One handler updates the local state and notifies the parent in the same step:** same value, same moment, no lag, no loop.
  </CodeVariant>
</CodeVariants>

If the parent is the one that really owns this value, because both the child and a sibling need it, don't notify at all. <Term definition="Move shared state to the closest common parent of the components that need it, so they all read one source of truth instead of copying it between each other.">Lift the state up</Term> to the parent and pass it down as a prop: with one copy, there is nothing to synchronize. That's the home concept from ["The four homes for state"](/024-hooks-for-holding-state/3-the-four-homes-for-state/), and the skill is reading "I'm notifying the parent through an effect" as a sign the state lives in the wrong place.
...
To make the reflex muscle memory, start with a fast sort. Each scenario belongs in exactly one bucket.
...
Now the real thing: a small two-file pull request from a teammate. It compiles and works in a quick click-through, and it carries the catalog's smells. Review it as you would for real, running the two questions on every effect you find.
...
<VideoCallout videoId="-yIsQPp31L0" videoTitle="All 12 useState & useEffect Mistakes Junior React Developers Still Make in 2026">
  ByteGrad's 46-minute walkthrough works every smell in this lesson through code — derived state, server vs client components, stale closures, fetching in an effect — landing on the same Server Component verdict.
</VideoCallout>
