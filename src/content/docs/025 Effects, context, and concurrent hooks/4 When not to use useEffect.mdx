...
1. **Is this value <Term definition="A value computed from existing props or state, recomputed on every render rather than stored. Lives in the render body, not in useState.">derived</Term> from props or state you already have?** Compute it in render. As you saw in ["Derive, don't mirror"](/024-hooks-for-holding-state/2-derive-in-render-do-not-mirror-into-state/), a value you can calculate from what you hold doesn't get its own `useState`.
