...
The first category is **code that's supposed to be pure**: your component function body; the initializers you hand to `useState`, `useReducer`, and `useMemo`, including the `useState(() => …)` lazy initializer from [The useState surface](/024-hooks-for-holding-state/1-the-usestate-surface-and-lazy-initialization/); and the updater functions you pass to a setter, like `setCount((c) => c + 1)`. A truly pure function returns the same result the second time and changes nothing outside itself, so the second run leaves no trace. You only notice the doubling when the function secretly mutates the outside world: now that change happens twice, and the bug surfaces. The second run doesn't break pure code; it exposes code that was never pure.

The second category is **effects**, and here Strict Mode checks cleanup rather than purity. On the first mount in dev, React runs the full lifecycle instead of setup alone: setup, cleanup, then setup again. React 19 gives <Term definition="A function passed to ref={} that React calls with the DOM node on mount and null on unmount.">callback refs</Term> the same treatment, so the function-form `ref` from [useRef](/024-hooks-for-holding-state/5-useref-for-non-rendering-values/) gets an extra call-and-cleanup cycle too. Anything that sets something up is run through a teardown to prove the teardown works.
