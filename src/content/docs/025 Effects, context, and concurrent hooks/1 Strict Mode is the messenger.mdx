...
The second category is **effects**, and here Strict Mode checks cleanup rather than purity. On the first mount in dev, React runs the full lifecycle instead of setup alone: setup, cleanup, then setup again. React 19 gives <Term definition="A function passed to ref={} that React calls with the DOM node on mount and null on unmount.">callback refs</Term> the same treatment, so the function-form `ref` from [useRef as the non-rendering escape hatch](/024-hooks-for-holding-state/5-useref-as-the-non-rendering-escape-hatch/) gets an extra call-and-cleanup cycle too. Anything that sets something up is run through a teardown to prove the teardown works.
