...
<CodeVariants>
  ...
  <CodeVariant label="With deps [roomId] — useEffect(fn, [roomId])">
    ...
    **Re-syncs when a listed value changes.** The setup re-runs whenever a listed value changes, compared with `Object.is`. This is the "reconnect when the room changes" shape, and the one you'll reach for most. You list the values the effect reads, and React re-syncs when any of them changes.
  </CodeVariant>
</CodeVariants>
...
What goes wrong if the array doesn't match what the setup reads? You get a <Term definition="A function capturing values from an earlier render, used after they've changed.">stale closure</Term>. The setup captured the value from the render where it last ran; if that value changes but the effect doesn't re-run, because you left it out of the array, the effect keeps using the old captured value forever. You met closures capturing by reference earlier in the course, and this is that exact trap, now inside an effect. The code doesn't error. It just quietly uses yesterday's data.

:::caution
An effect whose dependency array doesn't match what it reads won't throw an error; it silently keeps using values from the render where it last ran. Stale data with no stack trace is among the hardest bugs to chase. The lint warning is trying to prevent exactly this, so listen to it.
:::
...
:::note
If you remember one thing, make it this pair of questions to ask whenever you read or write an effect: *what external system does this synchronize with?* and *what does the cleanup tear down?* An effect that can't answer the first is probably the wrong tool. An effect that can't answer the second is probably leaking.
:::
