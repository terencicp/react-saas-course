...
Context comes with a catch, which the title of this lesson names. The naive version, where you bundle everything into one provider and read it anywhere, ships a performance bug that stays invisible until the app is large and the interface feels sluggish. This lesson covers both sides. It gives you context as the tool for propagating cross-cutting values, and it teaches the re-render cost that makes a careless context a liability. By the end you'll be able to decide when context is the right tool, write one that fails loudly when misused, explain why a bundled context re-renders the whole tree, and apply the three disciplines that keep it fast.

## What context is actually for

Begin with the mental model that keeps you out of trouble: **context is propagation, not a store.** A <Term definition="A React mechanism for passing a value down to every component in a subtree without threading it through props.">context</Term> doesn't manage state, slice it, or do anything clever with it. It takes one value you hand it and broadcasts that value to every descendant that asks. A <Term definition="The component that sets a context's value for everything rendered beneath it.">provider</Term> sits high in the tree, and any component below it can read the value directly, skipping every layer in between. That's the entire job. It removes the drilling, but it does not remove the need to manage the value yourself.

Keep that "propagation, not a store" idea in mind, because the rest of the lesson follows from it: the pitfall and all three fixes. A store would let you subscribe to one slice of a value. Context can't, because it broadcasts the whole thing. We'll see what that costs in a moment.
...
That first item under "not for" is the misuse you'll be most tempted by, so it's worth dwelling on: moving state into context is *not* the cure for prop-drilling that's only a few layers deep. Context earns its keep for genuinely cross-cutting concerns. Drilling a prop two components down is cheaper than it feels, and a context you reached for too early is more expensive than it looks.
...
Read the type carefully, because it tells the truth about how context behaves. The value is `User | null`, and the default you pass is `null`. Why allow `null` at all, when in a running app there's always a logged-in user? Because a component that calls `useContext` *outside* any provider doesn't get an error; it silently gets the default. The type has to allow for a reader rendered with no provider above it, and the honest default for "no user available" is `null`. In a moment you'll turn that loose end into a feature.
...
`}>
  <AnnotatedStep meta="{4}" color="blue">
    The context is declared once, at module scope. The default is `null` and the type is `User | null` because a reader rendered outside any provider falls back to this default, so the type has to allow for that case.
  </AnnotatedStep>
...
Each context gets its own fail-fast consumer hook, exactly as in the previous section: `useNotifications()` for the state and `useNotificationsDispatch()` for the sender. A missing provider then throws a clear error at the call site instead of a silent `null`. This pairing, a reducer shared via context and split into state and dispatch contexts, is a named, canonical React pattern. The official docs call it "Scaling Up with Reducer and Context," and it's worth recognizing by name when you see it in a codebase.
...
Here is the part that governs how you'll actually write this. **You don't write that `useMemo` in 2026.** The project ships with the React Compiler on, and on this stack manual `useMemo` and `useCallback` aren't your default reach, because the compiler auto-memoizes for you. When the provider component is pure, the compiler stabilizes the value object automatically, so the plain literal and the `useMemo` version compile to the *same* behavior. The ceremony disappears. The senior posture, then, is to write the provider plainly and let the compiler keep the value stable. Reach for a manual `useMemo` only as a *fallback*, when React DevTools shows the compiler skipped this component because of an impure body, an opt-out, or a shape it couldn't infer. The `useMemo` here is a *teaching* shape that makes the identity mechanism visible, along with the documented escape hatch, not code you sprinkle by hand.
...
Step back and the three mitigations collapse into one idea. Split the context so unrelated things don't share the same wire, split state from dispatch so read and write aren't bundled, and keep the value reference stable so unchanged contents don't hand out a fresh object. These are three angles on a single rule: **a context re-renders its consumers exactly when its value reference changes.** Each discipline is a way of making that reference change *only* when something a consumer actually cares about has changed. Once you hold the rule, the three fixes follow from it rather than being a list to memorize.

## Where context meets the server boundary

One constraint to know before you wire context into a real Next.js app. **React Context is a client-runtime mechanism, and Server Components can't call `useContext`**, because they have no hooks. A Server Component can *render* a provider, but it cannot *consume* one.
...
One last check. The mistake to watch for is reaching for context when the shape of the value has quietly outgrown it.
