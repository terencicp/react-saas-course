...
Three drills and one exercise. Each asks you to *derive* the verdict from the slot model, not recall a rule.
...
Next, the exception: which of these calls may sit inside an `if`?
...
Now sort every call site into the bucket that matches its verdict.
