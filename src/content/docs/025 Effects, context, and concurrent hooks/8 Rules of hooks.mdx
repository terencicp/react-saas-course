...
<CodeVariants>
  ...
  <CodeVariant label="Hooks above every return">
    ...
    **Hooks first, returns second.** Every hook moves above every early return. Now the loading render and the loaded render reach the exact same hooks in the exact same order, and the spinner short-circuits the JSX only *after* every hook has run. That ordering holds with no exceptions.
  </CodeVariant>
</CodeVariants>
...
Every regular hook needs call-order stability because it is tracked by *positional slot*, which is the whole story above. `use()` is not tracked that way at all. When you write `use(promise)`, React identifies that value by its <Term definition="Which exact object a value is in memory, the same notion Object.is compares, the one you met when reasoning about why two arrays with identical contents aren't equal.">referential identity</Term>, not by which numbered call it was. When you write `use(context)`, React resolves it by the component's *position in the tree*, walking up to find the nearest provider. Neither path assigns a slot.
...
<CodeVariants>
  ...
  <CodeVariant label="Conditional use() — legal">
    ...
    **No slot to lose.** `use(ThemeContext)` claims no slot, because context is resolved by tree position, not call order. Nothing can misalign whether this render reaches the call or not. Legal, and not as a special favor: there is simply no slot here to lose.
  </CodeVariant>
</CodeVariants>
...
A linter watches for these violations. It runs every time you save and again in <Term definition="Continuous integration, the automated checks that run on every push, before code can merge.">CI</Term>, and the part that matters day to day is that it is already in your project. `eslint-plugin-react-hooks` ships in the default Next.js ESLint config. You inherited it when you scaffolded the app: you did not wire it up, and you won't here. <Term definition="The linter that flags problematic code patterns statically, before the code ever runs.">ESLint</Term> does the watching so you don't have to.
