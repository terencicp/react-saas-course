...
A verified event can still arrive twice, and processing it twice is its own expensive bug. The next lesson, [Claim once, mutate once](/063-webhook-ingestion/2-claim-once-mutate-once), picks up where the success path stubs out.
