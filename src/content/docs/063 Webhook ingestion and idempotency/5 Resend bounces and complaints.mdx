...
The handler writes only `'hard_bounce'` and `'complaint'`. The enum's other values, `'soft_bounce_threshold'`, `'manual_unsubscribe'`, and `'invalid_address'`, live in the same schema but are written by other flows.
