...
<DiagramSequence>
  <DiagramStep>
    ...
    <Aside type="caution" title="How the cheap tier breaks">
      The function is killed mid-write the instant it hits the cap. You get a truncated file, no error the user can act on, and no way to resume where it stopped. This is the most observable condition: there's a hard number, and you either clear it or you don't.
    </Aside>
  </DiagramStep>
  ...
  <DiagramStep>
    ...
    <Aside type="caution" title="How the cheap tier breaks">
      `after()` fires once and is gone. Vercel Cron logs the 5xx and waits for the next tick. Neither one retries on its own. And a hand-rolled retry loop inside a request just burns the user's time budget, since they're now waiting through your backoff.
    </Aside>
  </DiagramStep>
  ...
  <DiagramStep>
    ...
    <Aside type="caution" title="How the cheap tier breaks">
      One cron tick is one invocation. Emailing 50,000 users one by one inside it times out, which is condition 1 all over again. Firing all 50,000 at once, unbounded, melts the downstream service. You need something that can hold tens of thousands of pending units of work and meter them out.
    </Aside>
  </DiagramStep>
  ...
  <DiagramStep>
    ...
    <Aside type="caution" title="How the cheap tier breaks">
      There is no way to pause a function invocation for six hours. You'd be paying for it the whole time, and it would die at the cap anyway. The fallback is polling: wake up every few minutes and check. Polling burns compute on every wake and can still miss the moment the work completes.
    </Aside>
  </DiagramStep>
</DiagramSequence>
...
<VideoCallout videoId="vZRiSPNAAM0" videoTitle="Background jobs with Trigger.dev — the missing piece for your Next.js project">
  A 10-minute Trigger.dev tour by Elie Steinbock covering long-running tasks past the function time wall, cron, durable waits, retries, and the run dashboard: all the capabilities this lesson maps onto the five conditions (filmed on v3, but the concepts carry straight over to v4).
</VideoCallout>
...
- **<Term definition={"A run that survives worker crashes, redeploys, and platform restarts by checkpointing between steps."}>Durable runs</Term>** that survive worker crashes and redeploys answer conditions 1 and 2 (past the time wall, multi-step with state). The run checkpoints between steps and resumes on a new worker.
- **Declared retries** with <Term definition={"Retry delays that grow geometrically, with jitter (randomization), so retries spread out instead of all stampeding at once."}>exponential backoff</Term> and jitter answer condition 3. You configure the policy; the platform runs it.
...
- **<Term definition={"A durable, resumable pause token: the run parks, the worker is freed, and an external signal resumes it."}>Waitpoints</Term>**, with `wait.for` and `wait.until` for durable pauses, answer condition 5. The run parks and the worker goes free.
