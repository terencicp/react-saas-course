...
The compiler ends that. As you saw in the last lesson, it auto-memoizes the five cases that mattered: derived values, object and array literals passed as props, callbacks defined inside components, the `value` you hand to a context provider, and JSX subtrees. That covers the overwhelming majority of what the discipline was for, so the role of manual memoization changes entirely. It becomes an <Term definition="A deliberate, documented departure from the default — pulled for a specific, named reason, never as a habit.">escape hatch</Term>: a deliberate opt-out you reach for when you need a *specific* guarantee the compiler cannot give you.
...
<VideoCallout videoId="14MZJtGAiVs" videoTitle="The Problem With the React Compiler — Cosden Solutions">
  Cosden Solutions (27 min) profiles a real app with the compiler on, finds components still re-rendering, and lands on the same point this lesson makes: the compiler is not a reason to stop understanding memoization.
</VideoCallout>
...
<CodeVariants>
  ...
  <CodeVariant label="useCallback">
    ...
    Sugar for a `useMemo` that returns a function: `useCallback(fn, deps)` is `useMemo(() => fn, deps)`. Same `Object.is` dependency check, and the compiler handles it by default just as it does `useMemo`, leaving the same residual job: hand a stable function reference to an effect or a library that reads it by reference.
  </CodeVariant>
...
:::danger
`const data = useMemo(() => fetch(url).then((r) => r.json()), []);` looks like it caches a request. It does not. It is a closure trick that breaks the moment you hit Suspense or concurrent rendering. React can discard and re-run a memo, so the request fires again. The promise isn't part of React's data flow at all, and the "cached" value doesn't survive a remount or get shared with any other component. You get something that *appears* to cache until the day it silently doesn't.
