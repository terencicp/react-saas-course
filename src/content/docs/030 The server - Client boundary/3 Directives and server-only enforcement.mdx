...
:::caution
A mistyped directive is a hard bug to catch: it's invisible at build time, easy to miss in code review, and triggered only when the wrong render path runs. No tool warns you, and the cost of a single transposed letter is a component that ships as the wrong kind and fails in front of a user.
:::
...
The dangerous word there is *transitive*. A <Term definition={"A module you don't import directly, but pull in through a chain — A imports B, B imports C, so A transitively depends on C.\nThe reason a leak can hide several hops below the file you actually edited."}>transitive import</Term> is one you never wrote down yourself: your file imports a helper, the helper imports another, and four files later something pulls in the database client. As far as the bundler is concerned, all of it counts as imported by your file. That's why these leaks hide. The damage isn't in the import you can see; it's at the bottom of a chain you'd have to follow by hand.
...
<CodeVariants>
  ...
  <CodeVariant label="Endpoint ('use server')">
    ...
    **The opposite intent.** `archiveInvoice` must be reachable from the client, since a button in a Client Component calls it to archive an invoice. The directive exposes it as an endpoint the browser can invoke. One file closes a door, the other opens a window.
  </CodeVariant>
</CodeVariants>
