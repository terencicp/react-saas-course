...
The rest of the chapter rests on one fact: **every component under `app/` is a <Term definition="React Server Component — a component that runs only on the server during the request and ships no JavaScript to the browser. The default for every file under app/.">React Server Component</Term> by default.** There's no directive at the top of the file, no special import, no opt-in. The `page.tsx` files you wrote in the previous chapter were Server Components, and nobody had to tell them to be.
...
`}>
  <AnnotatedStep meta={`"async"`} color="blue">
    `async` makes this a component you can `await` inside, which is only possible because it runs once, on the server, not in a live browser.
  </AnnotatedStep>
