...
The serialization rules ahead feel arbitrary until you can see what they describe, so start with the thing itself: the <Term definition={"React Server Components payload.\nThe serialized React tree the server sends to the browser alongside the HTML, used to reconcile what React renders."}>RSC payload</Term>.
