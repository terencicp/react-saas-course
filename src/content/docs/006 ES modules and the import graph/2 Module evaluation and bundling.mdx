...
Five snippets, one per core idea. Predict each outcome before reading the explanation.
...
<MultipleChoice>
  ...
  <McqWhy>`a.ts` starts evaluating and imports `b.ts`, which imports back into a still-evaluating `a.ts` where `fromA` is not assigned yet. `fromA` reads as `undefined`, and the arithmetic produces `NaN`. The build passes; this breaks only at runtime.</McqWhy>
</MultipleChoice>
