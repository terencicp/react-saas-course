...
The request hits your transfer action carrying no session, so `requireUser`, the validating read you built in [Reading the session everywhere with one call shape](/052-better-auth-setup/4-reading-the-session-everywhere-with-one-call-shape/), looks up the session, finds nothing, and returns 401.
