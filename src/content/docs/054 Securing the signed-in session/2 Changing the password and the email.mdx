...
`/settings/security` is a **protected route**: the proxy from the previous lesson bounces signed-out users away from anything under `/settings`, and the layout's `requireUser()` from [Reading the session everywhere](/052-better-auth-setup/4-reading-the-session-everywhere/) is the validating read that confirms a real session sits behind the cookie.
...
The forms are the same shape you've written since [useActionState](/044-forms-with-platform-primitives/3-useactionstate---pending-state-and-the-result/) — uncontrolled inputs with `defaultValue`, a `<SubmitButton>` reading pending from `useFormStatus` — so we cover only the actions behind them.
...
The token is the same `verification` row from [Email verification](/053-authentication-flows/3-verifying-email/), one-time and hashed at rest, under a different `identifier` namespace: `change-email:<userId>:<newEmail>`. Only the namespace and entry point differ, so there's nothing new to learn here.
