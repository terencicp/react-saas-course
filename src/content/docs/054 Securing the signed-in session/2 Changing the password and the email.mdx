...
The forms are the same shape you've written since [useActionState — pending state and the result](/044-forms-with-platform-primitives/3-useactionstate---pending-state-and-the-result/) — uncontrolled inputs with `defaultValue`, a `<SubmitButton>` reading pending from `useFormStatus` — so we cover only the actions behind them.
