...
This lesson builds the three-audience model as a durable scaffold, then walks the header families a SaaS engineer actually touches: content negotiation, caching, conditional requests, authorization, rate-limit signaling, the security baseline, request context, and custom headers. It closes with a concrete map from each header family to the Next.js file that sets it. By the end you'll place any header into one of three audiences, pick the senior default for the common cases, and know where in a Next.js app a given header needs to live.

## Headers have three audiences

The previous two lessons named the shape of an HTTP message in passing: a method or status line, a block of headers, and a body. The body carries the resource; the headers carry the metadata. This section answers the question an experienced engineer asks before writing a header value: who is going to read this?
...
The audience is the durable property of the header. Once you know who reads it, you know where in your stack to set it and what the consequence of getting it wrong is. The catalog of headers a SaaS engineer touches is wide, but the catalog is not what carries you; the audience model is. Memorize the headers and you'll lose them inside a year. Internalize the audiences and you can place new headers you meet later by reasoning.

A few headers blur the boundary, and that's fine. `Cache-Control` is mostly an infrastructure header, since CDNs and shared caches are the design audience, but the browser also caches and reads it. `Authorization` is application-audience, yet the infrastructure layer might still log it. Don't get stuck on edge cases. The mapping is one-to-one in practice for every header you'll touch, and the few boundary cases are called out where they come up.
...
Three headers follow one rule: name the format twice, once on the request side ("what I'm sending, what I want back") and once on the response side ("what you're actually getting").
...
`Content-Encoding` declares how the body was compressed before being put on the wire. The client advertises support via `Accept-Encoding`, the server picks one and stamps the response with `Content-Encoding`. The 2026 senior reach for new traffic is `zstd`: Chrome, Edge, Firefox, and Safari all support it now, Cloudflare and the other major CDNs negotiate it by default, and the bytes-per-CPU ratio is materially better than Brotli for the same fidelity. Brotli stays the cross-cutting default for older client support, and gzip is the legacy fallback. You almost never set `Content-Encoding` by hand, because the platform (Next.js plus the deploy target) negotiates compression and stamps the encoding for you. Setting it manually without actually compressing the body produces a content mismatch that downstream tooling will choke on.

`Vary` is the cache-key extender. By default, a shared cache keys responses by URL. `Vary: Accept-Encoding, Cookie` tells it that this response varies by the values of those request headers, so it should key the cache on them too. Without `Vary: Cookie` on a per-user response, the CDN serves user A's response to user B from cache. Without `Vary: Accept-Encoding`, the Brotli body gets handed to a client that only asked for gzip and can't decompress it. The 2026 reflex is that any response which varies meaningfully by request header sets `Vary`. The senior default for authenticated HTML (next section) is `private, no-store`, which makes `Vary: Cookie` moot because the response isn't cached at all, but on cacheable per-user surfaces `Vary` is mandatory.

Two more deserve a mention. `Content-Length` declares the body's size in bytes; the infrastructure layer reads it to frame the message, allocate buffers, and decide when the body has finished arriving. The platform sets it for you when it serializes the response, so it almost never appears in your code. `Accept-Language` carries the client's preferred locales, and Unit 17's `next-intl` reads it for locale negotiation.
...
`Cache-Control` is the header an infrastructure layer looks at first. Its value is a comma-separated list of directives, each one a knob on what caches are allowed to do with the response. The list is short, six or seven worth knowing, and four senior defaults will cover almost every response you ever ship.
...
- **Hashed static assets** (filename includes a content hash, like `/_next/static/chunks/main.a1b2c3.js`): `Cache-Control: public, max-age=31536000, immutable`. That's one year. New deploys ship new hashes, so the URL itself is the version.
- **Cacheable HTML on the CDN edge** (marketing pages, blog posts, anything anonymous): `Cache-Control: s-maxage=300, stale-while-revalidate=86400`. Five minutes fresh on the CDN, a day of stale-while-revalidate behind it. The user always sees a fast response, and the CDN refreshes in the background.
...
:::caution
If you ship `Cache-Control: public, max-age=...` on a response that varies by signed-in user, a shared cache (Vercel's edge, Cloudflare, a corporate proxy) can store one user's response and serve it to another. This is the worst cache header bug there is: one wrong directive leaks one user's data into another user's browser. The senior default for authenticated HTML is `private, no-store`. Reach for `public` only when you can prove the response is identical for every user, signed in or out.
:::

The two directives the trap usually involves are `no-store` and `no-cache`. They sound interchangeable, but they aren't. Here's the difference on the wire.
...
The conditional-request pair completes the caching story. It answers the question that follows from a cached response: it might be cached, but has the resource actually changed?

The pattern is small. The server stamps the response with `ETag: "v1"`, an opaque token identifying *this* version of the resource. On the next read, the client sends `If-None-Match: "v1"` in the request. If the resource hasn't changed, the server replies with `304 Not Modified` and no body, and the client uses its cached copy. The savings are the body bytes: the round-trip is still paid, but the payload isn't.
...
Two related headers earn one mention each. <Term definition="Pattern: server stamps an ETag on the response; client sends If-Match on a mutation; server replies 412 Precondition Failed if the resource has moved on. The write-side cousin of If-None-Match.">`If-Match`</Term> on a write request implements *optimistic concurrency*: "apply this mutation only if the resource is still at version v1; otherwise return `412 Precondition Failed` and let me reconcile." It's useful for collaborative-edit surfaces, but not a daily reach. `Last-Modified` and `If-Modified-Since` are the timestamp version of the same pattern. The 2026 senior default is `ETag`, because timestamps have second precision and can't disambiguate two writes inside the same second. `ETag` values are opaque to the client, so the server can hash whatever shape it likes: row version, content hash, or monotonic counter.
...
<VideoCallout videoId="Cy2ZJOBgk84" videoTitle="Deep Dive into HTTP Caching: cache-control, no-cache, no-store, max-age, ETag and etc.">
  Software Developer Diaries walks every `Cache-Control` directive and the `ETag`/`If-None-Match` revalidation flow with diagrams (21 min). It's the long-form companion to the last two sections.
</VideoCallout>
...
For programmatic clients (a mobile app, a server-to-server SDK, a public API consumer), the answer is `Authorization: Bearer <token>`. A bearer token is a raw credential carried in plaintext on every request. It has none of the browser-managed mitigations that cookies have, which is exactly why it's wrong for browser sessions and exactly *right* for clients that manage their own token lifecycle, store credentials in a vault or a keychain, and don't have a cookie store at all.
...
<CodeVariants maxLines={6}>
  ...
  <CodeVariant label="Machine API (Bearer)">
    ...
    **The SDK or HTTP client adds the header on every call.** The token lives in the client's config or vault, not in a browser cookie store. None of the cookie-attribute mitigations apply, because the token is a raw credential.
  </CodeVariant>
</CodeVariants>
...
The pair is `RateLimit` (the current state) and `RateLimit-Policy` (the policy that produced it). These are the response headers the IETF httpapi working group has been standardizing as `draft-ietf-httpapi-ratelimit-headers`, currently at draft 11 and intended for the Standards Track. They are *structured fields* per RFC 9651, lists of items with parameters parsed by a defined grammar, not the loose `X-RateLimit-*` set that the older Heroku and Twitter shape used. The 2026 senior reach is the IETF draft shape, and Upstash, Cloudflare, and most modern gateways emit it.
...
A request typically passes through several hops before hitting your application: CDN, load balancer, then application. Your application sees only the load balancer's IP as the source IP. To recover the real client IP, each proxy appends its known-good upstream IP to `X-Forwarded-For` (or to the structured `Forwarded` header standardized in RFC 7239), and the application then walks the chain.

Here's the trap: only the rightmost entry, the one appended by the immediate edge your application talks to, is trustworthy. Every entry to the left of that hop could have been forged by the original client sending a fake `X-Forwarded-For` header in the original request. If you rate-limit by the leftmost entry, you've given any attacker a free way to spoof their IP and bypass your limiter.
...
`Forwarded` (RFC 7239) is the standardized replacement for the older `X-Forwarded-*` set: one structured header (`Forwarded: for=192.0.2.60;proto=https;by=203.0.113.43`) instead of three separate `X-Forwarded-For`, `X-Forwarded-Proto`, and `X-Forwarded-Host`. The senior 2026 default is to emit `Forwarded` from your edge, but the legacy `X-Forwarded-*` set persists because most upstream tooling still reads it. Read whichever your edge actually sets, and configure the edge to set the modern one.
...
GitHub still emits `X-GitHub-Event` as a legacy holdout, named once here as the exception that proves the rule. Pick the IETF draft shape when one exists (`Idempotency-Key`, `RateLimit`), pick a vendor token for proprietary semantics, and pick no prefix for everything else.

## Where headers are set in a Next.js app

This is the closing payoff. The three audiences map cleanly onto the three places a Next.js app sets headers, and that map is what you'll look up in practice when shipping a new feature.
...
It runs per request on every request that matches the proxy's config, and the nonce is fresh each time. The snippet uses `crypto.randomUUID()` for parity with the `Idempotency-Key` reach in Lesson 1, but a CSP nonce is strictly meant to be a fresh random value encoded as base64 per the CSP spec. Chapter 81 swaps to the canonical `crypto.getRandomValues(...)` plus base64 pattern when it wires CSP in production.
...
That's the closing map: audience to setter to file. Every header in the lesson lands in one of those three slots.

## Closing recall

Here are six claims about the load-bearing reasoning in this lesson: applying the audience model, the senior defaults, and the request-context traps to specific cases. Mark each one true or false.
