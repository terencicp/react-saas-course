...
Two properties with two states each give four cells. Drop each of the five methods into the cell its properties pick out and the design space takes shape.

<Figure caption="The five-method palette placed by property. The grey cell has no occupants, because there's no use case for a read that produces different state each time. The red cell is the one that needs Idempotency-Key to become retry-safe.">
  <MethodGrid />
</Figure>

Walk the four corners in order. The top-left is GET, the only safe method in the everyday toolkit, which is why it's also the only method that caches, prefetchers, and retry layers can act on freely. `HEAD` is safe and idempotent too and lives in the same cell; it's `GET` without the response body, used to read metadata like content length or last-modified date without paying for the payload. You'll see it again later in the course when uploading to object storage. `OPTIONS` is also safe; it's what the browser sends as the CORS preflight, covered in the next chapter.
...
PUT and PATCH share a cell on the grid and a use case in the design: both update an existing resource. But they speak very different languages on the wire, and getting this pair right is the difference between a clean update endpoint and a scatter of small bugs.
...
<CodeVariants maxLines={10}>
  <CodeVariant label="merge-patch+json">
    ...
    ```
    **The default 2026 choice.** <Term definition="JSON Merge Patch (RFC 7396): the patch body is a partial JSON object; keys overwrite, null deletes. Default 2026 wire format for PATCH on object-shaped resources.">JSON Merge Patch</Term> (RFC 7396) lets you send a partial JSON object: keys overwrite, and an explicit `null` deletes a key. Its one limitation is that arrays can't be merged element by element, so to change one item in an array you have to send the whole new array. Reach for it when your patches are field-level on object-shaped resources, which describes most SaaS update endpoints.
  </CodeVariant>
  ...
  <CodeVariant label="json-patch+json">
    ...
    ```
    **The choice for operation semantics.** <Term definition="JSON Patch (RFC 6902): the patch body is an array of typed ops (add, remove, replace, move, copy, test). Reach for it when the diff needs operation semantics merge-patch can't express.">JSON Patch</Term> (RFC 6902) is an array of typed operations: `add`, `remove`, `replace`, `move`, `copy`, and `test`. It's verbose but unambiguous, it supports array mutation, and the `test` op lets you embed optimistic-concurrency checks in the patch itself. Reach for it when the diff has to be precise about ordering, array changes, or conditional application.
  </CodeVariant>
</CodeVariants>
...
The pattern works like this. The client generates a stable identifier *per logical operation*, one ID per "the user wants to pay this invoice" rather than one per HTTP attempt, and sends it as an <Term definition="Request header whose value is a stable per-operation identifier (typically a UUID). The server stores (key, response) and returns the cached response on replay, making non-idempotent methods retry-safe.">`Idempotency-Key`</Term> header. The server stores `(key, response)` before sending the response back. On a retry with the same key, the server *recognises* the replay, returns the cached response, and runs no business logic at all. Same logical operation in, same response out, exactly one side effect.

Walk through it as a sequence. Each step shows the same client-server pair as a two-column mini diagram inside one slide of the `<DiagramSequence>`, and the slider on the figure advances through the three slides.
...
:::caution
The implementation has to avoid three failure modes. **One**: generating a new key on each network attempt. That defeats the entire purpose, because every retry then looks like a new operation to the server. **Two**: reusing the same key across different operations. Each logical operation needs its own key; sharing keys means a retry of operation B replays the response of operation A. **Three**: writing the response to the store *after* sending it back. A server crash between "respond" and "persist" leaves no record of the work, so the retry re-runs it. Persist first, respond second. The route-handler and database side of all three lands in the Stripe webhook chapter; for now, hold the contract in mind.
:::
...
One file, one route, three handlers. The framework dispatches `GET /invoices/42` to the `GET` function, `PATCH /invoices/42` to `PATCH`, and so on. A `POST` arriving at this route, when no `POST` is exported, gets an automatic `405 Method Not Allowed`. The verb is the contract, the handler implements it, and the name on the exported function is where those two meet. Full coverage of route handlers, including the validation pipeline and the `authedRoute` wrapper that runs the auth and permission checks, comes later in the course.

One carve-out is worth a paragraph before the recall. The HTTP spec doesn't *forbid* a GET request from carrying a body; older drafts allowed it and the current spec is silent rather than prohibitive. But intermediaries such as proxies, CDNs, and some HTTP libraries silently strip GET bodies, and the request semantics ignore them anyway. In practice, a GET body won't survive the journey. If your read needs structure, such as filters, sort columns, or pagination, encode it in the query string, which the next chapter covers as the URL surface, including `URLSearchParams`.

:::tip
If you're tempted by "GET with a body," the real fix is usually POST with a clear name. An endpoint like `POST /searches` that takes the query parameters in the body and returns the result rows is a perfectly reasonable shape. It just isn't a *read* in the HTTP sense, and using POST signals that honestly. Don't try to dress an unsafe operation as a safe one.
:::

## A round on the corners

Here are four claims to apply the property model against. Each one tests whether you're reasoning about state and contract rather than pattern-matching method names.
