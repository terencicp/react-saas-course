...
One pillar is left: the `<Pagination cursor={cursor} hasNext={nextCursor != null} />` that [The list-view anatomy](/060-url-state-list-views/1-the-list-view-anatomy) left commented out in `page.tsx`.
...
Those links use the setter, which uses `replace`, the same in-list-navigation policy as every filter and sort control from [Filter shapes and sort encoding](/060-url-state-list-views/2-filter-shapes-and-sort-encoding).
