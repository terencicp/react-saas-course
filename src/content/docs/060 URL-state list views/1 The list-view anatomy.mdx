...
Picture the invoices screen of any SaaS app you've used. Someone filters it to **overdue**, sorts by amount so the biggest debts float to the top, types a customer's name into the search box, and clicks through to the second page of results. Then they copy the address bar, drop it in Slack, and write "can you chase these?" Their teammate clicks the link and lands on *the exact same view* — same filter, same sort, same search, same page. Nobody re-derives anything.

That last sentence is the whole job. A real list screen isn't one feature, it's four working together — filter, sort, search, paginate — and the bar it has to clear is that the screen's entire state lives in a link you can share. You already have the raw materials for this: in [URL state with searchParams and route params](/033-the-request-surface/4-url-state-with-searchparams-and-route-params) you learned to read one URL parameter in a Server Component and validate it, and in [Client-side navigation hooks](/033-the-request-surface/5-client-side-navigation-hooks) you learned to write one back from the client. Reading one and writing one is the primitive. This lesson is the composition.

By the end you'll hold a single mental model for the whole pattern — what belongs in the URL and what doesn't, which side of the server/client line reads it and which side writes it, and what the shared link actually guarantees. And you'll have stamped the production setup the rest of this chapter builds on: one shared parser module, one server-side cache, typed setters on the client. No new mechanics — just the architecture that turns four moving parts into one coherent screen. No code yet; the model comes first.

## The four pillars of a list view

Every list screen worth building leans on the same four pillars. It's tempting to treat them as four separate features you bolt on one at a time, but that framing is exactly what trips people up. They're not four decisions. They're one shape — one description of *what the user is currently looking at* — that happens to have four parts:
...
Read those four together and you get a complete sentence: *the overdue invoices mentioning Acme, largest first, page two.* That sentence is the state of the screen. And because it's one sentence, it wants to live in one place — the URL — where each pillar owns a small, readable fragment of the address.
...
Here's the reflex you want to build, because it's the one beginners get wrong in both directions — some reach for `useState` for everything and lose the shareable link, others stuff every twitch of the UI into the address and produce URLs nobody can read.
...
That third item in the component-state pile is subtler than it looks, and it's worth flagging now so it doesn't surprise you later: there's a split between the text a user is *typing* and the term they've *committed*. The committed term is URL state — it's what the query ran with. The in-progress keystrokes are not; you don't want a new history entry for every letter. Holding both correctly is its own small craft, and a later lesson in this chapter is dedicated to it. For now, just file it: typed is local, committed is in the URL.

A few things never belong in the URL no matter what the refresh question says, because the URL is public, durable, and human-readable by design. Don't put secrets in it — a shared link leaks them. Don't put large blobs in it — there's a length budget and you'll blow it. And don't put anything in it the user couldn't reasonably understand by reading it. An opaque encoded blob in the address is a smell, with exactly one sanctioned exception you'll meet later: a pagination cursor, which has to be opaque because it encodes a database position.

That's the rule stated. Stating a rule and *applying* it are different skills, though, and this one rewards practice. Walk the following decision tree a few times with different pieces of list-view state in mind — it forces you through the same questions a experienced engineer asks reflexively.
...
4. **Back button** — going back returns to the previous filter / sort / page combination, one step at a time.

This is the acceptance test for any URL-state list view. Not "does the filter work" — *does the URL hold the truth.* And there's a single litmus test that catches every violation of it:

:::tip
**If a click changes the result but not the URL, the contract is broken.** A filter that updates the table without updating the address is a bug, even when it looks like it works — refresh it and the user is back to square one.
:::

Keep that sentence. It's the most portable thing in this lesson; you'll reach for it every time you review a list screen.

One honest caveat, because the contract is precise about *what* it guarantees. The URL pins the **view parameters**, not a frozen photograph of the rows. If a teammate marks an invoice paid in the minute between you sharing the link and them opening it, they'll see the current data under the same filter — overdue, sorted by amount, page two of *whatever now matches*. That's almost always what you want; a shared link is a saved *question*, not a saved *answer*. (Cursor pagination adds a wrinkle here — a cursor points at a position, not a snapshot — but that's the pagination lesson's concern, not this one's.)
...
<VideoCallout videoId="qpczQVJMG1Y" videoTitle="Type-safe URL state in Next.js with nuqs">
  François Best, nuqs's creator, makes the whole case for URL-as-state at Next.js Conf 2025 — shareability, navigation, and the server/client split — in 25 minutes.
</VideoCallout>

## Read on the server, write on the client

So the state lives in the URL. The next question is the architectural one: who reads it, who writes it, and what happens in between? The answer is a clean split with no middle layer — and the "no middle layer" part is what makes it feel different from the single-page-app habits you might be carrying.
...
That's the entire loop. Change a control → write the URL → server re-renders → new table. The data round-trips through the URL and the server, never through a client-side fetch.

Two habits from the single-page-app world will actively fight you here, and naming them is half the battle:

- **No `useEffect` syncing state to the URL.** You will not hold the filter in `useState` and then write an effect to push it into the address. The URL *is* the state. The control reads from it and writes to it directly — there's no second copy to keep in sync.
- **No client-side data fetch.** The table is not fetched in the browser. The server already has the parsed parameters; it queries and renders. There is no `useEffect(() => fetch(...))`, and therefore no request waterfall where the page loads, *then* the data loads, *then* it pops in.
...
>
  ...
  <Phase id="hydrate">
    ...
    shipped **zero** client JavaScript — its work was finished on the server.
  </Phase>
</RequestTrace>

The two things that trace kills: the idea that a `"use client"` component skips the server (it doesn't — it renders on the server first, then hydrates), and the idea that the data is fetched on the client (it isn't — `InvoiceList` reads the parsed params and queries on the server, shipping no JS at all).

Now the write half. When a control changes the URL, it has a choice you met in [Client-side navigation hooks](/033-the-request-surface/5-client-side-navigation-hooks): `push` or `replace`. For list state, the default is **`replace`, with `{ scroll: false }`** — and the reasoning is pure user experience. Every filter tweak, sort flip, and page step is the *same screen reconfigured*, not a new destination. If each one pushed a history entry, a user who clicked five filters would have to hit back five times just to leave the page. `replace` swaps the current entry in place, so the back button still does the useful thing — it leaves the list — instead of rewinding chips one at a time. The `{ scroll: false }` keeps a long list from jumping to the top every time a control changes.

`push` isn't wrong; it's reserved. Clicking a *row* to open its detail page is genuine navigation — a new destination you'd want back to return from. That gets `push`. The policy is simple: reconfiguring the current view replaces, navigating to a new view pushes.
...
You can already write all of this by hand. Reading a parameter on the server, validating it, writing it back from the client with `router.replace` — those are the primitives from the request-surface chapter, and for *one* parameter they're completely fine. The trouble is that a list view doesn't have one parameter. It has four, and the hand-rolled approach degrades in a specific, visible way as they pile up.
...
**Defaults that strip themselves.** You attach a default with `.withDefault(...)`, and here's the payoff: when a parameter equals its default, `nuqs` *removes it from the URL entirely*. No by-hand `delete` dance. The empty URL `/invoices` becomes the home view, and the address only ever shows what differs from the baseline. The experienced engineer's reflex: pick defaults that match the most common view, so the cleanest URL is also the most useful one.
...
Let's set it up. Three concrete moves.
...
`}>
  ...
  <AnnotatedStep meta="{12}" color="green">
    `parseAsStringEnum(STATUS_VALUES)` constrains the parameter to exactly those literals. `.withDefault(null)` makes "no filter" the baseline — a hand-typed `?status=DROP TABLE` falls back to `null` instead of reaching the query.
  </AnnotatedStep>
  ...
  <AnnotatedStep meta="{14-15}" color="blue">
    The search term is a plain string defaulting to empty; the cursor is a plain string with no default (absent means "first page"). Both are simple because neither is constrained to a fixed set.
  </AnnotatedStep>
...
One `await searchParamsCache.parse(...)`, one typed object, and from there it's ordinary code: pass the slices to the query function, pass the current values down to the controls. The query itself is tenant-scoped — it always filters to the current organization — but that's the auth machinery from earlier, not something the URL touches.
...
<CodeVariants maxLines={0}>
  <CodeVariant label="searchParams.ts">
    ...
    ```
    **The single source of truth.** Both the page below and the control beside it read from these exact parsers — the shape is declared once.
  </CodeVariant>
...
What you've stamped is a scaffold, and the rest of the chapter fills it in:

- The next lesson builds the `<SortControl />` and the real filter shapes — single-select, multi-select, ranges — plus the invariant that changing a filter or the sort resets pagination.
