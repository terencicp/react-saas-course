...
The takeaway: a <Term definition="A setTimeout / setInterval callback, run by the timer queue. Advancing fake timers fires these.">macrotask</Term> firing and the <Term definition="A promise continuation — a .then callback or the code after an await. Drained to empty before the next macrotask runs.">microtask</Term> it schedules are one tick apart, and the synchronous timer API stops after the macrotask. The async variant keeps going until the <Term definition="The scheduler that alternates between draining all pending microtasks and running the next macrotask.">event loop</Term> has drained the microtasks too. That's the same micro/macrotask split you walked through back in the event-loop lesson, and it's why the rule is simple: any fake-timer test on a code path that `await`s should use the `*Async` variant.
