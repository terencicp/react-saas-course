...
<VideoCallout videoId="FsB_nRGdeLs" videoTitle="API Security Explained: Rate Limiting, CORS, SQL Injection, CSRF, XSS & More">
  Hayk Simonyan's 9-minute tour opens on exactly this decision — limiting per-endpoint, per-user, and per-IP, and why a botnet defeats per-IP alone — then widens to the rest of the abusable-API surface this chapter guards.
</VideoCallout>
