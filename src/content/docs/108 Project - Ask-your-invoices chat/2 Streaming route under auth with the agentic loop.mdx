---
title: Streaming chat route under auth
chapter-id: 108
...
The slice is deliberately narrow: a streaming answer with no tools yet, gated by auth, with the agentic loop's cost ceilings bolted on before there's anything expensive to run.
Tools, per-user budgets, and the real UI come in lessons 3 through 5, but all of it depends on this capped, authenticated route existing first.
Since the real chat UI is the last lesson, you'll stand up a throwaway box here that renders whatever streams back as plain text.
...
You're wiring `POST /api/chat` to stream an LLM answer, and the order is the point: **cap and wrap first, then add capability**.
...
The route is wrapped in `authedRoute('member', …)`, the `Request`/`Response` sibling of `authedAction` from [The authedRoute twin](/057-roles-authorization-audit-logs/3-the-authedroute-twin/).
...
Two seams from the Vercel AI SDK v5 appear once here, both owned by [streamText, generateText, and the route-handler seam](/106-text-objects-and-chat-uis/1-streamtext-generatetext-and-the-route-handler-seam/).
...
Every finished conversation writes one row, the same one-row-per-event discipline from [The append-only audit log](/057-roles-authorization-audit-logs/5-the-append-only-audit-log/).
...
`src/lib/llm/audit.ts` ships with both functions stubbed to no-ops and the argument types already in place. Fill in the two pushes:
...
Each writer is a single append-only `pushLlmAuditEvent` call, the in-memory stand-in for one `INSERT` into `llm_audit_events`.
The `event` discriminant (`'llm.step'` vs `'llm.finish'`) and a jsonb-shaped `payload` are the whole row.
This is the same one-row-per-event discipline from [The append-only audit log](/057-roles-authorization-audit-logs/5-the-append-only-audit-log/), pointed at a separate table: keeping the LLM events log apart from `auditLogs` keeps the lifecycle audit (who archived which invoice) from tangling with the LLM audit (what the model did per step).
...
`src/app/api/chat/route.ts` ships as a 501 stub. Replace it with the wrapped, capped streaming handler:
...
:::caution
The `route.ts` committed in the repo is the *finished* version, carrying more than this slice: a `tools` argument on `streamText`, an `onStepFinish` callback, and a `withLlmQuota` wrapper around the export. Those belong to the next two lessons. Build only what's shown above for now — copy the committed file and your chat fails to typecheck against this lesson's empty tool stub.
:::
...
This is scaffolding: the last lesson of the chapter throws it away for the typed client that renders tool-part cards across their lifecycle states.
So it does the bare minimum — send a message, render the text parts that come back.
...
Why messages are arrays of parts rather than one string is owned by [useChat, useObject, and the parts array](/106-text-objects-and-chat-uis/3-usechat-useobject-and-the-parts-array/).
...
This project ships no per-lesson test suite — `lesson-verification/` is a harness slot, so the project itself is the assessment.
...
A clean run confirms the *shape*: the slice typechecks and the production build succeeds.
