...
<VideoCallout videoId="x4hQ2Hmuy3k" videoTitle="Cosden Solutions — Better Auth in Next.js (Complete Tutorial)">
  Cosden Solutions builds the same spine you are wiring here — the `betterAuth` server instance, the `[...all]` handler, and the `nextCookies()` plugin (35 min); skip the UI section at the end.
</VideoCallout>
