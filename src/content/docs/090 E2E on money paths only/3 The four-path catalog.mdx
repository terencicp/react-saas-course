...
3. Invalid credentials surface an error alert and leave you on `/sign-in`. This is the `'invalid-credentials'` branch, seen from the browser.
4. The dual-key rate limiter blocks the sixth bad attempt and shows the lockout copy. This is the `'too-many-attempts'` branch.
...
:::note
The seed handles this. `pnpm db:e2e:reset` clears the Upstash rate-limit counters for `*@e2e.test` emails, so every run starts the sign-in tests from attempt zero. The reset owns the precondition, and the test just asserts the outcome.
:::
...
1. A fresh user receives an invitation. The token arrives via a seed-inserted row. *Sending the email itself* was the integration test's job from earlier, not this one's, so name that boundary and step over it.
