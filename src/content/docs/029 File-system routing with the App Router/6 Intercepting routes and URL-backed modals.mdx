...
The whole pattern rests on one idea: a single URL can render two different things, and which one you get depends on *how you arrived*. You met the two ways in the Navigation primitives lesson; the feature turns on the difference between them.
...
Almost every piece is something you already have: the `@modal` slot and its `default.tsx` from Parallel routes, the `[id]` segment and the `await params` reflex from Dynamic segments, and the shadcn `<Dialog>` from the accessibility chapter. The one new idea is the prefix. Everything else is recombination.
...
`}>
  ...
  <AnnotatedStep meta={`{3} "useRouter"`} color="blue">
    The router hook comes from `next/navigation` in the App Router, not the old `next/router`. A recap from the Navigation primitives lesson.
  </AnnotatedStep>
