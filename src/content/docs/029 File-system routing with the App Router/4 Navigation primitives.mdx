...
This is the everyday redirect. It sends the browser to a new URL with HTTP status <Term term="307">307</Term> (Temporary Redirect), which preserves the request method. Two cases dominate.
...
You've already half-met this one. `notFound()` throws a signal that makes the framework render the closest not-found UI and serve a real <Term term="404">404</Term> status. Every dynamic route is a candidate: you validate the `params`, query the row, and if it comes back `null`, the id pointed at an invoice that never existed or was deleted, so you call `notFound()` and let it throw. The not-found page itself is wired up by a `not-found.tsx` file, which you'll add in the next chapter.
