...
The export is `default`, and that is *not* optional. Next.js imports the default export of every `page.tsx` to find the component to render; it has no other way to know which thing in the file is the page. This cuts against a rule you'll see everywhere else in this course: prefer named exports, always. The App Router is one of the few places that rule yields, because the framework mandates a default export from its convention files (`page.tsx`, `layout.tsx`, `route.ts`, and a handful of others). Default exports are deliberate in these convention files and a code smell everywhere else; the files are the documented exception, not a loophole.

The component's *name* is irrelevant to routing. Call it `DashboardPage`, `Page`, or `Anything`: Next.js never reads the name, only the default export and the folder it sits in. The folder names the route; the file just provides the component. Name it for the humans reading the code (`DashboardPage` is clearer than a bare `Page`), not for the router.

It's also a <Term definition="A component that runs on the server during the request and ships zero JavaScript to the browser by default. The opposite is a Client Component, marked with 'use client'.">Server Component</Term> by default, with no `'use client'` at the top. For now that means it renders on the server and ships no client-side JavaScript unless something asks for it. What "Server Component" fully means is the next chapter's job; here, just register that `page.tsx` starts on the server side of the line.
...
:::note
When you read Next.js docs or a random GitHub repo, you'll often see bare `app/` at the project root instead of `src/app/`. They're the same thing; that project just skipped the `src/` directory. Every routing rule in this lesson applies identically, so mentally prepend `src/` and move on.
:::
...
First, the safe default you already half-know: only `page.tsx` (and one other file we'll meet shortly) creates a route. A bare `revenue-chart.tsx` sitting next to `page.tsx` is **already non-routable**; it's just a module you import, never a URL. Files in `app/` are safely co-located by default, so a stray component file won't leak as a page.
...
A *folder*, though, is a segment, so `charts/` is a real concern, and that's where the underscore comes in. **A folder whose name starts with `_` is invisible to the router.** `app/dashboard/_components/` and everything inside it opt out of routing entirely: `app/dashboard/_components/revenue-chart.tsx` is reachable by import but can never be a URL. The underscore is the explicit "this is supporting code, not a route" marker.
...
- **Future-proofing.** This is the one that matters most. Next.js keeps adding reserved filenames. A bare `default.tsx` you wrote as a component would silently become a parallel-route fallback, a framework convention you'll meet later in this chapter, and you'd have a confusing bug on your hands. A file at `_lib/default.ts` can never collide with a framework name, because the whole `_lib/` folder is off the router's radar.
...
One more standard worth stating now: when you import from `_lib/`, you import the *specific file* you need, such as `_lib/format-cents`, and there is no `index.ts` that re-exports everything from the folder. Those re-export files (barrels) are banned in this course, because a barrel pulls every file in the folder into the module graph at once and defeats the careful Server/Client split you'll spend the next chapter learning. Import the file, not the folder.
...
<CodeVariants>
  ...
  <CodeVariant label="Alias">
    ...
    import { formatCents } from '@/lib/format-cents';
    ```
    **Stable.** `@/` always means the source root, no matter how deep this file sits. Move the page anywhere and the import is unchanged, because it names the *destination*, not a relative walk.
  </CodeVariant>
</CodeVariants>

The `@/` is an <Term definition="A tsconfig.json path mapping that turns @/x into an absolute import from the source root, so it never depends on where the importing file sits.">import alias</Term>. A single line in `tsconfig.json` maps `@/*` to your source root, and from then on `@/lib/format-cents` resolves the same way from anywhere in the project. The contract an experienced developer follows: **`@/` for everything that crosses a feature boundary; relative paths only for intra-feature, same-folder or sibling imports.** Inside `app/dashboard/`, the page imports its own chart with `./_components/revenue-chart`: they're one feature, they move together, so the relative path is correct and reads as "right here." But the moment an import reaches *out* of the feature, to `lib/`, to `components/`, or to another route's code, it goes through `@/`.
...
You now have both halves of the placement decision: co-locate by feature, promote on the second use. The skill is running them as a single habit every time you create a file. Walk the decision below; it's the exact question order an experienced developer runs through when they drop a new file into the project.
