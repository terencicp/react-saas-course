...
`RootLayout` receives `DashboardLayout` as its `children`, and `DashboardLayout` receives `DashboardPage` as *its* `children`. That's the whole composition rule. Once you see the tree nested like this, the property the next section turns on follows almost for free.

## What re-renders on navigation: the layout/page boundary

Here is the idea the rest of the lesson builds on:
...
<VideoCallout videoId="auXXk38rGgk" videoTitle="Next.js Route Groups — Codevolution">
  Codevolution builds an `(auth)` route group from scratch, watching the URL stay flat as the folder organizes the routes. Four minutes on the exact move this section just described.
</VideoCallout>
