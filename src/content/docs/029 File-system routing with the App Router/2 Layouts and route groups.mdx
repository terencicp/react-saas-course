...
A `layout.tsx` is the framework's answer to "shared shell." Drop one into a folder, and every `page.tsx` at or below that folder renders *inside* it. Put your sidebar in `src/app/dashboard/layout.tsx` and it wraps `/dashboard`, `/dashboard/invoices`, and `/dashboard/settings`, every page in that subtree, written once.

The shape is small, but two facts about it trip up nearly everyone the first time. We'll walk the root layout, the one file every Next.js app has: `src/app/layout.tsx`, which wraps your entire application.
...
`}>
  ...
  <AnnotatedStep meta={`"{ children }: { children: ReactNode }"`} color="blue">
    The fact to internalize is that a layout takes a single prop, `children`, and **the framework fills it; you never pass it yourself**.
    Whatever route renders inside this layout arrives here as `children`. A beginner expects to hand `children` in like any other component, but you don't. You receive them.
  </AnnotatedStep>
  ...
  <AnnotatedStep meta="{9-14}" color="violet">
    This is where the shared chrome sits relative to the page. Anything you place around `{children}` appears on every page this layout wraps. Here that's the skip link the accessibility baseline asks every layout to carry, paired with the `<main id="main-content">` it jumps to.
...
That second step is worth one more beat, because the instinct to pass `children` yourself is strong. You don't construct the children; the framework hands them to you. The page that resolves at a URL becomes the `children` of the layout above it.
...
`RootLayout` receives `DashboardLayout` as its `children`, and `DashboardLayout` receives `DashboardPage` as *its* `children`. That's the whole composition rule. Once you see the tree nested like this, the most important property of layouts follows almost for free, which is the next section.
...
On the `layout.tsx` variant, the navigation lights up only the page, while both layouts hold steady. That single flash is the whole feature: it's *why* the sidebar's scroll survives and *why* the corner player keeps playing. This is not a bug to work around; it's the reason layouts exist. Flip to the React-only variant and the same navigation remounts the entire imported shell, so root, dashboard, and page all flash and the sidebar state is gone. That's the cost the intro warned about, made visible.
...
One trap rides along with this, and it's worth catching now, because the same property that makes layouts efficient makes a careless data fetch slow. Since a layout doesn't re-run on a page-only navigation, a layout that fetches data fetches it once and holds it. That's great for data the whole subtree shares, but a problem for data a single deep page needs. If you put a fetch for one page's data up in a high layout, you block that entire subtree from rendering until the fetch resolves, and you re-fetch nothing when the page below changes. The rule states cleanly in the positive: **fetch at the level that owns the data.** Page-specific data is fetched in the page. How a layout *streams* while that data loads, using `loading.tsx` and Suspense at the layout boundary, is owned by a later chapter; here you only need the placement rule.
...
:::caution
Route groups don't change the URL, which means the URL space stays flat and two groups can't both own the same path. Put `(marketing)/about/page.tsx` and `(shop)/about/page.tsx` in your tree and both resolve to `/about`; the framework can't decide which one `/about` should render, so it **fails the build**. This is a compile-time error, not a runtime surprise, so you'll catch it the moment you try to build. The fix is to make the paths distinct, because the groups themselves are invisible to the resolver.
:::
