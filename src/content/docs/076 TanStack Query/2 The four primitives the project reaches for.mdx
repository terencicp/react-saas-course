...
The screen you are building is the per-invoice comment thread: live, optimistic, and infinitely scrollable.
The previous lesson argued that a screen like this earns the library.
This lesson shows you what you actually type.
...
That branch is a wiring detail the next lesson covers; here, just hold that the schema is the shared contract on both sides.
...
The HTTP endpoint is still there for anything that speaks HTTP; you've layered a browser cache over it rather than replacing it.
:::
...
<DiagramSequence>
  ...
  <DiagramStep>
    ...
    <Fragment slot="caption">
      On failure, `onError(error, input, context)` fires, and it receives the same `context` that `onMutate` returned. This is where you roll back, using the snapshot you stashed in the first step.
    </Fragment>
  </DiagramStep>
...
There are two ways to fire the mutation, and the choice is simple.
...
`onMutate` runs *before* the request as part of setting it up, so firing another mutation from inside it leads to recursion and ordering bugs.
...
Optimism is where "knows the API" and "knows the library" part ways. The reason is that v5 gives you **two different shapes** for an optimistic update, and choosing the right one is the actual skill.
...
Cache-update is a whole sequence rather than a single write because the order is what keeps it correct.
...
`}>
  ...
  <AnnotatedStep meta={`"initialPageParam"`} color="blue">
    The cursor for the very first page. Here it's `null`, meaning "start from the beginning."
  </AnnotatedStep>

  <AnnotatedStep meta={`"getNextPageParam"`} color="blue">
    Given the last page loaded, return the cursor for the *next* page, or `undefined` to signal there are no more. `undefined` is the stop signal, and note it must be `undefined`, not `null`, because `null` is a valid *first* param and so can't double as "done".
  </AnnotatedStep>
...
- **`removeQueries({ queryKey })`** evicts matching entries entirely, gone from the cache.
