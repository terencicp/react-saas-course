...
You now have the reflex: the threshold that decides when to reach for the library, the four primitives you write at the call site, and the App Router wiring that keeps the cache from leaking across requests.
All of that has been abstract so far, a library waiting for a job.
This lesson gives it one. It takes a real screen in the invoicing app, the per-invoice comment thread, and runs everything you've learned against it, the way an experienced engineer would in a design review before a single file gets written.
...
Picture an invoice detail page in the app, at `/invoices/[id]`.
...
The rest of the lesson moves in three steps.
...
Here's the trap an inexperienced engineer falls into: they've just learned a powerful library, the screen looks "live and interactive," so they reach for it on instinct.
...
Hold each one against the thread, and for each, name the default it beats.
...
**Cross-view caching.** The same thread might get peeked at from a "recent activity" sidebar that mounts and unmounts as the user moves around the app, and a populated client cache renders it instantly on re-mount.
**Met, but this is the weakest of the four.**
On its own, cross-view caching would never justify pulling in a whole library.
...
The point worth sitting with is that **one screen hitting three triggers at once is rare.**
Most surfaces in a SaaS hit zero, which is why most surfaces stay Server Components and Server Actions.
This thread is the unusual case the library was actually built for, which is exactly why this chapter chose it as the worked example.
...
It owns the <Term definition={"FormData + native HTML <form> form contract. Submits without JavaScript and degrades gracefully — the page works before hydration."}>progressive-enhancement form contract</Term>, so the form submits even before JavaScript loads.
...
Pulling TanStack Query in added a client cache *on top of* that API; it didn't replace it, fork it, or sacrifice it.
...
A tighter poll multiplies the cancel/overwrite race you just saw, hammers the database connection pool, and drains mobile battery for no real gain, and a comment thread updating every ten seconds already feels live.
...
<VideoCallout videoId="cypK50wBCZs" videoTitle="Frontend System Design Essentials: Optimistic updates">
  Juntao Qiu builds this cache-update shape on a live comment feed: `onMutate` snapshots the previous cache with `getQueryData`, `setQueryData` writes the optimistic row, and `onError` rolls it back. It's the same snapshot-write-restore sequence, framed as a system-design call (19 min).
</VideoCallout>
...
- **`updateTag(invoiceTag(invoiceId))`**, *inside the action*, refreshes the **Server Component** layer with read-your-writes: the parent layout's summary and any server-rendered count.
...
The last move ties the whole chapter together: **scope the library to the one leaf that needs it, and leave everything else alone.**
