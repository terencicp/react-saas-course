...
The `additionalHooks` regex lists the hooks whose final argument is a dependency array. From then on the lint enforces exhaustive deps on them exactly as it does on `useEffect`. You met the `react-hooks` rules in the previous chapter; this is where you extend them to your own hooks.
