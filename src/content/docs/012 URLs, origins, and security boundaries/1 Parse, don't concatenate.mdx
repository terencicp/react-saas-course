...
You learned backticks as the clean way to interpolate a string, and they are, with one exception. The moment a string is a URL, `` `${base}/users?id=${id}` `` stops being clean and becomes a hidden bug. It looks fine in the editor and it works on the happy path, which is exactly why it survives review and then breaks in production. The problem is the premise: **URLs look like strings, but they aren't.** A URL is a structured value with its own grammar and escaping rules, and the browser already ships a parser that knows all of them, the one the <Term definition="Web Hypertext Application Technology Working Group — the body that maintains the living URL, HTML, and Fetch standards browsers actually implement.">WHATWG</Term> URL standard defines. So instead of treating the URL as a fill-in-the-blank template, hand the string to that parser. This is the same approach behind every API URL you'll assemble from Units 3 through 7, so it's worth learning properly once.
...
That consistency is the first concrete win, so it's worth seeing the failure it replaces. The classic concatenation bug is trailing-slash drift: the same logical URL, assembled from two slightly different inputs, produces two different strings. Now your cache, your logs, and your access checks see two URLs where you meant one.
...
While the parser is normalizing slashes, it normalizes two more things you'll rely on in the next lesson: it **lowercases the hostname** and **drops the default port**, so `:80` for `http` and `:443` for `https` collapse to no port at all. This is exactly why origin comparisons in the next lesson are reliable: two URLs for the same server normalize to the same `origin` string instead of differing on capitalization or a redundant port.
...
The exception is untrusted input. When the string you're parsing came from a user, such as a query param, a pasted link, or a redirect target, throwing is the wrong response to bad data you fully expected to receive. For that case, reach for `URL.canParse(input, base)`, a static method that returns a boolean instead of throwing.
...
So the rule splits cleanly by trust: use `new URL()` for config you control, and let it throw; use `URL.canParse()` to guard before parsing anything a user could send. (`URL.parse()` exists too, with the same idea, except it returns `null` instead of a boolean. Reach for `canParse` when you're guarding and `parse` when you want the parsed result in one step. They were added at the same time, and either is fine.)

## `URLSearchParams`: the end of escaping query strings

Here's the win, before the mechanics: you write `params.set('q', userInput)` and you never think about escaping a query value again. The whole point of `URLSearchParams` is that it owns the encoding so you don't. Every half hour you'd otherwise spend debugging a `&` that ate the rest of a query string is a half hour it gives back.
...
When you're done building, `params.toString()` hands you the finished query string, correctly percent-encoded, with every special character escaped to the right form. That sounds like the end of the story, but *how* it encodes is the one detail you have to understand, because it isn't the same encoding you'd reach for elsewhere. That's next.

## The `%20`-vs-`+` split

Here's the surprise, and it's one that experienced developers get wrong too: the WHATWG URL standard uses **different** percent-encode sets for different positions in a URL. Your intuition says encoding is encoding, that a space is a space. That intuition is the trap. The same character gets a different escape depending on where in the URL it sits.

It comes down to two cases. <Term definition="Replacing a character with % followed by its byte value in hex — %20 is a space. Also called URL-encoding.">Percent-encoding</Term> handles them differently:
...
That second rule is where the bug lives. If you encode with one model and decode with the other, the round trip silently corrupts your data. The clearest way to see it is to push a value that genuinely contains a plus, a search for `"a+b"`, through both paths.
...
The `%20`-vs-`+` split is the subtle case, but it sits inside a simpler, more common confusion: there are three encoding tools, each correct in exactly one place, and most bugs come from picking the wrong one. People reach for `encodeURIComponent` everywhere and double-encode values that `URLSearchParams` already handled, or reach for `encodeURI` thinking it escapes a value, when it's actually for whole URLs. This table settles it.
...
Now the payoff. Every one of these is a bug that string concatenation produces silently and that `new URL` plus `URLSearchParams` removes. They're worth seeing as a list, because the point isn't any single one; it's that you stop writing the whole category.
...
Read that list as five symptoms of one underlying problem. None of them is a one-off you patch when it bites. Each is a *category* of bug, and reaching for the parser removes the category rather than the single instance. That's the thesis of the lesson in one sentence: don't fix URL bugs, stop writing them.
