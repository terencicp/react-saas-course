...
:::note
This lesson wires only the *protection* around `audit_logs`. The table itself arrives in the lesson on audit logging in the next chapter: its columns, what events write a row, and the writer function that appends entries. Here we treat the table as already existing and focus on one thing: making sure a row written for one org is invisible to every other org.
:::

## How a policy decides which rows you see

Before any Drizzle syntax, you need a precise mental model of what a policy actually *is*. A <Term definition="Per-row boolean rule Postgres evaluates on every query against the table.">policy</Term> is a per-row boolean rule that Postgres evaluates on every query against a table. When a policy exists on a table and Row-Level Security is on, the database rewrites your query behind the scenes: every `SELECT`, `INSERT`, `UPDATE`, and `DELETE` gets the policy's condition added to it, and rows that fail the condition simply don't exist as far as that query is concerned. You don't opt in per query, and you can't forget it. The filter belongs to the database, not to you.
...
A <Term definition="Connection-scoped key/value, read with current_setting, that carries the tenant id into the policy.">session variable</Term> here means a connection-scoped key/value pair, read back with `current_setting`, that carries the current tenant's id into the policy. Keep that term in mind, because the second half of the lesson is almost entirely about setting it correctly.

<VideoCallout videoId="vZT1Qx2xUCo" videoTitle="Everything you need to know about Postgres Row Level Security">
  Supabase CEO Paul Copplestone's 18-minute POSETTE talk frames RLS as an implicit `WHERE` clause and walks through `USING` vs `WITH CHECK`, permissive-policy OR-ing, and the owner bypass, the exact concepts this section just introduced.
</VideoCallout>
...
:::note
One subtlety is worth naming: declaring a policy on a table *auto-enables* RLS, so `.enableRLS()` is technically redundant when a policy is present. Keep it anyway. It documents intent at the call site, so anyone reading the schema sees RLS is on without having to know the auto-enable rule, and the very next section needs RLS *forced*, not merely enabled, which is a separate thing entirely.
:::
...
Two terms to carry forward. <Term definition="Postgres command that sets a config value only until the current transaction ends.">SET LOCAL</Term> is the Postgres command that sets a config value only until the current transaction ends. <Term definition={"set_config(name, value, is_local).\nFunction form of SET; the third arg true makes it transaction-local.\nParameterizable, unlike raw SET."}>set_config</Term> is the function form of `SET`; its third argument `true` makes the setting transaction-local, and unlike raw `SET` it accepts a bind parameter, which is why the helper uses it.
...
:::caution
The in-browser Postgres these exercises normally run on (PGlite) **does not enforce RLS**. An embedded "prove it" sandbox here would pass even with the policy *deleted*, so it would teach the precise false confidence this entire lesson warns against. So this test ships as code to read, and the real one lives in the integration suite (built later in the course) against a real Postgres instance. The lesson inside that constraint is worth sitting with: **an RLS test that can pass without RLS is worse than no test at all**, because it converts "we're not sure" into a green checkmark.
:::
