...
<Aside type="note">
That `env.ts` validator earns its keep right here in the build. If a required server variable is missing or malformed, the build *fails* and the artifact never ships, so a misconfigured environment is caught before it can reach production rather than breaking at runtime. The full validator schema and the dev/preview/prod scoping that go with it come later in this chapter.
</Aside>
