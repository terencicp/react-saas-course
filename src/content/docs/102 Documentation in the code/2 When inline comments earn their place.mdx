...
- **A "must be called after auth" becomes a typed argument.** `// only call this with an authenticated user` becomes a function that takes an authenticated-session type, or runs behind the [`authedAction` wrapper](/057-roles-authorization-audit-logs/2-the-authedaction-wrapper/). Skipping auth is now a compile error, not an honor-system comment.
