...
:::note
The form that consumes this `Result`, the `useActionState` wiring that renders per-code copy and a pending state, is the exact form shape from the chapter on forms the platform way (["useActionState"](/044-forms-with-platform-primitives/3-useactionstate---pending-state-and-the-result/)).
...
Recall the <Term definition="An attack where the attacker fixes a known session ID in the victim's browser before they log in, then reuses it afterward; defeated by minting a fresh token on every sign-in.">session fixation</Term> threat from the chapter on the auth mental model (["Sessions vs JWTs"](/051-authentication-concepts/2-sessions-vs-jwts/)).
