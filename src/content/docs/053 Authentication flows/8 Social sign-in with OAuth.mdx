...
:::caution
Redirect URIs are matched exactly: scheme, host, port, path, trailing slash and all. `http://localhost:3000/api/auth/callback/google` and `http://localhost:3000/api/auth/callback/google/` are two different URIs to Google, and the second will silently fail. There are no wildcards and no "starts with." When one environment works and another mysteriously doesn't, this is almost always why: a `https` where you meant `http`, a missing port, or a stray trailing slash in one of the registered URLs.
:::
...
:::note
There is no Server Action wrapping the OAuth start. The password flows all run through the five-seam action shape: parse the FormData with Zod, return a `Result` the form renders. This call doesn't. `signIn.social` triggers a browser redirect, so there's no FormData to parse and no `Result` to return on the start, because the response is a redirect to Google, not data for your UI. The boundary that produces a session is the callback, the catch-all route the library owns, not a `signInWithGoogleAction` you write. Don't build one; there's nothing for it to do.
:::
...
:::danger
`account.encryptOAuthTokens` defaults to `false`, so stored tokens land in your database in plaintext. A leaked database dump then hands an attacker live, usable provider tokens, the keys to your users' Google accounts, not just your app. Whenever you actually persist and use tokens, set `account: { encryptOAuthTokens: true }`. The happy path works identically either way, and only a breach reveals the gap.
:::
...
<TabbedContent syncKey="provider-quirks">
  ...
  <TabbedItem label="GitHub" icon="simple-icons:github" caption="The one to read carefully: GitHub can hand you a null email.">
    - The catch: the default scope returns the public profile only, so `user.email` can be `null` when the user keeps their email private.
  ...
  <TabbedItem label="Apple" icon="simple-icons:apple" caption="Persist the email on first sign-in, because Apple won't hand it back again.">
    - Apple returns the user's email and name only on the very first sign-in. Every sign-in after returns just the `sub`. The app must persist the email at first sight, because Apple has no `userinfo` endpoint to re-fetch it. (Better Auth stores it on that first sign-in; trust that storage afterward.)
    - Requesting name and email requires `responseMode: 'form_post'`, so the callback arrives as a `POST`, not a `GET`. Your catch-all handles this transparently.
    - `email_verified` and `is_private_email` arrive as strings (`"true"`), not booleans, the same `=== true` catch called out in the `email_verified` section.
  </TabbedItem>

  <TabbedItem label="Microsoft" icon="simple-icons:microsoftazure" caption="One real decision: which tenant audience you accept.">
    - The `tenant` parameter picks the audience: `consumers` (personal Microsoft accounts), `organizations` (work or school accounts), or `common` (both).
    - B2B SaaS picks `organizations`; a consumer product picks `common`.
    - Everything else is the same `socialProviders` shape, so naming the tenant is the only real decision.
  </TabbedItem>
...
:::note
Side-effects after an OAuth sign-in ride the global `databaseHooks` and `hooks`, not a per-provider callback. If you went looking for an `onSuccess` on the provider config, that's why you didn't find one.
:::
