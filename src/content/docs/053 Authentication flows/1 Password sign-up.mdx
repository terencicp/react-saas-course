...
It stops the moment the verification email is *queued*: no email sent, no token minted, no session issued. Sending the email and handling the verify click come in [Email verification](/053-authentication-flows/3-verifying-email/). That is exactly where a hardened sign-up is supposed to stop.
...
The page deliberately dead-ends — no session, no `/dashboard`, no protected content — until the user clicks the verification link, which issues the session and opens the app (covered in [Email verification](/053-authentication-flows/3-verifying-email/)).
...
Each row is read later: [Password sign-in](/053-authentication-flows/2-password-sign-in/) checks `emailVerified`, and [Email verification](/053-authentication-flows/3-verifying-email/) consumes the `verification` row when the link is clicked.
