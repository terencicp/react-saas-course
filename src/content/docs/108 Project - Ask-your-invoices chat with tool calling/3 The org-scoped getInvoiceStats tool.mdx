...
The tool calling, the `inputSchema` / `outputSchema` contract, `execute` running server-side, and the agentic loop are taught in depth in [the tools lesson of chapter 107](/107-tools-generative-ui-and-retrieval/1-tools-and-the-agentic-loop/) — this lesson applies them. The typed-`UIMessage`-via-`InferUITools` mechanism is [the generative-UI lesson of chapter 107](/107-tools-generative-ui-and-retrieval/2-generative-ui-via-tool-parts/); the `scopedInvoices` builder and its keyset shape come from [the scoped-reads lesson of the production list view project](/062-project---production-invoice-list-view/3-scoped-reads-and-the-view-tabs/).
