...
<VideoCallout videoId="HnDsMehSSY4" videoTitle="HOW QUIC WORKS — Intro to the QUIC Transport Protocol — Chris Greer">
  Chris Greer draws the TCP-plus-TLS round-trip baseline against QUIC with TLS 1.3 folded in — the 1-RTT fresh handshake and the 0-RTT resumed path, the exact RTT story this section counts (8 min).
</VideoCallout>
...
<VideoCallout videoId="IjJtmfjp1Ls" videoTitle="Identifying backend connection latencies with chrome devtools — Hussein Nasser">
  Hussein Nasser reads the DevTools Timing breakdown segment by segment — connection latency, SSL, and the TLS-1.3-over-TCP-versus-QUIC round-trip difference — to name the slow stage on a real request (18 min), the exact skill the drill below rehearses.
</VideoCallout>
