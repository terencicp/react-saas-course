...
<CodeVariants>
  ...
  <CodeVariant label="HTTP/2 + TLS 1.3 · 2 RTT">
    ...
    Server -> Client : response (first byte)
    ```
    **TCP handshake first, then TLS, then the request.** That's two round trips to the first byte on a fresh connection. This is how the web worked before QUIC, and it's still where a request lands when UDP is blocked or before `Alt-Svc` has done its work.
  </CodeVariant>
</CodeVariants>
...
:::caution
TTFB measured from a warm connection isolates the server-and-network number: DNS is cached, the connection is alive, and TLS is resumed, so the only cost left is how long the server took to think plus how long the answer took to come back. TTFB on a cold connection bundles all four stages into one stopwatch reading. So when you read a TTFB value, always check which kind it is. A 600 ms TTFB on a cold connection might be a fast server sitting behind 400 ms of network setup, while a 600 ms TTFB on a warm connection is a genuinely slow server.
:::
...
<Figure caption="A schematic h3 document request Timing panel. Initial connection holds the QUIC + TLS handshake (Stages 2 and 3 folded), and Waiting (TTFB) is Stage 4. DNS Lookup is 0 ms because the lookup hit a warm cache; SSL is folded into Initial connection, so it never appears as its own bar.">
