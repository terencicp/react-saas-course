...
The first byte has arrived, but the user still sees nothing on screen and has nothing to click. Between the bytes on the wire and a page the user can actually use, the browser runs a second pipeline of six stages. Each stage has its own job, its own cost, and its own ways to go wrong. The last lesson named the four network stages from URL commit to first byte; this one names the six browser stages from first byte to interactive page. The framing carries forward: a slow page load is a problem with a specific stage, and the engineer who can point at the slow stage leads the debugging conversation.
...
If the compositor box stayed smooth while the `left` box stuttered, you've now seen the difference firsthand. Choose `transform` and `opacity` for animations that need to feel smooth: slide-ins, fades, hover scales, and drag handles. Reach for `width`, `top`, `margin`, or `height` only when you actually need the layout side effect, such as collapsing a panel that pushes its siblings down, and accept that those properties can't share the compositor's smoothness.
...
After first paint, the browser starts downloading the JavaScript bundle for any interactive components. When it lands and runs, React performs <Term definition="The process where React attaches event listeners and component state to server-rendered DOM nodes. The page is visible during this gap but not yet interactive — a button click won't fire until its component has hydrated.">hydration</Term>: it walks the DOM the server already produced and *attaches* event listeners and component state to the existing nodes. The page is visible during this entire process, so the user can scroll, read, and look around. But they can't click anything that requires React state until hydration finishes for that component.

**The page is visible before it's interactive.** That single sentence sums up the whole SSR-plus-hydration story, and keeping it in mind prevents a whole class of bugs: when a user reports that the button doesn't work for the first second after the page loads, that's not a broken click handler, it's the hydration gap.

<VideoCallout videoId="kZG3izJu7qE" videoTitle="Hydration Explained">
  A 4-minute deep dive walking through CSR vs SSR, the hydration gap, and how Suspense enables selective hydration, using the same framing as this section.
</VideoCallout>
