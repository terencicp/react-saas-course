...
That query takes around 800 milliseconds: not catastrophic, not instant, but long enough to notice. So here is the question this lesson exists to answer, and it is a question about the user rather than the code. For those 800 milliseconds, **what fills the screen?**
...
Three properties of that contract clear up the three misconceptions people arrive with, and all three are worth holding onto.
...
- It reacts to a signal that a child *throws*. When a component beneath the boundary is not ready, it emits a <Term definition={"The signal a component emits when it is not yet ready to render.\nReact treats it as 'wait', not as an error."}>suspend signal</Term>, and `<Suspense>` catches it the way a `try/catch` catches a throw. This means **the boundary does not need to know what its children are loading or how.** A child reading from a database, a child reading a streamed promise, and a lazily-loaded component all look the same to the boundary. It only knows that something below it said "not yet."
...
That last property is the one that drives the placement decision later, so let it land now: a boundary is a unit, and everything inside it appears together or not at all.
...
<CodeVariants maxLines={16}>
  ...
  <CodeVariant label="Suspense (server)">
    ...
    **Declarative: the boundary owns the loading state, and the component just awaits.** `InvoiceList` contains zero loading logic: no flag, no effect, no `'use client'`. The `<Suspense>` boundary, placed by whoever composes the component, is the single place loading is expressed.
  </CodeVariant>
</CodeVariants>

Look at what moved. In the first variant, the loading state is the component's problem: declared with `useState`, advanced inside an effect, read in a branch, and repeated for every new query. In the second, the component does not mention loading at all. It awaits its data and returns its UI, and the boundary, placed by whoever composes the component, is the single place the loading state is expressed. There is no flag to forget to flip, because there is no flag.
...
:::caution
There is a sharp edge here, and it is worth flagging now even though its full fix lives a few chapters away. The promise must be **created on the server and passed down**, or otherwise kept stable across renders. If you create the promise *inside* a Client Component that re-renders, you mint a brand-new promise on every render, so the boundary sees a fresh pending promise every time and never settles. The rules for keeping a promise stable, request memoization with `cache()`, are covered when we reach rendering and caching. For now the rule is simply to create the promise on the server and pass it down.
:::
...
This is the section the rest of the lesson was building toward. You now know what Suspense is, what suspends, and what a good fallback looks like. The senior decision, the one that is genuinely hard and genuinely matters, is *where the boundary goes*.
...
<CodeVariants maxLines={14}>
  ...
  <CodeVariant label="Two boundaries">
    ...
    **Each read resolves and reveals on its own, so the profile paints almost immediately.** Two boundaries means two independent units: the profile shows at 10ms with the feed's skeleton still beside it, and the feed fills in when it's ready.
  </CodeVariant>
</CodeVariants>
...
Walk the decision yourself. The drill below poses the questions in the order an experienced engineer asks them, and that order matters as much as any single answer.
...
The fix tells React the truth: this is a *different* thing. Put a <Term definition={"React's identity hint.\nA node with a new key is treated as a new instance, so React remounts it rather than reusing the old one."}>`key`</Term> on the suspending subtree, one that changes with the input.
...
<CodeVariants maxLines={10}>
  ...
  <CodeVariant label="With key">
    ...
    **A changed key is a fresh mount, so the boundary suspends again and the fallback returns.** The new key tells React this is a different instance, so it remounts, re-suspends, and shows the skeleton until the new invoice resolves.
  </CodeVariant>
</CodeVariants>
