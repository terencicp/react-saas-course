...
That's streaming. Here is the sequence the server actually runs, in order.
...
A few things about this are worth stating directly, because they're where the intuition usually goes wrong.
...
First, a quick drill to lock in the order.
...
This is the pattern that decides whether streaming actually buys you anything.
...
Now the trap, and it's the single most common latency bug you will ship. It looks like a perfectly reasonable Server Component, and it's wrong:
...
The simplest sniff test is the lesson's own question: **did the first chunk arrive while later chunks were still in flight?** If the whole body lands at once at the very end, streaming didn't happen, and the rest of this section is about the most common reason why, even when your code is perfect.
...
Three quick checks on the things this lesson is easy to get wrong.
