...
Second: never reach for `Math.random()` for anything that isn't visual jitter. Shuffling a deck for a non-security animation or nudging a particle a few pixels is fine. But the moment a random value is a token, an ID someone could guess, a nonce, or anything an attacker would benefit from predicting, `Math.random()` is a bug. It is not a <Term definition="Cryptographically secure pseudo-random number generator. Its output is unpredictable even to an attacker who has already seen many earlier values — unlike Math.random(), whose internal state can be reconstructed.">CSPRNG</Term> (a cryptographically secure pseudo-random number generator). Its output is statistically random but predictable: an attacker who watches enough values can reconstruct its internal state and forecast the next one. Every random byte in this lesson comes from `crypto`, which is the platform default for exactly this reason.
...
There is exactly one constraint, and it's the first appearance of a thread that runs through this whole chapter. In the **browser**, `crypto.randomUUID`, like nearly all of Web Crypto, only exists in a <Term definition="A page served over HTTPS or from localhost. Browsers gate Web Crypto, the clipboard, and other powerful APIs to secure contexts; a plain http:// page can't use them.">secure context</Term>: a page served over HTTPS, or over `localhost`. On a plain `http://` page, such as a raw LAN IP or a dev host that isn't `localhost`, the property is `undefined` and the call throws. This is the same constraint the `mkcert` setup earlier in the course unblocks for local development, so treat HTTPS-or-`localhost` as the prerequisite and don't re-litigate TLS here. On the **server**, meaning Node or the Edge runtime, there is no secure-context restriction at all, and the call always works. That split decides where the call can fail: the same `randomUUID` that's fine in your Server Component will throw in a component that runs in the browser over plain HTTP.

<Aside type="note">
  `randomUUID` returns a v4 UUID: random, and therefore **not** sortable by creation time. When you want database rows ordered by when they were inserted, which keeps the index tidy, the experienced reach is UUID **v7**. It's time-sortable and comes from the `uuid` npm package, and this course's project code standard picks it for user-facing primary keys. The platform one-liner is v4, which is what this lesson writes, but now you won't be surprised when a later lesson reaches for v7 instead.
</Aside>
...
<VideoCallout videoId="fzMIjWFYQl0" videoTitle="MAC / HMAC — Message Authentication Codes, explained">
  Practical Networking's 7-minute explainer covers *why* a keyed hash gives you both integrity and authentication, which is exactly the "only the secret-holder could have made this" property a webhook signature relies on.
</VideoCallout>
