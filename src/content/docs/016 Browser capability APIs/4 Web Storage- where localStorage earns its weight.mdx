...
The API is small. Using it safely in a Next.js app is not, and this is where most of the lesson's weight sits. The trouble is that `localStorage` is a *browser* object, and Next.js 16 renders a lot of your app on the *server* first.
...
:::note
This is the same boundary that gates object URLs and the clipboard from the last two lessons: browser-only, not the server. Web Storage just adds one wrinkle the others didn't have. Even after you've solved the server problem, a second and subtler hazard waits in how the server and client agree on what to render.
:::
...
`sessionStorage` is the same API, the same five methods, and the same strings-only constraint, with one difference: it's scoped to a single tab and wiped the instant that tab closes. The walker already drew the line; this is the confirmation.
...
Two production habits that a first draft always omits, and that separate code that works on your machine from code that survives real browsers.
