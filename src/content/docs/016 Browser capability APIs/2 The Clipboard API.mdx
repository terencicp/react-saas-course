...
So you write the obvious thing, `onClick={() => navigator.clipboard.writeText(url)}`, and ship it. It works on your machine. Then the support tickets start: for some users the button does nothing, for others it works only when the page is quick and fails when it's slow, and none of it reproduces locally. The call that looked like a one-liner has two invisible conditions on it, and neither one shows up during local development. **What conditions does this call depend on, and why does the obvious version break in production but not in dev?** That question is what the rest of the lesson answers.
...
:::note
`writeText` is the whole happy path. The depth in this lesson isn't the call itself. It's the two conditions that have to be true *before* the call can succeed, and what you do when they aren't.
:::
...
- **Insecure context.** On `http://`, recall that `navigator.clipboard` is `undefined`, so this never reaches your `catch` at all; it throws a `TypeError` when you read `.writeText`. And it's not a user error, it's a deployment misconfiguration. The fix isn't a friendly message to the user. It's making sure production is HTTPS so this can't happen, and surfacing it loudly in development with a console warning or a dev-only banner if it ever does. Don't try to handle the insecure-context case with a `catch (e) { if (e.name === '…') }` branch, because that's not where it shows up.
...
Everything so far has been *writing* plain text, which is the surface you'll actually reach for. The Clipboard API has two more corners worth recognizing so you know they exist, and worth knowing that you usually shouldn't reach for them.
