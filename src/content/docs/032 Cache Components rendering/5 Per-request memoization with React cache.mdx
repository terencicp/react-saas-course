...
If the function reads `cookies()`, `headers()`, the resolved session or current user, or a value derived from `params` or `searchParams`, anything that differs per request, then it's `cache()`. Here the choice isn't merely *better*, it's the *only* option. As you saw in [The use cache directive](/032-cache-components-rendering/3-the-use-cache-directive/), reading a request API inside a `'use cache'` boundary is a build error. You couldn't cache this work across requests if you wanted to, because its answer is different for every request. `cache()` is the per-request equivalent for exactly this work: dedupe within the render, forget at the end.
