...
The previous lesson, [Defending the no](/068-object-storage/1-defending-the-no--when-object-storage-earns-its-weight/), settled the question of *whether*: a trigger condition fired, since users are about to start handing the app files, so R2 is on the table for the right reasons. Now you open the Cloudflare dashboard to stand it up, and the first thing you see is a wall of toggles: location hints, lifecycle rules, public access, custom domains, CORS, token grades. None of it is labeled "start here." The senior question is not *how do I turn these on*; it's *which of these are trust boundaries I have to get right, and which are noise I can walk past?*
...
:::caution
R2 offers a **public bucket** mode that exposes objects at a public URL with no signing. It's a fine fit for genuinely public assets: marketing images, a logo, anything you'd happily put on a billboard. It is the wrong tool for tenant files, because a public bucket means anyone who learns or guesses a URL can read the object, with no per-user check. For files that belong to one organization, keep the bucket private and serve reads through signed URLs.
:::
...
The term for how much damage one leaked credential can do is its <Term definition="The scope of damage a single leaked credential or compromised component can cause. You shrink it by granting the least access the job actually needs.">blast radius</Term>, and shrinking it is the entire job here. R2 gives you two levers. On the scope side, a token can be **account-level**, reaching every bucket in the account including the power to create and delete buckets, or **bucket-scoped** to one or more specific buckets. On the permission side, the dashboard offers three grades:
...
:::caution
The secret access key is shown **once**, at creation, and never again. Copy it straight into your password manager (and your deployment's secrets) the moment it appears. If you miss it, you don't recover it; you roll the token and start over.
:::
