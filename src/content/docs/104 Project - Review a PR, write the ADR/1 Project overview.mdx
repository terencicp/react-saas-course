...
Before you go hunting for defects, spend the first pass calibrating your eye. Read the canonical helpers once — the [`authedAction` wrapper](/057-roles-authorization-audit-logs/2-the-authedaction-wrapper/), the [`logAudit` seam](/057-roles-authorization-audit-logs/5-the-append-only-audit-log/), the `tenantDb` facade, the [Temporal time primitive](/083-time-dates-and-timezones/5-arithmetic-with-temporal/), and the existing cached reads in `src/lib/invoices/queries.ts` with their `updateTag` mutation seams in `src/lib/invoices/actions.ts`. These are the established surface every finding is measured against — a review asks "what was the convention, and where does this code go around it?" You are not auditing yet; you are learning what *right* looks like so the bypass jumps out when you read `/plan` in the next lesson. The technology behind each helper is owned by its origin chapter — the [invalidation decision tree](/072-caching-strategy-and-invalidation/2-choosing-an-invalidation-call/) for the cached reads, the [roles chapter](/057-roles-authorization-audit-logs/2-the-authedaction-wrapper/) for the wrapper and the audit log, the [time chapter](/083-time-dates-and-timezones/5-arithmetic-with-temporal/) for Temporal — so read for the shape of the seam here, not the theory.
