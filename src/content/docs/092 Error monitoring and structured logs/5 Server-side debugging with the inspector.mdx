...
<StateMachineWalker title="Which surface answers this question?">
  ...
  <Leaf id="leaf-prod" verdict="Sentry + the log drain — never the inspector">
    ...
    An inspector on a live server is a remote-code-execution surface, for the reason covered at the end of this lesson.
  </Leaf>
  ...
  <Leaf id="leaf-debugger" verdict="Attach the inspector">
    The state that decides the bug was on the wire, not in any log, so pause on the suspicious line, replay the same input, and read it live.
...
<CodeVariants>
  ...
  <CodeVariant label="npm">
    ...
    **The lone `--` is load-bearing.** Without it, npm treats the flag as one of its own instead of forwarding it to the `dev` script, which is the single most common reason "the flag didn't work."
  </CodeVariant>
...
<VideoCallout videoId="_5mGxLZ61J0" videoTitle="Debugging Next.js in VSCode and Chrome DevTools (comprehensive guide)">
  Tom Dohnal walks the same setup end to end (16 min): the server-side, client-side, and full-stack `launch.json` configs, plus the Turbopack caveats.
</VideoCallout>
...
The Debug Console is a <Term definition={"Read-eval-print loop — an interactive prompt that evaluates expressions and shows the result.\nHere it runs in the scope of the paused frame."}>REPL</Term> bound to the *paused frame's scope*. You can evaluate against whatever variables are in scope at the breakpoint. Walk an object with `user.role`. Drill into `input.lineItems[0]`. You can even run a real query against the live database connection:
...
:::danger
Never start a production Node process with `--inspect`. An open inspector port lets anyone who can reach it run arbitrary code inside your live server, with full read and write access to your production database. The flag is for your machine only.
:::
