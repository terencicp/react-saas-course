...
<StateMachineWalker title="A test is failing intermittently — walk it to the cause">
  ...
  <Leaf id="leaf-resethandlers" verdict="`server.resetHandlers()` in `afterEach`">
    Wire it once in the integration setup file, never per test, since the next author won't remember a per-test reset. Each test then starts from the default handlers, and overrides don't leak forward. (Mechanics: Mocking outbound HTTP with MSW.)
  </Leaf>
