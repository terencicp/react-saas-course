...
<VideoCallout videoId="PvcRaMqtEPQ" videoTitle="React Query Tutorial - Polling">
  Codevolution shows polling in 3 minutes — `refetchInterval` as a millisecond cadence and `refetchIntervalInBackground`, including the hidden-tab pause — the exact two settings driving the ten-second poll and the battery line below.
</VideoCallout>
