...
<VideoCallout videoId="e74rB-14-m8" videoTitle="React Query Crash Course — Queries, Mutations, Caching, Optimistic Updates (PedroTech)">
  PedroTech builds the same optimism reflex from scratch — `onMutate` snapshots via `getQueryData`, `setQueryData` writes the row, the snapshot rides to `onError` for rollback, and `onSettled` invalidates; jump to 28:12 for the optimistic-updates section.
</VideoCallout>
