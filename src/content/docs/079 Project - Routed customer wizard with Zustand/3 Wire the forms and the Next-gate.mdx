...
For the `safeParse` and flattened-error semantics this gate runs on, see [parse, safeParse, and the error contract](/042-zod-4-schema-validation/5-parse-safeparse-and-the-error-contract/), and for the canonical `{ ok, error }` shape the action returns next lesson, [Result or throw](/043-server-actions/3-result-or-throw/) — the gate consumes a schema and a contract you've already met. The `'use client'` directive on every file here, and the Server/Client boundary it draws, is [Directives and server-only enforcement](/030-the-server---client-boundary/3-directives-and-server-only-enforcement/); this lesson applies that split rather than re-explaining it.
