...
<MultipleChoice>
  Inside `sendEmail`, the suppression query throws because the database connection timed out. What should the wrapper do?
  ...
  <McqWhy>The two outcomes aren't equally costly. A send that doesn't go out can be retried once the database recovers; nothing is lost. A send that reaches a suppressed address can't be taken back, and the bounce or complaint hits your reputation immediately. When a check can't run, refuse rather than risk the irreversible failure, and log it so it gets fixed.</McqWhy>
</MultipleChoice>
