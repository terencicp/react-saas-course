...
Here are three bugs from three different teams, all caused by the same mistake: the team kept using `{}` or `[]` past the point where either container fit the shape of the data.
...
The first is a `{}` used as a lookup table with user-controlled keys. Most of the time `cache[userInput]` returns a `User` or `undefined`, exactly as the type says. But when `userInput === 'toString'`, the property doesn't come from `cache`. It comes from `Object.prototype`, which every object literal inherits from. The lookup returns a built-in function, the next line tries to read `.name` on that function, and production logs a confusing `TypeError`. The previous lesson on objects named `Object.hasOwn` as the defensive read. The senior move goes one step further: when the keyspace is user-controlled, don't use `{}` at all.
...
The second is the one the previous lesson on array methods left hanging. With `bigInvoices.length === 10_000` and `watchedCustomerIds.length === 200`, the inner `.includes` walks the watch list once for each invoice, two million comparisons for what should be one walk. The result is correct, so the bug stays silent, and the slowdown only shows up once the dataset grows. A senior sees `.includes` inside a `.filter` callback and reaches for `Set`.
...
One pair of terms is worth learning now, because you'll lean on it throughout. A *record* has a fixed shape, like `{ id, email, name }`, and the object literal is the right container for it. A *dictionary* has a dynamic keyspace: *which* keys are present is part of the data, not the type, and `Map` is the right container for it. Most "I have a JavaScript object" intuitions blur the two, so before picking a container, ask *which one is this?*
...
`Set` is a collection of unique values. It has no keys, no order contract beyond insertion order, and no payload, only *"is this value in the set?"* and *"give me all the values."* Three triggers earn it.
...
`Set` decides "same value" using <Term definition={"JavaScript's equality semantics for Map keys and Set values: like ===, except NaN matches NaN. Two distinct objects are never equal even if their fields are identical — equality is by reference, not by shape."}>SameValueZero</Term>, which is like `===` except that `NaN` matches `NaN`. In practice this means two distinct object literals are never equal even when their fields look identical, because equality is by reference. The `new Set(arr)` form works for primitives; deduping an array of objects needs a key function and a different pattern.

### Membership inside a loop

This is the trigger from the lesson opener. `Array.prototype.includes` walks the array until it finds a match, which is `O(n)` for each call. `Set.prototype.has` is `O(1)` on average. The moment a membership check sits inside another loop, those two costs multiply.
...
In code review, watch for `.filter(x => other.includes(...))`, `.map(x => ... && other.includes(...))`, or a `for...of` loop with `.includes` in its body: anything where a membership check sits inside another walk. Drop a `new Set(other)` outside the walk and the inner check becomes constant time.

### Set algebra

As of ES2025, `Set` ships seven composition methods natively: union, intersection, difference, symmetric difference, plus three boolean predicates. They have been universally available since June 2024 (Chrome, Firefox, Safari, Node 22+), and Node 24 LTS, the runtime this course will pin a few lessons from now, has them all.
...
One rule carries over from the array lesson: `.add` and `.delete` *mutate the set in place*. That's fine for a `Set` you own inside a function. For a `Set` held in React state or otherwise shared, you don't mutate it, you replace it: `setSelected(new Set(selected).add(id))`. This is the same ownership rule as `.push` versus `[...arr, x]`. The React state mechanics arrive in the React chapters.
...
Plain objects coerce every key to a string. Write `obj[42]` and JavaScript stores it under `"42"`. Write `obj[someDate]` and JavaScript stores it under whatever `someDate.toString()` returns, something like `"Mon Jan 01 2026 00:00:00 GMT+0000"`. Two `Date` instances on the same calendar day collide, two different objects with identical shapes collide, and the original key type is lost.
...
Equality on `Map` keys uses the same SameValueZero rule as `Set`, which is by reference for objects. Two distinct object literals with identical fields are *different* keys, so you have to hand the same customer *instance* to `.set` and `.get` for the lookup to land.
...
`Map` is built for this operation. Plain objects in V8 (and other modern engines) are optimized through hidden classes: every shape the object passes through builds an internal transition. Frequent insert and delete patterns can break those optimizations, and the runtime falls back to a slower dictionary mode. `Map` skips the hidden-class machinery entirely, because it's a hash table by design.

You don't need to measure and prove this case by case. The rule is *"if adding and removing entries is the operation that dominates this container's lifetime, `Map` is the right tool by intent."* When a reader sees `Map`, they know churn is expected.
...
The same `Date`-as-object-key caveat from the non-string-keys section applies: two invoices built with separate `new Date(...)` calls land in *different* groups even when the dates represent the same instant. In practice you have two options. Either canonicalize upstream so equal logical days share one instance, or key by an ISO date string (`i.dueDate.toISOString().slice(0, 10)`) and use `Object.groupBy`. Choose based on which side of the string boundary your downstream code wants to work on.
...
`??` returns the right-hand side when the left is `null` or `undefined`. It was introduced in the previous chapter, and it's the most common reach here.
...
Two hard constraints follow. Keys must be objects, not primitives, because there's no way to weakly hold a string. And `WeakMap` is *not iterable* and has *no `.size`*: entries can disappear at any time, so iteration would yield inconsistent results and any size you read would be stale by the time you read it.
...
`WeakSet` is the set cousin: same weak-key semantics, no values. It fits "have I already processed this object?" checks where the set shouldn't keep the object alive. It's narrow enough that you'll rarely write one, but it's worth recognizing.

One historical footnote you may run into in older codebases: `WeakMap` was used to implement private state per instance before class private fields (`#field`) existed. The class private-field syntax replaces that use case entirely, so the modern way to hold private state in a class is `#field`, not `WeakMap`. Classes themselves come up in a later chapter; the point here is simply that *"private state in `WeakMap`"* is not the modern shape.

<VideoCallout videoId="WqNqeMjd28I" videoTitle="Only The Best Developers Understand How This Works" start="13:55">
  Web Dev Simplified shows `WeakMap` clearing a real memory leak in Chrome DevTools: the garbage-collection coupling this section describes, made visible in heap snapshots. The clip opens at the relevant part (13:55); you only need the couple of minutes from there, not the whole video.
</VideoCallout>
...
The reason: `JSON.stringify` only serializes own enumerable properties. `Map` and `Set` store their entries in internal slots, which are engine-managed storage that the serializer can't see and shouldn't try to.
...
One sort, eight items, four containers. Read each item, then drop it into the bucket whose triggers fit. This sort is the whole lesson in one drill: there's no syntax to remember, only the question *which container is this?*
...
A few items are worth a second thought after you finish. Item 4 looks like a record at first glance, since "lookup table" sounds like an object literal, but the *operation* is `O(1)` reads inside a hot loop with frequent inserts, which is the `Map` trigger. Item 6 lands in `Map` here because the four buckets don't include `WeakMap`, but the *better* answer in production is `WeakMap`: "DOM-node-keyed cache that should disappear when the node leaves the page" is the exact trigger from the `WeakMap` section. Item 8 is the dictionary-versus-record split. *Counting words* means the keyspace is whatever the input text contains, which is dynamic and user-controlled, so it's a `Map`. If you wrote it as `Record<string, number>` and a user submitted `"constructor"` as a word, you'd start counting how many times a built-in function appeared. That's the same trap as the opener.
