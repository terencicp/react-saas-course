...
The registry is the source of truth for what is notifiable. Each entry names the channels the event fans out to, its templates, its preference category, and its dedup configuration. The `as const satisfies Record<string, NotifiableEvent>` does two jobs at once: `satisfies` checks every entry against the `NotifiableEvent` shape, and `as const` freezes the keys so `EventType = keyof typeof notifiableEvents` infers the exact union of the three string literals — an unknown event type is then a compile error at every call site. This is the same `as const satisfies` move from [Derive types from values](/005-eliminating-bug-classes-with-types/5-derive-types-from-values/), applied to a real registry.
