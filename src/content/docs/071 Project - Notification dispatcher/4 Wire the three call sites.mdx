...
For the pieces these edits lean on but do not re-teach: `authedAction` and `withTenant` were built in [The authedAction wrapper](/057-roles-authorization-audit-logs/2-the-authedaction-wrapper/) and the audit write in [The append-only audit log](/057-roles-authorization-audit-logs/5-the-append-only-audit-log/); the webhook's verify-then-claim-then-commit shape is [Claim once, mutate once](/063-webhook-ingestion-and-idempotency/2-claim-once-mutate-once/) and the project version in [Claim the event inside one transaction](/065-project---stripe-webhook-to-entitlement/3-claim-the-event-inside-one-transaction/); and the dispatcher contract you are calling — `NotificationEvent` in, `DispatchResult` out — is [One seam, many channels](/070-the-notification-dispatcher/1-the-notification-dispatcher/) and this chapter's [Registry, dispatcher, and dedup](/071-project---notification-dispatcher/2-registry-dispatcher-and-dedup/).
