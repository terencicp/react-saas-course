...
:::caution
Run those two lines in your own browser console. The second one will print a different value than it does for a student in a different <Term definition="An identifier from the Internet Assigned Numbers Authority timezone database, like Europe/Madrid or Australia/Sydney. It names a region with its full DST history, not just a fixed UTC offset.">IANA tz</Term>. That divergence isn't a quirk of the runtime; it is the bug itself.
:::
...
<VideoCallout videoId="Q0BOHt6iHAY" videoTitle="Temporal API: A Decade of Waiting Was Worth It (ES2026)">
  Pixel Grid UI's 14-minute deep dive covers the same Date pitfalls, the same five Temporal types, and three migration gotchas from a different angle.
</VideoCallout>

## The runtime story, May 2026

Temporal is native code in Node 26, and a <Term definition="A small library that implements a not-yet-native API in JavaScript so existing code can use the same shape before the platform ships it.">polyfill</Term> everywhere else. The seam between those two is the one piece of plumbing this lesson installs.
...
:::caution
Never `import { Temporal } from 'temporal-polyfill'` outside `lib/temporal.ts`. Later in the course, an ESLint rule (`no-restricted-imports`) will enforce this automatically; for now, you keep the rule by discipline. Two `Temporal` instances in the same process break `instanceof` with no error to warn you.
:::
...
<CodeVariants>
  ...
  <CodeVariant label="In application code">
    ...
    ```
    **No `Date` in sight.** The adapter at the seam is the only place that touches the SDK's `Date` wrappers, and in unit 12 that adapter lives in `lib/billing/`. Domain code reads `Temporal.Instant` exclusively.
  </CodeVariant>
</CodeVariants>
