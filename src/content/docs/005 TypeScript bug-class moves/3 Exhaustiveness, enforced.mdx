...
<VideoCallout videoId="Wksz1DnJ2VM" videoTitle="Type Predicates Solve This Common TypeScript Error">
  Web Dev Simplified shows the exact problem this section addresses: extracting an inline `if` check into a function loses the narrow until you add `value is T`. Five minutes, with the inference failure on screen.
</VideoCallout>
...
<VideoCallout videoId="syBjKTOgFfo" videoTitle="Assertion Functions or Assertion Guards - TypeScript Narrowing">
  Lucas Paganini contrasts assertion functions with type guards, return versus throw, and lands on request-handler data validation as the dominant use case, the same parse-or-throw boundary this section describes. Five minutes.
</VideoCallout>
