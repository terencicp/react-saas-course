...
:::caution
`Partial<T>`, `Required<T>`, and `Readonly<T>` are all <Term definition="Operates on the top level only. Nested object fields keep their original shape; the transform doesn't recurse.">shallow</Term>. A nested object inside `Invoice` keeps its original shape, because `Partial<Invoice>` doesn't reach into nested fields and mark *their* keys optional. Deep variants exist in libraries, but in 2026 SaaS code the deep-partial problem usually lands on the wire boundary, where Zod handles it: the schema's `.partial()` produces the right shape with the right runtime validation.
:::
...
<CodeVariants>
  ...
  <CodeVariant label="Named intermediate — two levels, plus a label">
    ...
    The intermediate `EditableInvoiceFields` is a named landing point. The reader sees the name, knows what it holds, and the second line stays two utilities deep. You split the expression at the point where it would otherwise get hard to follow.
  </CodeVariant>
</CodeVariants>
