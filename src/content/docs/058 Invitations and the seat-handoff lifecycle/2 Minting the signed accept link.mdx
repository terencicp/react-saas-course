...
:::note
Two extensions you'll recognize and deliberately *not* build here. The year-two version pushes the send through a background queue (Trigger.dev) with automatic retries and a dead-letter queue, which comes later in the course; year one is the synchronous send plus the visible resend button. And invite-sending is a genuine abuse vector: a compromised admin firing thousands of invites can land your domain on a deliverability blocklist, so the limiter (roughly 20 invites per admin per hour, plan-configurable) rides the same middleware that gates your other actions. Both are named here and built later.
:::
...
:::caution
Three ways a well-meaning developer burns a live invitation. **Adding UTM tags** to the accept URL, where a tracker that fetches it consumes it. **Letting it be prefetched**, since some mail clients fetch links to generate previews, the same consume problem. And **surfacing the URL in the inviter's own UI in production**, where after the send that link is a credential the inviter has no business holding. In development it's fine to print the accept URL next to the "invite sent" toast so you can test without a mailbox; in production, never.
:::
