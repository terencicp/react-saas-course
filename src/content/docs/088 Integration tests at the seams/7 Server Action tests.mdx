...
<VideoCallout videoId="XMk1sG6ndwY" videoTitle="Unit Testing (Vitest) Tutorial #12 - Creating Spies">
  The Net Ninja builds a spy with `vi.spyOn`, then asserts on the calls it records with `toHaveBeenCalledTimes` and `toHaveBeenCalledWith`: the exact mechanics behind the `revalidatePath` cache spy above (11 min).
</VideoCallout>
