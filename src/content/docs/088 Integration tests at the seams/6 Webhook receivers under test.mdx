...
You already know how that header is built. You hand-rolled the HMAC in [Verify before parse](/063-webhook-ingestion/1-verify-before-parse/), reading the timestamp and the `v1` digest and comparing in constant time. The temptation now is to reuse that recipe: recompute the HMAC in the test fixture and sign your own payloads.
