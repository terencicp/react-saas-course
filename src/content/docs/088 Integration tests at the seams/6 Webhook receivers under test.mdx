...
That order is the whole design. It forms a <Term def="The line where untrusted input meets trusted code. Everything before the line is hostile until proven otherwise; the verifier is the gate.">trust boundary</Term>: nothing past step one is trusted until the signature checks out, and nothing past step three is allowed to run twice.
