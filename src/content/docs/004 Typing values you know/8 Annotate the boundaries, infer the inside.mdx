...
The strict tsconfig the course's projects use turns on <Term definition="TypeScript flag that emits each import statement exactly as written and requires import type for type-only imports, removing the compiler's old type-elision heuristic.">`verbatimModuleSyntax`</Term>. With that flag on, every import that's used only as a type must use `import type`.
