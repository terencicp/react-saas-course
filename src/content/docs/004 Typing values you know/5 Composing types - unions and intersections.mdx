...
- `A & B` is the <Term definition="The & operator on types. A & B is a value that satisfies both A and B. On shape types, the value has the fields of both — the set of fields grows even though the set of values shrinks.">intersection</Term> of constraints. A value of `A & B` satisfies *both* `A` and `B`. On shape types, that value has the fields of *both*. So the set of values shrinks, because fewer values satisfy two constraints than one, while the set of fields grows.
...
<CodeVariants>
  ...
  <CodeVariant label="Shape union">
    ...
    ```
    `value.name` reads cleanly because `name` lives on both variants. `value.email` errors because `Guest` doesn't have it. This is the shape-union trap the next section works through. The fix isn't to widen the type or assert past it; it's to narrow first.
  </CodeVariant>
...
:::tip
A discriminant only narrows if it stays a literal type. A value that property widening infers as `string`, named briefly in the lesson on primitives and four corners, won't narrow on equality with a specific literal. A later lesson in this chapter covers `as const`, the operator that fixes inline-literal discriminants. A `type` alias of a variant carries the literal type in its declaration, so it narrows correctly out of the box.
:::
