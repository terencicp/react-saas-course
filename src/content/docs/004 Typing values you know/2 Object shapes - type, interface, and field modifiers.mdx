...
The previous lesson covered the vocabulary for scalar values: primitives, literal unions, and the four corners. This lesson does the same for the next most common type you'll write, the object shape. Two production failures motivate it.
...
<CodeVariants>
  <CodeVariant label="The codebase uses both">
    ...
    You open two files in the same repo. One declares `User` with `type`; the other with `interface`. They produce the same shape at the call site, so neither one is wrong, but they can't both be the canonical form. New engineers waste an hour every onboarding deciding which one to write next. The fix is one rule with one narrow exception, and this lesson covers both.
  </CodeVariant>
  ...
  <CodeVariant label="The readonly that wasn't">
    ...
    `invoice` is meant to be the source of truth for a rendered invoice. Then a downstream helper appends a line to the array in place, the parent component holds a stale reference for a render cycle, and two screens display different totals before anyone notices. The fix is a `readonly` modifier, but `readonly` only locks one of the two things engineers assume it locks. This lesson draws that boundary clearly.
  </CodeVariant>
</CodeVariants>
...
The rule is simple: **`type` is the default for every alias the course writes**, whether that's objects, primitives, tuples, unions, intersections, generics, conditional types, or mapped types. The project's code conventions hold to this rule because it pays off in legibility: the team never debates the call.
...
<CodeVariants>
  ...
  <CodeVariant label="Two interface — silently merged">
    ...
    Both declarations merge into one `User` with both fields. This is a useful feature when you mean it, and a silent trap when two unrelated declarations accidentally collide in the same scope.
  </CodeVariant>
</CodeVariants>
...
:::note
Module augmentation is the full pattern (lesson 4 of chapter 6). This lesson only names the trigger, so you recognize the move when you meet it.
:::
...
**Performance at extreme scale.** `interface` checks marginally faster than very large unions of intersections in pathological cases, the kind library authors hit. At a single-app SaaS scale, the difference doesn't matter.
...
<VideoCallout videoId="e0AIkYrXAYE" videoTitle="Types vs Interfaces: What I Got Wrong In 2022 by Matt Pocock">
  Matt Pocock revisits his own type-vs-interface takes (10 min) and lands on the same evidence this section does: the single-object choice is cosmetic, and the only thing that matters at scale is intersections vs `extends`. He flips to an interface-first default while the course keeps `type`-first, but the reasoning he walks through is the part worth watching.
</VideoCallout>
...
<CodeVariants>
  ...
  <CodeVariant label="name: string | undefined">
    ...
    `name` must be present in the literal, even if its value is `undefined`. `'name' in user` is `true`. `Object.keys(user)` returns `['id', 'name']`. `JSON.stringify(user)` still produces `'{"id":"1"}'` because `JSON.stringify` drops `undefined` values, which is a surprise of its own.
  </CodeVariant>
  ...
  <CodeVariant label="name?: string | undefined">
    ...
    The combined form accepts both absence and an explicit `undefined` value. It's the most permissive shape, and under the project's default tsconfig it's what TypeScript treats `name?: string` as equivalent to.
  </CodeVariant>
</CodeVariants>
...
This is the field-level rule, and it's where most engineers form a wrong intuition. Walk through it line by line.
...
<CodeVariants>
  ...
  <CodeVariant label="Immutable array — readonly lines: readonly InvoiceLine[]">
    ...
    The array type is now `readonly InvoiceLine[]`. The mutating methods disappear from its surface, while the read methods (`.map`, `.filter`, `.find`, indexed read) stay. The push now errors at compile time, instead of after a stale-render incident.
  </CodeVariant>
</CodeVariants>
