...
That divergence is the heart of this whole lesson. Once you have seen it, "store colors in OKLCH" stops being a style preference and becomes the obvious choice for any system where you intend to derive variants by moving lightness.
...
<VideoCallout videoId="I9zHX-jSKpA" videoTitle="A deep dive into CSS color-mix()">
  Kevin Powell's deep dive on `color-mix()`, covering percentages, the interpolation spaces, and building lighter and darker schemes, runs about 18 minutes.
</VideoCallout>

:::caution
The interpolation space is mandatory, not optional. There is no such thing as a `color-mix()` without an `in <space>` keyword, and reaching for `in srgb` out of habit gives you the muddy midpoint. In this project the default is `in oklch`, so an `in srgb` in a code review is worth a comment.
:::
...
**The space here is `oklab`, not `oklch`.** Read that carefully, because it is a real distinction and not a typo. Tailwind's internal alpha mix uses `in oklab`, while the manual mixer you write for tints uses `in oklch`. <Term definition="The Cartesian form of the same color space behind OKLCH; it has no separate hue axis, which makes it ideal for straight-line mixes like fading to transparent.">OKLAB</Term> is the same color model as OKLCH but laid out on straight axes with no separate hue channel, which makes it ideal for a straight-line fade. The difference doesn't bite here because when you mix toward `transparent`, there is no second hue to preserve, so `oklab` and `oklch` produce the same visual result for the alpha case. The two spaces matter when you are blending two real colors; they are interchangeable when one side is transparent.
...
:::caution
`bg-transparent` is not `bg-none`. `bg-transparent` sets the background *color* to transparent; `bg-none` removes a background *image*. They are unrelated utilities that read like synonyms, so reaching for the wrong one is a common source of confusion.
:::
...
The Tailwind side fits in cleanly. The `dark:` variant compiles to a class-based selector, the `@custom-variant dark` rule from the dark-mode lessons, wrapped so it carries `.dark`'s single-class specificity and never fights utility ordering. So the markup below needs no `dark:` at all: you write `bg-card text-foreground` *once* and both themes work, because the tokens flip underneath. The only `dark:` you should still reach for is a genuine one-off, like a single shadow or overlay that needs a different value in dark mode.
...
**The hex-is-read-only rule.** This is the closing discipline of the lesson. Do not ship hex literals in new code, because OKLCH is the storage form for color. The only color keywords that compile to themselves, and so are always fine to write, are `transparent` and `currentColor`. You will still *read* hex constantly, in legacy files and copied snippets, and that is fine. The rule is about what you *write*.
