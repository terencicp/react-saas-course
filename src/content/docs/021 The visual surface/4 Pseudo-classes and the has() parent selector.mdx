...
Start with the five the browser sets from how the user is *interacting* with an element. You half-know most of them already, so the goal here is to land precisely which one fires when, because the differences between them are exactly where the bugs live.

Take the simplest ones first.

`:hover` matches while the pointer is over the element. You know this one. It quietly does nothing on a touchscreen, since there is no pointer to hover, which has consequences we will come back to at the end.

<Term definition="Matches an element while it is being pressed right now — mouse button or finger down — and releases the instant you let go.">`:active`</Term> is the pressed-feedback state, the tiny acknowledgement that a tap registered. It holds only while the press is down and lets go the moment you do. The canonical reach is `active:scale-[0.98]`, a barely-perceptible shrink that makes a button feel physical. That `scale-*` is a transform, and the *motion* layer that animates these is the subject of the next lesson. For now, just notice that `:active` is the state it hangs off.
...
So here is the rule, and it carries straight from the borders-and-elevation lesson earlier in this chapter: **`focus-visible:` is the default, and bare `focus:` is the bug.** This matters even more than it sounds, because Preflight, the reset you met in the cascade chapter, strips the browser's default focus outline to give you a blank canvas. Something has to put a visible focus state *back*, and that something is `:focus-visible`. Leave it out and you have an app no one can keyboard through.
...
This is app code, so it uses the semantic tokens, `bg-accent` and `ring-ring`, that your theme defines the way the earlier color and tokens lessons set up. The live exercises in this lesson swap in literal colors like `bg-slate-100` and `ring-blue-500`, because the sandbox can't load your project's theme; the shapes are identical, and only the color source differs. Notice what each piece owns. The `ring-2 ring-ring/50 ring-offset-2` and the `outline-none` are the focus-ring *geometry*, what the ring is physically made of and why it doesn't shove the layout around, which is the territory of the borders-and-elevation lesson. This lesson owns the other half: `focus-visible:` itself, the pseudo-class that decides *when* any of that paint appears.
...
There is a third you'll see named but rarely touch: `::file-selector-button`, the button inside an `<input type="file">`. Know it exists, and move on.

The placeholder bug is invisible until you see it live, so see it. The input below has *no* placeholder styling, so notice how the placeholder reads like real, typed text. Add the fix so it recedes to a proper hint.
