...
:::note
`text-base` is the body default: 1rem, the size running text should be unless you have a reason to change it. The scale is a system, not a menu, and staying on it is what keeps a hundred screens looking like one product.
:::
...
<Term definition="The horizontal spacing between letters. Set in CSS via letter-spacing; in Tailwind via the tracking-* utilities.">Letter-spacing</Term> is controlled by `tracking-*`, and it follows size in the opposite direction. Large headings look better with the letters pulled slightly together, using `tracking-tight`, or `tracking-tighter` for very large display type, because at big sizes the default gaps between letters start to look loose. Body text wants the normal default, so don't touch it. The one place you open spacing *up* is small all-caps labels, the little "OVERVIEW" or "SETTINGS" eyebrow above a section, where `tracking-wide` or `tracking-widest` keeps the capitals from crowding. (Negative tracking for display type is available through the bracket form, e.g. `tracking-[-0.04em]`; reach for it only on genuinely large headlines.)
...
<CodeVariants>
  ...
  <CodeVariant label="Body">
    ...
    **Roomy, default tracking.** Running text reads best with a little more line-height (`leading-relaxed`); letter-spacing stays at the default, since body copy never wants tracking touched.
  </CodeVariant>
...
Before the reflex, here's the honest support picture, because the two utilities are at different stages:
...
Two watch-outs belong right here, because this is where they come up.
...
:::caution
Match the tool to the block. `text-balance` is for *short* headings: the browser caps how many lines it will balance, so it does nothing useful on a long paragraph. `text-pretty` is the body-copy tool. Reaching for `balance` on a paragraph is a no-op, and reaching for `pretty` on a one-line heading is wasted.
:::
...
**Numbers and mono.** `font-mono` switches to the monospace stack for code and anything that should align character-by-character. And `tabular-nums`, a reflex you'll thank yourself for, sets `font-variant-numeric: tabular-nums`, which forces every digit to the same width.
