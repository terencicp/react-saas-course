...
The common instinct is to install an animation library, usually Framer Motion. That is more machinery than the job needs: another dependency, and animation running in JavaScript on the same <Term>main thread ((The single browser thread that runs your JavaScript and paints the page; busy work there freezes the interface.))</Term> that keeps your app responsive, all to fade in one dialog the platform can already animate for you.
