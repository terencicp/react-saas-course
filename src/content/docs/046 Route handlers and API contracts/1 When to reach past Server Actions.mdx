...
Recall from the envelope that the action is reachable only from React on this same app, and its action ID is an internal bundle artifact, not a public contract. The moment the caller is anything else, you need a route handler. Public REST endpoints and your <Term definition="Backend-for-Frontend — a thin server-side endpoint that proxies or aggregates third-party services for your own frontend, wearing your app's auth shape.">BFF</Term> surfaces both live in `route.ts`. This is the most common trigger you'll hit in a maturing SaaS.
...
<Figure caption="Same five seams; only the last two rows change shape. Parse, authorize, and mutate are identical on both sides, because a route handler is a public, untrusted-input boundary with the same trust posture as the action. Only revalidate and return take a different wire format.">
  <SeamMapping />
</Figure>
