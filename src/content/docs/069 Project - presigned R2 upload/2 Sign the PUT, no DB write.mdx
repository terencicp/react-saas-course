...
**`member`, not `admin`.** The role pin is the defense, not a convenience. The R2 credentials in `lib/r2.ts` are app-wide; nothing about the SDK call distinguishes one org from another. The only gate is `authedAction('member', …)`, which `requireOrgUser` enforces before the body runs — the same role-pin pattern from [The authedAction wrapper](/057-roles-action-wrappers-audit-trail/2-the-authedaction-wrapper/). Anyone below `member` gets a `forbidden` Result and never reaches the sign.
