...
**`ContentLength` is signed but R2 won't enforce it.** You pass the claimed size into the command so the SDK produces the canonical signed shape, but R2 does not reject an actual PUT that exceeds the signed length — this is the same quirk covered back in [Presigned URLs](/068-object-storage-with-r2/3-presigned-urls--signing-the-upload-seam/), where the real size enforcement is deferred to the post-upload HEAD. So treat this line as documentation of intent, not a guard. The guard lands next lesson, when `finalizeUpload` HEADs the stored object and reads the true byte count.
...
**`member`, not `admin`.** The role pin is the defense, not a convenience. The R2 credentials in `lib/r2.ts` are app-wide; nothing about the SDK call distinguishes one org from another. The only gate is `authedAction('member', …)`, which `requireOrgUser` enforces before the body runs — the same role-pin pattern from [The authedAction wrapper](/057-roles-authorization-audit-logs/2-the-authedaction-wrapper/). Anyone below `member` gets a `forbidden` Result and never reaches the sign.
...
**`validation` is mapped at the boundary, not in your body.** You'll notice the body has no error handling for a bad content type or an over-cap size. It doesn't need any. `authedAction` runs `schema.safeParse` before calling your function, and on failure returns `err('validation', …)` itself — the [Result](/043-server-actions/3-result-or-throw/) shape and the [Zod FormData boundary](/042-zod-4-schema-validation/6-coercing-formdata-strings/) you've used since the forms work. That's why requirements 2 and 3 short-circuit with no R2 call: a disallowed type or an over-cap claim fails the parse, and your `getSignedUrl` line never runs.
