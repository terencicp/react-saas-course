...
**The render does not audit, on purpose.** There is no `file.list_viewed` event in the system, and there shouldn't be — auditing belongs to actions and tasks, the things that *change* state, on the [append-only audit log](/057-roles-action-wrappers-audit-trail/5-the-append-only-audit-log/) discipline. A read surface that wrote an audit row per render would write twenty rows for one page view and turn the trail into noise. The file audit trail starts and ends with `finalizeUpload`; this page contributes nothing to it.
