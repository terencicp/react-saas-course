...
If the library publishes `string | undefined` and you augment to `string` to skip the narrowing, the augmentation is lying: the runtime value can still be `undefined` and your code will crash. Augmentation is for **adding** fields or **narrowing with a runtime guarantee**, like the Drizzle join. Erasing the library's safety isn't a valid use case; it only hides a crash that will still happen. Narrow at the call site with a real check instead.
