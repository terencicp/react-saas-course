...
Three rules fall out of that picture. Each one is short, and each one carries weight you will lean on for the rest of the course.
...
**Errors short-circuit upward.** A `throw` at the top of a leaf module prevents every upstream consumer from finishing. If `env.ts` throws because a required environment variable is missing, `db.ts` never gets its imports, `auth.ts` never runs, and your page never renders. That is exactly what you want: it is the mechanism behind <Term definition="A startup pattern where invalid configuration crashes the process immediately, so bad state never reaches a request.">fail-closed</Term> startup validation.

## Live bindings: imports point at the exporter's variable

Next, look at what the import statement actually gives you. The answer surprises most people the first time they see it, and it is the least obvious rule in ES modules. You need it clear in your head before the circular-dependency story makes sense.

An `import` is **not a copy**. It is a read-only window onto the exporter's variable. When the exporter mutates the variable, the importer sees the new value. This is not because some framework wired up an observer; it is what the ES module spec says happens at the binding level.

The clearest way to see it is to compare what actually happens with the mental model most newcomers bring.
...
<CodeVariants>
  ...
  <CodeVariant label="Value-copy intuition (the wrong mental model)">
    ...
    If imports were value copies, the second log would still be `0`. That mental model is wrong: ESM passes references at the binding level, not values. The red-marked lines mark a snapshot point that does not actually happen with a real import. A `const consumerCount = count` would freeze a number, but `import { count }` keeps a window onto the exporter's slot.
  </CodeVariant>
</CodeVariants>
...
**The importer cannot reassign the binding.** Inside the consumer, `count = 5` is a compile-time error. Only the exporter writes; everyone else reads. That is what "read-only window" means in practice, and the runtime enforces the asymmetry.

One note on the legacy contrast. CommonJS `require()` reads `module.exports` once and copies it. If you have seen that behavior in older Node code, set it aside, because modern ESM does not work that way.

## Circular dependencies: when the graph loops back on itself

A cycle in the import graph is the live-binding story applied to a graph that loops back on itself. Once you understand evaluation order and live bindings, circular dependencies stop being mysterious. They are predictable: there is a clear set of cases that crash and a clear set that resolve.

### When a cycle crashes

A cycle crashes when one module reads another's export at the top level before that export has been assigned. Two files import each other, and each one tries to read the other's export immediately rather than from inside a function.
...
Walk the runtime through it. Some entry module imports `a.ts`. `a.ts` starts evaluating and immediately reaches `import { fromB } from './b'`. The runtime starts evaluating `b.ts`. `b.ts` reaches `import { fromA } from './a'`. The runtime sees that `a.ts` is mid-evaluation, so its `fromA` export has not been assigned yet, and returns the **partial** module to `b.ts`. `b.ts` reads `fromA` and gets `undefined`. The arithmetic coerces `undefined` to `NaN` and produces `NaN`. The runtime detects the cycle, but the bug is the partial read.
...
When a value-level cycle does appear, the structural fix is to pull the shared symbol into a third module that neither side imports as a value. The cycle becomes a Y-shape: both `a.ts` and `b.ts` depend on `shared.ts`, and neither depends on the other. The runtime evaluates `shared.ts` first, then `a.ts` and `b.ts` in either order. Compare the two layouts:
...
This Y-shape pattern shows up again in Drizzle's relations API, where one shared file holds the relation declarations both tables reference, and you will see it used deliberately when the data layer arrives.

## Deferred edges: dynamic `import()` and code splitting

The previous lesson introduced `import()`, the function-call form rather than the statement, as a value-level expression that returns a `Promise<Module>`. This section covers the **bundling consequence**, which is the part that actually pays for itself.
...
One point worth getting straight: `await` is not what created the chunk. The bundler sees the `import()` form at build time and splits on that. Adding `await` in front of a static import does not split anything. People do attempt that, and now you can name why it does not work.
...
- **Conditional.** A locale-specific date module loaded based on the user's locale. A static import would force every locale into every bundle; the dynamic import loads just one.
...
Now the 800KB story from the opening makes sense. A single file, written without thought to the bundle boundary, can pull the entire server world into the browser the moment one client component imports a single harmless-looking helper from it. The fix is structural: split the file by responsibility, and let `'server-only'` enforce the cut.
...
<CodeVariants>
  <CodeVariant label="Before — one file, two responsibilities">
    ...
    A client component imports only `formatDate`. The bundler still walks every reachable edge, including `db`, `headers`, and the Drizzle relations graph, and pulls the server world along. The client bundle balloons with code the user never executes. The red-marked lines are the server-only seams that have no business being reachable from the browser.
  </CodeVariant>
  ...
  <CodeVariant label="After — split by responsibility">
    ...
    Same surface, split by responsibility. The client component imports `lib/format.ts` and never reaches `lib/auth.ts`. If some future client file ever tries to, the `'server-only'` import on the green-marked line fails the build with an error pointing at the import chain that leaked.
  </CodeVariant>
</CodeVariants>
