...
Everything so far has been about *correctness*: the right rows, in the right order, no skips or duplicates. But correctness rests on a performance condition that must ship alongside the query. Cursor pagination is fast **only** when an index covers the cursor's columns in the cursor's sort direction. Without it, "seek to the boundary" degrades into a <Term definition="Reading every row in a table in order, with no index shortcut to the matching rows. The slow path the next chapter's indexes exist to avoid.">sequential scan</Term>, and a cursor that scans is *slower* than the offset it replaced: all the complexity, none of the payoff.
