...
This is the full drop: reserve it for statements with no row shape worth typing or none the builder can express, like refreshing a <Term definition="A query whose result is computed once and stored on disk like a table; refresh recomputes it. Fast to read, stale until refreshed.">materialized view</Term>, a one-off maintenance `update`, or a migration helper.
