...
Sometimes both sides of a join are the *same* table, a <Term definition="A join where both sides are the same table, used to relate rows to other rows in that table (a reply to its parent comment).">self-join</Term>. Your schema already has this shape: a comment's `parentId` points back at another comment's `id`, so a reply and the comment it replies to both live in `comments`.
