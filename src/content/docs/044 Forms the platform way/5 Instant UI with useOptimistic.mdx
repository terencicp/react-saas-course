...
<StateMachineWalker kind="decision" title="Optimistic or pending?">
  ...
  <Leaf id="pending-not-watching" verdict="Pending state — useActionState / SubmitButton">
    The failed condition is *visible to the user*. The user isn't looking at the change, so an early frame buys nothing. A spinner that confirms completion is the honest signal here, as in a background autosave or a delete behind a long undo window.
  </Leaf>
  ...
  <Leaf id="pending-large" verdict="Pending state — useActionState / SubmitButton">
    The failed condition is *small UI change*. A large UI change is expensive to render optimistically and jarring to snap back. The perceived-speed win isn't worth the flicker when it reverts.
  </Leaf>
</StateMachineWalker>
...
`addOptimistic` only works **inside a <Term definition={"A React update React can interrupt and coordinate, so urgent work can preempt it. Optimistic state lives only inside one — opened by startTransition, or automatically by a form's action / a button's formAction prop."}>transition</Term>**, a React update that React is allowed to interrupt and reorder against more urgent work. Optimistic state is tied to that transition's lifecycle: when there's no transition open, there's no place for the optimistic value to live.
