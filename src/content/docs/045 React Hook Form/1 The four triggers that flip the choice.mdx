...
<StateMachineWalker>
  ...
  <Leaf id="leaf-arrays" verdict="Reach for RHF — useFieldArray">
    `useFieldArray` owns the add / remove / reorder bookkeeping and the per-row error tracking. It has its own lesson later in this chapter.
  </Leaf>

  <Leaf id="leaf-wizard" verdict="Reach for RHF — FormProvider">
    `FormProvider` carries one form instance across every step's component without prop-drilling. It's the chapter's final lesson.
  </Leaf>
...
The phrase doing the work there is <Term definition="The line past which data from a less-trusted zone, here the browser, must be re-validated before the system acts on it.">trust boundary</Term>: client-side validation is for the person filling in the form, server-side validation is for the system, and the two are not interchangeable.
