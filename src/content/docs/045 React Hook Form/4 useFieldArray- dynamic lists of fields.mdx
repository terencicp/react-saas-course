...
That is four bookkeeping jobs for *one* conceptual thing: a list of rows. None of them is about the invoice; all of them are about forcing a flat, string-keyed form to hold a nested, growable structure. This is the threshold the chapter named: when one form owns a set of repeated rows whose count the user controls, the native pattern's coordination cost is no longer worth paying, and <Term definition="The React Hook Form hook that manages a variable-length array field: it owns the rows' identity and ordering and coordinates their re-renders, so add / remove / reorder stops being manual bookkeeping.">`useFieldArray`</Term> is the standard tool to reach for. It owns all four jobs, the array structure, the identity keys, the re-indexing, and the per-row error wiring, behind one hook call.
...
`useFieldArray` does not create a form. It plugs into the one you already have. You hand it the `control` off the `form` object the page created back in the [primitives lesson](/045-react-hook-form/2-the-five-primitives--useform-register-controller-handlesubmit-formstate/), name the array field it should manage, and it returns an array to render plus a set of operations to mutate that array. Here is the whole call:
...
<Aside type="caution">
Always pass `append` (and `prepend` / `insert`) the **complete default row**, with every field the schema declares: `append({ description: '', amount: 0 })`, never `append({})`. An empty object gives the new inputs no value to start from, so they render uncontrolled and flip to controlled the instant the user types. That is the same uncontrolled-to-controlled warning `defaultValues` exists to prevent, now firing one row at a time.
</Aside>
...
Three details in that block carry weight. The format builders follow the Zod 4 conventions you have been using: top-level `z.uuid()`, not the deprecated string chain. The amount uses `z.coerce.number<number>()`, the same generic-pinned coercion from the [resolver lesson](/045-react-hook-form/3-zodresolver--one-schema-both-sides-of-the-wire/): it accepts the string an `<input>` produces, produces a `number`, and the pinned generic keeps the *input* type a clean `number` so `defaultValues` and the registered field type-check. And the `.min(1, 'Add at least one line')` on the array is the "you can't ship an empty invoice" rule.
...
The form now edits a list. The last question is what the *server* does with it, and this is where the `id?` field you added to the schema earns its place. The framing is this: the form owns the row set the user wants, and the action <Term definition="To compute the INSERT / UPDATE / DELETE writes by comparing the submitted list against the rows currently in the database. The action's job after it parses the submitted array.">reconciles</Term> that set against what is currently in the database. The submitted list is the new source of truth, and the action makes the database match it.
...
<Aside type="note">
Past a few hundred rows, a single client form is the wrong shape, not because RHF gets slow but because the *architecture* does. Every row ships to the client, lives in browser memory, and submits as one payload. At that scale the right reach is a different UI entirely: a paginated list or a bulk-edit table with row-level saves, where the client only ever holds one page. That is a list-and-table pattern from later in the course, not a `useFieldArray`. Recognize the threshold: reach for `useFieldArray` for the bounded list a single form should own, and reach past it when the list is really a dataset.
</Aside>
