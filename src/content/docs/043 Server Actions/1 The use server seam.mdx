...
This builds on the `'use server'` directive from [Directives and server-only enforcement](/030-the-server---client-boundary/3-directives-and-server-only-enforcement/). You will leave with an empty `createInvoice` skeleton that the rest of this chapter fills in, one seam at a time.
