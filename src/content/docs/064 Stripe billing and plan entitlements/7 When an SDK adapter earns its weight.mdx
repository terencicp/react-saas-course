...
By now you've wrapped two of the third-party libraries this app depends on. Billing got an interface in the last lesson, so app code calls `billing.upgrade`, never `stripe`. Authorization got one earlier: every privileged action goes through [`authedAction`](/057-roles-action-wrappers-and-the-audit-trail/2-the-authedaction-wrapper), never the raw auth check. You've also left at least one library *un*-wrapped: when the app sends a welcome email it calls `sendEmail`, a thin convenience function that reaches straight for Resend.
...
<CodeVariants>
  <CodeVariant label="Helper — sendEmail" icon="lucide:arrow-right">
    ...
    ```
    **The SDK lives _inside_ the function, and stays importable everywhere else.** `sendEmail` adds one pre-flight step, a suppression check, then calls `resend` directly. Another file is free to `import { resend }` and call it too, so the two coexist. This is a convenience, not a boundary.
  </CodeVariant>
  ...
  <CodeVariant label="Interface — billing.upgrade" icon="lucide:shield">
    ...
    ```
    **App code calls `billing.upgrade(planSlug)` and gets back a URL; it never imports `stripe`.** The Stripe client is constructed once inside `/lib/billing/` and is unreachable from anywhere else. The only path to Stripe is through this surface, and that's what makes it a boundary rather than a convenience.
  </CodeVariant>
</CodeVariants>
