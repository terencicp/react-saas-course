...
The walker below shows every state, every transition, and the Stripe event (like `invoice.payment_failed`) that triggers each move. Those events are the webhook signals you ingested in the [webhook ingestion chapter](/063-webhook-ingestion/1-verify-before-parse); this is the other side of that pipe. For each state, read what it *means* and, more importantly, **what your app does** while a subscription sits there.
