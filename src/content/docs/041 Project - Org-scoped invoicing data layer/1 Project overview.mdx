...
- Writing a deterministic, idempotent seed: `reset()` plus inserts from a fixed-seed <Term definition="Pseudo-random number generator: an algorithm whose 'random' stream is fully determined by a starting seed, so the same seed replays the same sequence.">PRNG</Term>, so every run produces byte-identical data.
