...
Walk through the two lifetimes, starting with the top lane.
...
You now hold two caching tools. Given a function, which one do you put on it?
...
:::note
`cache()` is not `useMemo`. `useMemo` is a client-side hook that lives in the React fiber and exists for client-render work. As you saw when you met the React Compiler earlier, the compiler handles that memoization for you, so you rarely write `useMemo` by hand anyway. `cache()` is a server-side, module-scope primitive for deduplicating server work. They sit at different layers and solve different problems, so don't reach for `useMemo` to dedupe a server fetch.
:::
