...
The first option runs Postgres on your laptop inside Docker. Four words of Docker vocabulary are enough to read its config file; Docker is a large subject with its own course, and this is the minimum to follow along.
...
The second option runs nothing on your laptop. You create one development branch on a Neon project, free while idle, and the console hands you a connection string to paste into `.env.local`. No compose file, no container, no `docker` command: the copy-paste *is* the setup.
