...
Your whole stack already agrees on this. Stripe's API takes `amount` as an integer in the currency's minor units: `1000` charges ten dollars, `10` charges ten yen. Unit 5 stores it the same way, in an integer cents column, so a value round-trips from JS to Postgres and back without touching a float.
