...
The takeaway goes beyond "watch out for emojis." `.length` is a <Term definition={"A property of how the value is stored or transmitted, not of what the value represents. `.length` reports the storage form, not the human-meaningful count."}>serialization detail</Term>: it tells you how the engine stored the string, not how a human reads it. The rest of this lesson covers the three counts you actually have available, and how a senior picks the right one for the job.

## Three counts, three reaches

There's no single "length of a string" in JavaScript. There are three, and each one answers a different question. Instead of asking "how long is this string?", the senior habit is to ask "do I need code units, code points, or <Term definition={"One character as the user perceives it on screen. Can span multiple Unicode code points — a base letter plus combining marks, an emoji built from a zero-width-joiner sequence, a regional-indicator pair forming a flag."}>grapheme clusters</Term>?"
...
The table shows the core pattern: each step up in fidelity (code units → code points → grapheme clusters) handles another class of input that the cruder count gets wrong. On ASCII all three counts agree, which is why `.length` looks fine on `'hello'` and breaks silently the moment a user pastes an emoji or an accented character.
...
The companion granularities are worth recognizing even if you rarely reach for them. `granularity: 'word'` is the right tool for word-count features; each segment carries an `isWordLike` flag so you can skip punctuation and whitespace. `granularity: 'sentence'` does the same for sentence-count features. Both are rare in SaaS UIs, but naming them here means you'll recognize `Intl.Segmenter` as the one tool that handles all three jobs when you see it elsewhere.
