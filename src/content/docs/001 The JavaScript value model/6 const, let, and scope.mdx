...
<ScriptCoding
  runner="vanilla"
  instructions="Run the snippet as-is. The first line accesses `x` before its `const` declaration on the next line; the engine refuses. The test asserts that access throws a ReferenceError."
