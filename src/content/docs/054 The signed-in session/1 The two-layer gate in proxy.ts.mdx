...
<VideoCallout videoId="zNgCFXZLoRk" videoTitle="Next.js 16 Middleware DEPRECATED — Authentication In Proxy Or Data Access Layer?">
  ByteGrad walks the exact split this lesson hangs on — why real auth doesn't belong in `proxy.ts` and why the data access layer is the authoritative check (23 min).
</VideoCallout>
