...
<DiagramSequence>
  ...
  <DiagramStep>
    <DesugarPanel step={2} />
    <Fragment slot="caption">
      A new shape, the same move: an attribute selector. `data-[state=open]:` becomes `&[data-state="open"]`, where the bracket carries the attribute and its value. Only the selector changed.
    </Fragment>
  </DiagramStep>
  <DiagramStep>
    <DesugarPanel step={3} />
  ...
  <DiagramStep>
    <DesugarPanel step={4} />
    <Fragment slot="caption">
      And *down*. `has-[:invalid]:` becomes `&:has(:invalid)`, where the element styles itself based on a descendant it contains. Four families, one idea: a selector wrapped around the utility.
    </Fragment>
  </DiagramStep>
</DiagramSequence>

Hold onto that sequence. Steps 2 through 4 are the three things you're here to learn, and they're all the same move: a selector, wrapped around a utility, reading some state. Once you read `data-[state=open]:` as "the rule `&[data-state="open"]`," there's nothing left to memorize; you're just writing CSS selectors with a shorter syntax.
...
**The arbitrary form** `aria-[attr=value]:` covers everything else: value-bearing attributes, and any attribute that doesn't have a shorthand. Here's the part that catches people. `aria-current` and `aria-invalid` have **no** built-in shorthand. Having just learned `aria-expanded:`, it's natural to assume `aria-current:` exists by analogy. It doesn't, and `aria-current:font-semibold` silently compiles to nothing: no error, no style. You write the arbitrary form instead: `aria-[current=page]:font-semibold` and `aria-[invalid=true]:border-destructive`. `aria-current` has no shorthand because it isn't really a boolean. Its value can be `page`, `step`, `location`, `date`, `time`, or `true`, so there's no single case for a shorthand to target.

One more point: **ARIA values are strings, not booleans.** `aria-expanded` is the string `"true"` or `"false"`, never a JavaScript `true`. The built-in `aria-expanded:` shorthand matches `="true"` specifically; if you ever need to style the false case, that's the explicit `aria-[expanded=false]:`. This trips people who expect JS truthiness to apply, since these are attribute strings, not booleans.
...
Try the reveal pattern now. The card below contains a title and a "Delete" button that starts fully transparent. Mark the card so it's a group, and give the button a class that brings it to full opacity when the card is hovered, to match the target, which shows the button revealed.
...
Mark an element with the `peer` utility, and a *later* sibling can read its state with a `peer-` variant: `peer-invalid:`, `peer-checked:`, `peer-focus:`, `peer-placeholder-shown:`, `peer-disabled:`. The constraint, stated up front because it's the thing people get wrong: **the `peer` reader only sees a `peer` source that comes *before* it in the markup.** Source order matters. This isn't a Tailwind choice but a CSS limitation: the variant compiles to the subsequent-sibling combinator `~`, and `~` only looks *forward*. So if you mark an input `peer`, the element that reads it has to appear *after* the input in your JSX.
...
:::caution
`peer` is single-direction. If your markup forces the message *before* the field, say a label above an input, `peer` can't reach it, because `~` only looks forward. Restructure so the field comes first, or fall back to another approach. This is the one constraint that will actually stop you.
:::
...
This is where the "whose state?" progression pays off the most: `has-` deletes a large class of state-mirroring outright. The parent no longer needs to be *told* what changed inside it, because it reads the change itself.

The radio-card exercise is the strongest demonstration in the lesson that DOM state alone can drive the UI. Below are three plan cards, each a `<label>` wrapping a radio input and its text. Add classes so that the card containing the *checked* radio highlights, with a colored border and a tinted background. Then click between the radios and watch the highlight follow. None of the code you wrote is running when you click; the DOM is doing all of it.
...
Three smaller families round out the surface. The goal with these is recognition, not drill.
...
Everything in this lesson served one reflex. Now it's time to make it explicit and turn it into a decision you can run on every state-driven style you'll ever write.

**Every state-driven style change starts with one question: can the DOM already tell me this?** If the change is driven by hover, focus, validity, a checked input, a disabled control, a library's `data-state`, an ARIA attribute, or a parent, sibling, or descendant's state, you write a variant: no state, no handler, no re-render. If the value comes from the server instead, or is a computed or derived value that no selector could ever match on, *then* React state, with a conditional class composed through the `cn()` helper you met earlier in this chapter, is the right tool. That's the fallback path, not the default.
...
<StateMachineWalker kind="decision" title="Read state, or mirror it?">
  ...
  <Leaf id="leaf-peer" verdict="peer-*">
    Mark the earlier sibling `peer`; a *later* sibling reads it with `peer-invalid:`, `peer-checked:`, … Source order is load-bearing, since `peer` only reaches forward.
  </Leaf>

  <Leaf id="leaf-has" verdict="has-[…]:">
    The parent reads its own descendants with `:has()`: `has-[:invalid]:`, `has-[:checked]:`. It's the one selector that reaches downward, and the one that deletes the most state-mirroring.
  </Leaf>
...
That `useState` lands later in the course is deliberate. You're learning the no-state path *first*, so it becomes your reflex, and state arrives as the exception you reach for with a reason, not the tool you reach for by default.
...
Read the buckets you sorted into "DOM already knows" again. Every one of them is a style you simply don't write the machinery for: no boolean, no handler, no effect keeping things in sync. That's the dividend the question pays, every time you remember to ask it. When you start pasting in real components in a later chapter and find them studded with `data-[state=open]:`, `aria-[invalid=true]:`, and `peer-disabled:`, you'll read them as exactly what they are: a parent, a sibling, or an element reading state that was already true.
