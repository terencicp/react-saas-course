...
:::note
The two seams this finding leans on each have a lesson that owns them. The canonical event set, the transaction discipline, and the `entity.verb-pasttense` naming convention come from [the audit-log policy lesson](/081-the-security-baseline/3-the-audit-log-policy/). The `logAudit(tx, event)` writer and its transaction-first signature come from [the append-only audit-log lesson](/057-roles-action-wrappers-audit-trail/5-the-append-only-audit-log/). The finding cites them; it does not re-teach them.
:::
