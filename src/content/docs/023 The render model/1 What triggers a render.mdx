...
:::note
You may see React's own docs describe only *two* reasons a component renders: the initial render, and "the component's or one of its ancestors' state has been updated." That's the same rule in compressed form, not a different one. An ancestor re-rendering and a context change both come down to some state somewhere above this component updating, and the update reaching it. The three-trigger split is the practical lens for debugging, and the docs fold those cases together into two.
:::
