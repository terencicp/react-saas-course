...
The reason per-step accounting exists at all is in that orange step. <Term content="Counting resource use — here, tokens — per unit of work so you can enforce a quota or bill against it.">Metering</Term> that only happens in `onFinish` bills the user *after* the cost is incurred. A loop that's about to run away has already run away by the time the aggregate arrives. Counting per step is how you stop it mid-flight.
