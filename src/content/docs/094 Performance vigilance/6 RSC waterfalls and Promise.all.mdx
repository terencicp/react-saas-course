...
<RequestTrace title="Render order is fetch order" url="/dashboard" phases="request,server-render">
  ...
  <Phase id="server-render">
    ...
    awaits serialize on render order alone, even though neither needs the other's data.
  </Phase>
</RequestTrace>
...
:::note
The deeper you would have to drill a hoisted prop, the more `cache()` wins. Hoisting one level is fine; threading a value through four components that ignore it is its own warning sign, and that is precisely when a module-level `cache()` beats the hoist.
:::
...
:::caution
One trap to watch for under streaming: the trace's **"Total" duration becomes misleading.** The request span can read 800ms while the user actually saw meaningful content at 50ms. When a page streams, read the *first-paint* metric, not the total, because the total is now measuring the slow tail, not the experience.
:::
...
Recall the cousin we opened with. This component-tree waterfall is the [N+1 query](/039-indexes-plans-and-transactions/2-spotting-n1/) one layer up: a list renders, each row is a child that awaits its own read, and the database sees N serial queries instead of one. Same shape, same family of fix: hoist the fetch and batch it into a single query, then pass the rows down. The SQL-side fix has its own lesson; the point here is to recognize it as the same bug.

:::caution
One more bottleneck to respect: a `Promise.all` that fans out over *many* reads can exhaust a small database connection pool, and now the fix for serialization has created a new wait. The next lesson covers bounded concurrency and pool sizing. For now, just know the trade-off exists so you don't quietly swap one failure for another.
:::
