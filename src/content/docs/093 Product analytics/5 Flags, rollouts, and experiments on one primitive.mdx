...
A <Term definition="A named decision, evaluated per user, that you can change remotely without shipping code.">feature flag</Term> is a named decision. Your code asks PostHog "what's the value of `new_onboarding` for this user?" and PostHog answers with `true`, or `'variant_a'`, or a config object. The targeting rules that decide the answer live in the PostHog dashboard. Your code never encodes the rule; it only reads the value.
...
<VideoCallout videoId="1X2gha80fsA" videoTitle="How to deploy UI changes with ZERO risk – PostHog Feature Flags Demo">
  PostHog ships code to prod behind a flag and toggles the UI on without a redeploy. It's the deploy-is-not-the-release idea in a 4-minute live demo.
</VideoCallout>
...
<Aside type="caution">
Bootstrapping flags means you evaluate per request, which forces the route into dynamic rendering: it can no longer be statically generated or cached with ISR. For an authenticated, per-user dashboard that's already the correct tradeoff, since it was dynamic anyway. But don't reflexively bootstrap flags on a static marketing page, or you'll silently lose its static optimization for no benefit.
</Aside>
...
<Figure expandable={false} caption="One identity feeds both evaluators. Because the server-side `posthog-node` evaluation and the client `posthog-js` SDK read the *same* `distinctId`, they compute the same variant, so there's no flip at hydration.">
...
<Aside type="caution">
Experiment readouts wobble. On any given day, before there's enough data, the readout can flicker "significant" purely by noise. If you stop the moment it goes green, you inflate your false-positive rate dramatically and declare winners that are nothing but luck. Wait for the sample size and duration you pre-declared. The cost of stopping early is that you ship the *losing* variant to 100% because it happened to look good on a noisy Tuesday, and you've now "proven" a change that did nothing, which is worse than running no experiment at all.
</Aside>
...
<VideoCallout videoId="C2ICJbICfsg" videoTitle="8 A/B testing mistakes every engineer should know">
  PostHog walks through eight ways experiments go wrong: peeking early, fuzzy hypotheses, missing counter-metrics. These are the process traps this section warns about, in six minutes.
</VideoCallout>
