...
Every event PostHog stores hangs off a <Term definition="PostHog's per-user, per-browser identifier. Every event is attached to one; it starts anonymous and gets linked to a stable user ID at sign-in.">distinct ID</Term>, the identifier introduced back when you wired the SDK, and now we'll spell out its full meaning. The lifecycle runs like this:
...
<VideoCallout videoId="LIJ_TuyMq74" videoTitle="Complete guide to event tracking - PostHog tutorial">
  PostHog's own 8-minute walkthrough wiring custom `capture` events, properties, `identify`, and `group` in a React app: the same moves this lesson just covered, seen end to end in the dashboard.
</VideoCallout>
...
<CodeVariants>
  ...
  <CodeVariant label="paywall_viewed (client)">
    ...
    **Pure event properties, fired in a handler rather than a render.** This is very likely the event the visitor fired *while still anonymous*, and tab 1's `identify` is what stitched it onto the now-known user. That stitch is why the funnel can start before signup.
  </CodeVariant>
  ...
  <CodeVariant label="plan_upgraded (server)">
    ...
    **The server-side close of the funnel.** There's no browser, so the distinct ID is looked up from the user row to keep the event on the real person. Bounded event properties only: `amount_cents`, never a formatted string.
  </CodeVariant>
</CodeVariants>
