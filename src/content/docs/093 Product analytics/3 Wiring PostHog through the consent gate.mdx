...
## A quick refresher: the two belts
...
One piece of vocabulary turns up in the env section. PostHog gives each project a <Term definition={"PostHog's public, write-only ingest key for a project. Safe to ship to the browser: it can send events but cannot read data back out. Distinct from a personal API key, which is read-capable and server-only."}>project key</Term>. It's safe in the browser by design, and that design decision is what the next section is about.
...
:::caution
Skip the deliberate pageview handling and the failure is quiet and confusing: client-side navigations either don't register at all or arrive doubled, your funnels look broken, and nothing errors to tell you why. Forget the `Suspense` boundary and the build fails outright on the `useSearchParams` reader.
:::
...
<CodeVariants maxLines={14}>
  ...
  <CodeVariant label="posthog-provider.tsx" icon="simple-icons:posthog">
    ...
    **The config changes.** `api_host: '/ingest'` sends events same-origin, through the rewrite. `ui_host: 'https://eu.posthog.com'` points the links the SDK generates, such as the toolbar and the replay viewer, at the real PostHog UI. Mind the dot: the ingest host is `eu.i.posthog.com` (with `.i.`), and the UI host is `eu.posthog.com` (no `.i.`). Set `api_host` without `ui_host` and the toolbar links break.
  </CodeVariant>
</CodeVariants>
