...
Let's name those three questions properly. They aren't one-time curiosities; they are the recurring questions every team running a SaaS asks from the first day of production onward.
...
<VideoCallout videoId="4pI8nBt98ys" videoTitle="Google Core Web Vitals: A Complete Guide For Beginners">
  DebugBear's 13-minute primer on LCP, INP, and CLS, and the field-versus-lab distinction this section turns on, with the 75th-percentile field data Speed Insights reports.
</VideoCallout>
...
Now the hands-on part, and it's short: two packages, two components, one dashboard toggle, one verification.
...
2. Add both components to your root layout, inside `<body>`, after `{children}`. This is the only code in the lesson, and it's walked through below.
...
4. Verify in production. Load a page on your live site and watch the dashboard. Give it time, because there's a first-load latency of roughly thirty seconds to a minute before the first data shows up, so an empty panel for the first minute is expected, not a bug. Once a view lands, you're done.
...
`}>
  <AnnotatedStep meta="{1-2}" color="blue">
    Both imports come from the Next.js-specific subpath of each package, the entry points built for the App Router. One import per capability, nothing more.
  </AnnotatedStep>

  <AnnotatedStep meta="{4}" color="blue">
    This is your existing root layout, and it stays a server component. You're not adding `'use client'` and you're not wrapping it in a provider, because each component handles its own client behavior internally.
  </AnnotatedStep>

  <AnnotatedStep meta={`{9-10} "<Analytics />" "<SpeedInsights />"`} color="green">
    Drop both in once, at the root, after `{children}`. Because every route renders inside the root layout, this single placement covers your entire app automatically, every page. There is nothing to add per route.
  </AnnotatedStep>
</AnnotatedCode>

:::note
In 2026, Vercel's dashboard tooling can perform this whole install for you end to end. We walk through the manual version anyway, because the point of this lesson is to know exactly what gets wired and why, not to watch it happen.
:::

## Why these two skip the consent gate

Here is the correction I promised at the start. You just spent a whole lesson in the security chapter learning that nothing non-essential fires before the user consents: every tracker routes through your `useConsent()` source of truth and stays dark until the user explicitly accepts. That lesson did its job a little too well. Having just installed two analytics tools, the natural next move is to wrap `<Analytics />` and `<SpeedInsights />` in that same gate. Do not do that.
...
1. **You added the package but never flipped the dashboard toggle.** The script ships, and the data goes nowhere. Enabling Web Analytics and Speed Insights in the project dashboard is a separate, required step.
...
You've shipped the floor. The last thing to take away isn't an install; it's knowing when to add the next tier, so you reach for it on purpose rather than out of anxiety.
...
The install is short, so the official quickstarts cover the wiring. Two deeper references back the two load-bearing ideas: why these tools skip the consent gate, and what the Speed Insights metrics actually mean.
