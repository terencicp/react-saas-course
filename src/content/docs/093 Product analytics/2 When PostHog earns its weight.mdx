...
<VideoCallout videoId="VBCYqp8l3Lc" videoTitle="How To Build Feature Flags Like A Senior Dev In 20 Minutes">
  Web Dev Simplified's 20-minute tour of feature flags, covering kill switches, percentage rollouts, and A/B tests. This is the second of the four needs, before any vendor enters the picture.
</VideoCallout>
...
<TabbedContent syncKey="one-vs-four">
  <TabbedItem label="Four vendors" icon="puzzle" caption="Four contracts, four SDKs in the bundle, and four identity models. The cross-tool question needs a join nobody maintains.">
    <OneVsFourSdks variant="four" />
  </TabbedItem>

  <TabbedItem label="One platform" icon="random" caption="One contract, one bundle, and one identity across all four primitives. The cross-primitive question is a single query.">
...
:::caution
Those numbers move, because PostHog adjusts the per-primitive tiers periodically. Treat the figures above as the shape ("generous free tier, billed per primitive"), not a contract. The pricing page is linked at the end of this lesson; check it for the current allowances before you commit a budget.
:::
...
<StateMachineWalker title="Do you reach past the cookieless floor?">
  ...
  <Leaf id="posthog-events" verdict="Add PostHog (Cloud EU)">
    The events need is your trigger: funnels, cohorts, retention. One platform covers this as well as flags, replay, and experiments when they arrive, so you're not re-deciding per need. Events are covered in depth in the next lessons; the consent gate and EU region apply to everything you capture.
  </Leaf>

  <Leaf id="posthog-flags" verdict="Add PostHog (Cloud EU)">
    Gradual rollouts and kill switches are your trigger. The same platform also gives you events, replay, and experiments on one identity, so there's no re-deciding later. Flags and experiments get their own lesson shortly; the consent gate and EU region apply.
  </Leaf>

  <Leaf id="posthog-replay" verdict="Add PostHog (Cloud EU)">
    The no-error bug-class is your trigger, where you need to see what the user actually did. The same platform folds in events, flags, and experiments on one identity. Replay gets its own lesson, with the masking catalog; the consent gate and EU region apply, and replay especially must be privacy-masked.
  </Leaf>
