...
<CodeVariants>
  ...
  <CodeVariant label="authedRoute — the HTTP door">
    ...
    **Returns a `Response` the HTTP client reads as a status code plus a body.** `201 Created` says "made it" in the vocabulary every HTTP client already speaks, with no envelope to teach a partner. Same `role`, same `schema`, same `ctx`; only the return type changed.
  </CodeVariant>
</CodeVariants>
...
:::caution
A tenant-scoped read that finds nothing for a *named* entity must return **404**, not an empty **200**. It's tempting to `Response.json(null)` or `Response.json([])` and call it a success, since the query didn't error. But a `GET /api/invoices/:id` for a row that isn't in your org returning `200 { }` tells the caller "this exists and is empty," which is wrong and leaks the shape of your data. No row for a named entity is a 404. This is precisely where one tenant quietly probes another's data without anyone noticing.
:::
