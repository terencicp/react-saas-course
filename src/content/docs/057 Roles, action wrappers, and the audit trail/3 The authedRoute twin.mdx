...
<VideoCallout videoId="XXagEUbrIc8" videoTitle="Your API Errors Suck (Here's How to Fix Them)">
  CodeOpinion makes the case for RFC 9457 from the consumer's side — why a `200 OK` carrying an error is the anti-pattern, and how the `type`/`title`/`status`/`detail` shape fixes it.
</VideoCallout>
