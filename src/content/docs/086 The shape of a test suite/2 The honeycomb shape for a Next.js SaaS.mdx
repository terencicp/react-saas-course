...
The second is **the <Term definition="The boundary where your application's own code meets an external system: the framework, the database, the auth layer, or a third-party API. Most SaaS bugs live here.">seams</Term>**, the places where your code stops being a pure function and starts talking to the outside world. These are your Server Actions wrapped in `authedAction`, your route handlers wrapped in `authedRoute`, your webhook receivers, your Drizzle query helpers, and your rate limiter `safeLimit`. A bug here isn't a wrong number; it's a query that returns another tenant's data, or a webhook that trusts an unsigned body.
