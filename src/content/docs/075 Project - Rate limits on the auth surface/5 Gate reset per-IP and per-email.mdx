...
The whole file. Set it next to the sign-in action from [Gate sign-in with dual-keying](/075-project---rate-limits-on-the-auth-surface/3-gate-sign-in/) and the import surface is identical: `resetLimiter` in place of `signInLimiter`, the same `getClientIp`, `safeLimit`, and `rateLimited`.
...
For the parts this lesson reuses but doesn't own — dual-keying, gate-before-work, the opaque message, and `safeLimit`'s fail-open — see [Gate sign-in with dual-keying](/075-project---rate-limits-on-the-auth-surface/3-gate-sign-in/) where they were first wired, and the Upstash primitives in [the rate-limiting chapter's dual-keying lesson](/074-rate-limiting-with-upstash-redis/3-rate-limiting-the-auth-endpoints/).
