...
Or drop `ANALYZE` and use plain `EXPLAIN` for an estimate-only plan that never touches the data. The hazard is real: run `EXPLAIN ANALYZE DELETE` in the wrong window and it deletes for real.
</Aside>
