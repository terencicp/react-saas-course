...
<VideoCallout videoId="K1a2Bk8NrYQ" videoTitle="Understanding B-Trees: The Data Structure Behind Modern Databases">
  Spanning Tree's 13-minute animation builds the sorted-tree structure under every B-tree index from the ground up, which is exactly the "shortcut" picture this section asks you to hold.
</VideoCallout>
...
That last line is the foreign-key trap from earlier, restated on purpose, because it is the one Postgres will not remind you about. You have to remember it yourself.
:::
...
Almost always, the answer is a <Term def="A balanced sorted tree; the default Postgres index type. It keeps keys in sorted order, which is what makes range scans and ordered reads fast.">B-tree</Term>, the default and the workhorse. Because it stores keys in sorted order, one B-tree serves an enormous range of query shapes: `=`, `<`, `<=`, `>`, `>=`, `BETWEEN`, `IN`, `IS NULL`, `IS NOT NULL`, and `ORDER BY` in either direction. Drizzle's plain `index(...).on(col)` gives you a B-tree. The mental rule is blunt and reliable: **if you are not sure, it is B-tree.** Roughly 95% of the indexes in a typical SaaS schema are B-trees.
...
<CodeVariants>
  ...
  <CodeVariant label="Partial index">
    ...
    **Indexes only the pending invoices.** When `pending` is 5% of the table, this index is roughly a twentieth of the size, updates only when a pending row changes, and serves the dashboard query just as fast. The non-selective column became selective once you carved out its hot slice. Note that the predicate is a raw `sql` template, not the `eq()` helper; the next paragraph says why.
  </CodeVariant>
</CodeVariants>
