...
The observability half of the audit is wired. Those gaps got fixed because they lose data, and lost data cannot be recovered after launch.

The performance half is different. The dashboard waterfall, the `lucide-react` barrel, and the N+1 in the invoice helper lose nothing: they render correct data, just heavier or later. A finding like that is slow, not bleeding, so you document it with measured impact and send it to the backlog instead of patching it on the spot. The one exception is the barrel import, which you fix in place to capture before-and-after evidence from the bundle analyzer.

:::note[Evidence you capture yourself]
Finding 006's evidence is a pair of analyzer treemaps you capture yourself, before and after the `next.config.ts` edit. Save them as `findings/screenshots/before-barrel.png` and `findings/screenshots/after-barrel.png`; the finding embeds both.
:::

## Your mission

Write the performance half of a launch-review audit report. Each finding names four things: the **rule** it violates (with the chapter 094 lesson that owns it), the **location** in source plus the diagnostic surface that exposed it, the **consequence** in operator- or user-visible terms, and the **fix**. Finding 7 is the template; you fill three more files to the same shape.

The contract is *document, don't patch*: the deliverable is findings, not code changes. The one exception is the barrel import, which you fix in `next.config.ts` because the bundle-analyzer treemap before and after **is** the required evidence. Every other defect stays in source.
...
The dashboard awaits four reads back to back:
...
<Figure caption="The authenticated /dashboard: the app-nav with its lucide icons, the Team panel, and the Recent invoices list — the surface the waterfall (005) and barrel-import (006) symptoms are read against.">
...
`}>
  <AnnotatedStep meta="{1-4}" color="violet">
    **Category** (RSC waterfall, chapter 094 lesson 6) and **Severity** with a two-line justification.
  </AnnotatedStep>
  <AnnotatedStep meta="{6-8}" color="blue">
    **Rule.** The dependency-check before every `await`: independent reads parallelize, a genuine data dependency stays sequential.
  </AnnotatedStep>
  <AnnotatedStep meta="{10-26}" color="orange">
    **Location.** The four-await chain, surfaced by the DevTools trace staircase then confirmed with `rg`: surface first, source second.
  </AnnotatedStep>
  <AnnotatedStep meta="{28-30}" color="red">
    **Consequence.** Operator-visible timing: ~320ms where ~240ms is reachable, the members read sitting as dead time.
  </AnnotatedStep>
  <AnnotatedStep meta="{32-49}" color="green">
    **Fix.** The `Promise.all` wraps the independent invoices+members pair *only*; `user → org` stays sequential. Wrapping all four is the half-credit trap.
  </AnnotatedStep>
</AnnotatedCode>

For the full diagnosis-and-rewrite treatment, see [RSC waterfalls and Promise.all](/094-performance-audits-and-fixes/6-rsc-waterfalls-and-promiseall/).
...
`}>
  <AnnotatedStep meta="{1-4}" color="violet">
    **Category** (bundle size — the barrel trap + the Turbopack analyzer, chapter 094 lessons 3/4) and **Severity** high: every authenticated page pays the weight.
  </AnnotatedStep>
  <AnnotatedStep meta="{6-8}" color="blue">
  ...
  <AnnotatedStep meta="{31-49}" color="green">
    **Fix.** The single `optimizePackageImports` line, plus the two embedded before/after screenshots that are this finding's required evidence.
  </AnnotatedStep>
</AnnotatedCode>

The two embedded screenshots are what the test checks for; produce them by running the analyzer before the config edit and again after. For the barrel trap and the analyzer, see [Barrel files and tree-shaking](/094-performance-audits-and-fixes/3-barrel-files-and-tree-shaking/) and [Reading the bundle treemap](/094-performance-audits-and-fixes/4-reading-the-bundle-treemap/).

### The one in-place fix — `next.config.ts`

The starter carries a `TODO(L6)` where the line belongs; you replace it with the `experimental` entry.
...
<CodeVariants>
  <CodeVariant label="Before">
    ...
    ```
    The starter has no `experimental` key, so nothing rewrites the barrel.
  </CodeVariant>
  ...
  <CodeVariant label="After">
    ...
    ```
    `optimizePackageImports` sits **under** the `experimental` key, as Next.js 16.2 still requires.
  </CodeVariant>
</CodeVariants>

Placement is the one detail to get right, and the test asserts it: `optimizePackageImports` lives under `experimental`, not as a top-level key. With the line in, the build rewrites every `lucide-react` barrel import to per-icon paths, and a second `pnpm next experimental-analyze` run shows the collapsed tile that finding 006 embeds.

### Finding 008 — the N+1 in the invoice helper

The dashboard's invoice helper reads the invoice rows in one query, then loops to fetch each invoice's customer separately:
...
That is 1 + N queries per render; dumping the loop's statement with <Term definition={"A Drizzle query-builder method that returns the SQL string and bound parameters a query would emit, without running it.\nYou log it to see exactly how many statements a read fires and what shape they take."}>`.toSQL()`</Term> confirms the count.
...
`}>
  ...
  <AnnotatedStep meta="{10-22}" color="orange">
    **Location.** Only the dedicated `invoices-with-customer.ts` helper; the healthy `invoices.ts` stays healthy, which keeps the grep falsifiable. Surfaced by the 1+N trace, confirmed with `.toSQL()`.
  </AnnotatedStep>
  ...
  <AnnotatedStep meta="{28-43}" color="green">
    **Fix.** `findMany({ with: { customer: true } })`: one <Term definition={"A join where the right side is a subquery that runs once per left-side row, able to reference that row's columns.\nDrizzle's relations API emits one to fetch each parent's related rows in a single statement instead of N follow-up queries."}>lateral-join</Term> statement, verified with `.toSQL()`. A hand-written join is the half-credit path.
  </AnnotatedStep>
</AnnotatedCode>

For the N+1 anti-pattern and the relations API, see [Indexes and N+1 in production](/094-performance-audits-and-fixes/7-indexes-and-n1-in-production/).

### Bonus finding 010 — the missing composite index

The bonus reaches past the eight-finding floor, on the same `/dashboard` read path. The `invoices` table ships with only its primary key, so the org-scoped, `created_at`-ordered read has no `(organization_id, created_at)` index to serve it.
...
The plan shows a `Seq Scan on invoices` feeding an in-memory `Sort`: the planner reads every row, then sorts because nothing arrives pre-sorted. Those two nodes are the fingerprint.

The fix has two halves. First, declare the composite index in the schema, leftmost-prefix `organization_id`, then `created_at`, then `id` so the ordering is total and the cursor stable:
...
Second, generate and apply the migration: `pnpm db:generate --name index_invoices_org_created` emits the `CREATE INDEX`, then `pnpm db:migrate` runs it. The schema declaration is only intent until the migration runs, so naming the index without generating the migration is half-credit. Re-run `EXPLAIN ANALYZE` and the `Seq Scan` plus `Sort` collapses to an `Index Scan using idx_invoices_org_created`.

:::note
This is a by-hand step to produce the `EXPLAIN ANALYZE` evidence, using the migration mechanics from the Postgres and Drizzle unit, not a committed production patch.
:::

The full finding follows the same template in `findings/010-composite-index.md` in the solution tree. For indexes and reading query plans, see [Indexes and N+1 in production](/094-performance-audits-and-fixes/7-indexes-and-n1-in-production/).

### `SUMMARY.md` — the coverage scorecard

`SUMMARY.md` proves the audit was systematic. It opens with the coverage line, `10/10`, the 8/8 floor plus both bonuses, then a table with one row per finding:
...
The `Half` column records how each finding closed: the four observability findings are **wired** (the diff from starter to solution), the performance findings and both bonuses are **documented**, save finding 006's one config line. Below the table sit the scoring rubric and the per-finding reach detail, then the two verdicts:
...
Paste finding 006's after-fix treemap into `SUMMARY.md` as secondary evidence for the bundle-size half.

### `out-of-scope.md` — keeping the count honest

An audit scores against its declared categories. Two things fall outside them, worth recording so a later pass can pick them up but not counted toward coverage:
...
Each fix names a primary reference doc; pull these up as you write the Fix sections so the config key, the relations-API call, and the plan-node names match the source of truth.
...
A green run confirms the source-shape probes: each finding file has its four sections, cites the right chapter 094 rule, locates the defect, and names the fix; finding 006 embeds both screenshots; and `next.config.ts` lists `lucide-react` under `experimental.optimizePackageImports`. It also checks that you documented the waterfall and N+1 rather than patching them: `dashboard/page.tsx` still awaits sequentially, and the invoice helper still loops.
...
The tests check file shape, not whether your numbers are real. Confirm that by hand at three surfaces: the Turbopack analyzer treemap, the DevTools Performance panel or Sentry trace, and the `EXPLAIN ANALYZE` query plan.
...
The next lesson runs the full verification, commits the work, and self-grades it against the answer key, closing the audit.
