...
Before moving on, lock the ordering in.
The next exercise fixes the body and asks you to drag the eight steps into execution order, so the constraint lands in your hands: both writes live inside the transaction, and the send comes after it.
