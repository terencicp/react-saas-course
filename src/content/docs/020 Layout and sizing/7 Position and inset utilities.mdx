...
The 2026 answer is three utilities: `fixed bottom-4 right-4 z-50`. That string pins a toast to the bottom-right of the viewport in every real SaaS app shipping today. Two things in it should make you curious. Why `fixed` and not `absolute`? And what is that `z-50` doing? By the end of this lesson you'll understand all five `position` modes, you'll know exactly what each one anchors against, and you'll build badges, sticky headers, toasts, and drawers without hitting the "why is it in the wrong corner" bug that eats an afternoon the first time it happens.

There's one connection to make before we start. Every element you've laid out so far, whether a flex item, a grid item, or a stacked block, has lived in <Term definition="The browser's default layout: each block element takes a new line top-to-bottom, each inline element flows left-to-right, and every element reserves a slot that pushes its siblings around.">normal flow</Term>. Flow is what makes a paragraph push the next paragraph down, and a flex item make room for its neighbors. `position` is how a small, specific set of elements opt *out* of flow, and that set stays small. Most of your layout is still flex and grid. You reach for `position` only for the handful of elements that genuinely need to escape: a badge on a corner, a header that sticks while you scroll, a toast pinned to the screen.

## Five ways an element can be positioned

There are five values for the `position` property, and the way to keep them straight is to stop thinking of them as five vocabulary words and ask a single question of each: **what is this element positioned against?** Once you know what each one is measured *from*, the offsets (`top`, `left`, and the rest, which we'll get to) stop being mysterious.
...
- `relative` stays in flow, so its original slot is preserved and its siblings don't move, but offsets now shift it *visually* from that slot. Its real job in 2026 isn't that nudge, though. It's to **become an anchor** for `absolute` children inside it, which is the idea this whole lesson turns on.
...
This is the heart of the lesson, the idea that turns "positioning is fiddly and unpredictable" into "positioning is mechanical and obvious." When you give an element `position: absolute` and then set `top` and `left`, the browser has to measure those offsets from *something*. That something is the <Term definition="The rectangle an out-of-flow element's offsets are measured from — for an absolute element, the padding box of its nearest positioned ancestor.">containing block</Term>.

The rule for an `absolute` element is precise: its offsets are measured from the nearest ancestor that is *itself* positioned, meaning the nearest ancestor with `position` set to `relative`, `absolute`, `fixed`, or `sticky`. The browser walks up the DOM tree from the element, parent by parent, looking for the first one that's positioned. That one becomes the containing block. Here is the catch that bites everyone: if the browser walks all the way up and finds *no* positioned ancestor, it falls back to the <Term definition="The viewport-sized rectangle the browser uses as a last-resort containing block when nothing in the ancestor chain is positioned.">initial containing block</Term>, which is near enough the whole viewport.
...
There's one exception to the "`fixed` anchors to the viewport" rule. It belongs to a later lesson, so we won't unpack it here, but it's worth naming once so you recognize it when it bites. A `fixed` element normally ignores its ancestors and anchors to the viewport. The exception is when one of its ancestors has a `transform`, `filter`, or `perspective` set: that ancestor silently becomes the containing block instead, and your `fixed` modal ends up positioned relative to some transformed parent halfway down the page. The lesson on stacking context and `z-index`, later in this chapter, runs into the same trap from a different symptom. The fix in both cases is to portal the element out to `<body>` so it has no troublesome ancestors, a technique you'll meet in the chapter on portals and modals. For now, just remember the recognition: **a `transform` on an ancestor can capture a `fixed` child.**
...
You now have the model (containing block) and the vocabulary (inset). What turns that into a skill is recognizing the handful of *shapes* that come up over and over in a real SaaS app and that an experienced developer types without thinking. There are about five, and the gallery below renders each one live with its class string, so you can see the pattern and the code together and inspect either in DevTools.
...
A few gotchas from those tabs are worth pulling out, since each one turns a clean pattern into a bug report. The badge's negative offsets only overhang if the card doesn't clip its overflow; an `overflow-hidden` would shave off the half that pokes out (you'll meet overflow properly in the next lesson). The toast's `z-50` just means "stack above the page." The number itself, and *why* it sometimes fails, is the job of the stacking-context lesson later in this chapter. For any bottom-pinned chrome on iPhones, add `pb-[env(safe-area-inset-bottom)]` so the toast clears the home-indicator bar, one of the rare arbitrary values that earns its keep. And a count badge means nothing to a screen reader as a bare "3" in a corner: give it an `aria-label` like "3 unread" so the number reaches assistive tech. The accessibility depth there belongs to a later chapter.

## Sticky: in flow until it isn't

`sticky` earns its own section because it confuses more people than the other four combined, and almost always for one of three specific reasons. Get the model right and the confusion clears up.
...
Time to hit the central bug yourself and fix it. You're building a notification card: an avatar on the left, a title and message in the middle, and a small unread badge pinned to the card's top-right corner, overhanging the edge slightly. This exercises the reflex the whole lesson drives toward. The parent needs `relative`, or the badge flies to the corner of the preview instead of the card.
...
If your badge jumped to the corner of the editor preview when you added `absolute`, you just reproduced the lesson's central bug for yourself, and the one-class fix (`relative` on the `<article>`) is now wired into your fingers. That's the whole point.
...
Keep these open while the model settles.
