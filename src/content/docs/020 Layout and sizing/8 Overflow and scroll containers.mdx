...
:::note
`overscroll-behavior` is supported in every 2026 browser, with no polyfill and no fallback, so ship it directly. And if you find `-webkit-overflow-scrolling: touch` in old code, delete it: that was a hack to get momentum scrolling on inner containers in ancient iOS, and modern iOS scrolls them smoothly without it. Today it does nothing but add noise.
:::
...
<StateMachineWalker kind="decision" title="Page scroll or app-shell scroll?">
  ...
  <Leaf id="leaf-page" verdict="Page scroll — let <body> scroll">
    The default, and the one you reach for unless you have a concrete reason not to. You get scroll restoration, back and forward position, and anchor jumps for free. There's nothing special to write, since `<body>` already scrolls.
  </Leaf>

  <Leaf id="leaf-shell" verdict="App-shell scroll — inner <main> scrolls">
    The chrome stays fixed, and an inner `<main>` gets `overflow-y-auto` and `min-h-0` so it actually scrolls. You take over scroll restoration manually, either with a Next.js config flag or a small hook you wire yourself. It's worth it for a dashboard and overkill for a content page.
  </Leaf>
</StateMachineWalker>
...
<Figure caption="A real carousel, no library. The snapping is pure CSS, snap-x snap-mandatory on the row and shrink-0 snap-start on each card, with no JavaScript behind it. Open devtools and inspect the row: every utility doing the work is right there in the styles.">
  <SnapCarousel />
</Figure>
...
:::note
One accessibility note, since horizontal scrollers are easy to get wrong: a scroll container that genuinely overflows is keyboard-focusable by default, so keyboard users can scroll it with the arrow keys. But the *items* inside a carousel still need to be reachable on their own, a focusable link or button per card, or keyboard and screen-reader users can't get to them at all. The full treatment of accessible carousels lives in the chapter on the shadcn component surface. For now, the rule is simply that a carousel's cards must be reachable by keyboard.
:::
