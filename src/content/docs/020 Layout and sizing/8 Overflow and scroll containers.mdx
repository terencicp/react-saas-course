...
A <Term definition="A box that clips its overflowing content and lets the user scroll inside it to reveal the rest. The page itself is one; an inner element becomes one when you give it overflow plus a height it can exceed.">scroll container</Term> is a box that clips whatever overflows it and lets the user scroll *inside that box* to see the rest.
