...
Before any Next.js syntax, you need a clear picture of what goes wrong by default, because the tags only make sense once you've seen the failure.
...
<VideoCallout videoId="lbxyw-jdNDo" videoTitle="Hreflang Tag Explained - SEO for Multilanguage Sites">
  A 6-minute framework-agnostic walkthrough from magier of the raw `hreflang` tag: the self-reference rule, `x-default`, and language-versus-region, the same three rules above before any Next.js syntax.
</VideoCallout>
...
<CodeVariants>
  ...
  <CodeVariant label="Right — self-canonical">
    ...
    **Each variant ranks in its own language.** `generateAlternates` already sets `canonical` to *this* locale's URL, so the French page is its own authority. `hreflang` handles the relationship to the other languages, and the canonical never collapses them.
  </CodeVariant>
</CodeVariants>
...
:::tip
Self-canonical and `hreflang` are partners, not alternatives. The canonical claims authority *for this language*, and `hreflang` points to the *other* languages. The instant a translated page's canonical points at the default locale, you've told Google to delete that translation from search, the most expensive one-line mistake in i18n SEO.
:::
...
`}>
  <AnnotatedStep meta={`{7} "MARKETING_PATHS"`} color="violet">
    Marketing routes only. The authenticated app is `noindex`, so it's deliberately absent here. This is the dichotomy from the intro, made concrete in one constant.
  </AnnotatedStep>
  ...
  <AnnotatedStep meta={`{9-10} "abs"`} color="orange">
    `abs` wraps `getPathname` with the base URL. It draws on the *same* prefix source of truth `generateAlternates` uses, so the sitemap and the head tags can't drift apart.
  </AnnotatedStep>
...
`og:locale` is this page's locale, and `og:locale:alternate` (the `alternateLocale` array) is every *other* locale you offer. Note the `.filter((l) => l !== locale)` that excludes the page's own locale, so the alternates never repeat it. The French page declares "I'm the French card (`og:locale` = `fr_FR`), and English and German versions also exist."
...
This is the through-line of the lesson: this surface is invisible locally. It renders fine, it passes review, and the only feedback that it's *correct* comes from outside your dev loop. So you need to know where that feedback actually lives. There are two places.
