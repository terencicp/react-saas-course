...
Before any single formatter, learn the two ideas that all of them share. Once you have these two, the rest of the lesson is variations on a theme.
...
**`'currency'`** is the one you just saw. It *requires* a `currency` option, a three-letter <Term definition="The ISO 4217 standard of three-letter currency codes: USD, EUR, GBP, JPY. The code carries the currency identity; the locale decides how it's displayed.">ISO 4217</Term> code like `'USD'` or `'EUR'`. Make this a habit: **the currency code is data, not a UI constant.** It comes from `invoice.currency`, never from a string literal in your component, because an invoice in euros rendered with a hard-coded `'$'` is a lie about money. A second option, `currencyDisplay`, controls how the currency is shown. `'symbol'` is the default (`'$'`, but `'US$'` in some locales to disambiguate), `'narrowSymbol'` forces the short form (`'$'` even where the default would say `'US$'`) and is the better choice for compact UI, while `'code'` shows `'USD'` and `'name'` spells out `'US dollars'`.
...
`}>
  <AnnotatedStep meta="{5-8}" color="green">
    The options type makes `locale` and `timeZone` *required*: not optional, not defaulted. You cannot call `formatDate` without naming a zone, which is what makes the Vercel-UTC bug impossible to express. This is the shape last chapter promised.
  </AnnotatedStep>
