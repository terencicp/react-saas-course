...
A request can arrive carrying every signal at once. The resolver checks them in a fixed order and stops at the first one present, so put them in that order.
...
Now run that order against three requests.
