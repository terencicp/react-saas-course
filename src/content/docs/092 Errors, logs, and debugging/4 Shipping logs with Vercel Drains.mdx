...
The first lesson of this chapter wrote the `requestId` into each Sentry event's *context*, not as a tag, since a `requestId` is too high-cardinality to group on.
The logger lesson threaded that same `requestId` through every log line.
