...
<VideoCallout videoId="jyfQmtISr3Y" videoTitle="TIMESTAMP vs TIMESTAMPTZ - What's the difference? Which should I use?">
  CockroachDB walks through the `timestamp` vs `timestamptz` choice in 3 min, showing the same row render differently per session zone and why `timestamptz` is the one to reach for.
</VideoCallout>
