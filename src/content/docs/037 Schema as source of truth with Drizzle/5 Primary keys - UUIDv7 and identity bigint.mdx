...
You have written `.primaryKey()` since the start of the chapter without looking closely at it. Before we choose what fills it, pin down what it actually does, because it gives you more than it appears to.
...
:::caution
Never add a separate `.unique()` or `.index()` to a primary-key column. The uniqueness and the index are already there, so a second one is redundant. Seeing it in a diff tells a reviewer the author did not know what `.primaryKey()` includes.
:::
...
An experienced engineer asks two questions to settle a primary key, and asks them in a fixed order. Get the order right and the answer usually falls out on its own. The first question is the universal one, the one that applies to every table before you think about anything else:
...
Two facts ground this as real and current rather than a clever hack. UUIDv7 was standardized by <Term definition="The 2024 IETF specification that standardized UUID versions 6, 7, and 8.">RFC 9562</Term> in May 2024. And Postgres 18, which you put your project on in the previous chapter via Neon, ships a native `uuidv7()` function: there is no extension to install, it is just there. That native function is what makes the shape in the next section so clean.
...
<VideoCallout videoId="DHtf_46OxY8" videoTitle="UUID vs INT: What's Better For Your Primary Key?">
  Database Star walks through auto-increment, UUIDv4, and the time-sortable UUIDv7 in about ten minutes. It is the same scatter-versus-tail story this section just made, backed by a Postgres demo.
</VideoCallout>

:::caution
Drizzle's `.defaultRandom()` generates a UUIDv4, the random one with the locality problem above. It is not the v7 path. There is an open feature request for a native v7 helper, but until it ships, reach for the SQL default in the next section for a primary key, never `.defaultRandom()`.
:::

## The three canonical shapes

Now for the mechanics, which come down to exactly three shapes this course ships. Each tab below is one strategy, paired with the trigger that should make you reach for it. Read them as "when do I write this," not "how do I write this," because the *how* is one line in every case.
...
<CodeVariants>
  ...
  <CodeVariant label="identity bigint">
    ...
    **The choice for high-volume internal tables.** An auto-incrementing 8-byte integer, minted by Postgres from a sequence. Reach for it only when *all* of these hold: the table is high-volume and internal (an event log, analytics rows, a junction nobody fetches by id), the id never crosses a system boundary, and sharding is not on the roadmap. A bigint beats a UUID here for two reasons. First, 8 bytes instead of 16 gives a tighter index. Second, no outsider ever enumerates these rows, so the leak and locality arguments for UUIDs do not apply. `mode: 'number'` is required: it maps the column to a JS `number`, which is safe up to 2⁵³.
  </CodeVariant>
...
One mechanic is left, the one you were told to resist reaching for earlier. A primary key can span more than one column. You will need it later in this chapter, for the table that links invoices to tags, so meet the syntax now, along with the boundary around it.
...
The decision is the heart of the lesson, so the practice drills the decision before it drills the syntax. There are two passes: first sort tables into strategies, then write the shapes.
...
`organizations` takes the day-one default, a UUIDv7 surrogate minted SQL-side. `auditLogs` earns the identity bigint because all three conditions hold: it is internal, it is high-volume, and no id ever leaves the database. `countries` was already right, since a small reference table the world owns is the one place a natural key belongs.

</details>

## Where to read more

The two-version UUID story is recent enough that a current, primary source is worth a look. The links below cover the native Postgres 18 support and the locality argument in more depth than this lesson needs to.
