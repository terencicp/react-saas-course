...
One fact comes before any call-shape detail, because every other decision in this lesson depends on it.
...
<CodeVariants syncKey="invoice-call">
  ...
  <CodeVariant label="Hardened">
    ...
    **Each fix maps to one failure.** The `signal` gives it a deadline. The `if (!response.ok)` branch catches the `422`. The `parse` validates the wire. The `catch` handles the transport failures. We'll build this seam by seam. For now, the point to take away is that the destination is one function, roughly a dozen lines, with no library.
  </CodeVariant>
</CodeVariants>
...
This section gives you a vocabulary you'll use for the rest of the course. Every `fetch` call, in the browser or on the server, is a pipeline of **five seams**:
...
That brings us to the call's biggest gaps. The naive `createInvoice` had no deadline and no way to cancel, which were failures two and three. Both are fixed at the build seam, with the same primitive: an `AbortSignal` passed as `init.signal`.
...
Two things layer on top of plain `fetch`. You won't learn either one here; the goal is just to recognize them as "you'll meet this later," then move on.
