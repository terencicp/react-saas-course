...
<CodeVariants maxLines={6}>
  ...
  <CodeVariant label="After" icon="approve-check">
    ...
    **Add the namespace import and pass `schema`.** Now the adapter resolves each table by reference, with no name-guessing and no drift. This mirrors how you built `db` itself: you passed `schema` to `drizzle(...)` when you set up the client.
  </CodeVariant>
</CodeVariants>
...
A quick note before the first table, so the column names don't confuse you. You set `casing: 'snake_case'` on your Drizzle client back in [the smallest table — pgTable and the snake_case bridge](/037-schema-as-source-of-truth-with-drizzle/2-the-smallest-table---pgtable-and-the-snake_case-bridge/). That means the TypeScript reads in camelCase, `emailVerified`, while the actual Postgres column is snake_case, `email_verified`. It's the same column with two spellings, mapped automatically. When you see `emailVerified` in the schema and `email_verified` in the migration SQL later, that's why. I'll only flag it this once.
...
You have the schema file, but it's only a description: there are still no actual tables in Postgres. Turning a schema description into real tables is exactly what Drizzle Kit does, and you already learned the workflow back in [the Drizzle Kit daily loop](/040-migrations-and-seeding/1-the-drizzle-kit-daily-loop/). There's no new tooling, just two commands.
