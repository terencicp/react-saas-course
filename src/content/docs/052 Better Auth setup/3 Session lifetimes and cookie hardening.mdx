...
:::note
Relaxing the prefix in dev is a deliberate, scoped concession to the fact that localhost runs over HTTP, not a weakening of production. Production resolves to the full `__Host-` lock; only your machine runs with the relaxed prefix.
:::
...
## Tools this chapter names but doesn't reach for
