...
:::note
`page.tsx` is default-exported and framework-named. This is the one place the codebase's "named exports everywhere" rule steps aside, because Next.js dictates the filename and the default export. And `searchParams` arrives as a Promise you `await`, because in Next.js 16 the request-bound inputs are async.
:::
...
<StateMachineWalker kind="decision" title="Routing one accept URL">
  ...
  <Question id="has-account" prompt="Is there already an account on the invited email?"
    description="There is no session, but the invited address may or may not already have a user record.">
  ...
  <Leaf id="leaf-accept" verdict="Shape A — render the Accept button">
    Bob is signed in as exactly the invited person. Show "Accept invitation to Acme as member" and the consent button. This is the only leaf that reaches the write, and every other shape has to route Bob *to* this state first.
  </Leaf>

  <Leaf id="leaf-mismatch" verdict="Shape B — refuse (handled in a later lesson)">
    Someone is signed in, but as a *different* email than the invite was sent to. Render the strict refusal stub. It is a refusal rather than an "accept anyway," because invites get forwarded: if you let the logged-in account accept an invite addressed to someone else, you have handed it the wrong person's seat. The full treatment, including the exact copy and the case-insensitive compare, lands in "Orphans, mismatches, and the double-click race."
  </Leaf>

  <Leaf id="leaf-signin" verdict="Shape C — sign in, then come back">
    There is no session, but the invited email already has an account. Render a sign-in form prefilled with the invited address and a `next` param pointing back at this same accept URL. On success the page reloads, now with a matching session, which lands on Shape A.
  </Leaf>

  <Leaf id="leaf-signup" verdict="Shape D — sign up, then come back">
    There is no session and no account. Render a sign-up form prefilled and email-locked to the invited address, returning to this accept URL. After sign-up the page reloads with a session whose email matches, which lands on Shape A.
  </Leaf>
</StateMachineWalker>
