...
:::note
The pending list and the member list are siblings, not the same table. The member list shows accepted seats and routes role changes through `changeMemberRole`. The pending list shows offered-but-not-taken seats. They never join, because an invite's job ends the moment it's accepted, which is where the member's job begins. This lesson only touches the pending side.
:::
...
:::note
A resend is the same abuse vector as a send: an attacker on a compromised admin account can hammer "resend" to spam an inbox just as easily as "invite." The fix is rate limiting, applied through the same middleware that guards the send action. That's a later chapter's job, named here but not built.
:::
...
<CodeVariants maxLines={16}>
  ...
  <CodeVariant label="Let the index throw, then translate">
    ...
    **The database decides, atomically.** There's no pre-check: just INSERT, and let the partial unique index from the first lesson reject the duplicate at the database. The second INSERT fails, race-free, because the constraint check and the write are one operation. Your job shrinks to translation: catch the unique violation, turn it into a typed `conflict`, and rethrow anything else.
  </CodeVariant>
</CodeVariants>
...
:::note
Better Auth's organization plugin offers a `cancelPendingInvitationsOnReInvite` option that auto-cancels the existing pending invite when you re-invite, rather than erroring. That's the framework's built-in posture, and it's a reasonable one. This chapter doesn't use it for the same reason it hand-rolls everything else: the send path owns its own token, hash, signature, and audit, so the plugin's invite lifecycle isn't in play. And catch-and-translate teaches the transferable skill, since this exact shape recurs for any unique constraint you'll ever hit, including slugs, seat reservations, and idempotency keys. Learn the pattern once, apply it everywhere.
:::
...
:::caution
Never wire the expired shelf's button to `resendInvitation`. Resend rotates a live token in place, and an expired invite has no live token to rotate. Expired means dead, and the only honest action on a dead invite is a fresh send. That's two different verbs for two different states.
:::
