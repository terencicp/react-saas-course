...
<CodeVariants>
  ...
  <CodeVariant label="Wrapper nesting">
    ...
    **Produces invalid interactive HTML.** An `<a>` wrapping a `<button>` nests two interactive elements inside each other, which the HTML spec forbids. Browsers render it unpredictably and screen readers announce a confusing double control. You learned why `<button>` and `<a>` are distinct interactive elements earlier in the course, and that rule is exactly what this approach breaks.
  </CodeVariant>
</CodeVariants>
