...
A read-then-write reopens the race from [Newer wins, single writer](/063-webhook-ingestion-and-idempotency/3-newer-wins-single-writer): two handlers both read the old `lastEventAt`, both decide theirs is newer, both write.
...
`hasActiveAccess` encodes the decision table from [the subscription-status lesson](/064-stripe-billing-and-plan-entitlements/5-decoding-subscription-status-into-access): `trialing`, `active`, and `past_due` grant; `canceled` and `incomplete` deny.
