...
Everything else — `claimEvent`, the `processed_events` table, the `db.transaction` shape, the `Transaction` type — ships in the starter, carried in from [Claim once, mutate once](/063-webhook-ingestion/2-claim-once-mutate-once).
