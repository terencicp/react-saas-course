...
One quick refresher first. In the images lesson you met <Term definition="Cumulative Layout Shift — the Core Web Vital that measures how much visible content jumps around as the page loads. A font swapping to a different-width face is a classic source.">CLS</Term>, the Core Web Vital that measures how much content jumps around as the page loads. A font swapping from a wrong-width fallback to the real font is a common CLS source: the text reflows the instant the real font lands. The fallback-metric mechanism at the heart of `next/font` exists to remove exactly that jump, so keep CLS in the back of your mind.
...
<VideoCallout videoId="kvFChwzt0BI" videoTitle="How to Optimize Font Loading in Next.js">
  Blazity records the DevTools performance trace of a render-blocking Google Fonts `<link>`: the FOUT and a 0.88 CLS. Both vanish once `next/font` self-hosts the file (4 min).
</VideoCallout>
