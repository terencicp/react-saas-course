...
It didn't, and nothing broke. The instant you added the `@import "tailwindcss"` line back in the last chapter, Tailwind quietly ran a small, deliberate reset called <Term definition="Tailwind's small built-in CSS reset, loaded automatically by the Tailwind import, that normalizes away the browser's default styles.">Preflight</Term>, and Preflight *deleted the browser's default styling on purpose.* The big bold heading, the bullets, the native button look: those styles were never yours. They came from the browser, and Tailwind stripped them so you could start from nothing.
