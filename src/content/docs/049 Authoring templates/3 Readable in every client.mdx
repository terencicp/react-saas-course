...
The part that matters most, because it determines how you actually work, is that you do not write that text body. The Resend <Term definition="Software Development Kit — the official Resend client library you call from your application code to send email.">SDK</Term> derives it automatically from the same `react` node you pass to send, the same render that produced the dual-format output in the first lesson. Your application code calls nothing extra. You pass the component, and both bodies come out the other side.
...
:::caution
There is one tempting wrong move here worth naming before you make it. When the text version looks noisy, do **not** "clean it up" by setting an *informational* image's `alt=""`. Emptying the alt does silence the text, but it also blinds every screen-reader user to real content. Stripping `alt` is the fix for *decorative* noise only. For an image that carries meaning, the cure for noisy text is reworded HTML around it, never a stripped `alt`.
:::
...
<CodeVariants>
  ...
  <CodeVariant label="Plain text (what shipped)" icon="document">
    ...
    **This is the exact text body a plain-text client receives.** The sparkle image left no trace because its `alt` was empty, and the link text reads cleanly beside its URL.
  </CodeVariant>
</CodeVariants>
