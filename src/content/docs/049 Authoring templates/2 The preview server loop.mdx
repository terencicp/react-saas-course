...
- The **test-send** is where you *verify*. It's slow and real, and it's the only way to see what an actual client does with your message: the dark-mode inversion the toggle could only emulate, and the Outlook <Term tooltip="Vector Markup Language, the legacy markup older Outlook uses to render buttons and shapes. React Email generates the fallback for you, but only a real Outlook confirms it landed.">VML</Term> button path nothing local can reproduce.
