...
<CodeVariants>
  <CodeVariant label="Nested ifs">
    ...
    **Four nesting levels, with the happy path buried at indentation four.** The reader has to track which `else` belongs to which `if` to know what fires on which path. There's also a hidden hazard: forget a `return` inside any branch and an implicit `undefined` leaks out of one of the paths without a compile error. Every level you add doubles the surface area where that bug can hide.
  </CodeVariant>
  ...
  <CodeVariant label="Guard clauses">
    ...
    **Each invalid case checks and returns at the top.** The happy path lives unindented at the bottom. The function reads top-to-bottom; the reader doesn't track nesting because there is none. Every `if` is followed immediately by a `return`, so there's nowhere for an implicit `undefined` to hide.
  </CodeVariant>
</CodeVariants>

The change between the two versions is a polarity flip. The nested version reads "if everything is good, do the work, else handle errors"; the guard version reads "if anything is wrong, exit; then do the work." A <Term definition="A check at the top of a function that exits early when an invalid or edge case is detected, so the rest of the function can assume the happy path.">**guard clause**</Term> is just that: a check at the top, an immediate return, no `else`. Each condition gets one return path, and the single happy path sits at the outer indentation level.
...
Biome catches this for you. The `noUselessElse` rule flags every `else` block that follows an `if` whose body terminates with `return`, `throw`, `continue`, or `break`, and its autofix dedents the trailing block in one keystroke. You don't have to remember the rule while you write; the linter catches the dead `else` for you.

The broader reflex this section builds is that **indentation deeper than two levels in a 2026 SaaS function is a smell.** When a reviewer opens a file and sees nesting depth three or four in a function body, they refactor into guards before reading further. The shape itself is what the reviewer is fixing: the conditions might be correct, but the nesting pays interest on every future read.
...
<CodeVariants>
  ...
  <CodeVariant label="Return">
    ...
    ```
    **A value being returned from a function.** Same shape: two possible return values, and the ternary picks between them. The function's contract is "returns either the admin badge or null"; the ternary reads as that contract.
  </CodeVariant>
...
The verb-shaped branching reads as control flow, which is what it is. The runtime behavior is the same; the shape is honest.
...
A note on Biome: it doesn't enforce "no ternaries for side effects"; that part stays a human review reflex. It does enforce `noUselessTernary`, which flags trivially wrong shapes like `x ? true : false` (the whole expression collapses to `x`). The side-effect rule is yours to enforce in review.
...
`}>
  ...
  <AnnotatedStep meta="{6-8}" color="violet">
    `assertNever` is the compile-time exhaustiveness helper. It takes one parameter typed `never`, TypeScript's <Term definition="TypeScript's bottom type — the type with no values. Useful as a parameter type to force a compile error when an unhandled union variant could reach the function. Full treatment in the chapter on TypeScript's narrowing model.">**`never` type**</Term>, the bottom type that represents "no possible value." If the function is ever actually called, it throws at runtime. The function lives once per codebase, in `lib/assert-never.ts`, and gets imported everywhere a `switch` over a discriminated union appears. Why `never` does the compile-time work lands fully in the chapter on TypeScript narrowing; the final step of this walkthrough shows the behavior in action.
  </AnnotatedStep>
...
<CodeVariants>
  <CodeVariant label="Switch">
    ...
    ```
    **Verbose for a flat mapping.** Eleven lines for three cases, and the `case`/`return` scaffolding adds nothing each branch actually needs. The `assertNever` payoff is real, but for a pure value-to-value mapping the form is heavier than the work.
  </CodeVariant>
