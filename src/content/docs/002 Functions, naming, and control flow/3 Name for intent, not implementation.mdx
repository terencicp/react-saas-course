...
Open any production repo and you'll find three kinds of bugs waiting in the names. A variable called `data` whose shape nobody can know without opening three other files. A function called `processOrder` whose body the next reader has to read end to end to learn what "process" means in this codebase. A boolean called `notDisabled` that someone negates six months later as `!notDisabled`, a double negative the next reader has to cancel out, and one day cancels wrong.
...
The principle fits in one sentence:
...
The lesson walks through four naming surfaces (variables, functions, parameters, types), the boolean-prefix convention that tells the reader a value's type before they read the annotation, three classes of bad names that account for almost every naming failure you'll see in code review, and a short pair of rules on abbreviations and consistency. It closes with two recognition exercises: a sorting drill on abbreviations and a PR review where you flag the bad names in a small file.

## The principle and its asymmetry

What makes the principle work is that it's asymmetric. Read it again:

> A name says what the value is or what the function does, not how it's computed.

The principle tolerates one direction and forbids the other. A name that is *vague but fitting*, like `user`, `total`, or `invoices`, is acceptable: the reader learns what the value is, even if they don't learn every detail about it. A name that *leaks the implementation*, like `userArray`, `totalReducer`, or `invoicesQueryResult`, is the smell: the reader learns how today's code happens to compute the value, and a future refactor breaks the name. The principle isn't "be specific." It's "describe what the value is, not how it's produced."
...
One nuance before moving on: "vague is acceptable" doesn't mean vague is the goal. The right name is as specific as the value warrants. `pendingInvoices` reads better than `invoices` when the pending filter is the point of the variable; the principle only asks that the extra specificity describe the value, not the implementation behind it.
...
`sum` and `invoice` are fine inside the one-line `.reduce`: each binding lives for one expression, and its position in the callback (accumulator first, item second) tells the reader what it is. `pendingInvoicesForCurrentMonth` carries its full intent because it lives at module scope, where a reader who jumps to its first use has none of the context the writer had at assignment time. The rule reads in one direction: a short name is acceptable only inside a short scope.
...
A reviewer skims the verbs and knows each function's category before reading any body. That at-a-glance categorization is what makes a file readable at scrolling speed.
...
The boolean-prefix convention is small but high-leverage. It's a **visual contract**: when the reader sees `is`, they know the value is a boolean before they read the annotation. The contract buys reading speed; it adds nothing to type safety.
...
The rule the course does legislate: **pick one across the codebase and stick to it.** Drift between synonyms in the same repo is the real smell, not the choice itself. A codebase that uses `fetchUser` in one file and `loadUser` in the next forces every reader to keep two mental models of "the read operation" running side by side, and the cognitive cost compounds across hundreds of files. Pick one verb per concept, write it down in the team's conventions doc, and let a linter or a custom check catch drift if the volume warrants it.
