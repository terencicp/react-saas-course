...
This extends the `const`-by-default reflex from the previous lesson, "const binds, it doesn't freeze," to functions. If every binding is `const`, then every function is a value bound to `const`, which is exactly `const fn = (args) => …`.
