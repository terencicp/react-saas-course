...
2. The browser walks every control, reads each one's `name` and value, and packs them into a <Term definition="A browser object that holds a form's fields as name/value pairs. The browser builds one from your named inputs; you can hand it straight to `fetch`.">`FormData`</Term> object.
