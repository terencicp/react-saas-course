...
:::caution
The unpooled-URL rule from the config section matters most right here. `migrate` runs the longest transactions of any command, by design, and the pooled URL will choke on them. The config already points at the unpooled URL, so don't override it.
:::
...
:::danger
Editing a migration file after it has already been applied is the most damaging mistake in this whole stack, so give it weight. Even a tiny tweak on your own laptop, after you ran the file once, leaves the snapshot on disk out of step with what's actually in the database. The next time anyone runs `generate`, it diffs the schema against a snapshot that no longer reflects reality, and it emits nonsense: migrations to "fix" differences that don't exist, or to redo things already done. The rule is absolute. **An applied migration is frozen.** If it's wrong, fix it forward with a new migration.
:::
...
:::caution
Studio's inline row editing writes straight to the connected database. Click delete on a row and the row is gone: there's no staging and no confirmation beyond the click. Pointed at a shared dev branch your whole team uses, a careless delete does the same damage as a `DELETE` with no `WHERE` clause, just behind a friendlier button. Treat every edit in Studio as a real, immediate mutation, because it is one.
:::
