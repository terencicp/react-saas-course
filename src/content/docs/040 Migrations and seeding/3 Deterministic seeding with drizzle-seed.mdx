...
Take the three arguments in turn. The first, `db`, is your Drizzle client, the same one your app queries through. The second, `schema`, is the **bag of every table you export from `db/schema.ts`**. That `import * as schema` pulls in all of them at once, and the seeder needs all of them, for a reason we'll get to. The third is an options object, and its `seed` key is the determinism knob: that `1` is the number that makes every run identical. (Those `@/db` import paths are the project's path aliases. Hold onto that detail; it comes back when we wire up the script to run.)
...
:::caution
Always pass the **whole schema bag**. If you hand the seeder a subset, say just `invoices`, and an invoice has a `NOT NULL` foreign key pointing at `organizations`, the seeder has no parent table to draw a valid id from, so it throws. Use `import * as schema` and pass all of it. There's an advanced escape hatch for partial exposure, but it's not your default; the default is to give the seeder everything.
:::
...
**Step two: column generators.** A `count` gives you more rows; a `columns` map controls what fills each one. This is where the `f` catalog comes in. There are around three dozen generators, so rather than memorizing them, learn them by category and reach for the docs for the rest:
...
`}>
  ...
  <AnnotatedStep meta="{5-11}" color="green">
    The weighted status distribution from above: mostly `paid`, fewer `pending`, a sliver `overdue`. That's the realistic shape, not a flat third each.
  </AnnotatedStep>

  <AnnotatedStep meta="{12}" color="orange">
    The money column. `total` is `numeric`, so `f.number` with `precision: 100` keeps two decimal places (cents) rather than a raw float that would lose precision. The generator has to respect the schema's money decision.
  </AnnotatedStep>

  <AnnotatedStep meta="{13-15}" color="violet">
    A curated `valuesFromArray` of real-sounding company names. For demo-facing columns, hand-picked strings beat lorem-grade noise on a screenshot.
  </AnnotatedStep>
...
We've called the seeder "deterministic" twice now without showing what that buys you. Here's the payoff, and it's the property that justifies the whole approach.
...
:::caution
`reset` deletes **everything** in the schema. It's the seed-script cousin of running a `DELETE` without a `where` clause: total and irreversible. It's safe here only because this script targets a dev or test database and is named as such. **Never wire it to a production credential.** Production data is not the seeder's concern, and a `reset` pointed at it would wipe out your real data.
:::

Here's the before-and-after, side by side. The first tab is the version that breaks; the second is the script you actually ship.
...
<CodeVariants>
  <CodeVariant label="seed alone">
    ...
    **Breaks on the second run.** The first run fills the table; run it again and the prior rows are still there, so you get four hundred invoices, or a unique or foreign-key violation aborts the run. Fine exactly once, useless as a repeatable tool.
  </CodeVariant>
  ...
  <CodeVariant label="reset + seed">
    ...
    **Idempotent.** `reset` clears the tables first, so every run starts from empty and lands on the same two hundred rows. Run it as many times as you like and you get the same state every time. This is the shape the script ships in.
  </CodeVariant>
</CodeVariants>
