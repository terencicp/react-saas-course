...
:::caution
`reset` deletes **everything** in the schema. It's the seed-script equivalent of running a `DELETE` without a `where` clause: total and irreversible. It's safe here only because this script targets a dev or test database. Never wire it to a production credential: a `reset` pointed at production would wipe out your real data.
:::
