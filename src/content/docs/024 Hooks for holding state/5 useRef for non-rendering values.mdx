...
What you need is a third kind of memory: one that *survives across renders* like state, but is *invisible to React* like a plain variable. That's <Term definition="A React hook that returns a stable mutable container whose changes never trigger a re-render.">`useRef`</Term>.
...
You attach a ref to an element with the `ref` prop, `<input ref={inputRef} />`, and React puts the live DOM node into `inputRef.current`. The catch is *when*: React assigns the node only **after it commits** the DOM to the screen. <Term definition="The moment React applies the render's changes to the real DOM — the point where the screen actually updates.">Commit</Term> is the same step you met in the render model. Before it there's no node to point at, so the ref is `null`; after it the node exists, and React fills the box.
...
All four are <Term definition="Telling the element to do something — focus, play, scroll — rather than describing what it should be.">imperative</Term>: you tell the element to do something. That distinction draws the line for the second beginner mistake.
