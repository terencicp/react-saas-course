...
With the vocabulary in place, we can write the function that consumes it. The <Term definition="A pure function that takes the current state and an action and returns the next state.">reducer</Term> takes the state and an action and returns the next state, and it does so as one `switch` over the action's discriminant. Let's read it case by case.
...
`}>
  ...
  <AnnotatedStep meta="{5-6}" color="green">
    ...
    The two can't drift apart, because one move owns both. There is no code path where the form is "submitting" but still showing a stale error, because that combination never gets a chance to exist.
  </AnnotatedStep>
...
Count the combinations. Three booleans give you eight of them, `true`/`false` cubed, and several are nonsense: submitting *and* saved, saved *and* errored, all three true at once. The scattered version let any handler produce any of those eight, and trusted you never to produce the bad five. A single `status` field has exactly three values, all of them legal. The illegal states aren't guarded against; they're **unrepresentable.** You cannot construct "submitting and saved" because no value of `status` means both. This is the lesson from [Impossible states, unrepresentable](/005-typescript-bug-class-moves/1-impossible-states-unrepresentable/), now enforced at runtime by a hook instead of only described by a type.
...
:::caution
React's Strict Mode calls your reducer twice in development to surface impurity, the same double-invocation it applies to render bodies and initializers. If your reducer is pure, calling it twice is harmless: same input, same output, no observable difference. If it isn't pure, because it logs, mutates, or reaches outside itself, the doubled calls will expose that. The mechanism behind this lands in the next chapter. For now, it's enough to know this is why a `console.log` inside a reducer appears to fire twice.
:::
