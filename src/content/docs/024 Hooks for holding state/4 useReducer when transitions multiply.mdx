...
First, **`dispatch` is stable.** Its identity never changes, so it's safe to list in effect dependency arrays and to pass to a child as a prop. That last part is worth pausing on, because in [Four homes for state](/024-hooks-for-holding-state/3-the-four-homes-for-state/) the advice was *not* to prop-drill raw `setState` setters. `dispatch` is the sanctioned exception: its identity is stable, and the child codes against the action vocabulary, not a setter. Passing `dispatch` down is idiomatic; passing `setIsSubmitting` down is a smell.
...
`}>
  ...
  <AnnotatedStep meta="{3-4}" color="green">
    ...
    This is the immutable-update reflex from [The useState surface](/024-hooks-for-holding-state/1-the-usestate-surface-and-lazy-initialization/), one level deeper.
...
The inputs stay controlled exactly as in [Four homes for state](/024-hooks-for-holding-state/3-the-four-homes-for-state/): `value` reads from `state.values`, and `onChange` calls a typed handler. The handler shape is unchanged, still a typed callback taking the field and the new value; only the destination moved, into a `dispatch` instead of a `setState`.
