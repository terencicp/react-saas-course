...
Here is the threshold as a rule you can apply without thinking: **switch to a reducer when three or more `useState` calls update together.** That means three or more values that move in lockstep, a <Term definition="A move from one state value to the next.">transition</Term> that has to maintain an <Term definition="A rule about which state combinations are legal — for example, never submitting and saved at once.">invariant</Term>, or a single value that gets set from many scattered handlers. Below that threshold, `useState` is correct and a reducer is just ceremony.

:::note
This is the sibling of the smell from [The four homes for state](/024-hooks-for-holding-state/3-the-four-homes-for-state/). There, the bug was state that should have been *lifted*: duplicated in two components and kept in sync with an effect. Here the state lives in the right component, but it's *scattered* across too many setters, with no single authority over what a legal transition looks like. These are two versions of the same family of problem. One is about *where* state lives, the other about *how its changes are governed.*
:::
...
- **<Term definition="The function that sends an action to the reducer.">`dispatch(action)`</Term>** is the function you call to make a change. It queues the action, React then calls your reducer with the current state plus that action, and re-renders with whatever the reducer returns. Like `useState`'s setter, **`dispatch` is referentially stable**: it's the same function across every render.
...
First, **`dispatch` is stable.** Its identity never changes between renders, which makes it safe to list in effect dependency arrays and safe to pass down to a child as a prop. This is worth pausing on, because in [The four homes for state](/024-hooks-for-holding-state/3-the-four-homes-for-state/) the advice was *not* to prop-drill raw `setState` setters. `dispatch` is the sanctioned exception: its identity is stable, and the contract a child codes against is the action vocabulary, not the setter. Passing `dispatch` down is idiomatic. Passing `setIsSubmitting` down is a smell.
...
`}>
  <AnnotatedStep meta="{1}" color="blue">
    ...
    The types live right here, and this is what `useReducer` reads to infer the types of `state` and `dispatch`.
  ...
  <AnnotatedStep meta="{3-4}" color="green">
    ...
    The rule that prevents the classic bug is to **always spread `...state` first, then override.** Forget it and you return a *partial* state, with every other field gone.
  </AnnotatedStep>
  ...
  <AnnotatedStep meta="{5-6}" color="green">
    ...
    The two can't drift, because one move owns both. There is no code path where the form is "submitting" but still showing a stale error, because the transition made that combination unrepresentable.
  </AnnotatedStep>
  ...
  <AnnotatedStep meta="{13-14}" color="red">
    ...
    If every action `type` is handled, `action` narrows to `never` here and `assertNever` type-checks. Add a new action and forget its `case`, and **this line fails to compile.**
    The red doesn't mean the code is broken. It means this is your safety net, catching the missing case at build time rather than in production.
  </AnnotatedStep>
</AnnotatedCode>
...
Do the counting. Three booleans give you eight combinations, `true`/`false` cubed, and several of them are nonsense: submitting *and* saved, saved *and* errored, all three true at once. The scattered version let any handler produce any of those eight, and trusted you never to produce the bad five. A single `status` field has exactly three values, all of them legal. The illegal states aren't guarded against; they're **unrepresentable.** You cannot construct "submitting and saved" because no value of `status` means both. This is the lesson from [Impossible states, unrepresentable](/005-typescript-bug-class-moves/1-impossible-states-unrepresentable/), now held at runtime by a hook instead of only described by a type.

That is the real argument for reaching past `useState`, and it's the one an experienced engineer leads with. The argument is not that the reducer is less code, because sometimes it's more. The argument is that the reducer makes the broken states impossible to reach.

Two things to watch for come with writing reducers, so they belong right here.

The first is the bailout, carried over from `useState`. If your reducer returns the *same reference* it received, with `return state`, React compares with `Object.is`, sees no change, and **skips the re-render entirely.** That's occasionally what you want, when you deliberately ignore an action that doesn't apply right now. Far more often it's a bug: you meant to compute a new state and accidentally returned the old one, and now the screen won't update. Prefer the `assertNever` default over a `return state` default, so a forgotten case is a compile error instead of a silent no-op.

:::caution
React's Strict Mode calls your reducer twice in development to surface impurity, the same double-invocation it applies to render bodies and initializers. If your reducer is pure, double-calling is harmless: same input, same output, no observable difference. If it isn't pure, if it logs, mutates, or reaches outside itself, the doubled calls will expose it. The mechanism behind this lands in the next chapter. For now, just know it's why a `console.log` inside a reducer appears to fire twice.
:::
...
The inputs are controlled exactly as you learned in [The four homes for state](/024-hooks-for-holding-state/3-the-four-homes-for-state/): `value` reads from `state.values`, and `onChange` calls a typed handler that dispatches `editField`. Notice that the handler *shape* didn't change at all. It's still a typed callback that takes the field and the new value. The only thing that changed is where the value goes when it gets there: into a `dispatch` instead of a `setState`. The controlled-input contract is identical, and you swapped only the storage underneath it.
...
<DiagramSequence>
  <DiagramStep caption="dispatch('submit') runs the reducer synchronously: idle → submitting. React re-renders right now, so the button is disabled and the spinner is on screen. The save hasn't happened yet.">
    <AsyncStateTimeline step="submit" />
  </DiagramStep>
  <DiagramStep caption="The handler is parked on the Promise from saveInvoice(…). No dispatch fires, so no re-render happens and the spinner keeps spinning. This is the gap the scattered version never reset out of.">
...
Notice the "No, independent" branch lands on **grouped `useState`**, a single `useState({ ... })` holding the related values, updated with spreads. That's the middle ground between scattered separate setters and a full reducer: when several values belong together but their changes are simple and local, one state object is plenty. (This is the separate-versus-grouped decision that [The useState surface and lazy initialization](/024-hooks-for-holding-state/1-the-usestate-surface-and-lazy-initialization/) pointed forward to, and here's where it lands.)
...
There's one signal that's louder than all of these. If you find yourself writing the tenth, eleventh, or twelfth `useState` in a single component, stop. You don't have twelve pieces of state. You have a machine you haven't modeled yet, and that's the unmissable cue to consolidate.
