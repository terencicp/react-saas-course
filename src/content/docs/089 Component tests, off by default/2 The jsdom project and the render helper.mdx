...
`}>
  ...
  <AnnotatedStep meta={`{14} "{ locale = 'en-US', messages = enMessages }"`} color="blue">
    The i18n thread. Options default to English, so most tests pass nothing; switching locale for one test is a single flip, `render(<X />, { locale: 'es-ES', messages: esMessages })`. That is what makes the next lesson's locale-aware tests cheap.
  </AnnotatedStep>
</AnnotatedCode>
