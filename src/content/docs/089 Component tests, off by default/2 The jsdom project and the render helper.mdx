...
Two terms before we start. <Term definition="A pure-JavaScript implementation of the browser's DOM that runs inside Node. No real browser, no window painting — just an in-memory document your component code can render into and your tests can query.">jsdom</Term> is a pure-JavaScript implementation of the browser's DOM that runs inside Node: no browser, no window painting, just an in-memory document your component code can render into and your tests can query. <Term definition="React Testing Library. The render-and-query library: it mounts a React component into a DOM and gives you functions to find elements the way a user would.">RTL</Term>, React Testing Library, is the render-and-query library: it mounts a React component into that DOM and gives you functions to find elements the way a user would.
...
<VideoCallout videoId="CxSL0knFxAs" videoTitle="React Vite Testing Tutorial — Vitest Testing Crash Course">
  PedroTech walks the whole rig end to end in about 38 minutes: Vitest and Testing Library setup, `render`, `screen`, `user-event`, and mocking a fetch, the same pieces this lesson wired.
</VideoCallout>
