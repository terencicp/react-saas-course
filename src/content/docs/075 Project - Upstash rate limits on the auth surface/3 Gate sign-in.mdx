...
The alternative — pointing Better Auth's built-in at Upstash through its `secondaryStorage` adapter — is the road not taken, covered as the named alternative in *Rate-limiting the auth endpoints*; we chose the application-wrapper seam because it gives one consistent budget, one opaque message, and one fail-open policy across every endpoint, auth and non-auth alike.
