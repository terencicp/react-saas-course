...
For the parts this lesson leans on but doesn't own — dual-keying, fail-open, gate-before-work, the budget riding the `Result`, and `after()` — see [Build the dual-keyed sign-in gate](/075-project---upstash-rate-limits-on-the-auth-surface/3-gate-sign-in/), and the Upstash primitives in [the rate-limiting chapter's dual-keying lesson](/074-rate-limiting-with-upstash-redis/3-rate-limiting-the-auth-endpoints/).
