...
The senior answer: this state belongs in the URL, and a Server Component reads it directly, with no client state, no effect, and no <Term definition="A chain of requests that must run in sequence because each one waits on the last — render, then fetch, then render again — so latency stacks up.">waterfall</Term>. The URL *is* the filter. By the end of this lesson you'll read and validate the URL on the server and render a filtered list with nothing in component memory.
