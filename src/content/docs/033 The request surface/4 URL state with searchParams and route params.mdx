...
Before any syntax, settle the decision, because everything after it is plumbing in service of it.
...
The rule earns its keep on the cases that aren't obvious, so let's work through the edges.
...
The two pairs that trip people up are worth saying out loud. A search query the user has *submitted* belongs in the URL, because they'd expect `?q=acme` to come back on refresh and to work in a shared link. The text *in the box before they hit enter* does not, because that's an in-progress edit and pure component state. Same word, two homes, split by whether the user has committed to it. Likewise, a selected tab is URL state, since it's a view they'd share, while an open dropdown is not, since nobody shares an open menu. Hold those two distinctions and the rest follows.
...
Aim for recognition here; the deep dive on the `Promise` wrapper is the next section.
...
That `await` is not ceremony. You met these request APIs in the previous chapter, where the point was that the route is dynamic by default, so resolving them *is* the request-time work: the value genuinely isn't known until the request arrives. Under the Cache Components model, the `await` is also the explicit signal that this part of the render is dynamic. Hold that thought; we come back to what it means for caching near the end of the lesson.

There's a client-side counterpart you should be able to recognize, though you won't write it here. A Client Component that's handed a `searchParams` Promise unwraps it with `React.use()` instead of `await`, the same unwrapping shape you saw when crossing the server/client boundary. But reading the URL on the client is rare, and it's the next lesson's subject. The senior default, the same reflex the cookies-and-headers lesson installed, is to read high on the server and pass resolved values down as props. You read once, at the top, where the request is, and everything below receives plain values.

## Validate at the boundary with Zod

This is the one rule in the lesson you don't get to skip, and it's a security decision rather than a style tip.
...
This is the payoff: the shape that replaces the whole client state machine. The contrast is what carries the point, so let's put the two approaches side by side.
...
<CodeVariants>
  <CodeVariant label="Client state machine">
    ...
    **The reflex version.** It renders, but the filter lives in memory: not shareable, not refresh-stable, and with an effect fetching data, exactly what the effects chapter told you to avoid.
  </CodeVariant>
...
Let's walk the server version line by line, because this is the page you'll write again and again.
...
`}>
  <AnnotatedStep meta={`{7-10} "async"`} color="blue">
    An `async` page component, handed both channels as Promises, which is why it has to be `async`. This signature is the only place the request enters, and everything below is plain values.
  </AnnotatedStep>

  <AnnotatedStep meta="{11}" color="blue">
    The identity read. `await props.params` resolves to `{ org }`, the org we're scoped to. This is the *who* from the path.
  </AnnotatedStep>

  <AnnotatedStep meta="{12}" color="blue">
    The validate-at-the-boundary line, the one to never skip. We `await` the raw, user-controlled query, hand it straight to the Zod helper, and get back typed, trusted `{ status, sort, cursor }`. Garbage in the URL becomes defaults out, not a crash.
  </AnnotatedStep>

  <AnnotatedStep meta="{14}" color="blue">
    The data read. The parsed, trusted values flow in as arguments. `listInvoices` is a black box here, since how it builds the query against the database is a later chapter's job. What matters is that *only validated values reach it*.
  </AnnotatedStep>
...
The narrative across those five steps is the whole pattern in five words: **URL in, typed filters, table out.** It runs top to bottom on the server, fresh, on every render.

That raises the question of when, exactly, "every render" happens. Here's the loop that makes the page feel alive, though notice that one step in it belongs to the next lesson.
...
```
  <Fragment slot="caption">The server is a pure function of the URL. The client's only job is to change the URL, which is the next lesson. The purple step is the only one the client owns.</Fragment>
</Figure>

That's the mental model to carry out of here: the server is a pure function of the URL. Give it the same URL, and it renders the same page. The only moving part the client owns is changing the URL, and that one piece is what the next lesson is about.
...
Now whether the user picks one tag or five, everything downstream of the parser receives a `string[]`. You normalized the surprise away at the boundary, which is exactly where surprises should be handled.
...
The fix is the same move you saw in the previous chapter, and it's the reason <Term definition="Partial Prerendering — a static shell rendered ahead of time with dynamic holes streamed in at request time.">PPR</Term> exists: keep the cached chrome outside the dynamic part of the tree. The sidebar, the header, and the org nav are chrome, the same for every filter, so they stream instantly from the static cache. The invoice table is the dynamic hole: it reads `searchParams` and runs at request time. You don't fight this; you lay the page out so the URL read happens only where the data is genuinely dynamic.

<StaticChromeDynamicTable />

The picture gives you the intuition: `searchParams` flows into the inner box and nowhere else. Cache the shell, and read the URL where the data lives.

## What the URL is not

You know what belongs in the URL. The senior judgment is just as much about what you'd never put there. Here are three boundaries.
...
**Not a place for transient UI state.** Here we close the loop back to the rule we opened with. An open dropdown, a hover, a half-typed input: the user would never bookmark those, never share them, never expect them back on refresh. They fail the one question, so they belong in `useState`.

## nuqs: the type-safe URL-state layer

Everything so far, meaning `searchParams`, a Zod schema, and defaults, is the right amount of tool for a page with one filter or two. Don't reach past it prematurely. But there's a threshold, and you should know where it sits.
...
Note one detail that catches people out: the server parsers import from **`nuqs/server`**, not the bare `nuqs`. The top-level `nuqs` entry carries a `'use client'` directive and would drag client code into your Server Component. The split is intentional, and server parsing lives behind `/server`.
