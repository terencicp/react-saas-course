...
A cursor encodes where the last page ended — the sort key of the final row, plus a <Term def="A secondary sort key that makes ordering deterministic when the primary key ties — so two invoices with the same date still have a stable, unambiguous order.">tiebreaker</Term> so the ordering is unambiguous. It's then base64-encoded into something <Term def="Data the user isn't meant to read or edit, only to receive and hand back unchanged.">opaque</Term> — meaningless to the user, perfectly deterministic for the server. The server hands it out; the browser carries it in the URL; the server decodes it on the next request to know where to resume.
