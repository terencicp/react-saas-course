...
tagline: Next.js 16's proxy.ts runs before your routes; the matcher controls what it costs.
chapter-id: 33
course-progress: 0.3086
sidebar:
  order: 2
...
<CodeVariants>
  ...
  <CodeVariant label="Legacy — middleware.ts (deprecated)">
    ...
    **The pre-16 name for the same file.** The body is byte-for-byte identical: the rename changes the shell, not the logic. `middleware.ts` is deprecated, warns at build, and is the form you'll find in older repos.
  </CodeVariant>
</CodeVariants>

Migrating an existing codebase is one command. Next.js ships a <Term definition="An automated script that rewrites your source code to a new API. Next.js ships codemods for breaking changes, so you don't hand-edit every call site.">codemod</Term> that renames both the file and the function:
...
That pattern is <Term definition="The library Next.js uses to turn matcher path patterns like /dashboard/:path* into regular expressions.">path-to-regexp</Term> syntax, the same shape Next.js uses for routes. The piece to recognize is `:path*`: a named segment plus a modifier. The `*` means *zero or more* segments, so `/dashboard/:path*` matches `/dashboard`, `/dashboard/settings`, and `/dashboard/team/billing` alike. The other modifiers (`?`, `+`, plain `:path`) follow the same shape; look them up when you need a precise one.
...
<VideoCallout videoId="zNgCFXZLoRk" videoTitle="Next.js 16 Middleware DEPRECATED — Authentication In Proxy Or Data Access Layer?">
  ByteGrad (23 min) walks the same line: why the rename signals "don't do the heavy thing here," and why auth belongs in a data access layer, not the proxy.
</VideoCallout>
...
Your `proxy` function receives a <Term definition="Next.js's extension of the web-platform Request object you met when we covered the browser platform.">`NextRequest`</Term> — the platform `Request` you already know, with a few Next.js conveniences on top:
...
- <Term definition="Next.js's extension of the web-platform Response, with static helpers (next, redirect, rewrite) for the moves a proxy can make.">`NextResponse`</Term>`.next()`: pass through, so the matched route renders. "I looked, I'm done, carry on."
