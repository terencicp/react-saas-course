...
`}>
  <AnnotatedStep meta="{2-3}" color="blue">
    Every request read for this layout lives here, at the top, and runs once. There are two `await`s, the cookie store and the headers, and nothing below this point touches the request directly.
  </AnnotatedStep>
  ...
  <AnnotatedStep meta={`{10} "getCurrentUser"`} color="orange">
    The session read hides behind the cached helper, so the layout never sees the raw token. Because `getCurrentUser` is React-`cache`d, children can call it themselves and share this one per-request lookup, rather than receiving `user` as a drilled prop.
  </AnnotatedStep>
